...
def repr_with_function_support(value):
    ...
    substitution then falls through as a no-op rather than emitting a broken
...
def py_descr2sphinx(ident, fw, descr, module_name, type_name, identifier, is_class):
    ...
    if type(descr) == GetSetDescriptorType:
        if not doc:
            raise ValueError(
                "C-API GETSET {:s}.{:s}.{:s} has empty/missing doc-string".format(
                    module_name, type_name, identifier,
                )
            )
        if ":type:" not in doc:
            raise ValueError(
                "C-API GETSET {:s}.{:s}.{:s} doc-string is missing ':type:'".format(
                    module_name, type_name, identifier,
                )
            )
...
class DunderInfo(NamedTuple):
    ...
    - ``overloads``: when set, emit one ``.. method::`` directive per entry
    ...
    for key in sorted(dunder_keys):
        ...
        if info.overloads is not None:
            ...
            for i, overload in enumerate(info.overloads):
                ...
                # `:noindex:` on subsequent overloads keeps Sphinx from emitting
