...
# Module-level singleton; rebound on update via `_replace` since NamedTuple is immutable.
GLOBAL = Global()
del Global
...
class TransparentDirective(Directive):  # type: ignore[misc]
    """
    Parses content and emits its children directly into the parent node.
...
def paragraph_to_type_str(para: nodes.paragraph) -> str:
    ...
    while i < len(children):
        ...
        if (isinstance(child, nodes.inline)
                and child.get("role") == "ref"
                and (alias_name := child.astext()) in enum_alias_map):
            # Check if wrapped in Literal[...]: previous text ends with
            # "Literal[" and next sibling text starts with "]".
            if (parts and parts[-1].endswith("Literal[")
...
def extract_class(node: ApiNode, source_path: Path) -> ClassInfo:
    ...
    # class. Overload signatures are emitted as `.. method::` directives sharing
...
# Module name where shared enum type aliases are emitted.
...
def contains_word(text: str, word: str) -> bool:
    ...
    while True:
        ...
        if idx == -1:
            return False
        if ((idx == 0 or not is_word_char(text[idx - 1]))
                and (idx + word_len >= len(text) or not is_word_char(text[idx + word_len]))):
            return True
        pos = idx + 1
        ...
        if is_word_char(text[pos]) and (pos == 0 or not is_word_char(text[pos - 1])):
            ...
            if pos < length and text[pos] == "[" and ident in collection_element_map:
                ...
                if pos < length and text[pos] == "]" and pos > elem_start:
                    pos += 1  # skip ]
                    # Emit span up to and including the identifier, skip [element].
...
# can't be expressed via RST. Lines are emitted verbatim into the class body
...
def write_stub(
    ...
    # Emit classes (hierarchy-sorted so locally-defined bases come first).
    ...
    # Emit submodule re-exports for package `__init__`.
    ...
    # emit `X = Any` aliases and ensure `Any` is imported.
    ...
    # Emit TypeVar declarations needed by generic classes.
    if ("typing", "TypeVar") in imports:
        fw("_T = TypeVar('_T')\n")
        fw("\n")

    # Emit `Any` aliases for types referenced but not defined in any module.
    if used_missing:
...
def write_class(
    ...
    # (e.g. Windows wraps Window), emit typed __getitem__ / __iter__.
    ...
    if extra_members is not None:
        ...
        # emit each non-empty line indented one level into the class body.
...
def write_method(
    ...
    *,
    ...
    # For __getitem__ with a non-primitive return type, emit overloads so that
    ...
    # each entry should be emitted as-is by the @overload path below.
    if (name == "__getitem__"
            and not method.is_overload
            and ret_type not in {"float", "int", "bool", "str", "None", "Any"}):
        imports.add(("typing", "overload"))
        fw("    @overload\n")
        fw("    def __getitem__(self, key: int) -> {:s}: ...\n".format(ret_type))
        if has_items_method:
...
def write_attribute(
    ...
    """Emit an attribute declaration. Returns True if emitted."""
    name = attr.name
...
def write_data(
    ...
    """Emit a module-level data declaration."""
    if data.type_str:
...
def build_collection_element_map(modules: dict[str, ModuleInfo]) -> None:
    ...
    to element class name so the stub generator can emit typed
...
def main() -> int:
    ...
    # Undefined referenced types are emitted as `Foo = Any` aliases by `write_stub`
