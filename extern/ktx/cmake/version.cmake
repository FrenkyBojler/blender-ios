...
function( create_version_header dest_path target )
    ...
    if(CMAKE_HOST_WIN32 AND (KTX_FORCE_CMAKE_VERSION_HEADER OR NOT BASH_EXECUTABLE))
        if(NOT KTX_GIT_VERSION_FULL)
            if(KTX_VERSION_FULL)
                set(_ktx_git_version "v${KTX_VERSION_FULL}")
            else()
                set(_ktx_git_version "v0.0.0-noversion")
            endif()
        else()
            set(_ktx_git_version "${KTX_GIT_VERSION_FULL}")
        endif()

        set(_ktx_version_content "/*\n// [API version]\n@par API Version\nv4.0\n// [API version]\n// [Code version]\n${_ktx_git_version}\n// [Code version]\n*/\n#define LIBKTX_VERSION ${_ktx_git_version}\n#define LIBKTX_DEFAULT_VERSION v4.0.__default__\n")

        if(EXISTS ${version_h_output})
            file(READ ${version_h_output} _ktx_version_existing)
        else()
            set(_ktx_version_existing "")
        endif()

        if(NOT _ktx_version_existing STREQUAL _ktx_version_content)
            file(WRITE ${version_h_output} "${_ktx_version_content}")
        endif()
