...
class Generator(base_generate.BaseGenerator):
    ...
    def __assign_layers(self):
        ...
        # Build a mapping from original bone name → metarig collection names,
        # used when rigify_def_group_by_collection is enabled.
        group_by_coll = self.metarig.data.rigify_def_group_by_collection
        if group_by_coll:
            meta_bone_colls = {}
            for meta_bone in self.metarig.data.bones:
                colls = [c.name for c in meta_bone.collections]
                if colls:
                  meta_bone_colls[meta_bone.name] = colls[0]

            def lookup_meta_coll(original_name):
                """Return the metarig collection for a DEF bone's original name.
                Some DEF bones (e.g. DEF-thigh.L.001) are generated by Rigify as
                extra bendy-bone segments and have no direct metarig counterpart.
                For these, fall back to the base name without the numeric suffix
                (e.g. thigh.L) which does exist in the metarig."""
                coll = meta_bone_colls.get(original_name)
                if coll:
                    return coll
            ...
            # Move all the bones with names starting with "DEF-" to their layer.
                ...
                if group_by_coll:
                    ...
                    if source_coll_name:
                        ...
                        if not sub_coll:
                            sub_coll = self.obj.data.collections.new(sub_name, parent=def_coll)
                        layers = [sub_coll]
                    else:
                        layers = [def_coll]
                else:
