    ...
    if (export_settings['gltf_bake_animation'] is True
        ...
        if export_settings['vtree'].nodes[obj_uuid].skin is None:
            if mode is None or mode == "OBJECT":
                ...
                used_slot_handle = blender_object.animation_data.action_slot if used_action else None
                animation, _ = gather_action_object_sampled(obj_uuid, None, used_slot_handle, animation_key, export_settings)
        ...
        if export_settings['gltf_morph_anim'] \
            ...
            if ignore_sk is False:
                ...
                used_slot_handle = blender_object.data.shape_keys.animation_data.action_slot if used_action else None
                channel = gather_sampled_sk_channel(obj_uuid, animation_key, used_slot_handle, export_settings)
        ...
        used_action = blender_object.animation_data.action if blender_object.animation_data else None
        used_slot_handle = blender_object.animation_data.action_slot if used_action else None
        animation, _ = gather_action_armature_sampled(obj_uuid, None, used_slot_handle, animation_key, export_settings)
        link_samplers(animation, export_settings)
