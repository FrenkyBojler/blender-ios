...
def gather_data_sampled_channels(blender_type_data, blender_id, blender_action_name, slot_handle,
    ...
    for path in export_settings['KHR_animation_pointer'][blender_type_data][blender_id]['paths'].keys():
        ...
        channel = gather_sampled_data_channel(
            ...
            list_of_animated_data_channels[path] if path in list_of_animated_data_channels.keys() else get_gltf_interpolation(export_settings['gltf_sampling_interpolation'], export_settings),
            additional_key,
            export_settings)
