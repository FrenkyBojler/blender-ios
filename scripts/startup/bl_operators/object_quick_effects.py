...
class QuickFur(ObjectModeOperator, Operator):
    ...
    def execute(self, context):
        ...
        with bpy.data.libraries.load(
            ...
            data_to.node_groups.extend([
                "Generate Hair Curves",
                "Set Hair Curve Profile",
                "Interpolate Hair Curves",
            ])
            if self.use_noise:
                data_to.node_groups.append("Hair Curves Noise")
            if self.use_frizz:
                data_to.node_groups.append("Frizz Hair Curves")

        # For convenient name lookups.
        node_groups_name_map = {id.name: id for id in data_to.node_groups}
