...
def compositor_effect_reserved_input_sockets(node_tree, image_input_count):
    # The first Float input is the effect fader, and the first N Color inputs are
    # the image inputs. This logic must be in sync with input mapping in `effect_compositor.cc`.
    reserved = set()
    fader_found = False
    images_found = 0
    for item in node_tree.interface.items_tree:
        if item.item_type != 'SOCKET' or item.in_out != 'INPUT':
            continue
        if not fader_found and item.socket_type == 'NodeSocketFloat':
            reserved.add(item.identifier)
            fader_found = True
        elif images_found < image_input_count and item.socket_type == 'NodeSocketColor':
            reserved.add(item.identifier)
            images_found += 1
    return reserved


def compositor_effect_socket_is_drawable(socket, reserved, inputs, usages):
    if socket.in_out != 'INPUT' or socket.hide_in_modifier:
        return False
    if socket.identifier in reserved:
        return False
    # Hidden by socket usage inference (e.g. input conditional on a menu selection).
    if not usages.get(socket.identifier, (True, True))[1]:
        return False
    socket_props = getattr(inputs, socket.identifier, None)
    # Some socket types (e.g. matrix, geometry) have no editable "value".
    return socket_props is not None and hasattr(socket_props, "value")


def compositor_effect_panel_toggle_socket(panel):
    # A panel can have a header toggle: first boolean input socket, flagged as a panel
    # toggle. Draw it as a checkbox in the panel header.
    # Mirrors `bNodeTreeInterfacePanel::header_toggle_socket`.
    items = panel.interface_items
    if len(items) == 0:
        return None
    first = items[0]
    if (first.item_type == 'SOCKET' and first.in_out == 'INPUT' and
            first.is_panel_toggle and first.socket_type == 'NodeSocketBool'):
        return first
    return None


def compositor_effect_panel_has_drawable_socket(panel, reserved, inputs, usages):
    for item in panel.interface_items:
        if item.item_type == 'PANEL':
            if compositor_effect_panel_has_drawable_socket(item, reserved, inputs, usages):
                return True
        elif item.item_type == 'SOCKET':
            if compositor_effect_socket_is_drawable(item, reserved, inputs, usages):
                return True
    return False


def compositor_effect_panel_affects_output(panel, reserved, inputs, usages):
    # A panel is grayed out in its header when none of its inputs currently affect the output.
    for item in panel.interface_items:
        if item.item_type == 'PANEL':
            if compositor_effect_panel_affects_output(item, reserved, inputs, usages):
                return True
        elif (item.item_type == 'SOCKET' and item.in_out == 'INPUT' and
              not item.hide_in_modifier and item.identifier not in reserved):
            if usages.get(item.identifier, (True, True))[0]:
                return True
    return False


def compositor_effect_draw_socket(layout, socket, inputs, usages, parent_name=None):
    socket_props = getattr(inputs, socket.identifier, None)
    if socket_props is None:
        return
    name = socket.name or ""
    # If the name is prefixed by the containing panel's name, drop the prefix.
    if parent_name and name.startswith(parent_name + " "):
        name = name[len(parent_name) + 1:]
    row = layout.row(align=True)
    # Gray out inputs that do not affect the output.
    row.active = usages.get(socket.identifier, (True, True))[0]
    if socket.socket_type == 'NodeSocketMenu' and socket.menu_expanded:
        # Use a single space when the name is empty to work around a bug with expanded enums.
        row.prop(socket_props, "value", expand=True, text=(name if name else " "))
    else:
        row.prop(socket_props, "value", text=name)


def compositor_effect_draw_items(layout, items, inputs, panels, reserved, usages, parent_name=None):
    for item in items:
        if item.item_type == 'PANEL':
            # Draw a panel
            if panels is None or not compositor_effect_panel_has_drawable_socket(
                    item, reserved, inputs, usages):
                continue
            header, body = layout.panel_prop(panels, "open_{:d}".format(item.identifier))

            # Draw a panel header toggle as a checkbox in the header.
            skip_first = False
            toggle = compositor_effect_panel_toggle_socket(item)
            toggle_props = None
            if toggle is not None and not toggle.hide_in_modifier:
                toggle_props = getattr(inputs, toggle.identifier, None)
            if toggle_props is not None and hasattr(toggle_props, "value"):
                header.use_property_split = False
                header.use_property_decorate = False
                header.prop(toggle_props, "value", text=item.name)
                skip_first = True
            else:
                header.label(text=item.name)

            if not compositor_effect_panel_affects_output(item, reserved, inputs, usages):
                header.active = False

            if body is not None:
                child_items = list(item.interface_items)
                if skip_first:
                    child_items = child_items[1:]
                compositor_effect_draw_items(
                    body, child_items, inputs, panels, reserved, usages, item.name,
                )
        elif item.item_type == 'SOCKET':
            # Draw a regular socket
            if compositor_effect_socket_is_drawable(item, reserved, inputs, usages):
                compositor_effect_draw_socket(layout, item, inputs, usages, parent_name)


def draw_compositor_effect_node_group_inputs(layout, strip):
    node_tree = strip.node_group
    if not node_tree or not node_tree.interface:
        return
    ...
    panels = getattr(properties, "panels", None)
    if inputs is None:
        return

    reserved = compositor_effect_reserved_input_sockets(node_tree, strip.input_count)

    # Query used/visible usage for the inputs, and key them by socket identifier.
    items_tree = node_tree.interface.items_tree
    input_identifiers = [
        item.identifier for item in items_tree
        if item.item_type == 'SOCKET' and item.in_out == 'INPUT'
    ]
    try:
        used, visible = strip.evaluate_input_usages()
    except (AttributeError, RuntimeError):
        used, visible = (), ()
    usages = {
        identifier: (is_used, is_visible)
        for identifier, is_used, is_visible in zip(input_identifiers, used, visible)
    }

    # Collect interface items not contained in any panel.
    child_indices = set()
    for item in items_tree:
        if item.item_type == 'PANEL':
            for child in item.interface_items:
                child_indices.add(child.index)
    root_items = [item for item in items_tree if item.index not in child_indices]

    compositor_effect_draw_items(layout, root_items, inputs, panels, reserved, usages)
...
class STRIP_PT_effect(StripButtonsPanel, Panel):
    ...
    def draw(self, context):
        ...
        if strip_type == 'COMPOSITOR':
            ...
            draw_compositor_effect_node_group_inputs(layout, strip)
