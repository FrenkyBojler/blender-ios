...
#  ifdef WITH_VULKAN_BACKEND
#    include <libdrm/drm_fourcc.h> /* For `DRM_FORMAT_ARGB8888`. */
...
#  ifdef WITH_VULKAN_BACKEND
    ...
    if (window_->ghost_context_type == GHOST_kDrawingContextTypeVulkan) {
      /* TESTING: try different buffer sizes. */
      // int X = window_->frame.size[0];
      // int Y = window_->frame.size[1];
      // /* Attempt a 1x1 pixel buffer. */
      // window_->frame.size[0] = 512;
      // window_->frame.size[1] = 512;
      ...
      const uint32_t modifier_lo = modifier & 0xffffffffl;
...
#    endif
      ...
      zwp_linux_dmabuf_v1 *linux_dmabuf = system_->wp_linux_dmabuf_get();
      zwp_linux_buffer_params_v1 *dmabuf_params = zwp_linux_dmabuf_v1_create_params(linux_dmabuf);

      zwp_linux_buffer_params_v1_add(
          dmabuf_params, fd, 0, 0, window_->frame.size[0], modifier_hi, modifier_lo);

      dummy_buffer = zwp_linux_buffer_params_v1_create_immed(
          dmabuf_params, window_->frame.size[0], window_->frame.size[1], format, 0);
      ...
      // window_->frame.size[0] = X;
      // window_->frame.size[1] = Y;
    }
