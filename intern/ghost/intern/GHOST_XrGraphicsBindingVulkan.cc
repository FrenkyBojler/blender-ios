...
std::optional<GHOST_XrSwapchainFormat> GHOST_XrGraphicsBindingVulkan::chooseSwapchainFormat(
    const std::vector<int64_t> &runtime_formats) const
{
  ...
  if (result) {
    ...
    swapchain_format.is_srgb_format = *result == VK_FORMAT_R8G8B8A8_SRGB ||
    ...
    XrSwapchainImageVulkan2KHR &swapchain_image, const GHOST_XrDrawViewInfo &draw_info)
{
  ...
  if (draw_info.swapchain_format.xr_format == GHOST_kXrSwapchainFormatRGBA16F ||
      draw_info.swapchain_format.xr_format == GHOST_kXrSwapchainFormatRGBA16)
  {
