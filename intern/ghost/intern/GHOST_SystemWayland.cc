...
/* #GWL_Display.wp_dmabuf */

static void gwl_registry_wp_linux_dmabuf_add(GWL_Display *display,
                                             const GWL_RegisteryAdd_Params &params)
{
  /* Needs version 2 (`create_immed`). */
  const uint version = GWL_IFACE_VERSION_CLAMP(params.version, 2u, 2u);

  display->wp.linux_dmabuf = static_cast<zwp_linux_dmabuf_v1 *>(wl_registry_bind(
      display->wl.registry, params.name, &zwp_linux_dmabuf_v1_interface, version));
  gwl_registry_entry_add(display, params, nullptr);
}
static void gwl_registry_wp_linux_dmabuf_remove(GWL_Display *display,
                                                void * /*user_data*/,
                                                const bool /*on_exit*/)
{
  zwp_linux_dmabuf_v1 **value_p = &display->wp.linux_dmabuf;
  zwp_linux_dmabuf_v1_destroy(*value_p);
  *value_p = nullptr;
}
...
static const GWL_RegistryHandler gwl_registry_handlers[] = {
    ...
    {
        /*interface_p*/ &zwp_linux_dmabuf_v1_interface.name,
        /*add_fn*/ gwl_registry_wp_linux_dmabuf_add,
        /*update_fn*/ nullptr,
        /*remove_fn*/ gwl_registry_wp_linux_dmabuf_remove,
    },
...
zwp_linux_dmabuf_v1 *GHOST_SystemWayland::wp_linux_dmabuf_get()
{
  return display_->wp.linux_dmabuf;
}
