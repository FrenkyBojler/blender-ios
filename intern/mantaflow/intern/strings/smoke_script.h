...
def smoke_step_$ID$():\n\
    ...
    if using_fire_s$ID$:\n\
        ...
        flame_s$ID$.multConst(flameVorticity_s$ID$ * s$ID$.timestep / (frameLengthUnscaled_s$ID$ * 32 * cellSize_s$ID$))\n\
    vorticityConfinement(vel=vel_s$ID$, flags=flags_s$ID$, strength=vorticity_s$ID$ * s$ID$.timestep / (frameLengthUnscaled_s$ID$ * 32 * cellSize_s$ID$), strengthCell=flame_s$ID$ if using_fire_s$ID$ else None)\n\
    \n\
    if using_heat_s$ID$:\n\
