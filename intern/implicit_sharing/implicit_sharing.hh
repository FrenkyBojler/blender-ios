...
class ImplicitSharingInfo {
 ...
 public:
  ImplicitSharingInfo() = default;
  ImplicitSharingInfo(const ImplicitSharingInfo &other) = delete;
  ImplicitSharingInfo &operator=(const ImplicitSharingInfo &other) = delete;
  ImplicitSharingInfo(ImplicitSharingInfo &&other) = delete;
  ImplicitSharingInfo &operator=(ImplicitSharingInfo &&other) = delete;
  virtual ~ImplicitSharingInfo()
  {
    assert(strong_users_ == 0);
    assert(weak_users_ == 0);
  }
                         ...
                         const ImplicitSharingInfo **r_dst_sharing_info)
{
  ...
  *r_dst_sharing_info = src_sharing_info;
  if (*r_dst_ptr) {
    assert(*r_dst_sharing_info != nullptr);
