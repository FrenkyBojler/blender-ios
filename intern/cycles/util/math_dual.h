...
{
  return select(is_zero_mask(b.val), make_zero<dual3>(), a - floor(a.val / b.val) * b);
}

template<class T> ccl_device_inline dual<T> safe_divide(const dual<T> f, const T g)
{
  return select(is_zero_mask(g), make_zero<dual<T>>(), f / g);
}

template<class T>
ccl_device_inline dual<T> safe_divide(const ccl_private dual<T> &f, const ccl_private dual<T> &g)
{
  return select(is_zero_mask(g.val), make_zero<dual<T>>(), f / g);
}
