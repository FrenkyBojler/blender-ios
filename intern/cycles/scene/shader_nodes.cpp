...
void GeometryNode::compile(SVMCompiler &compiler)
{
  ...
  const uint store_derivatives = need_derivatives() ? 1 : 0;
  ShaderOutput *out;
  ...
  if (!out->links.empty()) {
    compiler.add_node_derivative(
        NODE_GEOMETRY,
        use_derivative,
        compiler.encode_uchar4(NODE_GEOM_N, bump_offset, store_derivatives),
        compiler.stack_assign(out),
        __float_as_uint(bump_filter_width));
  }
  ...
  if (!out->links.empty()) {
    compiler.add_node_derivative(
        NODE_GEOMETRY,
        use_derivative,
        compiler.encode_uchar4(NODE_GEOM_T, bump_offset, store_derivatives),
        compiler.stack_assign(out),
        __float_as_uint(bump_filter_width));
  }
  ...
  if (!out->links.empty()) {
    compiler.add_node_derivative(
        NODE_GEOMETRY,
        use_derivative,
        compiler.encode_uchar4(NODE_GEOM_Ng, bump_offset, store_derivatives),
        compiler.stack_assign(out),
        __float_as_uint(bump_filter_width));
  }
  ...
  if (!out->links.empty()) {
    compiler.add_node_derivative(
        NODE_GEOMETRY,
        use_derivative,
        compiler.encode_uchar4(NODE_GEOM_I, bump_offset, store_derivatives),
        compiler.stack_assign(out),
        __float_as_uint(bump_filter_width));
  }
    ...
    if (compiler.background) {
      ...
      if (from_dupli) {
        compiler.add_node_derivative(
            NODE_TEX_COORD,
            use_derivative,
            compiler.encode_uchar4(NODE_TEXCO_DUPLI_GENERATED, bump_offset, store_derivatives),
            compiler.stack_assign(out),
            __float_as_uint(bump_filter_width));
      }
  ...
  if (!out->links.empty()) {
    if (from_dupli) {
      compiler.add_node_derivative(
          NODE_TEX_COORD,
          use_derivative,
          compiler.encode_uchar4(NODE_TEXCO_DUPLI_UV, bump_offset, store_derivatives),
          compiler.stack_assign(out),
          __float_as_uint(bump_filter_width));
    }
    else {
  ...
  if (!out->links.empty()) {
    if (compiler.background) {
      ...
      compiler.add_node_derivative(
          NODE_TEX_COORD,
          use_derivative,
          compiler.encode_uchar4(NODE_TEXCO_REFLECTION, bump_offset, store_derivatives),
          compiler.stack_assign(out),
          __float_as_uint(bump_filter_width));
    }
  }
...
void AttributeNode::compile(SVMCompiler &compiler)
{
  ...
  if (!color_out->links.empty() || !vector_out->links.empty()) {
    if (!color_out->links.empty()) {
      compiler.add_node_derivative(
          ...
          attr,
          compiler.encode_uchar4(
              compiler.stack_assign(color_out), NODE_ATTR_OUTPUT_FLOAT3, bump_offset,
              store_derivatives),
    ...
    if (!vector_out->links.empty()) {
      compiler.add_node_derivative(
          ...
          attr,
          compiler.encode_uchar4(
              compiler.stack_assign(vector_out), NODE_ATTR_OUTPUT_FLOAT3, bump_offset,
              store_derivatives),
          bump_filter_or_stochastic);
    }
  }
  ...
  if (!fac_out->links.empty()) {
    compiler.add_node_derivative(
        ...
        attr,
        compiler.encode_uchar4(
            compiler.stack_assign(fac_out), NODE_ATTR_OUTPUT_FLOAT, bump_offset, store_derivatives),
        bump_filter_or_stochastic);
  }
  ...
  if (!alpha_out->links.empty()) {
    compiler.add_node_derivative(
        ...
        attr,
        compiler.encode_uchar4(
            compiler.stack_assign(alpha_out), NODE_ATTR_OUTPUT_FLOAT_ALPHA, bump_offset,
            store_derivatives),
        bump_filter_or_stochastic);
  }
}
