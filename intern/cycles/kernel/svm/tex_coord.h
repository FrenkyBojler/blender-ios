...
template<typename T> ccl_device_inline T svm_texco_reflection(const ccl_private ShaderData *sd)
{
  T data = shading_incoming<T>(sd);
...
template<typename T>
ccl_device_inline T svm_texco_camera(KernelGlobals kg,
                                     const ccl_private ShaderData *sd,
                                     const ccl_private T &P)
{
  T data(P);
...
template<typename T>
ccl_device_noinline T svm_node_tex_coord_eval(KernelGlobals kg,
                                              ...
                                              ccl_private int *offset)
{
  ...
  switch ((NodeTexCoord)type) {
    ...
    case NODE_TEXCO_CAMERA: {
      const T P = shading_position<T>(sd);
      data = svm_texco_camera<T>(kg, sd, P);
      break;
    }
    case NODE_TEXCO_WINDOW: {
      ...
      {
        data = T(camera_world_to_ndc(kg, sd, sd->ray_P));
      }
      else {
        data = T(camera_world_to_ndc(kg, sd, sd->P));
        if constexpr (is_dual_v(T)) {
    ...
    case NODE_TEXCO_REFLECTION: {
      data = svm_texco_reflection<T>(sd);
      break;
    }
    case NODE_TEXCO_DUPLI_GENERATED: {
      data = T(object_dupli_generated(kg, sd->object));
      break;
    }
    case NODE_TEXCO_DUPLI_UV: {
      data = T(object_dupli_uv(kg, sd->object));
      break;
    }
...
#ifdef __VOLUME__
      if (sd->object != OBJECT_NONE) {
        data = volume_normalized_position<T>(kg, sd, data);
      }
#endif
      break;
    }
...
ccl_device_noinline int svm_node_tex_coord(KernelGlobals kg,
                                           ...
                                           int offset)
{
  const float3 data = svm_node_tex_coord_eval<float3>(kg, sd, path_flag, node.y & 0xFF, &offset);
...
ccl_device_noinline int svm_node_tex_coord_derivative(KernelGlobals kg,
                                                      ...
                                                      int offset)
{
  const NodeBumpOffset bump_offset = NodeBumpOffset((node.y >> 8) & 0xFF);
  const bool store_derivatives = (node.y >> 16) & 0xFF;

  dual3 data = svm_node_tex_coord_eval<dual3>(kg, sd, path_flag, node.y & 0xFF, &offset);
                                          ...
                                          const uint4 node)
{
  ...
  if (direction_type == NODE_TANGENT_UVMAP) {
    ...
    if (desc.offset == ATTR_STD_NOT_FOUND) {
      stack_store(stack, tangent_offset, T());
      return;
    }
    if constexpr (is_dual_v(T)) {
      if (desc.type == NODE_ATTR_FLOAT2) {
        ...
        tangent = primitive_surface_attribute<dual3>(kg, sd, desc);
      }
    }
    else {
      if (desc.type == NODE_ATTR_FLOAT2) {
        ...
        tangent = primitive_surface_attribute<float3>(kg, sd, desc);
      }
    }
  }
  else {
    ...
    T generated;
    if (desc.offset == ATTR_STD_NOT_FOUND) {
      generated = shading_position<T>(sd);
    }
    else {
      if constexpr (is_dual_v(T)) {
        if (desc.type == NODE_ATTR_FLOAT2) {
          ...
          generated = primitive_surface_attribute<dual3>(kg, sd, desc);
        }
      }
      else {
        if (desc.type == NODE_ATTR_FLOAT2) {
          ...
          generated = primitive_surface_attribute<float3>(kg, sd, desc);
        }
      }
    }
    ...
    if constexpr (is_dual_v(T)) {
      ...
      if (axis == NODE_TANGENT_AXIS_X) {
        tangent = make_float3(S(), -(generated.z() - 0.5f), (generated.y() - 0.5f));
      }
      else if (axis == NODE_TANGENT_AXIS_Y) {
        tangent = make_float3(-(generated.z() - 0.5f), S(), (generated.x() - 0.5f));
      }
      else {
        tangent = make_float3(-(generated.y() - 0.5f), (generated.x() - 0.5f), S());
      }
    }
    else {
      if (axis == NODE_TANGENT_AXIS_X) {
