                                                     ...
                                                     const AttributeDescriptor desc)
{
  ...
  if (desc.element & (ATTR_ELEMENT_OBJECT | ATTR_ELEMENT_MESH)) {
    return T(attribute_data_fetch<B>(kg, desc.element, desc.offset));
  }
...
template<typename T> ccl_device T primitive_tangent(KernelGlobals kg, ccl_private ShaderData *sd)
{
#if defined(__HAIR__) || defined(__POINTCLOUD__)
  if (sd->type & (PRIMITIVE_CURVE | PRIMITIVE_POINT)) {
#  ifdef __DPDU__
    return T(normalize(sd->dPdu));
  }
#  else
    return make_zero<T>();
#  endif
