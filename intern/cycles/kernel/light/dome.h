                                             ...
                                             const float3 D)
{
  ...
  if (tex_id == -1) {
    ...
    printf("DOME_DEBUG: No HDR texture (tex_id = -1)\n");
    return one_float3();
  }
  
  /* Additional safety check - verify texture ID is reasonable */
  if (tex_id < 0 || tex_id > 10000) {  /* Arbitrary high limit */
    printf("DOME_DEBUG: Invalid HDR texture ID: %d\n", tex_id);
    return one_float3();
  }
  
  printf("DOME_DEBUG: HDR texture evaluation - tex_id=%d, strength=%.3f, rotation=(%.3f,%.3f,%.3f)\n", 
         tex_id, (double)klight->dome.dome_hdr_strength,
         (double)klight->dome.dome_rotation.x, (double)klight->dome.dome_rotation.y, (double)klight->dome.dome_rotation.z);
  ...
  printf("DOME_DEBUG: HDR sampled color (%.3f,%.3f,%.3f,%.3f) at UV(%.3f,%.3f)\n", 
         (double)hdr_color.x, (double)hdr_color.y, (double)hdr_color.z, (double)hdr_color.w, (double)uv_clamped.x, (double)uv_clamped.y);
  ...
  printf("DOME_DEBUG: Final HDR result (%.3f,%.3f,%.3f) = gamma_corrected_color * intensity %.3f (gamma=%.3f)\n", 
         (double)result.x, (double)result.y, (double)result.z, (double)intensity, (double)gamma);
                                           ...
                                           ccl_private float *pdf)
{
  ...
  if (klight->dome.map_res_x > 0 && klight->dome.map_res_y > 0) {
    ...
    const float du = (u > 0) ? (rand.y - kernel_data_fetch(light_dome_conditional_cdf, v * cdf_width + u - 1).y) /
                               (kernel_data_fetch(light_dome_conditional_cdf, v * cdf_width + u).y - 
                                kernel_data_fetch(light_dome_conditional_cdf, v * cdf_width + u - 1).y) : rand.y;
    const float dv = (v > 0) ? (rand.x - kernel_data_fetch(light_dome_marginal_cdf, v - 1).y) /
                               (kernel_data_fetch(light_dome_marginal_cdf, v).y - 
                                kernel_data_fetch(light_dome_marginal_cdf, v - 1).y) : rand.x;
