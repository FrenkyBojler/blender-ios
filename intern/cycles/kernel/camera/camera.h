                                                     ...
                                                     float y)
{
  ...
  return calibrated_cam_to_direction(make_float2(Pcamera.x, Pcamera.y),
                                     cam->sensorwidth,
                                     cam->sensorheight,
                                     cam->calibrated_cam_fov,
                                     cam->calibrated_cam_f,
                                     BaseProjectionType(cam->calibrated_cam_type),
                                                ...
                                                ccl_private Ray *ray)
{
  ...
  float3 D = float4_to_float3(D_theta);
  ...
  P.x += 1e-3f * angle_to_noncentrality(D_theta.w, cam->calibrated_cam_nc);
                                     ...
                                     ccl_private Ray *ray)
{
  ...
  else if (kernel_data.cam.type == CAMERA_CALIBRATED) {
    ccl_global const DecomposedTransform *cam_motion = kernel_data_array(camera_motion);
    camera_sample_calibrated(&kernel_data.cam, cam_motion, raster, lens_uv, ray);
  }
}
