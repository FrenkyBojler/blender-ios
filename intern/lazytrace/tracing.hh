...
namespace lazytrace
{
	...
	enum class TraceStorageStrategy
	{
		...
		DirectSave, /* Collect trace information and write to disk as soon as trace events come in, this is _very_ expensive, and should only be used in situations where a crash in the spplication makes it impossible to write the trace data at a later time. */
		SaveOnExit, /* Collect trace information and write to disk as the application closes. */
		Chunked, /* Collect trace infromation and write to disk every N events, this is trying to strike a blance between memory use and disk activity. */
	};
	...
	/* Convenience helper, this will return false if Disabled is the current storage strategy, true for all others. */
	bool TraceEnabled(); 
	...
	struct TraceData
	{
		...
		std::string cat;  /* The event categories. This is a comma separated list of categories for the event. The categories can be used to hide events in the Trace Viewer UI. */
		char ph; /* The event type.This is a single character which changes depending on the type of event being output. */
		int64_t ts; /* The tracing clock timestamp of the event. The timestamps are provided at microsecond granularity. */
#if 0
		...
		TraceArgs args; /* Any arguments provided for the event. Some of the event types have required argument fields, otherwise, you can put any information you wish in here. The arguments are displayed in Trace Viewer when you view an event in the analysis section. */
		std::string id2; 
