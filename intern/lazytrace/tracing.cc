...
bool lazytrace::Save()
{
    ...
    else
    {
        ...
        std::filesystem::resize_file(_settings.file_name, std::filesystem::file_size(_settings.file_name) - 1);
...
{
    return NameThread(lazytrace::internal::CurrentProcessID(), lazytrace::internal::CurrentThreadID(), thread_name);
}
...
{
    return ThreadName(lazytrace::internal::CurrentProcessID(), lazytrace::internal::CurrentThreadID());
}
...
void lazytrace::Emit(const TraceData& event)
{
    if (TraceEnabled())
    {
      ...
      _lazylock.unlock();
      if (
             (StorageStrategy() == TraceStorageStrategy::DirectSave) || 
             (StorageStrategy() == TraceStorageStrategy::Chunked && _data.events_.size() >= _settings.chunk_size)
           )
        {
            Save();
            _lazylock.lock();
...
{
    return CreateData(name, ph, lazytrace::internal::CurrentProcessID(), lazytrace::internal::CurrentThreadID());
}

lazytrace::TraceData lazytrace::CreateData(const std::string& name, const char ph, size_t pid, size_t tid)
{
...
static size_t tracedata_to_json(lazytrace::TraceData &data, char *buffer, size_t size, const char* args, const char *id2)
{
...
void lazytrace::EmitCounter(const std::string& name, const std::string& variable, const TraceVariant& value)
{
