...
### `Paint` base struct (`source/blender/makesdna/DNA_scene_types.h`, ~line 1219)
...
`UnifiedPaintSettings` (`~line 1093`) holds shared brush size/strength/color that can be linked across modes via `eUnifiedPaintSettingsFlags`.

### Mode-specific structs (all in `DNA_scene_types.h`)

| Struct | ~Line | Extra notable fields |
|--------|-------|----------------------|
| `Sculpt` | 1491 | `eSculptFlags flags`, `float detail_size`, `float constant_detail`, `float gravity_factor`, `Object *gravity_object`, `eSculptTransformMode transform_mode` |
| `VPaint` | 1584 | (minimal — reused for both vertex color and weight paint, distinguished at runtime) |
| `ImagePaintSettings` | 1308 | `Image *stencil`, `Image *clone`, `Image *canvas`, `float clone_alpha`, `int mode` (material vs. image) |
| `GpPaint` | 1549 | `int mode` (materials or vertex color) |
| `GpVertexPaint` | 1557 | — |
| `GpSculptPaint` | 1564 | — |
| `GpWeightPaint` | 1571 | — |
| `CurvesSculpt` | 1536 | — |

`UvSculpt` (~line 1540) is separate: it is a 2D sculpt overlay in the UV editor and does **not** embed `Paint`.
...
| `BKE_paint_init(Main*, Scene*, PaintMode, bool)` | Initialize/reset a paint session (~line 158) |
...
- `sculpt_ops.cc` ~line 553: `sculpt_mode_toggle_exec()` (~553) / `SCULPT_OT_sculptmode_toggle` (~606)
- `paint_vertex.cc` ~line 863: `PAINT_OT_vertex_paint_toggle`
- `paint_weight.cc` ~line 1756: `PAINT_OT_weight_paint_toggle`
- `paint_image.cc` ~line 821: `PAINT_OT_texture_paint_toggle`
...
- `sculptmode_toggle_exec()` ~line 166 → `GREASE_PENCIL_OT_sculptmode_toggle` ~line 220
- `weightmode_toggle_exec()` ~line 254 → `GREASE_PENCIL_OT_weightmode_toggle` ~line 307
- `vertexmode_toggle_exec()` ~line 347 → `GREASE_PENCIL_OT_vertexmode_toggle` ~line 404
...
- `curves_sculptmode_enter()` ~line 318, `curves_sculptmode_exit()` ~line 343
- `curves_sculptmode_toggle_exec()` ~line 349 → `CURVES_OT_sculptmode_toggle` ~line 378
...
The modal loop (`PaintStroke::modal()`, declared ~line 198 in `paint_intern.hh`; defined in `paint_stroke.cc` ~line 1384) drives the whole pipeline:
...
**`VPaintData`** (`mesh/paint_vertex.cc`, ~line 915)
```cpp
...
class GreasePencilStrokeOperation : public PaintModeData {
  virtual void on_stroke_begin(const bContext &C, const InputSample &start) = 0;
  virtual void on_stroke_extended(const bContext &C, const InputSample &sample) = 0;
...
**`CurvesSculptStrokeOperation`** (`curves/sculpt_intern.hh`, ~line 62) — same virtual interface pattern as GP. Factories: `new_add_operation()`, `new_comb_operation()`, `new_delete_operation()`, `new_snake_hook_operation()`, etc. (~lines 69–90).
...
| Vertex Paint | `PAINT_OT_vertex_paint` | `mesh/paint_vertex.cc` ~2184 |
| Weight Paint | `PAINT_OT_weight_paint` | `mesh/paint_weight.cc` ~2032 |
| Texture Paint | `PAINT_OT_image_paint` | `mesh/paint_image_ops_paint.cc` ~625 |
| GP (all) | `GREASE_PENCIL_OT_brush_stroke` | `grease_pencil/draw_ops.cc` |
...
- `do_brush_action()` (~line 3536) — dispatches to the appropriate brush implementation
- `sculpt_apply_texture()` (~line 2479) — applies brush texture mask

### Brush implementations (`source/blender/editors/sculpt_paint/mesh/brushes/`)

Each brush type is in its own file:
`bmesh_topology_rake.cc`, `clay.cc`, `clay_strips.cc`, `clay_thumb.cc`, `crease.cc`, `draw.cc`, `draw_face_sets.cc`, `draw_sharp.cc`, `draw_vector_displacement.cc`, `elastic_deform.cc`, `enhance_details.cc`, `grab.cc`, `inflate.cc`, `layer.cc`, `mask.cc`, `multiplane_scrape.cc`, `multires_displacement_eraser.cc`, `multires_displacement_smear.cc`, `pinch.cc`, `plane.cc`, `relax.cc`, `rotate.cc`, `scene_project.cc`, `smooth.cc`, `smooth_mask.cc`, `snake_hook.cc`, `surface_smooth.cc`, `thumb.cc`, `topology_slide.cc`
...
All four GP modes live in `source/blender/editors/sculpt_paint/grease_pencil/` and share the `GreasePencilStrokeOperation` virtual interface. (The mode *toggle* operators are separate, in `source/blender/editors/grease_pencil/intern/grease_pencil_modes.cc`.)

| Sub-system | Files |
|------------|-------|
| Draw/Paint | `paint.cc`, `paint_common.cc`, `paint_cursor.cc`, `draw_ops.cc`, `erase.cc`, `fill.cc`, `interpolate.cc`, `tint.cc`, `trace.cc`, `trace_util.cc` |
...
| `sculpt_puff.cc` | Puff |
| `sculpt_selection.cc` | Selection utilities |
...
`BRUSH_OT_asset_activate`, `BRUSH_OT_asset_save`, `BRUSH_OT_asset_delete`, `BRUSH_OT_asset_revert`. See [brush-asset-system.md](./brush-asset-system.md) for details.
...
| `BrushPanel` (~line 374) | Brush settings panel base |
| `BrushSelectPanel` (~line 380) | Brush picker |
| `ColorPalettePanel` (~line 422) | Color palette |
| `ClonePanel` (~line 457) | Clone/stencil image controls |
| `TextureMaskPanel` (~line 517) | Texture mask settings |
| `StrokePanel` (~line 559) | Stroke method (dot/drag/airbrush/etc.) |
| `SmoothStrokePanel` (~line 646) | Stabilizer settings |
| `FalloffPanel` (~line 681) | Brush falloff curve |
| `DisplayPanel` (~line 738) | Cursor display options |
...
| Class (~line) | Mode |
|---------------|------|
| `VIEW3D_AST_brush_sculpt` (~9178) | Mesh Sculpt |
| `VIEW3D_AST_brush_sculpt_curves` (~9184) | Curves Sculpt |
| `VIEW3D_AST_brush_vertex_paint` (~9190) | Vertex Paint |
| `VIEW3D_AST_brush_weight_paint` (~9196) | Weight Paint |
| `VIEW3D_AST_brush_texture_paint` (~9202) | Texture Paint |
| `VIEW3D_AST_brush_gpencil_paint` (~9218) | GP Draw |
| `VIEW3D_AST_brush_gpencil_sculpt` (~9224) | GP Sculpt |

### Mode menus (header bar, `space_view3d.py`)

| Class (~line) | Mode |
|---------------|------|
| `VIEW3D_MT_sculpt` (~3722) | Mesh Sculpt |
| `VIEW3D_MT_paint_vertex` (~3538) | Vertex Paint |
| `VIEW3D_MT_paint_weight` (~3662) | Weight Paint |
| `VIEW3D_MT_sculpt_curves` (~3924) | Curves Sculpt |
| `VIEW3D_MT_paint_grease_pencil` (~2260) | GP Draw |

### Overlay panels (N-panel Overlays, `space_view3d.py`)

| Class (~line) | Mode |
|---------------|------|
| `VIEW3D_PT_overlay_sculpt` (~7599) | Mesh Sculpt |
| `VIEW3D_PT_overlay_texture_paint` (~7722) | Texture Paint |
| `VIEW3D_PT_overlay_vertex_paint` (~7744) | Vertex Paint |
| `VIEW3D_PT_overlay_weight_paint` (~7768) | Weight Paint |
