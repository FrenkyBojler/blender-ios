...
7. `BPY_RNA_operator_wrapper` (declared `rna_wm.cc:1799`, implemented in
...
1. `BPY_python_start()` (`source/blender/python/intern/bpy_interface.cc:374`)
   — initializes the embedded Python interpreter.
2. `wm_init_exit.cc:434` runs
   `BPY_run_string_eval(C, imports, "bpy.utils.load_scripts_extensions()")`
   during window-manager init (inside `wm_init_scripts_extensions_once`).
   (Unregister side: `wm_init_exit.cc:568` calls
   `BPY_run_string_eval(C, imports, "bpy.utils._on_exit()")`, which in turn
   runs `addon_utils.disable_all()` and unregisters all `"startup"` modules.)
...
5. `load_scripts_extensions()` (`bpy/utils/__init__.py:379`, called from
   `load_scripts()` at 344) separately loads add-ons/extensions, via
...
- `enable(module_name)` (line 337) imports the module and calls `mod.register()` (548).
- `disable(module_name)` (line 571) calls `mod.unregister()` (599).
- `_initialize_once()` (56), `modules()` (256), `check()` (276),
  `reset_all()` (624), `disable_all()` (661), `module_bl_info()` (712, reads
...
- `ToolDef` (lines 53-116) is a `namedtuple` describing one tool entry:
  `idname, label, icon, cursor, keymap, brush_type, data_block, operator, options`
  (among others). Line 101-103 documents `brush_type`: *"Optional brush type
  this tool is limited to. Ignored if `'USE_BRUSH'` isn't set in the options."*
...
- `ToolSelectPanelHelper` (191+) is the base class each space's tool panel
  ...
  `tools_from_context()`.
- `_tools_flatten` / `_tools_flatten_with_dynamic` (277-315) expand tuples
...
|---|---|---|
| `rna_enum_brush_sculpt_brush_type_items` | `sculpt_brush_type` | 148-193 / 2657 |
| `rna_enum_brush_vertex_brush_type_items` | `vertex_brush_type` | 195-201 / 2664 |
| `rna_enum_brush_weight_brush_type_items` | `weight_brush_type` | 203-209 / 2670 |
| `rna_enum_brush_image_brush_type_items` | `image_brush_type` | 211+ / 2676 |
| GP variants (`gpencil_brush_type`, `gpencil_vertex_brush_type`, `gpencil_sculpt_brush_type`, `gpencil_weight_brush_type`) | — | 2683-2707 |
| `rna_enum_brush_curves_sculpt_brush_type_items` | `curves_sculpt_brush_type` | 314 / 2713 |

Python consumes these enums **indirectly**, not through
`generate_from_enum_ex`. `BrushAssetShelf.has_tool_with_brush_type()` /
`brush_type_poll()` (`scripts/startup/bl_ui/properties_paint_common.py:35,75`)
...
- `View3DAssetShelf(BrushAssetShelf)` (`space_view3d.py:9169`) is the
...
- Per-mode subclasses, each pairing a `mode`, `mode_prop` (RNA bool checked
  ...
  |---|---|---|
  | `VIEW3D_AST_brush_sculpt` | 9178 | `sculpt_brush_type` |
  | `VIEW3D_AST_brush_sculpt_curves` | 9184 | `curves_sculpt_brush_type` |
  | `VIEW3D_AST_brush_vertex_paint` | 9190 | `vertex_brush_type` |
  | `VIEW3D_AST_brush_weight_paint` | 9196 | `weight_brush_type` |
  | `VIEW3D_AST_brush_texture_paint` | 9202 | `image_brush_type` |
  | `VIEW3D_AST_brush_gpencil_paint` | 9218 | `gpencil_brush_type` |
  | `VIEW3D_AST_brush_gpencil_sculpt` | 9224 | `gpencil_sculpt_brush_type` |
  | `VIEW3D_AST_brush_gpencil_vertex` | 9230 | `gpencil_vertex_brush_type` |
  | `VIEW3D_AST_brush_gpencil_weight` | 9236 | `gpencil_weight_brush_type` |

  All are appended to the module's `classes` tuple and registered generically
  at the bottom of `space_view3d.py` via
  `for cls in classes: register_class(cls)` (lines 9517-9518).
...
| `BPY_python_start` | `source/blender/python/intern/bpy_interface.cc` | 374 |
| WM calls `load_scripts_extensions` | `source/blender/windowmanager/intern/wm_init_exit.cc` | 434 |
| `bl_ui` module list + `register()` | `scripts/startup/bl_ui/__init__.py` | 13, 118 |
| `addon_utils.enable()` / `mod.register()` | `scripts/modules/addon_utils.py` | 337 / 548 |
...
| `ToolDef` namedtuple | `scripts/startup/bl_ui/space_toolsystem_common.py` | 53-116 |
...
| `View3DAssetShelf` + per-mode subclasses | `scripts/startup/bl_ui/space_view3d.py` | 9169-9236 |
