...
`Node *to`, a `name`, and a `flag` bitmask (`RELATION_FLAG_*`, lines 20-31).
...
| `DEG_id_tag_update` | `DEG_depsgraph.hh:123` | `intern/depsgraph_tag.cc:844` |
| `DEG_id_tag_update_ex` (explicit `Main*`) | — | `intern/depsgraph_tag.cc:849` |
| `DEG_id_tag_update_for_side_effect_request` (explicit `Depsgraph*`) | — | `intern/depsgraph_tag.cc:858` |
...
// sculpt.cc ~902/906, ~5429/5437, ~5734/5736
```

The same pattern recurs across ~42 files under `editors/sculpt_paint/`:
...
- `WM_operatortype_append(void (*opfunc)(wmOperatorType *ot))` — declared
  `WM_api.hh:1427`, defined `intern/wm_operator_type.cc:151`. Each operator
...
- `struct wmKeyMap` — `source/blender/makesdna/DNA_windowmanager_types.h:419`.
- `struct wmKeyMapItem` — same file, line 345.
...
- `wm_event_do_handlers(bContext *C)` — declared `wm_event_system.hh:155`,
  ...
  (`intern/wm.cc:617`) and from `intern/wm_window.cc:1961`. Walks windows,
...
`struct wmWindowManager` (`DNA_windowmanager_types.h:113`) is the top-level
singleton owning the list of `wmWindow`s, global keyconfigs, and
operator/notifier queues. `struct wmWindow` (`DNA_windowmanager_types.h:167`)
...
| `DEG_id_tag_update` | `depsgraph/intern/depsgraph_tag.cc` | 844 |
...
| `wmKeyMap` / `wmKeyMapItem` | `makesdna/DNA_windowmanager_types.h` | 419 / 345 |
...
| `wmWindowManager` / `wmWindow` | `makesdna/DNA_windowmanager_types.h` | 113 / 167 |
