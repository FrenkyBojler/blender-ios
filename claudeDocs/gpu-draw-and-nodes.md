...
- `class GPUBackend` — `source/blender/gpu/intern/gpu_backend.hh:39`. Pure
  virtual factory methods: `batch_alloc()` (63), `framebuffer_alloc()` (65),
  `shader_alloc()` (69), `context_alloc()` (61), etc. Every concrete backend
  (GL, Vulkan, Metal, headless "Dummy") implements this interface.
- Backend selection state: `g_backend_type` / `g_backend_type_override`
  (`intern/gpu_context.cc:362-367`), with
  `GPU_backend_type_selection_set()` / `_get()` (380 / 401) and
  auto-detection `GPU_backend_type_selection_detect()` (479).
- Instantiation: `gpu_context.cc:560,565,570,574` —
  `g_backend = MEM_new<GLBackend>(...)` /
  `MEM_new<VKBackend>(...)` /
  `MEM_new<MTLBackend>(...)` /
  `MEM_new<DummyBackend>(...)`, retrieved globally via `GPUBackend::get()` (630).
  The Vulkan branch is guarded by `#ifdef WITH_VULKAN_BACKEND` (~line 565).
...
| Topic | File | Line |
|---|---|---|
| `GPUBackend` interface | `gpu/intern/gpu_backend.hh` | 39 |
| Backend instantiation | `gpu/intern/gpu_context.cc` | 560-574 |
