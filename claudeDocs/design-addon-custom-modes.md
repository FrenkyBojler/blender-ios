...
3. **`rna_enum_object_mode_items[]`** — `makesrna/intern/rna_object.cc:36`.
   ...
   `object_edit.cc:2006`) but never adds to it. Worse, the header UI
   (`scripts/startup/bl_ui/space_view3d.py:843`) does
...
- **Threading**: sculpt pushes undo nodes *concurrently from brush-eval
  threads* under a mutex (`sculpt_undo.cc:258`, `:1842-1922`). Python's GIL
...
- **Protocol subtlety**: correct interop with memfile undo requires the
  ...
  `sculpt_undo.cc:2203`). Order-sensitive and identical for every client —
...
- **`GizmoGroup`** (`rna_wm_gizmo.cc:1365`) shows the
...
case that suppresses undo pushes during sculpt mode (`ed_undo.cc:442`) keys
