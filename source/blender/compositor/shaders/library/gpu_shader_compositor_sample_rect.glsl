...
float weight_box(float x, float w)
{
...
float weight_bspline(float x, float w)
{
  x = abs(x / w);
...
float weight_triangle(float x, float w)
{
  return 1 - abs(x / w);
}

float weight_cubic(float x, float w)
{
  x = abs(x / w);
  return x < 1 ? (1.5 * x - 15.0 / 6) * x * x + 1 : ((-0.5 * x + 15.0 / 6) * x - 4) * x + 2;
}

float weight_mitchell(float x, float w)
{
  x = abs(x / w);
...
float weight_lanczos5(float x, float w)
{
