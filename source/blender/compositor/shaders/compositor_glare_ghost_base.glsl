...
void main()
{
   ...
   * of 2.13f, while the big ghost is flipped and scaled up with the origin as the center of the
   * image by a factor of 0.97f. Note that 1) The negative scale implements the flipping. 2)
  ...
  /* Scale the coordinates for the small and big ghosts, pre subtract 0.5f and post add 0.5f to use
   * 0.5f as the origin of the scaling. Notice that the big ghost is flipped due to the negative
   * scale. */
  vec2 small_ghost_coordinates = (coordinates - 0.5f) * small_ghost_scale + 0.5f;
  vec2 big_ghost_coordinates = (coordinates - 0.5f) * big_ghost_scale + 0.5f;
