...
void main()
{
  ...
  float2 pixels = float2(texture_size(input_tx));
  float2 center = pixels / 2 - 0.5;

  float2 uv = float2(texel) - center; // use center coordinates
  ...
  float2 wh = clamp(abs(uv * dscale + scale), 1.0f, 63.0f); // derivative of uv * scale
  uv = uv * scale + center;
