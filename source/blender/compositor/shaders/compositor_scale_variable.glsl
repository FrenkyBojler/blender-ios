...
{
  return (texture_load(image, texel + int2(1, 0)).x - texture_load(image, texel - int2(1, 0)).x) /
         2;
}
...
void main()
{
  ...
  float2 scale = 1 /
                 max(float2(texture_load(x_scale_tx, texel).x, texture_load(y_scale_tx, texel).x),
                     0.0001f);
  float2 dscale = -float2(dx(x_scale_tx, texel), dx(y_scale_tx, texel)) * scale *
                  scale;                 // derivative of scale
