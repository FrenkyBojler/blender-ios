...
void main()
{
  ...
  constexpr float red = texture(input_tx, normalized_texel + float2(dispersion, 0.0f)).r;
  constexpr float green = texture_load(input_tx, texel).g;
  constexpr float blue = texture(input_tx, normalized_texel - float2(dispersion, 0.0f)).b;
