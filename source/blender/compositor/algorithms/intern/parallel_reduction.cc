...
static float sum_float_gpu(Context &context, const Result &input)
{
  gpu::Shader *shader = context.get_shader("compositor_sum_float", ResultPrecision::Full);
...
static float sum_float_cpu(const Result &input)
{
  return float(parallel_reduce(
      input.domain().size,
      0.0,
      [&](const int2 texel, double &accumulated_value) {
        accumulated_value += double(input.load_pixel<float>(texel));
      },
...
float sum_float(Context &context, const Result &input)
{
  ...
  if (context.use_gpu()) {
    return sum_float_gpu(context, input);
  }

  return sum_float_cpu(input);
}
...
static float4 sum_color_cpu(const Result &input)
{
  return float4(parallel_reduce(
      ...
      [&](const int2 texel, double4 &accumulated_value) {
        accumulated_value += double4(input.load_pixel<float4>(texel));
      },
...
static float sum_log_luminance_cpu(const Result &input, const float3 &luminance_coefficients)
{
  return float(parallel_reduce(
      ...
      0.0,
      [&](const int2 texel, double &accumulated_value) {
        const float luminance = math::dot(input.load_pixel<float4>(texel).xyz(),
                                          luminance_coefficients);
        accumulated_value += double(std::log(math::max(luminance, 1e-5f)));
      },
      [&](const double &a, const double &b) { return a + b; }));
}
...
static float sum_squared_difference_float_gpu(Context &context,
                                              const Result &input,
                                              const float subtrahend)
{
  gpu::Shader *shader = context.get_shader("compositor_sum_squared_difference_float",
...
static float sum_squared_difference_float_cpu(const Result &input, const float subtrahend)
{
  return float(parallel_reduce(
      input.domain().size,
      0.0,
      [&](const int2 texel, double &accumulated_value) {
        accumulated_value += double(math::square(input.load_pixel<float>(texel) - subtrahend));
      },
      [&](const double &a, const double &b) { return a + b; }));
}
...
float sum_squared_difference_float(Context &context, const Result &input, const float subtrahend)
{
  BLI_assert(input.type() == ResultType::Float);
  if (context.use_gpu()) {
    return sum_squared_difference_float_gpu(context, input, subtrahend);
  }

  return sum_squared_difference_float_cpu(input, subtrahend);
}
...
static float4 sum_squared_difference_color_cpu(const Result &input, const float4 subtrahend)
{
  return float4(parallel_reduce(
      ...
      double4(0.0),
      [&](const int2 texel, double4 &accumulated_value) {
        accumulated_value += double4(math::square(input.load_pixel<float4>(texel) - subtrahend));
      },
      [&](const double4 &a, const double4 &b) { return a + b; }));
}
...
{
  gpu::Shader *shader = context.get_shader("compositor_maximum_color", ResultPrecision::Full);
  GPU_shader_bind(shader);
...
static float4 maximum_color_cpu(const Result &input)
{
  return parallel_reduce(
      ...
      [&](const int2 texel, float4 &accumulated_value) {
        accumulated_value = math::max(accumulated_value, input.load_pixel<float4>(texel));
      },
...
{
  gpu::Shader *shader = context.get_shader("compositor_minimum_color", ResultPrecision::Full);
  GPU_shader_bind(shader);

  float *reduced_value = parallel_reduction_dispatch(
      input, shader, Result::gpu_texture_format(ResultType::Color, ResultPrecision::Full));
  const float4 minimum = float4(reduced_value);
  MEM_freeN(reduced_value);
  GPU_shader_unbind();
...
static float4 minimum_color_cpu(const Result &input)
{
  return parallel_reduce(
      ...
      [&](const int2 texel, float4 &accumulated_value) {
        accumulated_value = math::min(accumulated_value, input.load_pixel<float4>(texel));
      },
