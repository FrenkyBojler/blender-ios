...
/* -------------------------------------------------------------------- */
/** \name MemPool Chunk Iteration
 * \{ */

struct ParallelMempoolChunkState {
  void *userdata;
  TaskParallelMempoolChunkFunc func;
  /** Element sizes used for chunk iteration. */
  MempoolIterChunk iter_chunk_init;
};

static void parallel_mempool_chunk_func(TaskPool *__restrict pool, void *taskdata)
{
  ParallelMempoolChunkState *__restrict state = static_cast<ParallelMempoolChunkState *>(
      BLI_task_pool_user_data(pool));
  BLI_mempool_threadsafe_iter *iter = &(static_cast<ParallelMempoolTaskData *>(taskdata))->ts_iter;
  TaskParallelTLS *tls = &(static_cast<ParallelMempoolTaskData *>(taskdata))->tls;
  const MempoolIterChunk *iter_chunk = &state->iter_chunk_init;
  const uint chunk_size = iter_chunk->elem_num * iter_chunk->elem_size;

  void *elem_data;
  while ((elem_data = mempool_iter_threadsafe_chunk_data_step(iter)) != nullptr) {
    const void *elem_data_end = POINTER_OFFSET(elem_data, chunk_size);
    /* Skip to the start then execute, see #TaskParallelMempoolChunkFunc for details. */
    BLI_TASK_PARALLEL_MEMPOOL_CHUNK_ITER_BEGIN (elem_data, elem_data_end, iter_chunk->elem_size) {
      break;
    }
    BLI_TASK_PARALLEL_MEMPOOL_CHUNK_ITER_END;
    if (elem_data < elem_data_end) {
      state->func(state->userdata, elem_data, elem_data_end, iter_chunk->elem_size, tls);
    }
  }
}

void BLI_task_parallel_mempool_chunks(BLI_mempool *mempool,
                                      void *userdata,
                                      TaskParallelMempoolChunkFunc func,
                                      const TaskParallelSettings *settings)
{
  ...
  if (!settings->use_threading) {
    ...
    MempoolIterChunk iter_chunk;
    mempool_iter_chunk_init_sizes(mempool, &iter_chunk);

    const uint chunk_size = iter_chunk.elem_num * iter_chunk.elem_size;
    void *elem_data;
    while ((elem_data = mempool_iter_chunk_data_step(&iter)) != nullptr) {
      const void *elem_data_end = POINTER_OFFSET(elem_data, chunk_size);
      /* Skip to the start then execute, see #TaskParallelMempoolChunkFunc for details. */
      BLI_TASK_PARALLEL_MEMPOOL_CHUNK_ITER_BEGIN (elem_data, elem_data_end, iter_chunk.elem_size) {
        break;
      ...
      BLI_TASK_PARALLEL_MEMPOOL_CHUNK_ITER_END;
      if (elem_data < elem_data_end) {
        func(userdata, elem_data, elem_data_end, iter_chunk.elem_size, &tls);
      }
    }
  ...
  ParallelMempoolChunkState state;
  TaskPool *task_pool = BLI_task_pool_create(&state, TASK_PRIORITY_HIGH);
  const int threads_num = BLI_task_scheduler_num_threads();

  /* The idea here is to prevent creating task for each of the loop iterations
   * and instead have tasks which are evenly distributed across CPU cores and
   * pull next chunk to be crunched using the threaded-aware BLI_mempool_iter.
   */
  const int tasks_num = threads_num + 2;

  state.userdata = userdata;
  state.func = func;
  mempool_iter_chunk_init_sizes(mempool, &state.iter_chunk_init);
  ...
  for (int i = 0; i < tasks_num; i++) {
    ...
    /* Use this pool's pre-allocated tasks. */
    BLI_task_pool_push(
        task_pool, parallel_mempool_chunk_func, &mempool_iterator_data[i], false, nullptr);
  }
