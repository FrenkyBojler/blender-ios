...
template<typename T> void CDTArrangement<T>::delete_edge(SymEdge<T> *se)
{
  ...
  if (!v1_isolated && !v2_isolated && aface != bface) {
     ...
     *
     * Mark the face deleted so callers checking `deleted` (hole detection, output)
     * skip it. The outer face is the exception: it's dereferenced without null checks
     * so it must never be deleted, only clear its `symedge`
     * (matching the handling in #dissolve_symedge). */
