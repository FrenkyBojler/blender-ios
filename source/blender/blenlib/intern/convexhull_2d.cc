...
static float convexhull_2d_angle_iter_cross_for_debug(const HullAngleIter &hiter,
...
static void convexhull_2d_angle_iter_step(HullAngleIter &hiter)
{
  ...
  if (hiter.axis_ordered) {
    ...
    BLI_assert((hull_angle_canonical_cmp(angle_prev, hiter.axis_ordered->angle) > 0) ||
               /* Skip this for very small triangles, without this near co-linear edges can fail.
                ...
                * It's debatable - however with near co-linear edges it's unlikely to make a
                * noticeable improvement to the result, at the expense of having to calculate
                * unit length vectors for all edges twice (or store them for reuse). */
               (std::abs(convexhull_2d_angle_iter_cross_for_debug(hiter, angle_prev)) < 1e-6f));
    UNUSED_VARS_NDEBUG(angle_prev, hull_angle_canonical_cross);
  }
#endif
}
