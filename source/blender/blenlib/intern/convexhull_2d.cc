...
/**
 * The algorithm used to create the convex hull is susceptible to errors with large values.
 * Unlike most floating point precision errors this happens when the numbers are not *that* big.
 * Errors may occur in the range of 100-500 or more for polygons that are (near) degenerate.
 * For example a polygon where all points are close to being co-linear.
 * See the test: `convexhull_2d.Degenerate_CoLinear` which fails without this define.
 *
 * To ensure the resulting hull is convex, add additional convex cross-product checks
 ...
 * see: #USE_CONVEX_CROSS_PRODUCT_ENSURE
 */
 ...
 * \return the updated `top` value.
 */
static int convexhull_2d_stack_finalize(const blender::float2 *points, int r_points[], int top)
{
                                ...
                                int r_points[])
{
  ...
  top = convexhull_2d_stack_finalize(points, r_points, top);
  return top + 1;
}
