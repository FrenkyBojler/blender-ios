...
ScopedTimerAveraged::~ScopedTimerAveraged()
{
  ...
  if (window_size_ == 0 || total_count_ < window_size_) {
    rolling_average_ = total_time_ / total_count_;
  }
  else {
    rolling_average_ = (rolling_average_ * (window_size_ - 1) / window_size_) +
  ...
  if (window_size_ != 0) {
    fmt::format_to(fmt::appender(buf), " of last {} events", std::min(window_size_, total_count_));
  }
