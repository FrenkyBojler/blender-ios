...
static float oklab_find_gamut_intersection(float a, float b, float L1, float C1, float L0, float l_cusp, float c_cusp)
{
	float t;
...
static void oklab_get_cs(float L, float a_, float b_, float *c_0, float *c_mid, float *c_max)
{
  ...
  *c_mid = 0.9f * k * sqrtf(sqrtf(1.f / (1.0f / powf(c_mid_a, 4) + 1.0f / powf(c_mid_b, 4))));

  float c_0_a = L * 0.4f;
  float c_0_b = (1.f - L) * 0.8f;
  *c_0 = sqrtf(1.f / (1.f / (c_0_a * c_0_a) + 1.f / (c_0_b * c_0_b)));
}
