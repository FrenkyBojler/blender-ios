 ...
 *
 * The intended use is a pre-process for arrays stored by #BArrayStore
 * which are likely to contain large spans of contiguous data (which doesn't de-duplicate so well).
 *
 * Intended for byte arrays and doesn't have any special logic array for alignment.
 * Note that this this could be supported and would be useful to de-duplicate repeating
 * patterns of non-byte data.
 *
 * Notes:
 ...
 *   because there is a probability the data isn't random.
 *   Having said this - encoding random data must not be *slow* either.
 */
