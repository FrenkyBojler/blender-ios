...
#ifdef USE_FIND_FASTPATH
/**
 * Check if all bytes in a fast_int are identical.
 * E.g., 0x42424242 returns true, 0x42434242 returns false.
 */
static inline bool has_homogeneous_bytes(const uintptr_t x)
{
                                       ...
                                       size_t *span_skip_next_p)
{
  ...
  if (size - offset > min_size_for_fast_path) {
    ...
    if (p < p_end && !has_homogeneous_bytes(*p)) {
      p++;

      /* Skip non-homogeneous chunks. */
      while (p < p_end && !has_homogeneous_bytes(*p)) {
        p++;
      }
