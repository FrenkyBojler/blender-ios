...
template<typename T> struct AngleRadianBase {
 ...
 public:
  ...
  T degree() const
  {
    return value_ * T(180.0 / std::numbers::pi);
  }
  ...
  {
    return math::mod_periodic(value_ + T(std::numbers::pi), T(2 * std::numbers::pi)) -
           T(std::numbers::pi);
  }
  ...
  friend auto operator<=>(const AngleRadianBase &a, const AngleRadianBase &b) = default;
...
template<typename T> struct AngleCartesianBase {
 ...
 public:
  ...
  {
    return AngleCartesianBase(degrees * T(std::numbers::pi / 180.0));
  }
  ...
  T degree() const
  {
    return T(*this) * T(180.0 / std::numbers::pi);
  }
  ...
  friend bool operator==(const AngleCartesianBase &a, const AngleCartesianBase &b) = default;
 ...
 public:
  ...
  T radian() const
  {
    ...
    if (abs(numerator_) == denominator_ * 2) {
      return is_negative ? T(-std::numbers::pi * 2) : T(std::numbers::pi * 2);
    }
    if (abs(numerator_) == denominator_) {
      return is_negative ? T(-std::numbers::pi) : T(std::numbers::pi);
    }
    if (numerator_ == 0) {
      return T(0);
    }
    if (abs(numerator_) * 2 == denominator_) {
      return is_negative ? T(-std::numbers::pi * 0.5) : T(std::numbers::pi * 0.5);
    }
    if (abs(numerator_) * 4 == denominator_) {
      return is_negative ? T(-std::numbers::pi * 0.25) : T(std::numbers::pi * 0.25);
    }
    ...
    if (slice_numerator > 0xFFFFFFFF || denominator_ > 0xFFFFFFFF) {
      ...
      slice_of_pi = T(std::numbers::pi) * slice_numerator / T(denominator_);
    }
    else {
    ...
    return slice_of_pi + T(std::numbers::pi) * number_of_pi;
  }
  ...
  operator AngleCartesianBase<T>() const
  {
    ...
    if (rest == 0) {
      switch (octant) {
        ...
        case 1:
        case 3:
          x = y = math::rcp(T(std::numbers::sqrt2));
          break;
        default:
      ...
      T angle = T(std::numbers::pi) * (T(a.numerator_) / T(a.denominator_));
      x = math::cos(angle);
      y = math::sin(angle);
