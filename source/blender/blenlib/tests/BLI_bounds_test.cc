...
TEST(bounds, Intersect)
{
  ...
  EXPECT_FALSE(bounds2.intersects_all(data2.as_span()));

  Bounds<float2> bounds3(float2(-2.0f, -1.0f), float2(4.0f, 5.0f));
  Array<float2> data3 = {
      float2(-1.0f, -1.0f), float2(-0.5f, -1.0f), float2(3.5f, 2.0f), float2(2.0f, 5.0f)};
  EXPECT_TRUE(bounds3.intersects_all(data3.as_span()));
}
