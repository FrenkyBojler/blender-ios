...
TEST_F(BMainAllIDsIteratorTest, basics)
{
  ...
  EXPECT_EQ(1, bmain->libraries.size());
  EXPECT_EQ(1, bmain->collections.size());
  EXPECT_EQ(2, bmain->objects.size());
...
TEST_F(BMainMergeTest, basics)
{
  ...
  EXPECT_EQ(1, bmain_dst->collections.size());
  EXPECT_EQ(0, bmain_dst->objects.size());
  EXPECT_EQ(1, bmain_src->collections.size());
  EXPECT_EQ(1, bmain_src->objects.size());

  MainMergeReport reports = {};
  BKE_main_merge(bmain_dst, &bmain_src, reports);

  EXPECT_EQ(2, bmain_dst->collections.size());
  EXPECT_EQ(1, bmain_dst->objects.size());
  ...
  auto gen_main_new = [](Collection **r_coll, Object **r_ob) -> Main * {
    ...
    *r_ob = BKE_id_new<Object>(bmain_new, "Ob_src");
    ...
    EXPECT_EQ(1, bmain_new->collections.size());
    EXPECT_EQ(1, bmain_new->objects.size());
  ...
  EXPECT_EQ(3, bmain_dst->collections.size());
  EXPECT_EQ(1, bmain_dst->objects.size());
   ...
   * in `bmain_src`. */
  EXPECT_EQ(1, coll_2->gobject.size());
  ...
  EXPECT_EQ(3, bmain_dst->collections.size());
  EXPECT_EQ(2, bmain_dst->objects.size());
  EXPECT_EQ(1, reports.num_merged_ids);
  EXPECT_EQ(0, reports.num_unknown_ids);
  EXPECT_EQ(1, reports.num_remapped_ids);
  EXPECT_EQ(0, reports.num_remapped_libraries);
...
#  define ABS_ROOT SEP_STR
  ...
  EXPECT_EQ(1, bmain_dst->collections.size());
  EXPECT_EQ(0, bmain_dst->objects.size());
  EXPECT_EQ(0, bmain_dst->libraries.size());
  EXPECT_EQ(1, bmain_src->collections.size());
  EXPECT_EQ(1, bmain_src->objects.size());
  EXPECT_EQ(1, bmain_src->libraries.size());
  ...
  EXPECT_EQ(2, bmain_dst->collections.size());
  EXPECT_EQ(1, bmain_dst->objects.size());
  EXPECT_EQ(1, bmain_dst->libraries.size());
  ...
  EXPECT_EQ(1, bmain_src->collections.size());
  EXPECT_EQ(2, bmain_src->objects.size());
  EXPECT_EQ(1, bmain_src->libraries.size());
  ...
  EXPECT_EQ(3, bmain_dst->collections.size());
  EXPECT_EQ(3, bmain_dst->objects.size());
  EXPECT_EQ(1, bmain_dst->libraries.size());
  EXPECT_EQ(ob_1, bmain_dst->objects.first);
  ...
  EXPECT_EQ(4, bmain_dst->collections.size());
  EXPECT_EQ(4, bmain_dst->objects.size());
  EXPECT_EQ(2, bmain_dst->libraries.size());
  EXPECT_EQ(ob_1, bmain_dst->objects.first);
...
TEST_F(BMainMergeTest, link_lib_packed)
{
  ...
  EXPECT_EQ(4, bmain_dst->libraries.size());
  ...
  EXPECT_EQ(5, bmain_dst->objects.size());
