...
void BKE_particle_system_blend_write(BlendWriter *writer, ListBaseT<ParticleSystem> *particles)
{
  for (ParticleSystem &psys : *particles) {
    ...
    if (psys.particles) {
      writer->write_struct_array<ParticleData>(psys.totpart, psys.particles);

      if (psys.particles->hair) {
        ParticleData *pa = psys.particles;

        for (int a = 0; a < psys.totpart; a++, pa++) {
          writer->write_struct_array<HairKey>(pa->totkey, pa->hair);
        }
      }

      if (psys.particles->boid && (psys.part->phystype == PART_PHYS_BOIDS)) {
        writer->write_struct_array<BoidParticle>(psys.totpart, psys.particles->boid);
      }

      if (psys.part->fluid && (psys.part->phystype == PART_PHYS_FLUID) &&
          (psys.part->fluid->flag & SPH_VISCOELASTIC_SPRINGS))
      {
        writer->write_struct_array<ParticleSpring>(psys.tot_fluidsprings, psys.fluid_springs);
      }
    }
    ...
    if (psys.child) {
      writer->write_struct_array<ChildParticle>(psys.totchild, psys.child);
    }
...
void BKE_particle_system_blend_read_after_liblink(BlendLibReader * /*reader*/,
                                                  ...
                                                  ListBaseT<ParticleSystem> *particles)
{
  for (ParticleSystem &psys : particles->items_mutable()) {
    if (psys.part) {
      if (psys.clmd) {
