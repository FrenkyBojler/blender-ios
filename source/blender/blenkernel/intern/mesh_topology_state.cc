...
MeshTopologyState::MeshTopologyState(const Mesh &mesh)
{
  ...
  edge_verts_ = attribute_reader_to_array_state(attributes.lookup<int2>(".edge_verts"_ustr));
  corner_verts_ = attribute_reader_to_array_state(attributes.lookup<int>(".corner_vert"_ustr));
  corner_edges_ = attribute_reader_to_array_state(attributes.lookup<int>(".corner_edge"_ustr));
...
bool MeshTopologyState::same_topology_as(const Mesh &mesh) const
{
  ...
  if (!attribute_reader_matches_array_state(edge_verts_,
                                            attributes.lookup<int2>(".edge_verts"_ustr)))
  {
    return false;
  }
  if (!attribute_reader_matches_array_state(corner_verts_,
                                            attributes.lookup<int>(".corner_vert"_ustr)))
  {
    return false;
  }
  if (!attribute_reader_matches_array_state(corner_edges_,
                                            attributes.lookup<int>(".corner_edge"_ustr)))
  {
    return false;
  }
