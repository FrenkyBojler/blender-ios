                            ...
                            const int flag)
{
  ...
  brush_dst->curve_hue_jitter = BKE_curvemapping_copy(brush_src->curve_hue_jitter);
  brush_dst->curve_sat_jitter = BKE_curvemapping_copy(brush_src->curve_sat_jitter);
  brush_dst->curve_val_jitter = BKE_curvemapping_copy(brush_src->curve_val_jitter);
  ...
  if (brush_src->gpencil_settings != nullptr) {
    ...
    brush_dst->gpencil_settings->curve_hue_jitter = BKE_curvemapping_copy(
        brush_src->gpencil_settings->curve_hue_jitter);
    brush_dst->gpencil_settings->curve_sat_jitter = BKE_curvemapping_copy(
        brush_src->gpencil_settings->curve_sat_jitter);
    brush_dst->gpencil_settings->curve_val_jitter = BKE_curvemapping_copy(
        brush_src->gpencil_settings->curve_val_jitter);
  }
...
static void brush_free_data(ID *id)
{
  ...
  BKE_curvemapping_free(brush->curve_hue_jitter);
  BKE_curvemapping_free(brush->curve_sat_jitter);
  BKE_curvemapping_free(brush->curve_val_jitter);
  ...
  if (brush->gpencil_settings != nullptr) {
    ...
    BKE_curvemapping_free(brush->gpencil_settings->curve_hue_jitter);
    BKE_curvemapping_free(brush->gpencil_settings->curve_sat_jitter);
    BKE_curvemapping_free(brush->gpencil_settings->curve_val_jitter);
...
static void brush_blend_write(BlendWriter *writer, ID *id, const void *id_address)
{
  ...
  if (brush->curve_hue_jitter) {
    BKE_curvemapping_blend_write(writer, brush->curve_hue_jitter);
  }
  if (brush->curve_sat_jitter) {
    BKE_curvemapping_blend_write(writer, brush->curve_sat_jitter);
  }
  if (brush->curve_val_jitter) {
    BKE_curvemapping_blend_write(writer, brush->curve_val_jitter);
  }
  ...
  if (brush->gpencil_settings) {
    ...
    if (brush->gpencil_settings->curve_hue_jitter) {
      BKE_curvemapping_blend_write(writer, brush->gpencil_settings->curve_hue_jitter);
    }
    if (brush->gpencil_settings->curve_sat_jitter) {
      BKE_curvemapping_blend_write(writer, brush->gpencil_settings->curve_sat_jitter);
    }
    if (brush->gpencil_settings->curve_val_jitter) {
      BKE_curvemapping_blend_write(writer, brush->gpencil_settings->curve_val_jitter);
    }
  }
...
static void brush_blend_read_data(BlendDataReader *reader, ID *id)
{
  ...
  BLO_read_struct(reader, CurveMapping, &brush->curve_hue_jitter);
  if (brush->curve_hue_jitter) {
    BKE_curvemapping_blend_read(reader, brush->curve_hue_jitter);
  }
  else {
    brush->curve_hue_jitter = BKE_paint_default_curve();
  }

  BLO_read_struct(reader, CurveMapping, &brush->curve_sat_jitter);
  if (brush->curve_sat_jitter) {
    BKE_curvemapping_blend_read(reader, brush->curve_sat_jitter);
  }
  else {
    brush->curve_sat_jitter = BKE_paint_default_curve();
  }

  BLO_read_struct(reader, CurveMapping, &brush->curve_val_jitter);
  if (brush->curve_val_jitter) {
    BKE_curvemapping_blend_read(reader, brush->curve_val_jitter);
  }
  else {
    brush->curve_val_jitter = BKE_paint_default_curve();
  }
  ...
  if (brush->gpencil_settings != nullptr) {
    ...
    BLO_read_struct(reader, CurveMapping, &brush->gpencil_settings->curve_hue_jitter);
    BLO_read_struct(reader, CurveMapping, &brush->gpencil_settings->curve_sat_jitter);
    BLO_read_struct(reader, CurveMapping, &brush->gpencil_settings->curve_val_jitter);
    ...
    if (brush->gpencil_settings->curve_hue_jitter) {
      BKE_curvemapping_blend_read(reader, brush->gpencil_settings->curve_hue_jitter);
    }

    if (brush->gpencil_settings->curve_sat_jitter) {
      BKE_curvemapping_blend_read(reader, brush->gpencil_settings->curve_sat_jitter);
    }

    if (brush->gpencil_settings->curve_val_jitter) {
      BKE_curvemapping_blend_read(reader, brush->gpencil_settings->curve_val_jitter);
    }
  }
...
void BKE_brush_init_gpencil_settings(Brush *brush)
{
  ...
  brush->gpencil_settings->curve_hue_jitter = BKE_curvemapping_add(1, 0.0f, 0.0f, 1.0f, 1.0f);
  brush->gpencil_settings->curve_sat_jitter = BKE_curvemapping_add(1, 0.0f, 0.0f, 1.0f, 1.0f);
  brush->gpencil_settings->curve_val_jitter = BKE_curvemapping_add(1, 0.0f, 0.0f, 1.0f, 1.0f);
}
    ...
    const Scene *scene, const Paint *paint, const Brush *brush)
{
  ...
  return BrushColorJitterSettings{
      ...
      brush->hsv_jitter[1],
      brush->hsv_jitter[2],
      brush->curve_hue_jitter,
      brush->curve_sat_jitter,
      brush->curve_val_jitter,
  };
}
