                                  ...
                                  ReshapeSubdivCoarsePositionWithStorageCb coarse_position_cb)
{
  ...
  CLOG_DEBUG(&LOG, "SETTING %d VERTS", num_vertices);
...
{
  const MultiresReshapeContext *reshape_context = reshape_smooth_context->reshape_context;
  const GridCoord *grid_coord = reshape_subdiv_refine_vert_grid_coord(vertex);
    ...
    blender::MutableSpan<blender::float3x3> tangent_matrix_storage)
{
  ...
  foreach_reshape_ptex_face(
      reshape_smooth_context, [&](const PTexCoord *ptex_coord, int idx, int corner) {
        ...
        tangent_matrix_storage[idx] = BKE_multires_construct_tangent_matrix(
            dPdu, dPdv, corner % 4);
      });
