...
static bNode *nodetree_uv_node_recursive(bNode *node)
{
  LISTBASE_FOREACH (bNodeSocket *, sock, &node->inputs) {
    if (sock->link) {
      ...
      if (inode->typeinfo->nclass == blender::bke::NodeClass::Input &&
          inode->typeinfo->type == SH_NODE_UVMAP)
      {
        return inode;
      }
                                            ...
                                            ePaintSlotFilter slot_filter)
{
  ...
  for (bNode *node : nodetree->all_nodes()) {
    if (do_image_nodes && node->typeinfo->nclass == blender::bke::NodeClass::Texture &&
