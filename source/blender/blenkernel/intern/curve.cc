...
static void curve_blend_write(BlendWriter *writer, ID *id, const void *id_address)
{
  ...
  if (cu->ob_type == OB_FONT) {
    ...
    writer->write_struct_array<CharInfo>(cu->len_char32 + 1, cu->strinfo);
    writer->write_struct_array<TextBox>(cu->totbox, cu->tb);
  }
  else {
    ...
    for (Nurb &nu : cu->nurb) {
      if (nu.type == CU_BEZIER) {
        writer->write_struct_array<BezTriple>(nu.pntsu, nu.bezt);
      }
      else {
        writer->write_struct_array<BPoint>(nu.pntsu * nu.pntsv, nu.bp);
        if (nu.knotsu) {
