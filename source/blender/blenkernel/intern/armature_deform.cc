    ...
    const bool use_dverts)
{
  ...
  if (deform_params.use_dverts) {
    ...
    deform_params.pchan_from_defbase.reinitialize(defbase_len);
    ...
    LISTBASE_FOREACH_INDEX (bDeformGroup *, dg, defbase, i) {
      ...
      deform_params.pchan_from_defbase[i] = (pchan && !(pchan->bone->flag & BONE_NO_DEFORM)) ?
                                                pchan :
                                                nullptr;
    }
  }
...
template<typename MixerT>
static void armature_vert_task_with_dvert(const ArmatureDeformParams &params,
                                          const int i,
                                          ...
                                          MixerT &mixer)
{
  ...
  if (params.use_dverts && dvert) {
    const Span<bPoseChannel *> pose_channels = params.pchan_from_defbase;
                                          ...
                                          const bool use_quaternion)
{
  ...
  if (use_quaternion) {
    if (full_deform) {
      ...
      armature_vert_task_with_dvert(deform_params, i, dvert, mixer);
    }
    else {
