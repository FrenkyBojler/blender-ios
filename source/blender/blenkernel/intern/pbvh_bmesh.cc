...
static BMVert *pbvh_bmesh_vert_create(BMesh &bm,
                                      ...
                                      const int cd_vert_color_offset)
{
  BMeshNode *node = &nodes[node_index];
  ...
  /* Log the new vertex. */
  BM_log_vert_added(&bm_log, v, cd_vert_mask_offset, cd_vert_color_offset);
...
#  define EDGE_QUEUE_DISABLE(e) \
      ...
      if (node->bm_faces_) {
        ...
        GSET_ITER (gs_iter, node->bm_faces_) {
          ...
          BLI_assert((EDGE_QUEUE_TEST(e_tri[0]) == false) &&
                     (EDGE_QUEUE_TEST(e_tri[1]) == false) && (EDGE_QUEUE_TEST(e_tri[2]) == false));
        }
      }
    }
  }
   ...
   *
   * The priority is used to control the order in which edges are handled for both splitting of
   * long edges and collapsing of short edges. For long edges we start by splitting the longest
   * edge and for collapsing we start with the shortest.
   *
...
#  undef EVEN_GENERATION_SCALE
                                    ...
                                    BMEdge *e)
  {
    ...
    for (const int i : edge_loops.index_range()) {
      ...
      BMFace *f_new_second = pbvh_bmesh_face_create(bm,
                                                    nodes,
                                                    node_changed,
                                                    cd_face_node_offset,
                                                    bm_log,
   ...
   * to \a v_conn.
   * \param flap_face: Face which is adjacent to \a v_conn and will form a flap when merging \a
   * v_del to \a v_conn. \param e: An edge which is being collapsed. It connects \a v_del and \a
   * v_conn. \param v_del: A vertex which will be removed after the edge collapse. \param l_del: A
   * loop of del_face which is adjacent to v_del. \param v_conn: A vertex which into which geometry
   * is reconnected to after the edge collapse.
   */
                                   ...
                                   BMVert *v_conn)
  {
    ...
    /* Faces around `e` (which connects `v_del` to `v_conn`) are to the handled separately from
     * this function. Help troubleshooting cases where these faces are mistakenly considered flaps.
     */
    ...
    const std::array<const BMEdge *, 4> source_edges{
         ...
         * These edges are implicitly merged with the ones from the `flap_face` upon collapsing
         * edge `e`. */
  ...
  /**
   * Find vertex which can be an outer for the flap face: the vertex will become loose when the
   * face and its edges are removed. If there are multiple of such vertices, return null.
   */
                                       ...
                                       EdgeQueueContext *eq_ctx)
  {
    ...
    for (BMFace *f_del : deleted_faces) {
      ...
      /* if its sa flap face merge its "outer" edge data into "base", so that boundary is
       * propagated from edges which are about to be deleted to the base of the triangle and will
       * stay attached to the mesh. */
      ...
      for (const int j : IndexRange(3)) {
        if ((v_tri[j] != v_del) && (v_tri[j]->e == nullptr)) {
          ...
          BM_log_vert_removed(
              &bm_log, v_tri[j], eq_ctx->cd_vert_mask_offset, eq_ctx->cd_color_offset);
    ...
    if (v_conn != nullptr && !is_boundary_vert(*v_conn)) {
      BM_log_vert_before_modified(
          &bm_log, v_conn, eq_ctx->cd_vert_mask_offset, eq_ctx->cd_color_offset);
    ...
    BM_log_vert_removed(&bm_log, v_del, eq_ctx->cd_vert_mask_offset, eq_ctx->cd_color_offset);
                                              ...
                                              BMLog &bm_log)
  {
    ...
    CLOG_INFO(
        &LOG, 2, "Short edge collapse took %f seconds.", BLI_time_now_seconds() - start_time);
        ...
        if (ray_face_intersection_tri(ray_start, isect_precalc, cos[0], cos[1], cos[2], depth)) {
          ...
          if (r_active_vertex) {
            ...
            for (const int i : IndexRange(3)) {
              if (i == 0 || len_squared_v3v3(location, cos[i]) <
                                                ...
                                                MemArena *arena)
  {
    ...
    for (int i = 0; i < node->totface; i++) {
      ...
      const float3 center = math::midpoint(face_bounds[face_index].min,
                                           face_bounds[face_index].max);
      math::min_max(center, cb.min, cb.max);
    }
   ...
   * BMesh vertex. Attempts to retrieve the value from the BMLog, falls back to the vertex's
   * current coordinates if it is either not found in the log or not requested. */
