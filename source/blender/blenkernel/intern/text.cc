...
bool BKE_text_reload(Text *text)
{
  ...
  BLI_path_apply_variables(
      filepath_abs, BKE_build_path_variables(ID_BLEND_PATH_FROM_GLOBAL(&text->id), &text->id));
                       ...
                       const bool is_internal)
{
  ...
  BLI_path_apply_variables(filepath_abs, BKE_build_path_variables(relbase, nullptr));
...
int BKE_text_file_modified_check(const Text *text)
{
  ...
  BLI_path_apply_variables(
      filepath, BKE_build_path_variables(ID_BLEND_PATH_FROM_GLOBAL(&text->id), &text->id));
