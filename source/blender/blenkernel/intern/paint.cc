...
{
  return mtex.brush_angle_mode & MTEX_ANGLE_RAKE;
}

static bool paint_rake_rotation_active(const Brush &brush, const PaintMode paint_mode)
{
  return (paint_rake_rotation_active(brush.mtex) || paint_rake_rotation_active(brush.mask_mtex)) &&
         (brush.mtex.tex || BKE_brush_has_cube_tip(&brush, paint_mode));
}
