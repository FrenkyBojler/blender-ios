...
bool MutableAttributeAccessor::rename(const UString old_name,
                                      const UString new_name,
                                      const bool overwrite)
{
  Map<UString, UString> map;
  map.add_new(old_name, new_name);
  return fn_->rename(owner_, map, overwrite).is_empty();
}

Set<UString> MutableAttributeAccessor::rename(const Map<UString, UString> &map,
                                              const bool overwrite)
{
