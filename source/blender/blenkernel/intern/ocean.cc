                    ...
                    void *update_cb_data)
{
  ...
  for (f = och->start, i = 0; f <= och->end; f++, i++) {
    ...
    ibuf_foam = IMB_allocImBuf(res_x, res_y, ImColorMode::RGBA, IB_float_data);
    ibuf_disp = IMB_allocImBuf(res_x, res_y, ImColorMode::RGBA, IB_float_data);
    ibuf_normal = IMB_allocImBuf(res_x, res_y, ImColorMode::RGBA, IB_float_data);
    ibuf_spray = IMB_allocImBuf(res_x, res_y, ImColorMode::RGBA, IB_float_data);
    ibuf_spray_inverse = IMB_allocImBuf(res_x, res_y, ImColorMode::RGBA, IB_float_data);
