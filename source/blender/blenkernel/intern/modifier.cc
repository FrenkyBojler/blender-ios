...
void BKE_modifier_blend_write(BlendWriter *writer, const ID *id_owner, ListBase *modbase)
{
  ...
  LISTBASE_FOREACH (ModifierData *, md, modbase) {
    ...
    if (md->type == eModifierType_Cloth) {
      ...
      writer->write_struct<ClothCollSettings>(clmd->coll_parms);
      writer->write_struct<EffectorWeights>(clmd->sim_parms->effector_weights);
      ...
      if (fmd->type & MOD_FLUID_TYPE_DOMAIN) {
        writer->write_struct<FluidDomainSettings>(fmd->domain);
        ...
        if (fmd->domain) {
          ...
          if (fmd->domain->coba) {
            writer->write_struct<ColorBand>(fmd->domain->coba);
          }
          ...
          writer->write_struct<EffectorWeights>(fmd->domain->effector_weights);
        }
      }
      else if (fmd->type & MOD_FLUID_TYPE_FLOW) {
        writer->write_struct<FluidFlowSettings>(fmd->flow);
      }
      else if (fmd->type & MOD_FLUID_TYPE_EFFEC) {
        writer->write_struct<FluidEffectorSettings>(fmd->effector);
      }
    }
      ...
      if (pmd->canvas) {
        writer->write_struct<DynamicPaintCanvasSettings>(pmd->canvas);

        /* write surfaces */
        LISTBASE_FOREACH (DynamicPaintSurface *, surface, &pmd->canvas->surfaces) {
          writer->write_struct<DynamicPaintSurface>(surface);
        }
        /* write caches and effector weights */
        LISTBASE_FOREACH (DynamicPaintSurface *, surface, &pmd->canvas->surfaces) {
          BKE_ptcache_blend_write(writer, &(surface->ptcaches));

          writer->write_struct<EffectorWeights>(surface->effector_weights);
        }
      }
      if (pmd->brush) {
        writer->write_struct<DynamicPaintBrushSettings>(pmd->brush);
        writer->write_struct<ColorBand>(pmd->brush->paint_ramp);
        writer->write_struct<ColorBand>(pmd->brush->vel_ramp);
      }
    }
