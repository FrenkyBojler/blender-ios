...
static void sound_device_close_no_lock()
{
  if (g_state.sound_device) {
    CLOG_DEBUG(LOG_SOUND, "Closing audio device");
...
static void sound_device_open_no_lock(AUD_DeviceSpecs requested_specs)
{
  ...
  CLOG_DEBUG(LOG_SOUND, "Opening audio device name:%s", g_state.device_name);
...
static void delayed_close_thread_run()
{
  ...
  while (!g_state.need_exit) {
    if (!g_state.use_delayed_close) {
      CLOG_DEBUG(LOG_SOUND, "Delayed device close is disabled");
    ...
    if (g_state.need_exit) {
      CLOG_DEBUG(LOG_SOUND, "System exit requested");
      break;
    }
    ...
    if (!g_state.sound_device) {
      CLOG_DEBUG(LOG_SOUND, "Device is not open, nothing to do");
      continue;
    }

    CLOG_DEBUG(LOG_SOUND, "Checking last device usage and timestamp");

    if (g_state.num_device_users) {
      CLOG_DEBUG(LOG_SOUND, "Device is used by %d user(s)", g_state.num_device_users);
      continue;
    }
  ...
  CLOG_DEBUG(LOG_SOUND, "Delayed device close thread finished");
}
...
void BKE_sound_init_once()
{
  ...
  if (sound_use_close_thread()) {
    CLOG_DEBUG(LOG_SOUND, "Using delayed device close thread");
