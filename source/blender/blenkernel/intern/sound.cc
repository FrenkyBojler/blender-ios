...
bSound *BKE_sound_new_file(Main *bmain, const char *filepath)
{
  ...
  BLI_path_apply_variables(filepath_abs, BKE_build_path_variables(blendfile_path, nullptr));
...
bSound *BKE_sound_new_file_exists_ex(Main *bmain, const char *filepath, bool *r_exists)
{
       ...
       sound = static_cast<bSound *>(sound->id.next))
  {
    ...
    BLI_path_apply_variables(
        filepath_test, BKE_build_path_variables(ID_BLEND_PATH(bmain, &sound->id), &sound->id));
...
#  endif
  {
    ...
    BLI_path_apply_variables(
        fullpath, BKE_build_path_variables(ID_BLEND_PATH(bmain, &sound->id), &sound->id));
