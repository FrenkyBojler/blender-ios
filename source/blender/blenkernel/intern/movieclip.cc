                                  ...
                                  char filepath[FILE_MAX])
{
  ...
  BLI_path_apply_variables(
      filepath, BKE_build_path_variables(ID_BLEND_PATH_FROM_GLOBAL(&clip->id), &clip->id));
...
static void movieclip_open_anim_file(MovieClip *clip)
{
  ...
  if (!clip->anim) {
    ...
    BLI_path_apply_variables(
        filepath_abs, BKE_build_path_variables(ID_BLEND_PATH_FROM_GLOBAL(&clip->id), &clip->id));
    ...
    if (clip->anim) {
      if (clip->flag & MCLIP_USE_PROXY_CUSTOM_DIR) {
        ...
        STRNCPY(dir, clip->proxy.dir);
        BLI_path_apply_variables(
            dir, BKE_build_path_variables(BKE_main_blendfile_path_from_global(), nullptr));
...
MovieClip *BKE_movieclip_file_add_exists_ex(Main *bmain, const char *filepath, bool *r_exists)
{
  ...
  STRNCPY(filepath_abs, filepath);
  /* TODO: is this right?  It's not clear to me that applying `BLI_path_abs()`
       ...
       clip = static_cast<MovieClip *>(clip->id.next))
  {
    ...
    BLI_path_apply_variables(filepath_test,
                             BKE_build_path_variables(ID_BLEND_PATH(bmain, &clip->id), &clip->id));
