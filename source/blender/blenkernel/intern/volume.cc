...
bool BKE_volume_load(const Volume *volume, const Main *bmain)
{
  ...
  CLOG_INFO(LOG_GEOM_VOLUME, "Volume %s: load %s", volume_name, filepath);

  /* Test if file exists. */
  if (!BLI_exists(filepath)) {
    grids.error_msg = BLI_path_basename(filepath) + std::string(" not found");
    CLOG_INFO(LOG_GEOM_VOLUME, "Volume %s: %s", volume_name, grids.error_msg.c_str());
    return false;
  }
...
void BKE_volume_unload(Volume *volume)
{
  ...
  if (grids.filepath[0] != '\0') {
    const char *volume_name = volume->id.name + 2;
    CLOG_INFO(LOG_GEOM_VOLUME, "Volume %s: unload", volume_name);
    grids.clear_all();
  }
#else
  UNUSED_VARS(volume);
