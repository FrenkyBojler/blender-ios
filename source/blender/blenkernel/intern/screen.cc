  ...
  if (region->regiondata) {
    ...
    switch (spacetype) {
      case SPACE_VIEW3D:
        if (region->regiontype == RGN_TYPE_WINDOW) {
          RegionView3D *rv3d = static_cast<RegionView3D *>(region->regiondata);
          writer->write_struct<RegionView3D>(rv3d);

          if (rv3d->localvd) {
            writer->write_struct<RegionView3D>(rv3d->localvd);
          }
          if (rv3d->clipbb) {
            writer->write_struct<BoundBox>(rv3d->clipbb);
          }
        }
        else {
...
{
  writer->write_struct<uiList>(ui_list);
...
static void write_area(BlendWriter *writer, ScrArea *area)
{
  LISTBASE_FOREACH (ARegion *, region, &area->regionbase) {
    ...
    LISTBASE_FOREACH (PanelCategoryStack *, pc_act, &region->panels_category_active) {
      writer->write_struct<PanelCategoryStack>(pc_act);
    }
    ...
    LISTBASE_FOREACH (uiPreview *, ui_preview, &region->ui_previews) {
      writer->write_struct<uiPreview>(ui_preview);
    }

    LISTBASE_FOREACH (uiViewStateLink *, view_state, &region->view_states) {
      writer->write_struct<uiViewStateLink>(view_state);
    }
  }
...
void BKE_screen_area_map_blend_write(BlendWriter *writer, ScrAreaMap *area_map)
{
  ...
  LISTBASE_FOREACH (ScrArea *, area, &area_map->areabase) {
    ...
    writer->write_struct<ScrArea>(area);

    writer->write_struct<ScrGlobalAreaData>(area->global);
