...
ARegion *BKE_area_region_copy(const SpaceType *st, const ARegion *region)
{
  ...
  for (const uiTextboxStateLink &link : region->textbox_states) {
    uiTextboxStateLink *copy = MEM_new<uiTextboxStateLink>("uiTextboxStateLink", link);
    copy->idname = BLI_strdup(link.idname);
...
void BKE_area_region_free(SpaceType *st, ARegion *region)
{
  ...
  for (uiTextboxStateLink &link : region->textbox_states.items_mutable()) {
    BLI_remlink(&region->textbox_states, &link);
    MEM_delete(link.idname);
    MEM_delete(&link);
  }
...
static void direct_link_region(BlendDataReader *reader, ARegion *region, int spacetype)
{
  ...
  BLO_read_struct_list(reader, uiTextboxState, &region->textbox_states);
  for (uiTextboxStateLink &textbox_state : region->textbox_states) {
