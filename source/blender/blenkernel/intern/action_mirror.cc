...
static void action_flip_pchan_rna_paths(bAction *act)
{
  ...
  for (FCurve *fcu : animrig::legacy::fcurves_all(act)) {
    if (!STRPREFIX(fcu->rna_path_ptr, path_pose_prefix)) {
      continue;
    }

    const char *name_esc = fcu->rna_path_ptr + path_pose_prefix_len;
...
void BKE_action_flip_with_pose(bAction *act, Span<Object *> objects)
{
  ...
  for (Object *object : objects) {
    ...
    for (FCurve *fcu : fcurves) {
      fcu_cache.add({fcu->rna_path_ptr, std::nullopt, fcu->array_index}, fcu);
    }
