...
{
  CLOG_DEBUG(LOG_UNDO, "addr=%p, name='%s', type='%s'", us, us->name, us->type->name);
  ...
  if (ok == false) {
    CLOG_DEBUG(LOG_UNDO, "encode callback didn't create undo step");
  }
  return ok;
}
...
{
  UNDO_NESTED_ASSERT(false);
  CLOG_DEBUG(LOG_UNDO, "steps=%d", BLI_listbase_count(&ustack->steps));
...
static bool undosys_stack_push_main(UndoStack *ustack, const char *name, Main *bmain)
{
  ...
  CLOG_DEBUG(LOG_UNDO, "Push main '%s'", name);
...
{
  UNDO_NESTED_ASSERT(false);
  CLOG_INFO(LOG_UNDO, "Initialize type='%s'", ut->name);
...
void BKE_undosys_stack_limit_steps_and_memory(UndoStack *ustack, int steps, size_t memory_limit)
{
  ...
  CLOG_DEBUG(LOG_UNDO, "Limit steps=%d, memory_limit=%zu", steps, memory_limit);
  UndoStep *us;
  ...
  CLOG_DEBUG(LOG_UNDO, "Total steps %zu: data_size_all=%zu", us_count, data_size_all);
                                   ...
                                   const bool use_skip)
{
  ...
  if (us_target == nullptr) {
    CLOG_ERROR(LOG_UNDO, "called with a nullptr target step");
    return false;
  }
  undosys_stack_validate(ustack, true);
  ...
  if (us_reference == nullptr) {
    CLOG_ERROR(LOG_UNDO, "could not find a valid initial active target step as reference");
    return false;
  }
