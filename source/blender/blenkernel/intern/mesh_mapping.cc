                                        ...
                                        Array<int> &r_indices)
{
  ...
  /* Group edges by vertex. Each edge stores two vertices consecutively, so the position in
   * #edge_verts is mapped back to the edge index by dividing by two. */
  offset_indices::reverse_indices_in_groups(edge_verts, offsets, r_indices);
  threading::parallel_for(r_indices.index_range(), 8192, [&](const IndexRange range) {
                                ...
                                MutableSpan<int> face_indices)
{
  ...
  offset_indices::reverse_indices_in_groups(corner_verts, offsets, face_indices, corner_to_face);
}
                                        ...
                                        Array<int> &r_indices)
{
  ...
  offset_indices::reverse_indices_in_groups(
      corner_edges, OffsetIndices<int>(r_offsets), r_indices, corner_to_face);
  return {OffsetIndices<int>(r_offsets), r_indices};
}
