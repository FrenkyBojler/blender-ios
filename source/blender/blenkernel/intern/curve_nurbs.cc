                                      ...
                                      int &r_start_index)
{
  ...
  int start = 0;
  int end = 0;
  for (const int i : IndexRange(wrapped_point_count + degree)) {
  ...
  for (const int i_order : IndexRange(2, degree)) {
    if (end + i_order >= knots.size()) {
      end = wrapped_point_count + degree - i_order;
    }
                           ...
                           BasisCache &basis_cache)
{
  ...
  const int wrapped_point_count = control_points_num(points_num, order, cyclic);

  int eval_point = 0;

  for (const int knot_span : IndexRange::from_begin_end(degree, wrapped_point_count)) {
  ...
  if (!cyclic) {
    calculate_basis_for_point(knots[wrapped_point_count],
                              wrapped_point_count,
                              degree,
                              knots,
                              basis_weights.slice(eval_point * order, order),
