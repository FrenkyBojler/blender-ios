...
size_t GLTexture::calc_read_texture_size(int mip, eGPUDataFormat type) const
{
...
size_t GLTexture::read_size(int mip, eGPUDataFormat type) const
{
  /* AMD Pro driver have a bug that write 8 bytes past buffer size
   * if the texture is big. Note, this seemingly only affected cube map arrays. (see #66573) */
  return calc_read_texture_size(mip, type) + 8;
}
...
void GLTexture::read(int mip, eGPUDataFormat type, void *data)
{
  ...
  size_t texture_size = calc_read_texture_size(mip, type);
