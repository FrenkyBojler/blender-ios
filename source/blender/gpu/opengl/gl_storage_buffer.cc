...
void GLStorageBuf::init()
{
  ...
  const size_t alloc_size = ceil_to_multiple_ul(size_in_bytes_, 16);
  glGenBuffers(1, &ssbo_id_);
  glBindBuffer(GL_SHADER_STORAGE_BUFFER, ssbo_id_);
  glBufferData(GL_SHADER_STORAGE_BUFFER, alloc_size, nullptr, to_gl(this->usage_));
...
void GLStorageBuf::async_flush_to_host()
{
  ...
  if (read_ssbo_id_ == 0) {
    ...
    glBindBuffer(GL_SHADER_STORAGE_BUFFER, read_ssbo_id_);
    glBufferStorage(
        GL_SHADER_STORAGE_BUFFER, alloc_size, nullptr, GL_MAP_PERSISTENT_BIT | GL_MAP_READ_BIT);
    persistent_ptr_ = glMapBufferRange(
        GL_SHADER_STORAGE_BUFFER, 0, alloc_size, GL_MAP_PERSISTENT_BIT | GL_MAP_READ_BIT);
  ...
  if (GLContext::direct_state_access_support) {
    glCopyNamedBufferSubData(ssbo_id_, read_ssbo_id_, 0, 0, alloc_size);
  }
  else {
