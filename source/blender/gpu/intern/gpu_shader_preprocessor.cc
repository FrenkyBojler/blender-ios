...
class Parser {
 ...
 public:
  explicit Parser(Token start) : tok(start) {}

  int64_t parse()
  {
    int64_t v = expression(0);
    if (peek() != Invalid) {
 ...
 private:
  int64_t expression(int right_binding_power)
  {
  ...
  int64_t nud(const Token &t)
  {
    switch (t.type()) {
      ...
      case Plus:
        return +expression(unary_binding_power);
      case Minus:
        return -expression(unary_binding_power);
      case Not:
        return !expression(unary_binding_power);
      ...
      case ParOpen: {
        ...
        int64_t v = expression(binding_power(ParOpen));
  ...
  int64_t led(int64_t left, const Token &t)
  {
    switch (t.type()) {
      case Multiply:
        return left * expression(binding_power(Multiply));
      case Divide:
        return left / expression(binding_power(Divide));
      case Modulo:
        return left % expression(binding_power(Modulo));
      case Plus:
        return left + expression(binding_power(Plus));
      case Minus:
        return left - expression(binding_power(Minus));
...
#endif
      case LThan:
        return left < expression(binding_power(LThan));
      case LEqual:
        return left <= expression(binding_power(LEqual));
      case GThan:
        return left > expression(binding_power(GThan));
      case GEqual:
        return left >= expression(binding_power(GEqual));
      case Equal:
        return left == expression(binding_power(Equal));
      case NotEqual:
        return left != expression(binding_power(NotEqual));
      case And:
        return left & expression(binding_power(And));
      case Xor:
        return left ^ expression(binding_power(Xor));
      case Or:
        return left | expression(binding_power(Or));
      case LogicalAnd:
        return left && expression(binding_power(LogicalAnd));
      case LogicalOr:
        return left || expression(binding_power(LogicalOr));
      case Question: {
        int64_t tval = expression(binding_power(Question));
        if (consume().type() != Colon) {
          throw std::runtime_error("Expected ':'");
        }
        int64_t fval = expression(binding_power(Colon));
