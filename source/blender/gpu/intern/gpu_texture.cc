...
{
  BLI_assert(texture);
  size_t size = GPU_texture_read_size(texture, data_format, mip_level);
...
size_t GPU_texture_read_size(Texture *texture, eGPUDataFormat data_format, int mip_level)
{
  BLI_assert(texture);
  return texture->read_size(mip_level, data_format);
}
