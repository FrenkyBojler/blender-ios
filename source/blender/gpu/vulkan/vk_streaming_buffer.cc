...
namespace blender::gpu {
    VKStreamingBuffer::VKStreamingBuffer(VKBuffer&buffer):vk_buffer_dst_(buffer.vk_handle()), vk_buffer_size_(buffer.size_in_bytes())
    ...
    VkDeviceSize VKStreamingBuffer::update(VKContext&context, const void*data, size_t data_size){
        ...
        const bool allocate_new_buffer = !(host_buffer_.has_value() && data_size < host_buffer_.value().get()->size_in_bytes() - offset_);
        ...
        offset_ += data_size;
        memcpy(static_cast<void*>(static_cast<uint8_t*>(host_buffer.mapped_memory_get()) + start_offset), data, data_size);


            render_graph::VKCopyBufferNode::Data &copy_buffer_data = render_graph.get_node_data(copy_buffer_handle_);
