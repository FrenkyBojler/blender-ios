...
VKBuffer &VKImmediate::ensure_space(VkDeviceSize bytes_needed, VkDeviceSize offset_alignment)
{
  ...
  {
    CLOG_DEBUG(LOG_GPU_VULKAN, "Activating recycled buffer");
    buffer_offset_ = 0;
  ...
  CLOG_DEBUG(LOG_GPU_VULKAN, "Allocate buffer (size=%d)", int(alloc_size));
  buffer_offset_ = 0;
...
{
  if (!recycling_buffers_.is_empty()) {
    CLOG_DEBUG(LOG_GPU_VULKAN, "Discarding %d unused buffers", int(recycling_buffers_.size()));
  }
