...
{
  functions->vkCmdSetStencilCompareMask(vk_command_buffer_, VK_STENCIL_FACE_FRONT_AND_BACK, compare_mask);
}
void VKCommandBufferWrapper::set_stencil_write_mask(const uint32_t write_mask)
{
  functions->vkCmdSetStencilWriteMask(vk_command_buffer_, VK_STENCIL_FACE_FRONT_AND_BACK, write_mask);
}
void VKCommandBufferWrapper::set_stencil_reference(const uint32_t reference)
{
  functions->vkCmdSetStencilReference(vk_command_buffer_, VK_STENCIL_FACE_FRONT_AND_BACK, reference);
}
...
{
  const VKDevice &device = VKBackend::get().device;
  device.functions.vkCmdBindDescriptorBuffersEXT(
      vk_command_buffer_, buffer_count, p_binding_infos);
}
...
{
  const VKDevice &device = VKBackend::get().device;
  device.functions.vkCmdSetDescriptorBufferOffsetsEXT(vk_command_buffer_,
                                                      pipeline_bind_point,
                                                      layout,
                                                      first_set,
                                                      set_count,
