                     ...
                     uint32_t bindless_table_size)
{
  ...
  /* Bindless table information. */
  align<LayoutT>(shader::Type::uint_t, bindless_table_size, &offset);
  reserve<LayoutT>(shader::Type::uint_t, bindless_table_size, &offset);
...
VKPushConstants::StorageType VKPushConstants::Layout::determine_storage_type(
    ...
    const uint32_t bindless_table_size)
{
  if (info.push_constants_.is_empty() && bindless_table_size == 0) {
    return StorageType::NONE;
  }

  uint32_t max_push_constants_size =
      device.physical_device_properties_get().limits.maxPushConstantsSize;
  uint32_t size = struct_size<Std430>(info.push_constants_, bindless_table_size);
                 ...
                 uint32_t *r_offset)
{
  ...
  // Add bindings table if it exists.
  if (interface.bindings_table_size_get() > 0) {
    shader::ShaderCreateInfo::PushConst bindings_table_push_constant{
        shader::Type::uint_t, "bindings_table", (int)interface.bindings_table_size_get()};
    const ShaderInput *shader_input = interface.uniform_get(
        bindings_table_push_constant.name.c_str());
    r_struct.append(init_constant<LayoutT>(bindings_table_push_constant, *shader_input, r_offset));
  }
