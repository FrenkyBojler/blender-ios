...
void mapping_point(float3 vector, float3 location, float3 rotation, float3 scale, out float3 result)
{
  result = (from_rotation(as_EulerXYZ(rotation)) * (vector * scale)) + location;
}

void mapping_texture(float3 vector, float3 location, float3 rotation, float3 scale, out float3 result)
{
  result = safe_divide(transpose(from_rotation(as_EulerXYZ(rotation))) * (vector - location),
                       scale);
}

void mapping_vector(float3 vector, float3 location, float3 rotation, float3 scale, out float3 result)
{
  result = from_rotation(as_EulerXYZ(rotation)) * (vector * scale);
}

void mapping_normal(float3 vector, float3 location, float3 rotation, float3 scale, out float3 result)
{
