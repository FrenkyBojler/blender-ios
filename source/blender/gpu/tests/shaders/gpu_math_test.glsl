...
void main()
{
  ...
  {
    float2x2 m2 = float2x2(vec2(1, 2), vec2(3, 4));
    float3x3 m3 = float3x3(vec3(1, 2, 3), vec3(-3, 4, -5), vec3(5, -6, 7));
    float4x4 m4 = float4x4(
        vec4(1, 2, -3, 3), vec4(3, 4, -5, 3), vec4(5, 6, 7, -3), vec4(5, 6, 7, 1));
  ...
  TEST(math_matrix, MatrixAdjoint)
  {
    ...
    float4x4 m4 = float4x4(
        vec4(1, 2, -3, 3), vec4(3, 4, -5, 3), vec4(5, 6, 7, -3), vec4(5, 6, 7, 1));
    float2x2 expect2 = transpose(float2x2(vec2(4, -3), vec2(-2, 1)));
    float3x3 expect3 = transpose(float3x3(vec3(-2, -4, -2), vec3(-32, -8, 16), vec3(-22, -4, 10)));
    float4x4 expect4 = transpose(float4x4(vec4(232, -184, -8, -0),
                                          vec4(-128, 88, 16, 0),
                                          vec4(80, -76, 4, 28),
                                          vec4(-72, 60, -12, -28)));
  ...
  TEST(math_matrix, MatrixInit)
  {
    ...
    float4x4 m = from_location(vec3(1, 2, 3));
    expect = float4x4(vec4(1, 0, 0, 0), vec4(0, 1, 0, 0), vec4(0, 0, 1, 0), vec4(1, 2, 3, 1));
    EXPECT_TRUE(is_equal(m, expect, 0.00001f));

    expect = transpose(float4x4(vec4(0.411982f, -0.833738f, -0.36763f, 0),
    ...
    expect = float4x4(vec4(1, 0, 0, 0), vec4(0, 2, 0, 0), vec4(0, 0, 3, 0), vec4(0, 0, 0, 4));
    EXPECT_TRUE(is_equal(m, expect, 0.00001f));

    m = to_float4x4(from_scale(vec3(1, 2, 3)));
    expect = float4x4(vec4(1, 0, 0, 0), vec4(0, 2, 0, 0), vec4(0, 0, 3, 0), vec4(0, 0, 0, 1));
    EXPECT_TRUE(is_equal(m, expect, 0.00001f));

    m = to_float4x4(from_scale(vec2(1, 2)));
    expect = float4x4(vec4(1, 0, 0, 0), vec4(0, 2, 0, 0), vec4(0, 0, 1, 0), vec4(0, 0, 0, 1));
    EXPECT_TRUE(is_equal(m, expect, 0.00001f));

    m = from_loc_rot(vec3(1, 2, 3), EulerXYZ(1, 2, 3));
    expect = float4x4(vec4(0.411982f, -0.0587266f, -0.909297f, 0),
  ...
  TEST(math_matrix, MatrixModify)
  {
    ...
    float4x4 m1 = float4x4(vec4(0, 3, 0, 0), vec4(2, 0, 0, 0), vec4(0, 0, 2, 0), vec4(0, 0, 0, 1));

    expect = float4x4(vec4(0, 3, 0, 0), vec4(2, 0, 0, 0), vec4(0, 0, 2, 0), vec4(4, 9, 2, 1));
    result = translate(m1, vec3(3, 2, 1));
    EXPECT_NEAR(result, expect, epsilon);

    expect = float4x4(vec4(0, 3, 0, 0), vec4(2, 0, 0, 0), vec4(0, 0, 2, 0), vec4(4, 0, 0, 1));
    result = translate(m1, vec2(0, 2));
    EXPECT_NEAR(result, expect, epsilon);

    expect = float4x4(vec4(0, 0, -2, 0), vec4(2, 0, 0, 0), vec4(0, 3, 0, 0), vec4(0, 0, 0, 1));
    result = rotate(m1, AxisAngle(vec3(0, 1, 0), M_PI_2));
    EXPECT_NEAR(result, expect, epsilon);

    expect = float4x4(vec4(0, 9, 0, 0), vec4(4, 0, 0, 0), vec4(0, 0, 8, 0), vec4(0, 0, 0, 1));
    result = scale(m1, vec3(3, 2, 4));
    EXPECT_NEAR(result, expect, epsilon);

    expect = float4x4(vec4(0, 9, 0, 0), vec4(4, 0, 0, 0), vec4(0, 0, 2, 0), vec4(0, 0, 0, 1));
  ...
  {
    float4x4 m1 = float4x4(vec4(0, 3, 0, 0), vec4(2, 0, 0, 0), vec4(0, 0, 2, 0), vec4(0, 0, 0, 1));
    float4x4 m2 = float4x4(vec4(0, 3.001f, 0, 0),
                           vec4(1.999f, 0, 0, 0),
                           vec4(0, 0, 2.001f, 0),
                           vec4(0, 0, 0, 1.001f));
    float4x4 m3 = float4x4(
        vec4(0, 3.001f, 0, 0), vec4(1, 1, 0, 0), vec4(0, 0, 2.001f, 0), vec4(0, 0, 0, 1.001f));
    float4x4 m4 = float4x4(vec4(0, 1, 0, 0), vec4(1, 0, 0, 0), vec4(0, 0, 1, 0), vec4(0, 0, 0, 1));
    float4x4 m5 = float4x4(vec4(0, 0, 0, 0), vec4(0, 0, 0, 0), vec4(0, 0, 0, 0), vec4(0, 0, 0, 0));
    float4x4 m6 = float4x4(vec4(1, 0, 0, 0), vec4(0, 1, 0, 0), vec4(0, 0, 1, 0), vec4(0, 0, 0, 1));
  ...
  {
    float4x4 m = float4x4(vec4(0, 3, 0, 0), vec4(2, 0, 0, 0), vec4(0, 0, 2, 0), vec4(0, 1, 0, 1));
  ...
  {
    float4x4 m = float4x4(vec4(1, 2, 3, 4), vec4(5, 6, 7, 8), vec4(9, 1, 2, 3), vec4(2, 5, 6, 7));
    float4x4 expect = float4x4(
        vec4(1, 5, 9, 2), vec4(2, 6, 1, 5), vec4(3, 7, 2, 6), vec4(4, 8, 3, 7));
  ...
  TEST(math_matrix, MatrixInterpolationRegular)
  {
    ...
    float4x4 m2 = transpose(float4x4(vec4(0.224976f, -0.333770f, 0.765074f, 0.100000f),
    ...
    float4x4 expect = transpose(float4x4(vec4(0.690643f, -0.253244f, 0.484996f, 0.050000f),
  ...
  TEST(math_matrix, MatrixTransform)
  {
    ...
    float4x4 m4 = from_loc_rot(vec3(10, 0, 0), EulerXYZ(M_PI_2, M_PI_2, M_PI_2));
    ...
    float3x3 pers3 = float3x3(vec3(1, 0, 0.1f), vec3(0, 1, 0.1f), vec3(0, 0.1f, 1));
