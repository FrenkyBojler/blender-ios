...
static void blend_write(BlendWriter *writer, const ID * /*id_owner*/, const ModifierData *md)
{
  ...
  writer->write_struct_array<NodesModifierBake>(nmd->bakes_num, nmd->bakes);
  for (const NodesModifierBake &bake : Span(nmd->bakes, nmd->bakes_num)) {
    BLO_write_string(writer, bake.directory);

    writer->write_struct_array<NodesModifierDataBlock>(bake.data_blocks_num, bake.data_blocks);
    ...
    if (bake.packed) {
      ...
      writer->write_struct_array<NodesModifierBakeFile>(bake.packed->meta_files_num,
                                                        bake.packed->meta_files);
      writer->write_struct_array<NodesModifierBakeFile>(bake.packed->blob_files_num,
  ...
  writer->write_struct_array<NodesModifierPanel>(nmd->panels_num, nmd->panels);
