...
static void skin_fix_hull_topology(BMesh *bm, SkinNode *skin_nodes, int verts_num)
{
  ...
  for (v = 0; v < verts_num; v++) {
    ...
    for (j = 0; j < sn->totframe; j++) {
      ...
      if (f->detached) {
        /* It is important to check for coincident vertices *before* detaching because
         * detaching duplicates the frame vertices, losing their identity with hull vertices.
         * Note: `skin_hole_target_face` uses vertex positions, not identity, so the call order doesn't
         * matter for it - but the coincidence check requires the original vertex pointers. */
