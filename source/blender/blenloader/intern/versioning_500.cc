  ...
  if (!MAIN_VERSION_FILE_ATLEAST(bmain, 500, 115)) {
    FOREACH_NODETREE_BEGIN (bmain, node_tree, id) {
      if (ELEM(GS(id->name), ID_MA, ID_LA, ID_WO, ID_TE, ID_SCE, ID_LS)) {
        /* These node trees should not have interface sockets. However, it was possible to add
         * interface sockets through the Python API. Remove these interface sockets here before
         * they cause problems further down the line. */
