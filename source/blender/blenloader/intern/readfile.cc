...
static bool scene_validate_setscene__liblink(Scene *sce, const int totscene)
{
  ...
  for (a = 0, sce_iter = sce; sce_iter->set; sce_iter = sce_iter->set, a++) {
     ...
     * #ID_Readfile_Data::Tags.is_id_need_link the library pointer check is sufficient. */
    ...
    FileData *fd, Main *main, const ID * /*id*/, ID *id_old, BHead *bhead, const int id_tag)
{
  ...
  /* Also do not set #ID_Readfile_Data::Tags.is_id_need_link, this ID will never be re-liblinked,
...
static void read_libblock_undo_restore_at_old_address(FileData *fd, Main *main, ID *id, ID *id_old)
{
  ...
  BLO_readfile_id_runtime_tags_for_write(*id_old).is_id_need_link = true;
                            ...
                            ID **r_id)
{
   ...
   * to be done still. */
  id_tag |= ID_TAG_NEW;
  id_read_tags.is_id_need_link = true;
...
static void lib_link_all(FileData *fd, Main *bmain)
{
  ...
  FOREACH_MAIN_ID_BEGIN (bmain, id) {
    ...
    if ((id->tag & (ID_TAG_UNDO_OLD_ID_REUSED_UNCHANGED | ID_TAG_UNDO_OLD_ID_REUSED_NOUNDO)) != 0)
    {
       ...
       * it at all. */
      BLI_assert(!BLO_readfile_id_runtime_tags(*id).is_id_need_link);
    ...
    if (BLO_readfile_id_runtime_tags(*id).is_id_need_link) {
      ...
      BLO_readfile_id_runtime_tags_for_write(*id).is_id_need_link = false;
    }
...
static void expand_doit_library(void *fdhandle, Main *mainvar, void *old)
{
  ...
  if (bhead->code == ID_LINK_PLACEHOLDER) {
    ...
    if (id == nullptr) {
      ID_Readfile_Data::Tags id_read_tags{};
      id_read_tags.is_id_need_expand = true;
      read_libblock(
...
void BLO_expand_main(void *fdhandle, Main *mainvar, BLOExpandDoitCallback callback)
{
  ...
  for (bool do_it = true; do_it;) {
    ...
    FOREACH_MAIN_ID_BEGIN (mainvar, id_iter) {
      if (!BLO_readfile_id_runtime_tags(*id_iter).is_id_need_expand) {
        continue;
      }
      ...
      do_it = true;
      BLO_readfile_id_runtime_tags_for_write(*id_iter).is_id_need_expand = false;
    }
    FOREACH_MAIN_ID_END;
  }
}
    ...
    FileData *basefd, FileData *fd, Main *mainvar, ID *id, ID **r_id)
{
  ...
  BLO_readfile_id_runtime_tags_for_write(*id).is_id_link_placeholder = false;
  id->flag &= ~ID_FLAG_INDIRECT_WEAK_LINK;

  if (bhead) {
    BLO_readfile_id_runtime_tags_for_write(*id).is_id_need_expand = true;
