...
void blo_do_versions_pre250(FileData *fd, Library *lib, Main *bmain)
{
  ...
  if (bmain->versionfile == 100) {
    ...
    while (tex) {
      if (BLO_readfile_id_runtime_tags(tex->id).is_id_need_link) {
        if (tex->extend == 0) {
  ...
  if (!MAIN_VERSION_FILE_ATLEAST(bmain, 245, 8)) {
         ...
         ob = static_cast<Object *>(ob->id.next))
    {
      ...
      if ((paf = BKE_object_do_version_give_parteff_245(ob))) {
        ...
        part->id.us--;
        BLO_readfile_id_runtime_tags_for_write(part->id).is_id_need_link =
            BLO_readfile_id_runtime_tags(ob->id).is_id_need_link;
