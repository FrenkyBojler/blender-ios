                                        ...
                                        bool *have_function)
{
  ...
  LISTBASE_FOREACH (Link *, link, lb) {
    ...
    PyObject *item;
    PyObject_GetOptionalAttrString(py_class, RNA_function_identifier(func), &item);
  ...
  LISTBASE_FOREACH (Link *, link, lb) {
    ...
    switch (PyObject_GetOptionalAttrString(py_class, identifier, &item)) {
      ...
      case 0: {
        /* Not found, use fallbacks. */

        /* Sneaky workaround to use the class name as the `bl_idname`. */
        int i;
        for (i = 0; i < ARRAY_SIZE(bpy_property_substitutions); i += 1) {
          const char *rna_attr = bpy_property_substitutions[i].rna_attr;
