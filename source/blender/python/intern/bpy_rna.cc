...
PyDoc_STRVAR(
    ...
    "   Fast access to a basic-type attribute within a collection.\n"
    "\n"
    "   :param attr: Name of the item attribute to write (for example ``co``, ``normal`` or\n"
    "      ``select``). The attribute must be a basic type (bool, int or float).\n"
    "      If unsure, inspect the available properties on the collection item type.\n"
    "      For ``Attribute.data`` collections, common mappings are:\n"
    "      ``FLOAT``, ``INT``, ``BOOLEAN``, ``INT32_2D``, ``QUATERNION`` and ``FLOAT4X4``\n"
    "      use ``value``; ``FLOAT_VECTOR``, ``FLOAT2`` and ``FLOAT4`` use ``vector``;\n"
    "      ``FLOAT_COLOR`` and ``BYTE_COLOR`` use ``color``.\n"
    "   :type attr: str\n"
