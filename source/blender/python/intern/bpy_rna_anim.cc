                                       ...
                                       eBezTriple_KeyframeType *r_keytype)
{
  ...
  /* Missing or `None` leaves the caller-initialized `FLT_MAX` sentinel in place,
   * which is later resolved to `bpy.context.scene.frame_current`. */
  if (cfra.has_value()) {
    *r_cfra = *cfra;
  }
...
char pyrna_struct_keyframe_insert_doc[] =
    ".. method:: keyframe_insert(data_path, *, index=-1, frame=None, "
...
char pyrna_struct_keyframe_delete_doc[] =
    ".. method:: keyframe_delete(data_path, *, index=-1, frame=None, "
    "group=\"\")\n"
    "\n"
