...
/* -------------------------------------------------------------------- */
/** \name Quaternion Type: `Vectorcall Protocol` / `mathutils.Quaternion()`
 * \{ */
                                       ...
                                       PyObject *kwnames)
{
  ...
  switch (nargs) {
    case 0:
      break;
    case 1: {
      int size = mathutils_array_parse(quat, 3, QUAT_SIZE, args[0], "mathutils.Quaternion()");
    ...
    case 2: {
      {
        ...
        if (UNLIKELY(angle == -1.0 && PyErr_Occurred())) {
          PyErr_Format(PyExc_TypeError,
                       "mathutils.Quaternion(): angle must be a real number, not '%.200s'",
    ...
    default: {
      PyErr_Format(
          PyExc_TypeError, "mathutils.Quaternion() takes at most 2 arguments (%zd given)", nargs);
      return nullptr;
    }
  }
