...
{
  /* `info.buf` is asserted non-null at construction; #MEM_delete_void also
   * accepts null defensively. */
...
PyObject *PyC_MemoryView_FromBufferOwned(const Py_buffer *info)
{
  ...
  if (PyType_Ready(&BPyMemoryViewOwned_Type) < 0) {
