...
#define PY_IMBUF_CHECK_EXPORTS(obj) \
 ...
 */
static void py_imbuf_region_sanitize(const ImBuf *ibuf, rcti *region, bool *r_use_region)
{
  ...
  *r_use_region = !(region->xmin == 0 && region->ymin == 0 && region->xmax == ibuf->x &&
                    region->ymax == ibuf->y);
}
...
static PyObject *py_imbuf_with_buffer(Py_ImBuf *self, PyObject *args, PyObject *kw)
{
  ...
  if (region.has_value()) {
    py_imbuf_region_sanitize(ibuf, &region.value(), &use_region);
  }
