...
const EnumPropertyItem *USD_build_hook_enum()
{
  ...
  while (hook_iter != hook_list().end()) {
    ...
    ++hook_iter;
    item_tmp.identifier = hook->idname;
    item_tmp.description = hook->description;
                                ...
                                ReportList *reports)
{
  ...
  on_material_import.call(import_params.filter_hooks);
