...
void process_scene_graph_instances(pxr::UsdStageRefPtr stage)
{
  ...
  for (const pxr::SdfPath& proto_path : protos)
  {
    ...
    if (!dest_prim)
    {
      CLOG_ERROR(&LOG, "Couldn't create destination prim %s for copying protoype %s",
   ...
   */
  for (PathMap::reverse_iterator riter = proto_to_copy_map.rbegin(); riter != proto_to_copy_map.rend(); ++riter)
  {
    const pxr::SdfPath &src_path = riter->first;
    const pxr::SdfPath &dst_path = riter->second;
    if (!pxr::SdfCopySpec(stage->GetRootLayer(), riter->first, stage->GetRootLayer(), riter->second)) {
      CLOG_WARN(&LOG,
