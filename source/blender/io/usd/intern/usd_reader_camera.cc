...
void USDCameraReader::read_object_data(Main *bmain, const double motionSampleTime)
{
  ...
  bcam->clip_start = max_ff(1e-6f, clippingRangeVal.UncheckedGet<pxr::GfVec2f>()[0]) * import_params_.scale;
  bcam->clip_end = clippingRangeVal.UncheckedGet<pxr::GfVec2f>()[1] * import_params_.scale;

  bcam->dof.focus_distance = focalDistanceVal.Get<float>() * import_params_.scale;
