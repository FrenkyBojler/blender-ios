...
struct VolumeModule {
  ...
  blender::gpu::Texture *grid_default_texture(eGPUDefaultValue default_value)
  {
                                   ...
                                   ListBaseWrapper<GPUMaterialAttribute> &attrs)
{
  ...
  for (const GPUMaterialAttribute *attr : attrs) {
    ...
    const blender::gpu::Texture *grid_tex = (drw_grid) ?
                                                drw_grid->texture :
                                            (volume_grid) ?
                                                module.dummy_zero :
                                                module.grid_default_texture(attr->default_value);
    /* TODO(@pragma37): bind_texture const support ? */
    sub->bind_texture(attr->input_name, (blender::gpu::Texture *)grid_tex);
