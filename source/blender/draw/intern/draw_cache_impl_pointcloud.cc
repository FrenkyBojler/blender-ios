...
static void pointcloud_extract_indices(const PointCloud &pointcloud, PointCloudBatchCache &cache)
{
   ...
   * - pointcloud.totpoint is an int we assume that we can safely use 31 bits.
   */
  const uint32_t max_totpoint = INT32_MAX / uint32_t(tri_count_per_point * GPU_indexbuf_primitive_len(GPU_PRIM_TRIS));
