...
static void *viewport_system_gpu_context = nullptr;
/** GPUContext associated to the viewport_system_gpu_context. */
static GPUContext *viewport_blender_gpu_context = nullptr;
/**
 ...
 */
static TicketMutex *viewport_system_gpu_context_mutex = nullptr;

/** Unique ghost context used by Preview jobs. */
static void *preview_system_gpu_context = nullptr;
/** GPUContext associated to the preview_system_gpu_context. */
static GPUContext *preview_blender_gpu_context = nullptr;
static TicketMutex *preview_system_gpu_context_mutex = nullptr;

void DRW_gpu_context_create()
{
  BLI_assert(viewport_system_gpu_context == nullptr); /* Ensure it's called once */

  viewport_system_gpu_context_mutex = BLI_ticket_mutex_alloc();
  preview_system_gpu_context_mutex = BLI_ticket_mutex_alloc();
  DRW_submission_mutex_init();

  /* This changes the active context. */
  preview_system_gpu_context = WM_system_gpu_context_create();
  WM_system_gpu_context_activate(preview_system_gpu_context);
  /* Be sure to create viewport_blender_gpu_context too. */
  preview_blender_gpu_context = GPU_context_create(nullptr, viewport_system_gpu_context);

  /* This changes the active context. */
  viewport_system_gpu_context = WM_system_gpu_context_create();
  WM_system_gpu_context_activate(viewport_system_gpu_context);
  /* Be sure to create viewport_blender_gpu_context too. */
  viewport_blender_gpu_context = GPU_context_create(nullptr, viewport_system_gpu_context);
  ...
  WM_system_gpu_context_release(viewport_system_gpu_context);
...
void DRW_gpu_context_destroy()
{
  ...
  if (viewport_system_gpu_context != nullptr) {
    DRW_shader_exit();
    WM_system_gpu_context_activate(viewport_system_gpu_context);
    GPU_context_active_set(viewport_blender_gpu_context);
    GPU_context_discard(viewport_blender_gpu_context);
    WM_system_gpu_context_dispose(viewport_system_gpu_context);
    DRW_submission_mutex_exit();
    BLI_ticket_mutex_free(viewport_system_gpu_context_mutex);

    if (preview_system_gpu_context != nullptr) {
      WM_system_gpu_context_activate(preview_system_gpu_context);
      GPU_context_active_set(preview_blender_gpu_context);
      GPU_context_discard(preview_blender_gpu_context);
      WM_system_gpu_context_dispose(preview_system_gpu_context);
      BLI_ticket_mutex_free(preview_system_gpu_context_mutex);
    }
  }
}
...
void DRW_gpu_context_enable_ex(bool /*restore*/)
{
  if (viewport_system_gpu_context != nullptr) {
     ...
     * multiple threads. */
    BLI_ticket_mutex_lock(viewport_system_gpu_context_mutex);
    GPU_render_begin();
    WM_system_gpu_context_activate(viewport_system_gpu_context);
    GPU_context_active_set(viewport_blender_gpu_context);
    GPU_context_begin_frame(viewport_blender_gpu_context);
  }
}
...
{
  if (viewport_system_gpu_context != nullptr) {
    GPU_context_end_frame(viewport_blender_gpu_context);
    ...
    BLI_ticket_mutex_unlock(viewport_system_gpu_context_mutex);
  }
}
...
static void drw_gpu_preview_context_enable()
{
  if (preview_system_gpu_context != nullptr) {
     ...
     * This shall remain in effect until immediate mode supports
     * multiple threads. */
    BLI_ticket_mutex_lock(preview_system_gpu_context_mutex);
    GPU_render_begin();
    WM_system_gpu_context_activate(preview_system_gpu_context);
    GPU_context_active_set(preview_blender_gpu_context);
    GPU_context_begin_frame(preview_blender_gpu_context);
  }
}
...
{
  if (preview_system_gpu_context != nullptr) {
    GPU_context_end_frame(preview_blender_gpu_context);

    WM_system_gpu_context_release(preview_system_gpu_context);
    GPU_context_active_set(nullptr);
    ...
    BLI_ticket_mutex_unlock(preview_system_gpu_context_mutex);
  }
}
...
void DRW_gpu_context_enable()
{
  ...
  if (G.background && viewport_system_gpu_context == nullptr) {
    WM_init_gpu();
  }
...
void DRW_render_context_enable(Render *render)
{
  ...
  if (re_viewport_system_gpu_context != nullptr) {
    ...
    void *re_viewport_blender_gpu_context = RE_blender_gpu_context_ensure(render);
    DRW_blender_gpu_render_context_enable(re_viewport_blender_gpu_context);
  }
  else {
...
void DRW_render_context_disable(Render *render)
{
  ...
  if (re_viewport_system_gpu_context != nullptr) {
    ...
    DRW_blender_gpu_render_context_disable(re_viewport_blender_gpu_context);
    GPU_render_end();
...
void *DRW_system_gpu_context_get()
{
  ...
  return viewport_system_gpu_context;
}
...
void *DRW_xr_blender_gpu_context_get()
{
  ...
  return viewport_blender_gpu_context;
}
...
bool DRW_gpu_context_release()
{
  ...
  if (GPU_context_active_get() != viewport_blender_gpu_context) {
