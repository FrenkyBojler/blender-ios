...
static VectorSet<UString> mesh_extract_uv_format_init(GPUVertFormat *format,
                                                      ...
                                                      const MeshExtractType extract_type)
{
  ...
  VectorSet<UString> uv_layers;
  for (const UString name : cache.cd_used.uv.as_span().take_front(MAX_MTFACE)) {
    uv_layers.add_new(name);
  }

  const UString active_name = mesh.active_or_default_uv_map_name();
  const UString default_name = mesh.default_uv_map_name();

  /* HACK to fix #68857 */
  if (extract_type == MeshExtractType::BMesh && cache.cd_used.edit_uv == 1) {
    if (!bke::attribute_name_is_anonymous(default_name.ref())) {
      uv_layers.add(default_name);
    }
  }

  const UString stencil_name = [&]() -> UString {
    UString name(mesh.stencil_uv_map_attribute);
    if (name.is_empty()) {
  ...
  for (const UString name : uv_layers) {
...
gpu::VertBufPtr extract_uv_maps(const MeshRenderData &mr, const MeshBatchCache &cache)
{
  ...
  const VectorSet<UString> uv_layers = mesh_extract_uv_format_init(
    ...
    if (mr.extract_type == MeshExtractType::BMesh) {
      ...
      for (const int i : uv_layers.index_range()) {
        ...
        const int offset = CustomData_get_offset_named(
            &bm.ldata, CD_PROP_FLOAT2, uv_layers[i].ref());
...
static VectorSet<UString> all_uv_map_attributes(const Mesh &mesh)
{
  return mesh.uv_map_names();
}
                                       ...
                                       const MeshBatchCache &cache)
{
  ...
  const VectorSet<UString> all_uv_maps = all_uv_map_attributes(*coarse_mesh);
  ...
  for (const int pack_layer_index : uv_layers.index_range()) {
    const UString name = uv_layers[pack_layer_index];
