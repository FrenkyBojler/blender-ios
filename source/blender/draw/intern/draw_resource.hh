  ...
  switch (GS(reinterpret_cast<ID *>(ref.object->data)->name)) {
    case ID_VO: {
      std::optional<const blender::Bounds<float3>> bounds = BKE_volume_min_max(
          &DRW_object_get_data<const Volume>(*ref.object));
      if (bounds) {
    ...
    case ID_ME: {
      BKE_mesh_texspace_get(&DRW_object_get_data<Mesh>(*ref.object), orco_add, orco_mul);
      break;
    }
    case ID_CU_LEGACY: {
      Curve &cu = DRW_object_get_data<Curve>(*ref.object);
    ...
    case ID_MB: {
      MetaBall &mb = DRW_object_get_data<MetaBall>(*ref.object);
