...
void main() {
  ...
  /* Early exit with reduced constant cache pressure */
  ...
  vec3 thread_min = vec3(INIT_MIN);
  vec3 thread_max = vec3(INIT_MAX);
  bool found_valid = false;

  /* Fully unrolled loop - eliminates loop overhead and improves instruction scheduling */
  uint idx0 = base_idx;
  ...
  if (idx0 < num_vertices) {
    vec3 pos0 = skinned_positions[idx0].xyz;
  ...
  if (idx1 < num_vertices) {
    vec3 pos1 = skinned_positions[idx1].xyz;
  ...
  if (idx2 < num_vertices) {
    vec3 pos2 = skinned_positions[idx2].xyz;
  ...
  if (idx3 < num_vertices) {
    vec3 pos3 = skinned_positions[idx3].xyz;
  ...
  if (idx4 < num_vertices) {
    vec3 pos4 = skinned_positions[idx4].xyz;
  ...
  if (idx5 < num_vertices) {
    vec3 pos5 = skinned_positions[idx5].xyz;
  ...
  if (idx6 < num_vertices) {
    vec3 pos6 = skinned_positions[idx6].xyz;
  ...
  if (idx7 < num_vertices) {
    vec3 pos7 = skinned_positions[idx7].xyz;
