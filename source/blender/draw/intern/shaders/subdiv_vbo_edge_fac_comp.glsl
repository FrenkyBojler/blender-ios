...
float compute_line_factor(uint corner_index, vec3 face_normal)
{
  ...
  vec3 v0 = subdiv_get_vertex_pos(positions[start_corner_index_other + 0]);
  vec3 v1 = subdiv_get_vertex_pos(positions[start_corner_index_other + 1);
  vec3 v2 = subdiv_get_vertex_pos(positions[start_corner_index_other + 2]);
