...
bool intersect_view(IsectBox i_box, uint view_id = drw_view_id)
{
  ...
  for (int p = 0; p < 6; ++p) {
    bool is_any_vertex_on_positive_side = false;
    for (int v = 0; v < 8; ++v) {
      float test = dot(drw_view_culling(view_id).frustum_planes.planes[p],
  ...
  for (int p = 0; p < 6; ++p) {
    ...
    for (int v = 0; v < 8; ++v) {
      float test = dot(i_box.planes[p],
                       float4(drw_view_culling(view_id).frustum_corners.corners[v].xyz, 1.0f));
      if (test > 0.0f) {
        is_any_vertex_on_positive_side = true;
        break;
      }
...
bool intersect_view(Sphere sphere, uint view_id = drw_view_id)
{
  ...
  for (int p = 0; p < 6 && intersects; ++p) {
    float dist_to_plane = dot(drw_view_culling(view_id).frustum_planes.planes[p],
