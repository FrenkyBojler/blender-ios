...
void main()
{
  vec4 texel = vec4(gl_FragCoord.xyxy) * 2.0f + vec4(0.0f, 0.0f, 1.0f, 1.0f);
  texel = (texel - 0.5f) / vec4(textureSize(sceneColorTex, 0).xyxy);
   ...
   * texel values. */
  vec4 color1 = textureLod(sceneColorTex, texel.xy, 0.0f);
  vec4 color2 = textureLod(sceneColorTex, texel.zw, 0.0f);
  vec4 color3 = textureLod(sceneColorTex, texel.zy, 0.0f);
  vec4 color4 = textureLod(sceneColorTex, texel.xw, 0.0f);

  vec2 cocs1 = textureLod(inputCocTex, texel.xy, 0.0f).rg;
  vec2 cocs2 = textureLod(inputCocTex, texel.zw, 0.0f).rg;
  vec2 cocs3 = textureLod(inputCocTex, texel.zy, 0.0f).rg;
  vec2 cocs4 = textureLod(inputCocTex, texel.xw, 0.0f).rg;
