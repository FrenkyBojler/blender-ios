...
void Empties::begin_sync(Resources &res, const State &state)
{
...
void Empties::begin_sync(CallBuffers &call_buffers)
{
...
void Empties::object_sync(Manager &manager,
...
void Empties::object_sync(const select::ID select_id,
...
void Empties::end_sync(Resources &res, const State &state)
{
  if (!enabled_) {
    return;
  }
...
void Empties::end_sync(Resources &res,
                       const State &state,
                       PassSimple::Sub &ps,
                       CallBuffers &call_buffers)
{
...
void Empties::image_sync(const ObjectRef &ob_ref,
                         select::ID select_id,
                         Manager &manager,
                         Resources &res,
                         const State &state,
...
PassMain::Sub &Empties::create_subpass(const State &state,
                                       const Object &ob,
...
PassMain::Sub &Empties::create_subpass(
    const State &state, const float4x4 &mat, Resources &res, PassSortable &parent, bool depth_bias)
{
