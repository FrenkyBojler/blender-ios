...
void Fade::begin_sync(Resources &res, const State &state)
{
...
void Fade::object_sync(Manager &manager,
...
void Fade::pre_draw(Manager &manager, View &view)
{
  if (!enabled_) {
    return;
  }
...
void Fade::draw(Framebuffer &framebuffer, Manager &manager, View &view)
{
  if (!enabled_) {
    return;
  }
