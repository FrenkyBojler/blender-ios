...
void MotionPath::begin_sync(Resources &res, const State &state)
{
...
void MotionPath::object_sync(Manager & /*manager*/,
...
static gpu::VertBuf *mpath_vbo_get(bMotionPath *mpath)
{
  if (!mpath->points_vbo) {
...
static gpu::Batch *mpath_batch_points_get(bMotionPath *mpath)
{
...
void MotionPath::draw_color_only(Framebuffer &framebuffer, Manager &manager, View &view)
{
  if (!enabled_) {
    return;
  }
...
void MotionPath::motion_path_sync(const State &state,
                                  const Object *ob,
                                  ...
                                  bMotionPath *mpath)
{
  ...
  const bool selected = (pchan) ? (pchan->flag & POSE_SELECTED) : (ob->base_flag & BASE_SELECTED);
