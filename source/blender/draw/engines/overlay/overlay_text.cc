...
void Text::begin_sync(Resources &res, const State &state)
{
  ...
  res.select_bind(ps_);
  {
    ...
    /* Highlight text within selection boxes. */
    {
      ...
      sub.state_set(DRW_STATE_WRITE_COLOR | DRW_STATE_BLEND_ALPHA | DRW_STATE_DEPTH_GREATER_EQUAL,
...
void Text::edit_object_sync(Manager &manager,
...
void Text::end_sync(Resources &res, const State &state)
{
  if (!enabled_) {
    return;
  }
...
void Text::draw(Framebuffer &framebuffer, Manager &manager, View &view)
{
  if (!enabled_) {
    return;
  }
...
void Text::add_select(Manager &manager, const Curve &cu, const float4x4 &ob_to_world)
{
...
void Text::add_cursor(Manager &manager, const Curve &cu, const float4x4 &ob_to_world)
{
...
void Text::add_boxes(const Resources &res, const Curve &cu, const float4x4 &ob_to_world)
{
