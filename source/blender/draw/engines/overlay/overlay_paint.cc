...
void Paints::begin_sync(Resources &res, const State &state)
{
  enabled_ =
  ...
  if (state.ctx_mode == CTX_MODE_PAINT_WEIGHT) {
    ...
    weight_masked_transparency_ps_ = weight_subpass("Masked Transparency",
  ...
  if (state.ctx_mode == CTX_MODE_PAINT_TEXTURE) {
    ...
    if (show_paint_mask_) {
      ...
      gpu::Texture *mask_texture = BKE_image_acquire_gpu_texture(paint_settings.stencil, nullptr);
...
void Paints::object_sync(Manager &manager,
...
void Paints::draw(Framebuffer &framebuffer, Manager &manager, View &view)
{
  if (!enabled_) {
    return;
  }
