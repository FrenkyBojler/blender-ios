...
static float wire_discard_threshold_get(float threshold)
{
...
void Wireframe::begin_sync(Resources &res, const State &state)
{
...
void Wireframe::object_sync_ex(Manager &manager,
                               ...
                               const bool in_edit_mode)
{
  ...
  switch (ob_ref.object->type) {
    case OB_CURVES_LEGACY: {
      ...
      coloring.curves_ps_->draw(geom, manager.unique_handle(ob_ref), res.select_id(ob_ref).get());
      break;
    }
    case OB_FONT: {
    ...
    case OB_GREASE_PENCIL: {
      if (show_surface_wire) {
        gpu::Batch *geom = DRW_cache_grease_pencil_face_wireframe_get(state.scene, ob_ref.object);
...
void Wireframe::pre_draw(Manager &manager, View &view)
{
  if (!enabled_) {
    return;
  }
...
void Wireframe::copy_depth(TextureRef &depth_tx)
{
...
void Wireframe::draw_line(Framebuffer &framebuffer, Manager &manager, View &view)
{
  if (!enabled_) {
    return;
  }
