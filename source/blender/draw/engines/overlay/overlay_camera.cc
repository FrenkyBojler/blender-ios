...
void Cameras::begin_sync(Resources &res, const State &state)
{
...
void Cameras::object_sync(Manager &manager,
...
void Cameras::end_sync(Resources &res, const State &state)
{
  if (!extras_enabled_ && !motion_tracking_enabled_) {
    return;
  }
...

  {
    ...
    sub_pass.state_set(DRW_STATE_WRITE_COLOR | DRW_STATE_BLEND_ALPHA | DRW_STATE_DEPTH_LESS_EQUAL |
                           DRW_STATE_CULL_BACK,
...

  {
    ...
    sub_pass.state_set(DRW_STATE_WRITE_COLOR | DRW_STATE_WRITE_DEPTH | DRW_STATE_DEPTH_LESS_EQUAL,
                       state.clipping_plane_count);
    sub_pass.shader_set(res.shaders->extra_shape.get());
...
void Cameras::pre_draw(Manager &manager, View &view)
{
  if (!images_enabled_) {
    return;
  }
...
void Cameras::draw_line(Framebuffer &framebuffer, Manager &manager, View &view)
{
  if (!extras_enabled_ && !motion_tracking_enabled_) {
    return;
  }
...
void Cameras::draw_scene_background_images(gpu::FrameBuffer *framebuffer,
                                           Manager &manager,
                                           View &view)
{
  if (!images_enabled_) {
...
void Cameras::draw_background_images(Framebuffer &framebuffer, Manager &manager, View &view)
{
  if (!images_enabled_) {
    return;
  }
...
void Cameras::draw_in_front(Framebuffer &framebuffer, Manager &manager, View &view)
{
  if (!images_enabled_) {
    return;
  }
...
void Cameras::object_sync_extras(const ObjectRef &ob_ref,
                                 select::ID select_id,
                                 const State &state,
                                 Resources &res)
{
...
void Cameras::object_sync_motion_paths(const ObjectRef &ob_ref, Resources &res, const State &state)
{
       ...
       ListBaseWrapper<MovieTrackingObject>(&tracking->objects))
  {
    ...
    for (MovieTrackingTrack *track : ListBaseWrapper<MovieTrackingTrack>(&tracking_object->tracks))
    {
...
void Cameras::object_sync_images(const ObjectRef &ob_ref,
                                 select::ID select_id,
                                 Manager &manager,
                                 const State &state,
                                 Resources &res)
...
gpu::Texture *Cameras::image_camera_background_texture_get(const CameraBGImage *bgpic,
                                                           const State &state,
                                                           Resources &res,
                                                           float &r_aspect,
 ...
 */
void Cameras::sync_stereoscopy_extra(const Main &bmain,
