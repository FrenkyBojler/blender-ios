...
void Facing::begin_sync(Resources &res, const State &state)
{
...
void Facing::object_sync(Manager &manager,
...
void Facing::pre_draw(Manager &manager, View &view)
{
  if (!enabled_) {
    return;
  }
...
void Facing::draw(Framebuffer &framebuffer, Manager &manager, View &view)
{
  if (!enabled_) {
    return;
  }
