...
void GreasePencil::begin_sync(Resources &res, const State &state)
{
  ...
  switch (state.object_mode) {
    ...
    case OB_MODE_EDIT:
      /* Edit mode. */
      show_points_ = ELEM(ts->gpencil_selectmode_edit, GP_SELECTMODE_POINT, GP_SELECTMODE_SEGMENT);
...

  {
    ...
    DRWState depth_write_state = (grid_xray) ? DRW_STATE_DEPTH_ALWAYS : DRW_STATE_DEPTH_LESS_EQUAL;
    auto &pass = grid_ps_;
    pass.init();
...
void GreasePencil::edit_object_sync(Manager &manager,
                                    ...
                                    const State &state)
{
  ...
  if (show_points_) {
    gpu::Batch *geom = show_weight_ ? DRW_cache_grease_pencil_weight_points_get(state.scene, ob) :
...
void GreasePencil::object_sync(Manager & /*manager*/,
                               const ObjectRef &ob_ref,
                               Resources & /*res*/,
                               const State &state)
{
...
float4x4 GreasePencil::grid_matrix_get(const Object &object, const Scene *scene)
{
...
void GreasePencil::compute_depth_planes(Manager &manager,
                                        View &view,
                                        Resources &res,
                                        const State & /*state*/)
{
  for (auto i : IndexRange(res.depth_planes_count)) {
    ...
    plane.plane = depth_plane_get(object_to_world, plane.bounds, view);
  }
}

void GreasePencil::draw_line(Framebuffer &framebuffer, Manager &manager, View &view)
{
  if (!enabled_) {
    return;
  }
...
void GreasePencil::draw_color_only(Framebuffer &framebuffer, Manager &manager, View &view)
{
  if (!enabled_) {
    return;
  }
...
void GreasePencil::draw_grease_pencil(Resources &res,
                                      PassMain::Sub &pass,
                                      const Scene *scene,
                                      Object *ob,
...
void GreasePencil::draw_material_names(const ObjectRef &ob_ref, const State &state, Resources &res)
{
  ...
  for (const ed::greasepencil::DrawingInfo &info : drawings) {
    ...
    const bke::AttrDomain domain = show_points_ ? bke::AttrDomain::Point : bke::AttrDomain::Curve;
