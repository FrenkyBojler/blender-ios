                                          ...
                                          Object *custom)
{
  ...
  if (ELEM(custom->type, OB_CURVES_LEGACY, OB_FONT, OB_SURF)) {
    drw_shgroup_custom_bone_curve(ctx,
                                  &DRW_object_get_data<Curve>(*custom),
                                  bone_mat,
                                  outline_color,
                                  wire_width,
                                  select_id,
                                         ...
                                         Object *custom)
{
  ...
  if (ELEM(custom->type, OB_CURVES_LEGACY, OB_FONT, OB_SURF)) {
    drw_shgroup_custom_bone_curve(
        ctx, &DRW_object_get_data<Curve>(*custom), bone_mat, color, wire_width, select_id, custom);
  }
}
...
{
  bArmature &arm = DRW_object_get_data<bArmature>(*ctx->ob);
...
void Armatures::draw_armature_edit(Armatures::DrawContext *ctx)
{
  ...
  bArmature &arm = DRW_object_get_data<bArmature>(*ob_orig);
...
void Armatures::draw_armature_pose(Armatures::DrawContext *ctx)
{
  ...
  bArmature &arm = DRW_object_get_data<bArmature>(*ob);
  int index = -1;
  const bool show_text = DRW_state_show_text();
