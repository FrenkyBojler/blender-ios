...
void Curves::begin_sync(Resources &res, const State &state)
{
...
void Curves::edit_object_sync(Manager &manager,
...
void Curves::edit_object_sync_legacy(Manager &manager,
                                     const ObjectRef &ob_ref,
                                     Resources & /*res*/)
{
  if (!enabled_) {
...
void Curves::draw_line(Framebuffer &framebuffer, Manager &manager, View &view)
{
  if (!enabled_) {
    return;
  }
...
void Curves::draw_color_only(Framebuffer &framebuffer, Manager &manager, View &view)
{
  if (!enabled_) {
    return;
  }
