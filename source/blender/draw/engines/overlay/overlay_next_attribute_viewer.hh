...
class AttributeViewer : Overlay {
 ...
 private:
  ...
  void populate_for_geometry(const ObjectRef &ob_ref, const State &state, Manager &manager)
  {
    ...
    switch (object.type) {
      case OB_MESH: {
        Mesh &mesh = DRW_object_get_data<Mesh>(object);
      ...
      case OB_POINTCLOUD: {
        PointCloud &pointcloud = DRW_object_get_data<PointCloud>(object);
        if (const std::optional<bke::AttributeMetaData> meta_data =
      ...
      case OB_CURVES_LEGACY: {
        Curve &curve = DRW_object_get_data<Curve>(object);
        if (curve.curve_eval) {
      ...
      case OB_CURVES: {
        ::Curves &curves_id = DRW_object_get_data<::Curves>(object);
