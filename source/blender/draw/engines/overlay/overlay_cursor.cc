...
void Cursor::begin_sync(Resources &res, const State &state)
{
  ...
  if (state.is_space_v3d()) {
    ...
    float4x4 mvp_lines = float4x4(state.rv3d->winmat) * float4x4(state.rv3d->viewmat) * cursor_mat;
...
void Cursor::draw_output(Framebuffer &framebuffer, Manager &manager, View & /*view*/)
{
  if (!enabled_) {
    return;
  }
...
bool Cursor::is_cursor_visible_3d(const State &state)
{
