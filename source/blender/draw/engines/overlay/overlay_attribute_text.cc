...
[[maybe_unused]] static void add_lines_to_cache(DRWTextStore *dt,
...
[[maybe_unused]] static void add_text_to_cache(DRWTextStore *dt,
                                               const float3 &position,
                                               const StringRef text,
                                               const uchar4 &color)
{
  DRW_text_cache_add(
      dt, position, text.data(), text.size(), 0, 0, DRW_TEXT_CACHE_GLOBALSPACE, color, true, true);
}

static void add_values_to_text_cache(DRWTextStore *dt,
...
static void add_attributes_to_text_cache(DRWTextStore *dt,
...
static void add_mesh_attributes_to_text_cache(const State &state,
                                              const Mesh &mesh,
                                              const float4x4 &object_to_world)
{
  ...
  if (domain == bke::AttrDomain::Corner) {
    ...
    if (type.is<int2>() || type.is<float2>() || type.is<float3>() || type.is<ColorGeometry4b>() ||
        type.is<ColorGeometry4f>() || type.is<math::Quaternion>())
    {
      offset_by_type = 1.5f;
    }
...
static void add_instance_attributes_to_text_cache(DRWTextStore *dt,
                                                  bke::AttributeAccessor attribute_accessor,
                                                  const float4x4 &object_to_world,
                                                  int instance_index)
{
...
void AttributeTexts::begin_sync(Resources &res, const State &state)
{
  enabled_ = !res.is_selection() && state.show_attribute_viewer_text();
}

void AttributeTexts::object_sync(Manager & /*manager*/,
