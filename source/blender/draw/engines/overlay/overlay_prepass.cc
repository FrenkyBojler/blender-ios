...
void ImagePrepass::begin_sync(Resources &res, const State &state)
{
...
void ImagePrepass::draw_on_render(gpu::FrameBuffer *framebuffer, Manager &manager, View &view)
{
  if (!enabled_) {
    return;
  }
...
void Prepass::begin_sync(Resources &res, const State &state)
{
...
void Prepass::particle_sync(Manager &manager,
...
void Prepass::sculpt_sync(Manager &manager, const ObjectRef &ob_ref, Resources &res)
{
...
void Prepass::object_sync(Manager &manager,
                          const ObjectRef &ob_ref,
                          Resources &res,
                          const State &state)
{
  ...
  for (int material_id : geom_list.index_range()) {
    ...
    if (res.is_selection() && (pass == mesh_ps_)) {
      /* Conservative shader needs expanded draw-call. */
      pass->draw_expand(geom_list[material_id], GPU_PRIM_TRIS, 1, 1, res_handle, select_id.get());
    }
    else {
...
void Prepass::pre_draw(Manager &manager, View &view)
{
  if (!enabled_) {
    return;
  }
...
void Prepass::draw_line(Framebuffer &framebuffer, Manager &manager, View &view)
{
  if (!enabled_) {
    return;
  }
