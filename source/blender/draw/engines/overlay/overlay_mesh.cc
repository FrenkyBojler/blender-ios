...
void Meshes::begin_sync(Resources &res, const State &state)
{
...
void Meshes::edit_object_sync(Manager &manager,
...
void Meshes::draw_line(Framebuffer &framebuffer, Manager &manager, View &view)
{
  if (!enabled_) {
    return;
  }
...
void Meshes::draw_color_only(Framebuffer &framebuffer, Manager &manager, View &view)
{
  if (!enabled_) {
    return;
  }
...
bool Meshes::mesh_has_edit_cage(const Object *ob)
{
...
uint4 Meshes::data_mask_get(const int flag)
{
...
bool Meshes::mesh_has_skin_roots(const Object *ob)
{
  Mesh &mesh = DRW_object_get_data_for_drawing<Mesh>(*ob);
...
void MeshUVs::begin_sync(Resources &res, const State &state)
{
  ...
  if (show_face_overlay_ || select_face_) {
    const float opacity = (object_mode_is_edit && space_mode_is_uv) ? space_image->uv_opacity :
...
void MeshUVs::object_sync(Manager &manager,
                          const ObjectRef &ob_ref,
                          Resources & /*res*/,
                          const State &state)
{
...
void MeshUVs::edit_object_sync(Manager &manager,
                               const ObjectRef &ob_ref,
                               Resources & /*res*/,
                               const State &state)
{
...
void MeshUVs::end_sync(Resources &res, const State &state)
{
  if (!enabled_) {
    return;
  }
...
void MeshUVs::draw(Framebuffer &framebuffer, Manager &manager, View &view)
{
  if (!enabled_) {
    return;
  }
...
void MeshUVs::draw_on_render(gpu::FrameBuffer *framebuffer, Manager &manager, View &view)
{
  if (!enabled_) {
    return;
  }
...
void MeshUVs::paint_mask_texture_ensure(Mask *mask, const int2 &resolution, const float2 &aspect)
{
