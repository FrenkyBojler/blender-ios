...
void Grid::begin_sync(Resources &res, const State &state)
{
  enabled_ = init(state);
  if (!enabled_) {
    grid_ps_.init();
  ...
  /* Grid and axis line draws. */
  {
    ...
    const auto grid_draw_state = ps_draw_state | DRW_STATE_DEPTH_LESS_EQUAL | DRW_STATE_BLEND_ADD;
...
void Grid::draw_line(Framebuffer &framebuffer, Manager &manager, View &view)
{
  if (!enabled_) {
    return;
  }
...
bool Grid::init(const State &state)
{
...
bool Grid::init_space_image(const State &state)
{
...
bool Grid::init_v3d(const State &state)
{
