...
GPU_SHADER_INTERFACE_INFO(overlay_grid_mesh_iface)
SMOOTH(float2, edgePos)
FLAT(float2, edgeStart)
SMOOTH(float3, axis_tag)
SMOOTH(float4, finalColor)
GPU_SHADER_INTERFACE_END()

GPU_SHADER_CREATE_INFO(overlay_grid_mesh)
DO_STATIC_COMPILATION()
TYPEDEF_SOURCE("overlay_shader_shared.hh")
VERTEX_OUT(overlay_grid_mesh_iface)
FRAGMENT_OUT(0, float4, fragColor)
FRAGMENT_OUT(1, float4, lineOutput)
PUSH_CONSTANT(int3, origin_offset)
PUSH_CONSTANT(int, axis)
PUSH_CONSTANT(float, unit_scale)
PUSH_CONSTANT(int, next_divider)
PUSH_CONSTANT(float, far_clip)
PUSH_CONSTANT(bool, show_axis_x)
PUSH_CONSTANT(bool, show_axis_y)
PUSH_CONSTANT(bool, show_axis_z)
VERTEX_SOURCE("overlay_grid_mesh_vert.glsl")
FRAGMENT_SOURCE("overlay_grid_mesh_frag.glsl")
ADDITIONAL_INFO(draw_view)
ADDITIONAL_INFO(draw_globals)
GPU_SHADER_CREATE_END()
