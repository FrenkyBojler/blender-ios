...
{
  /* TODO: Theme? */
  const vec4 pinned_col = vec4(1.0f, 0.0f, 0.0f, 1.0f);

  bool is_selected = (flag & (VERT_UV_SELECT | FACE_UV_SELECT)) != 0u;
  bool is_pinned = (flag & VERT_UV_PINNED) != 0u;
  vec4 deselect_col = (is_pinned) ? pinned_col : vec4(color.rgb, 1.0f);
  fillColor = (is_selected) ? colorVertexSelect : deselect_col;
  outlineColor = (is_pinned) ? pinned_col : vec4(fillColor.rgb, 0.0f);

  vec3 world_pos = vec3(au, 0.0f);
  /* Move selected vertices to the top
   * Vertices are between 0.0f and 0.2f, Edges between 0.2f and 0.4f
   * actual pixels are at 0.75f, 1.0f is used for the background. */
