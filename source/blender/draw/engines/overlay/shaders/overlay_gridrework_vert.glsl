...
LineData get_line_data(in uint vertex_id)
{
  ...
  line.P.x = float(grid_buf.num_lines_per_level >> 1u); /* 255 */
  line.P.y = float(vertex_id) - line.P.x;               /* (<511) - 255 */
...
void main()
{
  ...
  if (drw_view_is_perspective()) {
    ...
    if (all(greaterThan(abs(P - P_offset), P_clip))) {
      gl_Position = float4(NAN_FLT);
      return;
    }
    /* Otherwise, clamp to clipping distance around the offset point. */
    P = clamp(P, P_offset - P_clip, P_offset + P_clip);
  }
