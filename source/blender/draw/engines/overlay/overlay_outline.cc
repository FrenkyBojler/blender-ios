...
void Outline::begin_sync(Resources &res, const State &state)
{
...
void Outline::object_sync(Manager &manager,
...
void Outline::flat_objects_pass_sync(Manager &manager,
                                     View &view,
                                     Resources &res,
                                     const State &state)
{
...
void Outline::pre_draw(Manager &manager, View &view)
{
  if (!enabled_) {
    return;
  }
...
void Outline::draw_line_only_ex(Framebuffer &framebuffer,
                                Resources &res,
                                Manager &manager,
                                View &view)
{
  ...
  prepass_fb_.ensure(GPU_ATTACHMENT_TEXTURE(tmp_depth_tx_), GPU_ATTACHMENT_TEXTURE(object_id_tx_));
