...
static bool everything_selected(const blender::Curves &curves_id)
{
...
void Sculpts::begin_sync(Resources &res, const State &state)
{
...
void Sculpts::object_sync(Manager &manager,
...
void Sculpts::curves_sync(Manager &manager, const ObjectRef &ob_ref, const State &state)
{
  ...
  if (show_mask_ && !everything_selected(curves)) {
    ...
    if (is_valid) {
      ...
      gpu::Batch *geometry = curves_sub_pass_setup(*curves_ps_, state.scene, ob_ref.object, error);
...
void Sculpts::mesh_sync(Manager &manager, const ObjectRef &ob_ref, const State &state)
{
...
void Sculpts::draw_line(Framebuffer &framebuffer, Manager &manager, View &view)
{
  if (!enabled_) {
    return;
  }
...
void Sculpts::draw_on_render(gpu::FrameBuffer *framebuffer, Manager &manager, View &view)
{
  if (!enabled_) {
    return;
  }
