...
void LightProbes::begin_sync(Resources &res, const State &state)
{
...
void LightProbes::object_sync(Manager & /*manager*/,
...
void LightProbes::end_sync(Resources &res, const State &state)
{
  if (!enabled_) {
    return;
  }
  ...
  DRWState pass_state = DRW_STATE_WRITE_COLOR | DRW_STATE_WRITE_DEPTH | DRW_STATE_DEPTH_LESS_EQUAL;
  {
...
void LightProbes::pre_draw(Manager &manager, View &view)
{
  if (!enabled_) {
    return;
  }
...
void LightProbes::draw_line(Framebuffer &framebuffer, Manager &manager, View &view)
{
  if (!enabled_) {
    return;
  }
...
void LightProbes::draw_color_only(Framebuffer &framebuffer, Manager &manager, View &view)
{
  if (!enabled_) {
    return;
  }
