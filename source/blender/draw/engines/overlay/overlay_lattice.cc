...
void Lattices::begin_sync(Resources &res, const State &state)
{
...
void Lattices::edit_object_sync(Manager &manager,
...
void Lattices::object_sync(Manager &manager,
                           const ObjectRef &ob_ref,
                           Resources &res,
                           const State &state)
{
  ...
  if (geom) {
    ...
    ResourceHandleRange res_handle = manager.resource_handle(ob_ref, &draw_mat, nullptr, nullptr);
    lattice_ps_->draw(geom, res_handle, res.select_id(ob_ref).get());
  }
}

void Lattices::pre_draw(Manager &manager, View &view)
{
  if (!enabled_) {
    return;
  }
...
void Lattices::draw_line(Framebuffer &framebuffer, Manager &manager, View &view)
{
  if (!enabled_) {
    return;
  }
