...
void Relations::begin_sync(Resources &res, const State &state)
{
...
void Relations::object_sync(Manager & /*manager*/,
  ...
  if (ob->rigidbody_constraint) {
    ...
    if (rbc_ob1 && (DRW_object_visibility_in_active_context(rbc_ob1) & OB_VISIBLE_SELF)) {
      ...
      relations_buf_.append(
          rbc_ob1->object_to_world().location(), ob->object_to_world().location(), relation_color);
    }
    if (rbc_ob2 && (DRW_object_visibility_in_active_context(rbc_ob2) & OB_VISIBLE_SELF)) {
      relations_buf_.append(
          rbc_ob2->object_to_world().location(), ob->object_to_world().location(), relation_color);
    }
  }
    ...
    for (bConstraint *constraint : ListBaseWrapper<bConstraint>(ob->constraints)) {
      if (ELEM(constraint->type, CONSTRAINT_TYPE_FOLLOWTRACK, CONSTRAINT_TYPE_OBJECTSOLVER)) {
        ...
        if (constraint->type == CONSTRAINT_TYPE_FOLLOWTRACK) {
          ...
          bObjectSolverConstraint *data = static_cast<bObjectSolverConstraint *>(constraint->data);
          camob = data->camera ? data->camera : scene->camera;
        }
...
void Relations::end_sync(Resources &res, const State &state)
{
  if (!enabled_) {
    return;
  }
...
void Relations::draw_line(Framebuffer &framebuffer, Manager &manager, View &view)
{
  if (!enabled_) {
    return;
  }
