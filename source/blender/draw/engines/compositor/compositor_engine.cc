 ...
 public:
  ...
  compositor::Result get_pass(const Scene *scene, int view_layer, const char *pass_name) override
  {
    ...
    if (STREQ(pass_name, RE_PASSNAME_COMBINED)) {
      blender::gpu::Texture *combined_texture =
          DRW_context_get()->viewport_texture_list_get()->color;
    ...
    blender::gpu::Texture *pass_texture = DRW_viewport_pass_texture_get(pass_name).gpu_texture();
    if (pass_texture) {
