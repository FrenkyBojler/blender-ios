...
void main()
{
  ...
  float4 data_src = colorspace_scene_to_perceptual(max(float4(0.0f), imageLoadFast(src_img, texel)));
  float4 data_dst = colorspace_scene_to_perceptual(max(float4(0.0f), imageLoadFast(dst_img, texel)));
