...
float4 get_color(float2 uv, float2 dx, float2 dy)
{
  ...
  /* When `gp_interp.color_mul` is interpolated for each fragment, it might not always be exactly 1.0
   * in the default case. To fix this, clamp any value very close to 1.0 to 1.0. See #156278. */
