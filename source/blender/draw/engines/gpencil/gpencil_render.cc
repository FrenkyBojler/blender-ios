                                ...
                                const bool use_separated_pass)
{
  ...
  if (inst.render_depth_tx.is_valid() && !do_clear_z) {
    ...
    inst.render_depth_tx.ensure_2d(blender::gpu::TextureFormat::SFLOAT_32_DEPTH,
                                   int2(size),
                                   usage,
                                   do_region ? nullptr : pix_z);
  }
  if (inst.render_color_tx.is_valid() && !do_clear_col) {
    ...
    eGPUTextureUsage usage = GPU_TEXTURE_USAGE_SHADER_READ | GPU_TEXTURE_USAGE_ATTACHMENT |
                             GPU_TEXTURE_USAGE_HOST_READ;
    inst.render_color_tx.ensure_2d(blender::gpu::TextureFormat::SFLOAT_16_16_16_16,
                                   int2(size),
                                   usage,
                                   do_region ? nullptr : pix_col);
  }
