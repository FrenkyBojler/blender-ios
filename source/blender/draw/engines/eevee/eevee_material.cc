...
{
  /* Compile material first to be able to still push to the texture queue. */
  material_shaders_compile();
  material_textures_load();
}

void MaterialModule::material_shaders_compile()
{
  ...
  auto for_each_material_in_queue = [&](GPUMaterialCompileMode compile_mode) {
    for (MaterialPassRequest &request : material_queue_) {
      ...
      GPUMaterial *gpumat = inst_.shaders.material_shader_get(request.blender_mat,
                                                              ntree,
...
void MaterialModule::material_textures_load()
{
