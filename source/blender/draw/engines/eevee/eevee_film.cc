...
{
  blender::gpu::Texture *pass_tx = this->get_aov_texture(aov);
...
blender::gpu::Texture *Film::get_aov_texture(ViewLayerAOV *aov)
{
  bool is_value = (aov->type == AOV_TYPE_VALUE);
...
void Film::accumulate(View &view, blender::gpu::Texture *combined_final_tx)
{
  if (inst_.is_viewport()) {
...
{
  blender::gpu::Texture *pass_tx = this->get_pass_texture(pass_type, layer_offset);
...
blender::gpu::Texture *Film::get_pass_texture(eViewLayerEEVEEPassType pass_type, int layer_offset)
{
  ePassStorageType storage_type = pass_storage_type(pass_type);
...
void Film::write_viewport_compositor_passes()
{
  ...
  for (const int i : IndexRange(EEVEE_RENDER_PASS_MAX_BIT + 1)) {
    ...
    for (const int64_t pass_offset : IndexRange(pass_names.size())) {
      blender::gpu::Texture *pass_texture = this->get_pass_texture(pass_type, pass_offset);
      if (!pass_texture) {
        continue;
      }
  ...
  LISTBASE_FOREACH (ViewLayerAOV *, aov, &inst_.view_layer->aovs) {
    ...
    blender::gpu::Texture *pass_texture = this->get_aov_texture(aov);
    if (!pass_texture) {
      continue;
    }
