...
void ShadingView::render()
{
  ...
  GPU_debug_group_scope_transient(name_);
...
void CaptureView::render_world()
{
  ...
  GPU_debug_group_scope("World.Capture", ProfileCategory::Draw);
...
void CaptureView::render_probes()
{
  ...
  while (const auto update_info = inst_.sphere_probes.probe_update_info_pop()) {
    GPU_debug_group_scope("Probe.Capture", ProfileCategory::Draw);
...
void LookdevView::render()
{
  ...
  GPU_debug_group_scope("Lookdev", ProfileCategory::Draw);
