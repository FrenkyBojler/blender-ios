...
void main()
{
  ...
  if (gl_LocalInvocationIndex == 0u) {
    sunlight_buf[0].color = local_radiance[0];
    ...
    sunlight_buf[0].object_to_world.x = float4(tx[0], 0.0f);
    sunlight_buf[0].object_to_world.y = float4(tx[1], 0.0f);
    sunlight_buf[0].object_to_world.z = float4(tx[2], 0.0f);
    ...
    sunlight_buf[0].power[LIGHT_DIFFUSE] = shape_power;
    sunlight_buf[0].power[LIGHT_SPECULAR] = shape_power;
    sunlight_buf[0].power[LIGHT_TRANSMISSION] = shape_power;
    sunlight_buf[0].power[LIGHT_VOLUME] = point_power;
