...
{
  return ambient_occlusion_data(vec4(M_PI, -M_PI, M_PI, -M_PI), 1.0f);
}

vec4 ambient_occlusion_pack_data(OcclusionData data)
{
  return vec4(1.0f - data.horizons * vec4(1, -1, 1, -1) * M_1_PI);
}

OcclusionData ambient_occlusion_unpack_data(vec4 v)
{
  return ambient_occlusion_data((1.0f - v) * vec4(1, -1, 1, -1) * M_PI, 0.0f);
}
                                       ...
                                       const float dir_sample_count)
{
  ...
  vec3 vI = (drw_view_is_perspective() ? normalize(-vP) : vec3(0.0f, 0.0f, 1.0f));

  OcclusionData data = (inverted != 0.0f) ? ambient_occlusion_data(vec4(0, 0, 0, 0), 1.0f) :
                                            ambient_occlusion_disabled_data();

  for (int i = 0; i < 2; i++) {
    Ray ray;
    ray.origin = vP;
    ray.direction = vec3(dir, 0.0f);
    ray.max_time = radius;
                            ...
                            out vec3 bent_normal)
{
  ...
  for (int i = 0; i < 2; i++) {
    vec3 T = drw_normal_view_to_world(vec3(dir, 0.0f));
    ...
    /* Inner integral (Eq. 7). */
    float a = dot(-cos(2.0f * h - angle_N) + N_cos + 2.0f * h * N_sin, vec2(0.25f));
...
float ambient_occlusion_multibounce(float visibility, vec3 albedo)
{
  ...
  float lum = dot(albedo, vec3(0.3333f));
