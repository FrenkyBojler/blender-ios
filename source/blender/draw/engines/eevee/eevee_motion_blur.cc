...
void MotionBlurModule::render(View &view, gpu::Texture **input_tx, gpu::Texture **output_tx)
{
  ...
  if (inst_.is_viewport()) {
    float step_delta = fabsf(inst_.velocity.step_time_delta_get(STEP_PREVIOUS, STEP_CURRENT));
    /* Avoid highly disturbing blurs, during navigation with high shutter time. */
    if (step_delta > 0.0f && !inst_.is_navigating) {
       ...
       * position. */
      data_.motion_scale = float2(0.5f, 0.5f) * float2(shutter_time_ / step_delta);
    }
    else {
