...
#define CHECK_PASS_DENOISING(name, type, channels, chanid) \
  if (view_layer->passflag & (SCE_PASS_DENOISING)) { \
    RE_engine_register_pass( \
        engine, scene, view_layer, RE_PASSNAME_##name, channels, chanid, type); \
  } \
  ((void)0)
  ...
  CHECK_PASS_DENOISING(DENOISING_NORMAL, SOCK_FLOAT, 3, "XYZ");
  CHECK_PASS_DENOISING(DENOISING_ROUGHNESS, SOCK_FLOAT, 1, "X");
  CHECK_PASS_DENOISING(DENOISING_DIFFUSE_ALBEDO, SOCK_FLOAT, 3, "RGB");
  CHECK_PASS_DENOISING(DENOISING_SPECULAR_ALBEDO, SOCK_FLOAT, 3, "RGB");
