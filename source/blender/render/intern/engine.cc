                                     ...
                                     const bool use_grease_pencil)
{
  ...
  if (engine->has_grease_pencil && use_grease_pencil && re->result->passes_allocated) {
    ...
    if (!RE_engine_test_break(engine) && engine->depsgraph != nullptr) {
      CLOG_INFO(LOG_RENDER, "Rendering grease pencil");
...
bool RE_engine_render(Render *re, bool do_all)
{
  ...
  if (type->render) {
    FOREACH_VIEW_LAYER_TO_RENDER_BEGIN (re, view_layer_iter) {
      CLOG_INFO(
          LOG_RENDER, "Start rendering: %s, %s", re->scene->id.name + 2, view_layer_iter->name);
      CLOG_INFO(LOG_RENDER, "Engine: %s", engine->type->name);
  ...
  if (re->r.mode & R_EDGE_FRS) {
    CLOG_INFO(LOG_RENDER, "Rendering freestyle");
