                  ...
                  const rcti *disprect)
{
  ...
  if (re->r.scemode & R_BUTS_PREVIEW) {
    ...
    if (border_overlay) {
      // Free again, so that in engine.cc the result can be properly initialized. The
      // initialization here is only needed to be able to create a render_result_view.
      render_result_free(re->result);
      re->result = nullptr;
    }
  }
