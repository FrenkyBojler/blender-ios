    ...
    LISTBASE_FOREACH (const wmWindow *, win, &wm->windows) {
      ...
      LISTBASE_FOREACH (const ScrArea *, area, &screen->areabase) {
        ...
        if (space.spacetype == SPACE_NODE) {
          ...
          if (snode.nodetree == scene->compositing_nodetree) {
            do_free = false;
          }
        }
...
{
  bNodeTree *ntree = scene->compositing_nodetree;
...
{
  if (re->scene->compositing_nodetree == nullptr) {
    return;
  }
  ...
  for (bNode *node : re->scene->compositing_nodetree->all_nodes()) {
    Scene *node_scene = get_scene_referenced_by_node(node);
    if (!node_scene) {
      continue;
    }
    ...
    node->typeinfo->updatefunc(re->scene->compositing_nodetree, node);
  }
...
{
  bNodeTree *ntree = re->pipeline_scene_eval->compositing_nodetree;
  ...
  if (compositor_needs_render(re->pipeline_scene_eval)) {
     ...
     * more information. */
    if (!node_tree_has_composite_output(re->pipeline_scene_eval->compositing_nodetree)) {
...
{
  return node_tree_has_any_compositor_output(scene->compositing_nodetree);
}
