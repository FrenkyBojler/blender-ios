...
{
  EXPECT_EQ(total_depth_from_total(100), 4);

  EXPECT_EQ(total_joints_at_start(0), 0);
  EXPECT_EQ(total_joints_at_start(1), 1);
  EXPECT_EQ(total_joints_at_start(2), 3);
  EXPECT_EQ(total_joints_at_start(3), 7);

  EXPECT_EQ(total_joints_at_depth(0), 1);
  EXPECT_EQ(total_joints_at_depth(1), 2);
  EXPECT_EQ(total_joints_at_depth(2), 4);
  EXPECT_EQ(total_joints_at_depth(3), 8);

  EXPECT_EQ(joint_size_at_depth(1, 0), 1);

  EXPECT_EQ(joint_size_at_depth(2, 0), 2);
  EXPECT_EQ(joint_size_at_depth(2, 1), 1);

  EXPECT_EQ(joint_size_at_depth(3, 0), 4);
  EXPECT_EQ(joint_size_at_depth(3, 1), 2);
  EXPECT_EQ(joint_size_at_depth(3, 2), 1);

  EXPECT_EQ(joints_range_at_depth(0), IndexRange::from_begin_size(0, 1));
  EXPECT_EQ(joints_range_at_depth(1), IndexRange::from_begin_size(1, 2));
  EXPECT_EQ(joints_range_at_depth(2), IndexRange::from_begin_size(1 + 2, 4));
  EXPECT_EQ(joints_range_at_depth(3), IndexRange::from_begin_size(1 + 2 + 4, 8));

  EXPECT_EQ(joint_buckets_range_at_depth(4, 0, 0), IndexRange::from_begin_size(0, 8));

  EXPECT_EQ(joint_buckets_range_at_depth(4, 1, 0), IndexRange::from_begin_size(0, 4));
  EXPECT_EQ(joint_buckets_range_at_depth(4, 1, 1), IndexRange::from_begin_size(4, 4));

  EXPECT_EQ(joint_buckets_range_at_depth(4, 2, 0), IndexRange::from_begin_size(0, 2));
  EXPECT_EQ(joint_buckets_range_at_depth(4, 2, 1), IndexRange::from_begin_size(2, 2));
  EXPECT_EQ(joint_buckets_range_at_depth(4, 2, 2), IndexRange::from_begin_size(4, 2));
  EXPECT_EQ(joint_buckets_range_at_depth(4, 2, 3), IndexRange::from_begin_size(6, 2));

  EXPECT_EQ(joint_buckets_range_at_depth(4, 3, 0), IndexRange::from_begin_size(0, 1));
  EXPECT_EQ(joint_buckets_range_at_depth(4, 3, 1), IndexRange::from_begin_size(1, 1));
  EXPECT_EQ(joint_buckets_range_at_depth(4, 3, 2), IndexRange::from_begin_size(2, 1));
  EXPECT_EQ(joint_buckets_range_at_depth(4, 3, 3), IndexRange::from_begin_size(3, 1));
  EXPECT_EQ(joint_buckets_range_at_depth(4, 3, 4), IndexRange::from_begin_size(4, 1));
  EXPECT_EQ(joint_buckets_range_at_depth(4, 3, 5), IndexRange::from_begin_size(5, 1));
  EXPECT_EQ(joint_buckets_range_at_depth(4, 3, 6), IndexRange::from_begin_size(6, 1));
  EXPECT_EQ(joint_buckets_range_at_depth(4, 3, 7), IndexRange::from_begin_size(7, 1));
}
...
TEST(AbstractKDimensionalHierarchy, BasicLoops)
{
  ...
  EXPECT_EQ(total_depth, 4);
  EXPECT_EQ(total_buckets, 8);
  EXPECT_EQ(total_joints, (8 + 4 + 2 + 1));
  ...
  EXPECT_EQ_SPAN(bucket_indices.as_span(), test_indices.as_span());
  ...
  for_each_leaf(buckets_offsets,
                ...
                [&](const IndexRange bucket_range, const int joint_index, const int depth_i) {
                  EXPECT_EQ(total_depth - 1, depth_i);
                  EXPECT_TRUE(leaf_joint_to_bucket.add(joint_index, bucket_range));
                });
  ...
  EXPECT_EQ(test_leaf_joint_to_bucket, leaf_joint_to_bucket);
  ...
  for_each_to_bottom(
      ...
      [&](const IndexRange bucket_range, int joint_index, int depth_i) {
        EXPECT_TRUE(joints_to_bottom.add(std::pair<int, int>(joint_index, depth_i), bucket_range));
      });
  ...
  EXPECT_EQ(joints_to_bottom, test_joints_to_bottom);
  ...
  for_each_to_top(
      ...
      [&](const IndexRange bucket_range, int joint_index, const int2 sub_joints, int depth_i) {
        EXPECT_TRUE(joints_to_top.add(std::pair<int, int>(joint_index, depth_i),
  ...
  EXPECT_EQ(joints_to_top, test_joints_to_top);
}
