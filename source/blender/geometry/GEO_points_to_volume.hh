...
enum class PointRasterizeType {
  ...
  Scalar,
  /** Store the gradient of a scalar attribute at grid locations. */
  ScalarGradient,
  /** Rasterize a vector attribute into a vector grid. */
  Vector,
  /** Store the divergence of a vector attribute at grid locations. */
  VectorDivergence,
  /** Store the divergence of a 3x3 matrix attribute at grid locations. */
  TensorDivergence,
  /**
   * Transfer a velocity vector attribute combined with affine momentum.
   * The point attribute is a 4x4 matrix with the affine momentum stored
   * in the 3x3 rotational part and the velocity vector stored in the
   * translational part.
   */
  AffineMomentum,
};
