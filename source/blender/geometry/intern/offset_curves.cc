      ...
      [&](const int64_t curve_i) {
        ...
        for (const int i : src_pos.index_range()) {
            ...
            if (corner_type == OffsetCornerType::Miter && cos_theta < miter_limit) {
              ...
              if (src_handles_left) {
                ...
                offset_handles_left.append(
                    (A - B) * (1.0f + math::safe_divide(offset_distance, 1.0f + cos_theta)) +
                    pos_1);
                ...
                offset_handles_right.append(
                    (C - B) * (1.0f + math::safe_divide(offset_distance, 1.0f + cos_theta)) +
                    pos_2);
              }
          ...
          if (src_handles_left) {
            /* TODO: Use a better approximation. */
            offset_handles_left.append(
                (A - B) * (1.0f + math::safe_divide(offset_distance, 1.0f + cos_theta)) + B +
                offset);
            offset_handles_right.append(
                (C - B) * (1.0f + math::safe_divide(offset_distance, 1.0f + cos_theta)) + B +
                offset);
          }
        }
