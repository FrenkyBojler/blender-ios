...
static void dissolve_valence2_verts(MeshAssembly &ma)
{
  ...
  for (const int old_v : IndexRange(vnum)) {
    ...
    if (new_v >= 0) {
      const int new_start = new_v * ma.vertpos_stride;
      const int old_start = old_v * ma.vertpos_stride;
      BLI_assert(ma.vertpos_stride == 3);
      ma.vertpos[new_start] = ma.vertpos[old_start];
      ma.vertpos[new_start + 1] = ma.vertpos[old_start + 1];
      ma.vertpos[new_start + 2] = ma.vertpos[old_start + 2];
    }
  }
