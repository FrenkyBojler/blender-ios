                         ...
                         const std::optional<IndexRange> sampler_to_bucket_range)
{
  ...
  while (!depth_stack.is_empty()) {
    ...
    const int total_to_pass_to_childs = ispc::count_floats_less_than(
        batch_distances_buffer.data(), joint_min_distance - offset_value, prefix_to_visit);
    ...
    if (!all_pass_to_childs) {
      ...
      if (!all_end_on_joint) {
        ...
        BLI_assert(std::all_of(
            partition.begin(), partition.begin() + pertition_mapping_total, [&](const int i) {
              BLI_assert(i >= 0);
