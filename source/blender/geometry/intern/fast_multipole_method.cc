                         ...
                         const std::optional<IndexRange> sampler_to_bucket_range)
{
  ...
  std::array<Array<float, 0, GuardedAlignedAllocator<>>, 3> batch_positions_data;
  for (const int axis_i : IndexRange(3)) {
    batch_positions_data[axis_i].reinitialize(sample_position[axis_i].size());
    batch_positions_data[axis_i].as_mutable_span().copy_from(sample_position[axis_i]);
  }

  const int data_axes_num = src_bucket_value.size();
  Array<Array<float, 0, GuardedAlignedAllocator<>>, 3> batch_values_data(data_axes_num);
  for (const int axis_i : IndexRange(data_axes_num)) {
    batch_values_data[axis_i].reinitialize(batch_size);
    batch_values_data[axis_i].as_mutable_span().fill(0);
  }
  ...
  while (!depth_stack.is_empty()) {
    ...
    const MutableSpan<float> batch_positions_x = batch_positions_data[0].as_mutable_span().take_front(prefix_to_visit);
    const MutableSpan<float> batch_positions_y = batch_positions_data[1].as_mutable_span().take_front(prefix_to_visit);
    ...
    if (!sampler_to_bucket_range->intersect(joint_backet).is_empty()) {
      const IndexRange local_range = joint_backet.shift(-sampler_to_bucket_range->start());
