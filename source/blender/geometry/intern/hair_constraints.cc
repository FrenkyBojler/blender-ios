    ...
    const fn::Field<float> &damping_field)
{
  ...
  const VArray<float3> goal_position = VArray<float3>::from_single(float3(0.0f), num_points);
    ...
    const fn::Field<float> &damping_field)
{
  ...
  const VArray<float> damping = evaluator.get_evaluated<float>(1);
  const VArray<math::Quaternion> goal_rotation = VArray<math::Quaternion>::from_single(
