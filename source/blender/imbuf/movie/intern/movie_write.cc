...
static ImBuf *alloc_imbuf_for_colorspace_transform(const ImBuf *input_ibuf)
{
  ...
  ImBuf *result_ibuf = IMB_allocImBuf(input_ibuf->x, input_ibuf->y, input_ibuf->color_mode, 0);
...
static const AVCodec *get_prores_encoder(const ImageFormatData *imf, int rectx, int recty)
{
  ...
  if (imf->planes == ImColorMode::RGBA) {
                                    ...
                                    int error_size)
{
  ...
  if (codec_id == AV_CODEC_ID_FFV1) {
    ...
    else if (imf->planes == ImColorMode::RGBA) {
      c->pix_fmt = AV_PIX_FMT_RGB32;
      if (is_10_bpp) {
  ...
  if (codec_id == AV_CODEC_ID_VP9 && imf->planes == ImColorMode::RGBA) {
