                                    ...
                                    int height)
{
  ...
  /* Calculate actual data window dimensions */
  ...
  /* Build channel_names in RGBA order and map file channels to output positions */
  Vector<std::string> channel_names;
  std::map<std::string, int> channel_to_output_idx;  // Maps "R" -> 0, "G" -> 1, etc.

  const char *rgba_order[] = {"R", "G", "B", "A"};
  for (int i = 0; i < 4; i++) {
    bool found = false;
    for (ChannelList::ConstIterator iter = channels.begin(); iter != channels.end(); ++iter) {
      if (strcmp(iter.name(), rgba_order[i]) == 0) {
        channel_names.append(rgba_order[i]);
        channel_to_output_idx[rgba_order[i]] = channel_names.size() - 1;
        found = true;
        break;
      }
    }
  ...
  const int data_channels = channel_names.size();

  /* Find Z channel */
  int z_channel = -1;
  int channel_idx = 0;
  for (ChannelList::ConstIterator i = channels.begin(); i != channels.end(); ++i, ++channel_idx) {
    if (strcmp(i.name(), "Z") == 0) {
      z_channel = channel_idx;
      break;
    }
  }

  if (z_channel < 0) {
    CLOG_ERROR(&LOG, "Deep image missing Z channel");
    return false;
  }

  deep_buffer.sample_counts.resize(pixel_count);

  DeepFrameBuffer frame_buffer;
  frame_buffer.insertSampleCountSlice(
      Slice(Imf::UINT,
            (char *)(deep_buffer.sample_counts.data() - dw.min.x - dw.min.y * data_width),
            sizeof(int),
            sizeof(int) * data_width));

  deep_in.setFrameBuffer(frame_buffer);
  deep_in.readPixelSampleCounts(dw.min.y, dw.max.y);

  int total_samples = 0;
  deep_buffer.sample_offsets.resize(pixel_count + 1);
  deep_buffer.sample_offsets[0] = 0;
  for (int i = 0; i < pixel_count; i++) {
    total_samples += deep_buffer.sample_counts[i];
  ...
  deep_buffer.channels_per_sample = data_channels;
  deep_buffer.channel_data.resize(total_samples * data_channels);
  deep_buffer.channel_names = channel_names;

  /* Build pointer arrays - ONE per output channel position */
  Vector<float *> depth_pointers(pixel_count);
  Vector<Vector<float *>> channel_pointers(data_channels);

  for (int ch = 0; ch < data_channels; ch++) {
    channel_pointers[ch].resize(pixel_count);
  }

  for (int pixel_idx = 0; pixel_idx < pixel_count; pixel_idx++) {
    const int sample_offset = deep_buffer.sample_offsets[pixel_idx];
    const int sample_count = deep_buffer.sample_counts[pixel_idx];

    if (sample_count > 0) {
      depth_pointers[pixel_idx] = &deep_buffer.depths[sample_offset];

      for (int ch = 0; ch < data_channels; ch++) {
        channel_pointers[ch][pixel_idx] =
            &deep_buffer.channel_data[sample_offset * data_channels + ch];
      }
    }
    else {
      depth_pointers[pixel_idx] = nullptr;
      for (int ch = 0; ch < data_channels; ch++) {
        channel_pointers[ch][pixel_idx] = nullptr;
      }
    }
  }

  /* Set up frame buffer for reading - map each file channel to correct output position */
  ...
  for (ChannelList::ConstIterator i = channels.begin(); i != channels.end(); ++i, ++channel_idx) {
    ...
    if (channel_idx == z_channel) {
      /* Z channel */
      frame_buffer.insert(
          channel_name,
          DeepSlice(Imf::FLOAT,
                    (char *)(depth_pointers.data() - dw.min.x - dw.min.y * data_width),
                    sizeof(float *),
                    sizeof(float *) * data_width,
                    sizeof(float)));
    }
    else {
      /* Data channel - find its output position */
      auto it = channel_to_output_idx.find(channel_name);
      if (it != channel_to_output_idx.end()) {
        int output_idx = it->second;
        frame_buffer.insert(channel_name,
                            DeepSlice(Imf::FLOAT,
                                      (char *)(channel_pointers[output_idx].data() - dw.min.x -
                                               dw.min.y * data_width),
                                      sizeof(float *),
                                      sizeof(float *) * data_width,
                                      sizeof(float) * data_channels));
      }
    }
  }
...
/* Read deep tiled EXR into ImBufDeepBuffer */
static bool imb_read_deep_tiles(Imf::DeepTiledInputPart &deep_in,
                                ImBufDeepBuffer &deep_buffer,
                                int width,
                                int height)
{
  ...
  /* Allocate sample counts - use data_width for stride */
  deep_buffer.sample_counts.resize(pixel_count);

  /* Read sample counts for all pixels */
  DeepFrameBuffer frame_buffer;
  frame_buffer.insertSampleCountSlice(
      Slice(Imf::UINT,
            (char *)(deep_buffer.sample_counts.data() - dw.min.x - dw.min.y * data_width),
  ...
  deep_in.readPixelSampleCounts(0, deep_in.numXTiles() - 1, 0, deep_in.numYTiles() - 1);

  /* Calculate total samples and build offsets */
  int total_samples = 0;
  deep_buffer.sample_offsets.resize(pixel_count + 1);
  deep_buffer.sample_offsets[0] = 0;
  for (int i = 0; i < pixel_count; i++) {
  ...
  /* Build pointers - iterate over actual pixel count */
  for (int pixel_idx = 0; pixel_idx < pixel_count; pixel_idx++) {
    const int sample_offset = deep_buffer.sample_offsets[pixel_idx];
    const int sample_count = deep_buffer.sample_counts[pixel_idx];
    ...
    if (sample_count > 0) {
      ...
      /* Pixel has no samples - set to nullptr */
      depth_pointers[pixel_idx] = nullptr;
      for (int ch = 0; ch < data_channels; ch++) {
        channel_pointers[ch][pixel_idx] = nullptr;
      }
  ...
  /* Re-insert sample count slice - use data_width */
  frame_buffer.insertSampleCountSlice(
      Slice(Imf::UINT,
            (char *)(deep_buffer.sample_counts.data() - dw.min.x - dw.min.y * data_width),
            sizeof(int),
  ...
  /* Read all tile data */
  deep_in.setFrameBuffer(frame_buffer);
  deep_in.readTiles(0, deep_in.numXTiles() - 1, 0, deep_in.numYTiles() - 1);
                                    ...
                                    ImFileColorSpace &r_colorspace)
{
  ...
  /* Allocate ImBuf (no regular pixel data, only deep data) */
  ...
  ibuf->foptions.flag |= openexr_header_get_compression(header);

  imb_exr_set_known_colorspace(header, r_colorspace);
  ...
  try {
    ...
    if (header.type() == DEEPSCANLINE) {
      ...
      DeepTiledInputPart deep_in(file, 0);
      if (!imb_read_deep_tiles(deep_in, ibuf->deep_buffer, width, height)) {
        IMB_freeImBuf(ibuf);
        return nullptr;
      }
    }
    else {
      CLOG_ERROR(&LOG, "Unsupported deep image type");
      IMB_freeImBuf(ibuf);
      return nullptr;
    }
  }
  catch (const std::exception &exc) {
    CLOG_ERROR(&LOG, "Failed to read deep EXR: %s", exc.what());
    IMB_freeImBuf(ibuf);
    return nullptr;
  }
