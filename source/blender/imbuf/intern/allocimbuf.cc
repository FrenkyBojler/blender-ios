    ...
    uint8_t *byte_buffer, float *float_buffer, uint w, uint h, uint channels)
{
  ...
  ImBuf *ibuf = IMB_allocImBuf(w, h, ImColorMode::RGBA, 0);
    ...
    const uint8_t *byte_buffer, const float *float_buffer, uint w, uint h, uint channels)
{
  ...
  ibuf = IMB_allocImBuf(w, h, ImColorMode::RGBA, 0);
...
ImBuf *IMB_allocImBuf(uint x, uint y, ImColorMode color_mode, uint flags)
{
  ImBuf *ibuf = MEM_new<ImBuf>("ImBuf_struct");

  if (ibuf) {
    if (!IMB_initImBuf(ibuf, x, y, color_mode, flags)) {
      IMB_freeImBuf(ibuf);
      return nullptr;
    }
  }
...
bool IMB_initImBuf(ImBuf *ibuf, uint x, uint y, ImColorMode color_mode, uint flags)
{
  *ibuf = ImBuf{};
...
ImBuf *IMB_dupImBuf(const ImBuf *ibuf1)
{
  ...
  ImBuf *ibuf2 = IMB_allocImBuf(ibuf1->x, ibuf1->y, ibuf1->color_mode, 0);
  if (ibuf2 == nullptr) {
    return nullptr;
  }
  ibuf2->x = ibuf1->x;
