...
static void imb_scale_box(const BufferT *src_buffer,
                          ...
                          const bool threaded)
{
  BufferT *tmp_buffer = MEM_new_array_uninitialized_aligned<BufferT>(
      int64_t(channels) * dst_size.x * src_size.y, IMBUF_FLOAT_ALIGNMENT, __func__);
    ...
    case IMBScaleFilter::Nearest: {
      if (const float *src = ibuf->float_data()) {
        float *dst = MEM_new_array_uninitialized_aligned<float>(
            size_t(ibuf->channels) * new_size.x * new_size.y, IMBUF_FLOAT_ALIGNMENT, __func__);
