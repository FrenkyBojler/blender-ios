                                                ...
                                                const bool has_keep_vert)
{
  ...
  auto deduplicate_target_calc_fn = [&verts](const int *cluster, const int cluster_num) -> int {
    ...
    for (int i = 0; i < cluster_num; i++) {
      center += blender::float3(verts[cluster[i]]->co);
    }
    center /= float(cluster_num);
    ...
    float dist_sq_best = len_squared_v3v3(center, verts[cluster[i_best]]->co);
    for (int i = 0; i < cluster_end; i++) {
      const float dist_sq_test = len_squared_v3v3(center, verts[cluster[i]]->co);
