...
#define SPLIT_INPUT 1
  ...
  BMO_mesh_delete_oflag_context(
      ...
      [&bm, &dupeop, &splitop]() {
        ...
        BMO_slot_copy(
            &dupeop, slots_out, "isovert_map.out", splitop, slots_out, "isovert_map.out");

        /* `boundary_map.out` can't use #BMO_slot_copy` because some of the "source"
         * vertices will be removed. In this case the source - destination map.
         * In this case there is not an especially good option. */
        BMOpSlot *splitop_slot_boundary_map = BMO_slot_get(splitop->slots_out, "boundary_map.out");
        BMOIter siter;
        BMElem *ele_key;
        BMO_ITER (ele_key, &siter, dupeop.slots_out, "boundary_map.out", 0) {
          BMElem *ele_val = static_cast<BMElem *>(BMO_iter_map_value_ptr(&siter));
          if (BMO_elem_flag_test(bm, ele_key, SPLIT_INPUT)) {
            ele_key = ele_val;
          }
          BMO_slot_map_elem_insert(splitop, splitop_slot_boundary_map, ele_key, ele_val);
        }
      });
