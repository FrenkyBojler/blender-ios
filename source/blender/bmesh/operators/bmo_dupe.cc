  ...
  for (int a = 0; a < steps; a++) {
    ...
    if (do_dupli) {
      ...
      if ((use_merge && (a == steps - 1)) == false) {
         ...
         * runs as a single fresh rotation from the original position rather than
         * compounding the previous step's rotation (avoids precision loss from chained
         * rotations, see: #148890). */
        if (a != 0) {
