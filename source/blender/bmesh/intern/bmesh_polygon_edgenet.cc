        ...
        if (index_other != -1) {
...
#endif
          {
            ...
            if ((edge_net_new[edge_net_new_index] = e_new)) {
              ...
              /* If a duplicate already exists this will assert during cleanup,
               * catch the error on the input to avoid confusion. */
              BLI_assert(BM_edge_find_double(e_new) == nullptr);
              edge_net_new_index++;
