...
/**
 * Check loops have matching winding.
 * Topological dista
 */
bool bm_loop_match_verts_same_winding(BMLoop *l_a, BMLoop *l_a_end, BMLoop *l_b, BMLoop *l_b_end)
{
#ifndef NDEBUG
  /* Ensure the loops have correct topological distance. */
  {
    BMLoop *l_a_iter = l_a;
    BMLoop *l_b_iter = l_b;
    while (true) {
...
bool bm_loop_match_verts_swap_winding(BMLoop *l_a, BMLoop *l_a_end, BMLoop *l_b, BMLoop *l_b_end)
{
#ifndef NDEBUG
  /* Ensure the loops have correct topological distance. */
  {
...
bool BM_vert_splice_check_double_face(BMVert *v_a, BMVert *v_b)
{
  ...
  while (a_beg < loops_pair[0].size() && b_beg < loops_pair[1].size()) {
    ...
    for (int ai = a_beg; ai < a_end; ai++) {
      BMLoop *l_a = loops_pair[0][ai];
      for (int bi = b_beg; bi < b_end; bi++) {
        BMLoop *l_b = loops_pair[1][bi];
        if ((l_a->next->v == l_b->next->v) && (l_a->prev->v == l_b->prev->v)) {
          if (bm_loop_match_verts_same_winding(
          ...
          if (bm_loop_match_verts_swap_winding(
    ...
    /* Advance past this length range. */
    a_beg = a_end;
    b_beg = b_end;
  }
  return false;
  ...
  do {
    ...
    do {
      ...
      if (swap_winding) {
        ...
        if (bm_loop_match_verts_swap_winding(l_collapse_beg->next,
                                             l_collapse_end->prev,
                                             l_exists_beg->prev,
                                             l_exists_end->next)) /* Wrap. */
        {
          return true;
        }
      }
        ...
        if (bm_loop_match_verts_same_winding(l_collapse_beg->next,
                                             l_collapse_end->prev,
                                             l_exists_beg->next,
                                             l_exists_end->prev)) /* Wrap. */
        {
          return true;
        }
      }
