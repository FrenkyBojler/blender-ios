...
bool BM_mesh_is_valid(BMesh *bm)
{
  ...
  BM_ITER_MESH_INDEX (f, &iter, bm, BM_FACES_OF_MESH, i) {
    ...
    BMLoop *l_iter_vert_min = l_first;
    j = 0;
    l_iter = l_first;
    do {
      ...
      if (BM_elem_index_get(l_iter->v) < BM_elem_index_get(l_iter_vert_min->v)) {
        l_iter_vert_min = l_iter;
      }
      j++;
    } while ((l_iter = l_iter->next) != l_first);
    ...
    if (BM_elem_index_get(l_iter_vert_min->next->v) < BM_elem_index_get(l_iter_vert_min->prev->v))
    {
      l_iter = l_iter_vert_min;
      do {
        face_verts.append_unchecked(BM_elem_index_get(l_iter->v));
      } while ((l_iter = l_iter->next) != l_iter_vert_min);
    }
    else {
      l_iter = l_iter_vert_min;
      do {
        face_verts.append_unchecked(BM_elem_index_get(l_iter->v));
      } while ((l_iter = l_iter->prev) != l_iter_vert_min);
    }
