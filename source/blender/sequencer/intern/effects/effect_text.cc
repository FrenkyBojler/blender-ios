                                ...
                                Vector<CharInfo> &characters)
{
  ...
  for (CharInfo &character : characters) {
    ...
    if (character.do_wrap) {
      runtime->lines.append(LineInfo());
      cur_pixel_pos.x = 0;
      cur_pixel_pos.y -= runtime->line_height * data->space_line + data->abs_space_line;
    }
  }
}
...
static void calc_boundbox(const TextVars *data, TextVarsRuntime *runtime, const int2 image_size)
{
  ...
  const int text_height = (runtime->lines.size() - 1) * (runtime->line_height * data->space_line + data->abs_space_line) +
                                 ...
                                 const int2 image_size)
{
  ...
  const int box_height = runtime->line_height +
                         (runtime->lines.size() - 1) * ( data->abs_space_line + runtime->line_height * data->space_line);
