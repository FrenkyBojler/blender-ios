...
{
  const int effect_strip_length = strip->right_handle_frame_get(scene) -
  ...
  for (int frame_index = 1; frame_index < effect_strip_length; frame_index++) {
    target_frame += evaluate_fcurve(fcu, strip->left_handle_frame_get() + frame_index);
    const int target_frame_max = strip->input1->length_get(scene);
                                          ...
                                          int input)
{
  ...
  switch (s->speed_control_type) {
    case SEQ_SPEED_STRETCH: {
      ...
      const float target_content_length = source->length_get(scene) - source->startofs;
      const float speed_effetct_length = strip_speed->right_handle_frame_get(scene) -
                                         strip_speed->left_handle_frame_get();
    ...
    case SEQ_SPEED_LENGTH:
      target_frame = source->length_get(scene) * (s->speed_fader_length / 100.0f);
      break;
  ...
  CLAMP(target_frame, 0, source->length_get(scene));
