...
static void flag_users_for_removal(Scene *scene, ListBase *seqbase, Strip *strip)
{
  LISTBASE_FOREACH (Strip *, user_strip, seqbase) {
    /* Look in meta-strips for usage of strip. */
    if (user_strip->type == STRIP_TYPE_META) {
      flag_users_for_removal(scene, &user_strip->seqbase, strip);
    }

    /* Clear strip from modifiers. */
    LISTBASE_FOREACH (SequenceModifierData *, smd, &user_strip->modifiers) {
    ...
    if (relations_is_effect_of_strip(user_strip, strip)) {
      user_strip->flag |= SEQ_FLAG_DELETE;
      /* Strips can be used as mask even if not in same seqbase. */
      flag_users_for_removal(scene, &scene->ed->seqbase, user_strip);
    }
  }
}
...
void flag_strips_for_removal(Scene *scene, ListBase *seqbase, Strip *strip)
{
  ...
  if (strip->type == STRIP_TYPE_META) {
    LISTBASE_FOREACH (Strip *, meta_child, &strip->seqbase) {
      flag_strips_for_removal(scene, &strip->seqbase, meta_child);
    }
  }

  strip->flag |= SEQ_FLAG_DELETE;
  flag_users_for_removal(scene, seqbase, strip);
}
...
void remove_flagged_strips(Scene *scene, ListBase *seqbase)
{
  LISTBASE_FOREACH_MUTABLE (Strip *, strip, seqbase) {
    if (strip->flag & SEQ_FLAG_DELETE) {
      if (strip->type == STRIP_TYPE_META) {
        remove_flagged_strips(scene, &strip->seqbase);
      }
...
{
  /* Move to meta. */
  BLI_remlink(seqbase_src, strip);
  BLI_addtail(seqbase_dst, strip);
  relations_invalidate_cache_preprocessed(scene, strip);

  /* Update meta. */
  if (transform_test_overlap(scene, seqbase_dst, strip)) {
    transform_seqbase_shuffle(seqbase_dst, strip, scene);
  }
                        ...
                        const char **r_error_str)
{
  ...
  ListBase *seqbase = seqbase_by_strip_get(scene, strip_src);

  if (meta_strip_dst->type != STRIP_TYPE_META) {
    *r_error_str = N_("Cannot move strip to non-meta strip");
    return false;
  }

  if (strip_src == meta_strip_dst) {
    *r_error_str = N_("Strip cannot be moved into itself");
    return false;
  }

  if (seqbase == &meta_strip_dst->seqbase) {
    *r_error_str = N_("Moved strip is already inside provided meta strip");
    return false;
  }

  if (strip_src->type == STRIP_TYPE_META && exists_in_seqbase(meta_strip_dst, &strip_src->seqbase))
  {
    *r_error_str = N_("Moved strip is parent of provided meta strip");
    return false;
  }

  if (!exists_in_seqbase(meta_strip_dst, &ed->seqbase)) {
    *r_error_str = N_("Cannot move strip to different scene");
    return false;
  }

  blender::VectorSet<Strip *> strips;
  strips.add(strip_src);
  iterator_set_expand(scene, seqbase, strips, query_strip_effect_chain);

  for (Strip *strip : strips) {
    /* Move to meta. */
    strip_move_to_seqbase(scene, seqbase, strip, &meta_strip_dst->seqbase);
  }

  return true;
}

static void split_set_right_hold_offset(Main *bmain,
                                        Scene *scene,
                                        Strip *strip,
                                        int timeline_frame)
{
...
static void split_set_left_hold_offset(Main *bmain, Scene *scene, Strip *strip, int timeline_frame)
{
  const float content_start = time_start_frame_get(strip);
  const float content_end = time_content_end_frame_get(scene, strip);
...
static bool split_intersect_check(const Scene *scene, const Strip *strip, const int timeline_frame)
{
  return timeline_frame > time_left_handle_frame_get(scene, strip) &&
         timeline_frame < time_right_handle_frame_get(scene, strip);
}

static void split_handle_strip_offsets(Main *bmain,
                                       Scene *scene,
                                       Strip *left_seq,
                                       Strip *right_seq,
...
{
  if (split_intersect_check(scene, right_seq, timeline_frame)) {
    switch (method) {
      case SPLIT_SOFT:
        time_left_handle_frame_set(scene, right_seq, timeline_frame);
        break;
      case SPLIT_HARD:
        split_set_left_hold_offset(bmain, scene, right_seq, timeline_frame);
        break;
    }
  }

  if (split_intersect_check(scene, left_seq, timeline_frame)) {
    switch (method) {
      case SPLIT_SOFT:
        time_right_handle_frame_set(scene, left_seq, timeline_frame);
        break;
      case SPLIT_HARD:
        split_set_right_hold_offset(bmain, scene, left_seq, timeline_frame);
        break;
    }
  }
}

static bool effect_inputs_intersect(const Scene *scene,
                                    const Strip *strip,
                                    const int timeline_frame)
{
  bool input_does_intersect = false;
  if (strip->seq1) {
    input_does_intersect |= split_intersect_check(scene, strip->seq1, timeline_frame);
    if ((strip->seq1->type & STRIP_TYPE_EFFECT) != 0) {
      input_does_intersect |= effect_inputs_intersect(scene, strip->seq1, timeline_frame);
    }
  }
  if (strip->seq2) {
    input_does_intersect |= split_intersect_check(scene, strip->seq2, timeline_frame);
    if ((strip->seq1->type & STRIP_TYPE_EFFECT) != 0) {
      input_does_intersect |= effect_inputs_intersect(scene, strip->seq2, timeline_frame);
    }
  }
  return input_does_intersect;
}

static bool split_operation_permitted_check(const Scene *scene,
                                            ...
                                            const char **r_error)
{
  for (Strip *strip : strips) {
    ...
    if (!split_intersect_check(scene, strip, timeline_frame)) {
      continue;
    }
    ...
    if (!effect_inputs_intersect(scene, strip, timeline_frame)) {
                   ...
                   const char **r_error)
{
  ...
  for (Strip *strip : strips_old) {
    ...
    connections.remove_if([&](Strip *connection) {
      return !(connection->flag & SELECT) ||
             !split_intersect_check(scene, connection, timeline_frame);
    });
  ...
  if (!split_operation_permitted_check(scene, strips, timeline_frame, r_error)) {
    return nullptr;
  }
  ...
  while (left_strip && right_strip) {
    if (time_left_handle_frame_get(scene, left_strip) >= timeline_frame) {
      flag_strips_for_removal(scene, seqbase, left_strip);
    }
    else if (time_right_handle_frame_get(scene, right_strip) <= timeline_frame) {
      flag_strips_for_removal(scene, seqbase, right_strip);
    }
    ...
    split_handle_strip_offsets(bmain, scene, left_strip, right_strip, timeline_frame, method);
    left_strip = left_strip->next;
    right_strip = right_strip->next;
  }

  remove_flagged_strips(scene, seqbase);
  animation_restore_original(scene, &animation_backup);

  return return_strip;
}

bool remove_gaps(Scene *scene,
                 ListBase *seqbase,
                 const int initial_frame,
                 const bool remove_all_gaps)
{
  GapInfo gap_info = {0};
  time_gap_info_get(scene, seqbase, initial_frame, &gap_info);
