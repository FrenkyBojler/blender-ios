                                  ...
                                  const bool is_proxy_image)
{
  ...
  if (strip->modifiers.first) {
    ...
    ModifierApplyContext mod_context(
        *context, *state, *strip, matrix, timeline_frame, result.image);
  ...
  if (view3d_fn && is_preview && camera) {
    ...
    for (int view_id = 0; view_id < totviews; view_id++) {
      ...
      if (rres.ibuf && rres.ibuf->float_data()) {
        ibufs_arr[view_id].image = IMB_allocImBuf(
            rres.rectx, rres.recty, ImBufFlags::FloatData | ImBufFlags::UninitializedPixels);
        memcpy(ibufs_arr[view_id].image->float_data_for_write(),
        ...
        memcpy(ibufs_arr[view_id].image->byte_data_for_write(),
               rres.ibuf->byte_data(),
