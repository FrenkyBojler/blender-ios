                               ...
                               const bool is_proxy_image)
{
      ...
      context->recty != ibuf->y || modifier_translation != float2(0, 0))
  {
    ...
    ImBuf *transformed_ibuf = IMB_allocImBuf(
        x, y, ImColorMode::RGBA, ibuf->float_data() ? IB_float_data : IB_byte_data);
                                           ...
                                           float timeline_frame)
{
  ...
  if (!sh.execute) {
    ...
    out = IMB_allocImBuf(context->rectx, context->recty, ImColorMode::RGBA, IB_byte_data);
    return out;
  }
...
static ImBuf *create_missing_media_image(const RenderData *context, int width, int height)
{
  ...
  ImBuf *ibuf = IMB_allocImBuf(
      max_ii(width, 1), max_ii(height, 1), ImColorMode::RGBA, IB_byte_data);
                       ...
                       bool make_float)
{
  ...
  ImBuf *ibuf = IMB_allocImBuf(width,
                               height,
                               ImColorMode::RGBA,
                               (make_float ? IB_float_data : IB_byte_data) |
    ...
    for (int view_id = 0; view_id < totviews; view_id++) {
      ...
      if (rres.ibuf && rres.ibuf->float_data()) {
        ibufs_arr[view_id] = IMB_allocImBuf(
            rres.rectx, rres.recty, ImColorMode::RGBA, IB_float_data | IB_uninitialized_pixels);
        ...
        ibufs_arr[view_id] = IMB_allocImBuf(
            rres.rectx, rres.recty, ImColorMode::RGBA, IB_byte_data | IB_uninitialized_pixels);
        ...
        ibufs_arr[view_id] = IMB_allocImBuf(
            rres.rectx, rres.recty, ImColorMode::RGBA, IB_byte_data);
      }
                        ...
                        float timeline_frame)
{
  ...
  if (ibuf == nullptr) {
    ibuf = IMB_allocImBuf(context->rectx, context->recty, ImColorMode::RGBA, IB_byte_data);
  }
      ...
      case StripEarlyOut::DoEffect:
        if (i == 0) {
          ...
          ImBuf *ibuf1 = IMB_allocImBuf(context->rectx,
                                        context->recty,
                                        ImColorMode::RGBA,
                                        use_float ? IB_float_data : IB_byte_data);
