...
static void strip_transform_handle_overwrite_trim(
    Scene *scene, ListBaseT<Strip> *seqbasep, const Strip *source, Strip *target, bool covers_left)
{
  ...
  for (int i = 0; i < targets.size(); i++) {
    ...
    for (Strip *source : source_strips) {
      ...
      /* Source strip only partially overlaps target strip. */
        ...
        strip_transform_handle_overwrite_trim(scene, seqbasep, source, target, covers_left);
      }
