...
BLI_INLINE static void lMatrix_reserve_elems(lMatrix &m, int num)
{
  m.reserve(Eigen::VectorXi::Constant(m.cols(), num));
}

BLI_INLINE static float *lVector_v3(lVector &v, int vertex)
{
  return v.data() + 3 * vertex;
}

BLI_INLINE static const float *lVector_v3(const lVector &v, int vertex)
{
  return v.data() + 3 * vertex;
}

#  if 0
BLI_INLINE static void triplets_m3(TripletList &tlist, float m[3][3], int i, int j)
{
  i *= 3;
  j *= 3;
  for (int l = 0; l < 3; l++) {
...
BLI_INLINE static void triplets_m3fl(TripletList &tlist, float m[3][3], int i, int j, float factor)
{
  i *= 3;
  j *= 3;
  for (int l = 0; l < 3; l++) {
...
BLI_INLINE static void lMatrix_add_triplets(lMatrix &r, const TripletList &tlist)
{
...
BLI_INLINE static void lMatrix_madd_triplets(lMatrix &r, const TripletList &tlist, float f)
{
  lMatrix t(r.rows(), r.cols());
  t.setFromTriplets(tlist.begin(), tlist.end());
  r += f * t;
...
BLI_INLINE static void lMatrix_sub_triplets(lMatrix &r, const TripletList &tlist)
{
  lMatrix t(r.rows(), r.cols());
  t.setFromTriplets(tlist.begin(), tlist.end());
  r -= t;
...
BLI_INLINE static void cross_m3_v3m3(float r[3][3], const float v[3], float m[3][3])
{
...
BLI_INLINE static void madd_m3_m3fl(float r[3][3], float m[3][3], float f)
{
  r[0][0] += m[0][0] * f;
  r[0][1] += m[0][1] * f;
  r[0][2] += m[0][2] * f;
...
BLI_INLINE static void madd_m3_m3m3fl(float r[3][3], float a[3][3], float b[3][3], float f)
{
  r[0][0] = a[0][0] + b[0][0] * f;
  r[0][1] = a[0][1] + b[0][1] * f;
  r[0][2] = a[0][2] + b[0][2] * f;
...
BLI_INLINE static void apply_spring(
...
BLI_INLINE static void spring_angbend_forces(Implicit_Data *data,
                                             int i,
                                             int j,
                                             int k,
                                             const float goal[3],
...
BLI_INLINE static void spring_angbend_estimate_dfdx(Implicit_Data *data,
                                                    int i,
                                                    int j,
                                                    int k,
                                                    const float goal[3],
...
/* Finite Differences method for estimating the jacobian of the force */
BLI_INLINE static void spring_angbend_estimate_dfdv(Implicit_Data *data,
                                                    int i,
                                                    int j,
                                                    int k,
                                                    const float goal[3],
