...
static CursorSize window_size_calc()
{
...
#endif
  constexpr float cursor_size_default = 24;
  const uint32_t cursor_size = WM_cursor_preferred_logical_size();

  const float cursor_scale = cursor_size ? (float(cursor_size) / cursor_size_default) : 1.0f;
  ...
  const int dpi_system = int((U.dpi * cursor_scale) / U.ui_scale);
