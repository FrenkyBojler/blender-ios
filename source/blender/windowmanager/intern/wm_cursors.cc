...
enum CursorSize { CURSOR_SMALL = 0, CURSOR_MEDIUM, CURSOR_LARGE, TOT_CURSOR_SIZES };

/* Blender custom cursor. */
struct BCursor {
  char *bitmap[TOT_CURSOR_SIZES];
  char *mask[TOT_CURSOR_SIZES];
  char hotspot[TOT_CURSOR_SIZES][2];
...
static bool window_set_custom_cursor(wmWindow *win, BCursor *cursor)
{
  ...
  return GHOST_SetCustomCursorShape(static_cast<GHOST_WindowHandle>(win->ghostwin),
                                    (uint8_t *)cursor->bitmap[CURSOR_SMALL],
                                    (uint8_t *)cursor->mask[CURSOR_SMALL],
                                    16,
                                    16,
                                    cursor->hotspot[CURSOR_SMALL][0],
                                    cursor->hotspot[CURSOR_SMALL][1],
                                    cursor->can_invert_color);
}
