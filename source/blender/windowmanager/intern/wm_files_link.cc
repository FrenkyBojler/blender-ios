                                     ...
                                     const bool do_append)
{
  ...
  if (!group || !name) {
    CLOG_WARN(LOG_BLEND_LINK, "Skipping %s", filepath);
    return false;
  }
...
static wmOperatorStatus wm_lib_relocate_exec_do(bContext *C, wmOperator *op, bool do_reload)
{
  ...
  if (BLI_path_cmp(lib->runtime->filepath_abs, filepath) == 0) {
    CLOG_DEBUG(
        LOG_BLEND_LINK, "We are supposed to reload '%s' lib (%d)", lib->filepath, lib->id.us);
    ...
    if (totfiles) {
      RNA_BEGIN (op->ptr, itemptr, "files") {
        ...
        CLOG_DEBUG(LOG_BLEND_LINK, "\tCandidate new lib to reload datablocks from: %s", filepath);
        BKE_blendfile_link_append_context_library_add(lapp_context, filepath, nullptr);
      }
      RNA_END;
    }
