...
{
  wmWindow *win = CTX_wm_window(C);
  ui::popup_block_close(C, win, static_cast<ui::Block *>(arg_block));
}
...
static ui::Block *block_create_unused_warning_dialog(bContext *C, ARegion *region, void *arg1)
{
  ui::Block *block = ui::block_begin(C, region, __func__, blender::ui::EmbossType::Emboss);
  ui::block_flag_enable(
  ...
  ui::Layout *layout = uiItemsAlertBox(block, 34, ui::AlertIcon::Warning);

  /* Title. */
  uiItemL_ex(layout, RPT_("File Contains Unused Data"), ICON_NONE, true, false);
  layout->separator(0.1f);
  layout->label(RPT_("This file contains some data that does not have any users."), ICON_NONE);
  ...
  if (windows_layout) {
    ...
    ui::Button *but = uiDefIconTextBut(block,
                                       ...
                                       nullptr,
                                       std::nullopt);
    button_func_set(but, wm_block_unused_warning_manage, block, nullptr);
    ...
    but = uiDefIconTextBut(block,
                           ...
                           nullptr,
                           std::nullopt);
    button_func_set(but, wm_block_unused_warning_okay, block, nullptr);
    button_drawflag_disable(but, ui::BUT_TEXT_LEFT);
  ...
  if (success) {
    ...
    if (U.warn_unused_data) {
      ...
      if (data.num_total[INDEX_ID_NULL] > 0) {
        ...
        CTX_wm_window_set(C, win);
        ui::popup_block_invoke(C, block_create_unused_warning_dialog, nullptr, nullptr);
