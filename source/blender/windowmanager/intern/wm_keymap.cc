...
{
  if (wm_keymap_update_flag != 0) {
    /* NOTE: this could be limited to the keymaps marked for updating.
     * However #KEYMAP_UPDATE is only cleared for `wm->userconf`
     * so only check the global flag for now. */
    BKE_reportf(
        reports,
        RPT_ERROR,
        "KeyMap cannot run when an update is pending, "
        "call `context.window_manager.keyconfigs.update()` to ensure matches can be found.");
    return nullptr;
  }

  if (km_base == km_match) {
    /* We could also return `kmi_match` (it's technically correct)
     * however this hints at API misuse. */
  ...
  /* Returning null here is not an error. */
