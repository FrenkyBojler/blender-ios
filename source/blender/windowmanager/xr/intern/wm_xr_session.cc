...
{
  if (!WM_xr_session_is_ready(xr) || !xr->runtime->session_state.is_view_data_set ||
      (subaction_idx >= xr->runtime->session_state.controllers.size()))
  {
    zero_v3(r_location);
    return false;
  }
                                                 ...
                                                 wmXrSessionState *state)
{
  ...
  BLI_assert(grip_action->count_subaction_paths == state->controllers.size());
                                            ...
                                            const GHOST_XrDrawViewInfo *draw_view)
{
  ...
  if (draw_view->view_idx >= surface_data->viewports.size()) {
