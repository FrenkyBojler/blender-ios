...
namespace blender::nodes::node_shader_volume_coeffs_cc {
...
static void node_shader_init_coeffs(bNodeTree * /*ntree*/, bNode *node)
{
  node->custom1 = SHD_PHASE_HENYEY_GREENSTEIN;
}

static void node_shader_update_coeffs(bNodeTree *ntree, bNode *node)
{
...
static int node_shader_gpu_volume_coeffs(GPUMaterial *mat,
                                         bNode *node,
                                         ...
                                         GPUNodeStack *out)
{
  ...
  return GPU_stack_link(mat, node, "node_volume_coeffs", in, out);
}

#undef SOCK_ABS_COEFFICIENTS_ID
#undef SOCK_SCT_COEFFICIENTS_ID

}  // namespace blender::nodes::node_shader_volume_coeffs_cc

/* node type definition */
void register_node_type_sh_volume_coeffs()
{
  namespace file_ns = blender::nodes::node_shader_volume_coeffs_cc;

  static blender::bke::bNodeType ntype;

  sh_node_type_base(&ntype, "ShaderNodeVolumeCoeffs", SH_NODE_VOLUME_COEFFS);
  ntype.ui_name = "Volume Coeffs";
  ntype.ui_description = "Scatter and absorb light as it passes through the volume";
  ntype.enum_name_legacy = "VOLUME_COEFFS";
  ntype.nclass = NODE_CLASS_SHADER;
  ntype.declare = file_ns::node_declare;
  ntype.add_ui_poll = object_shader_nodes_poll;
  ntype.draw_buttons = file_ns::node_shader_buts_coeffs;
  blender::bke::node_type_size_preset(ntype, blender::bke::eNodeSizePreset::Middle);
  ntype.initfunc = file_ns::node_shader_init_coeffs;
  ntype.gpu_fn = file_ns::node_shader_gpu_volume_coeffs;
  ntype.updatefunc = file_ns::node_shader_update_coeffs;
