...
static void math_input_defaults(bNode &node, NodeMathOperation mode)
{
  bNodeSocket *socket_2 = bke::node_find_socket(node, eNodeSocketInOut::SOCK_IN, "Value_001");
  BLI_assert(socket_2 != nullptr);
  float &value_2 = socket_2->default_value_typed<bNodeSocketValueFloat>()->value;

  bNodeSocket *socket_3 = bke::node_find_socket(node, eNodeSocketInOut::SOCK_IN, "Value_002");
