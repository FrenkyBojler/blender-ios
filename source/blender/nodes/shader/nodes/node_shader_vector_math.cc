...
static void sh_node_vector_math_gather_link_searches(GatherLinkSearchOpParams &params)
{
       ...
       item++)
  {
    if (item->name != nullptr && item->identifier[0] != '\0') {
      ...
      {
        params.add_item(
            CTX_IFACE_(BLT_I18NCONTEXT_ID_NODETREE, item->name),
            SocketSearchOp{.socket_name = "Value", .mode = NodeVectorMathOperation(item->value)},
            weight);
      }
      else {
        params.add_item(
            CTX_IFACE_(BLT_I18NCONTEXT_ID_NODETREE, item->name),
            SocketSearchOp{.socket_name = "Vector", .mode = NodeVectorMathOperation(item->value)},
            weight);
      }
    }
  }
