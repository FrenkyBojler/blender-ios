...
static void node_shader_update_tex_gabor(bNodeTree *ntree, bNode *node)
{
  ...
  bNodeSocket *orientation_2d_socket = bke::node_find_socket(*node, SOCK_IN, "Orientation 2D"_ustr);
  bke::node_set_socket_availability(
      *ntree, *orientation_2d_socket, storage.type == SHD_GABOR_TYPE_2D);

  bNodeSocket *orientation_3d_socket = bke::node_find_socket(*node, SOCK_IN, "Orientation 3D"_ustr);
  bke::node_set_socket_availability(
