...
struct GeometryNodesLazyFunctionBuilder {
 private:
  ...
  /**
   * Context sockets that need to be linked in the end.
   */
  MultiValueMap<StringRef, const bNodeSocket *> context_output_sockets_by_identifier_;
 ...
 public:
  ...
  void build()
  {
    ...
    for (const bNode *node : btree_.all_nodes()) {
      ...
      if (node->type_legacy == GEO_NODE_CONTEXT_INPUT) {
        ...
        context_output_sockets_by_identifier_.add(context_identifier, &node->output_socket(0));
        continue;
      }
      ...
      for (const bNodeSocket *socket : node->output_sockets()) {
        ...
        context_output_sockets_by_identifier_.add(context_identifier, socket);
      }
    }
...

    {
      ...
      for (const int interface_input_i : interface_inputs.index_range()) {
        ...
        const Span<const bNodeSocket *> context_sockets =
            context_output_sockets_by_identifier_.lookup(context_identifier);
