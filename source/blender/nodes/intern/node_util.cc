...
{
  const NodeShaderMath *storage = static_cast<const NodeShaderMath *>(node->storage);
  /* Modern: Access NodeShaderMath storage for operation. */
  BLI_assert(storage);
  const int op = storage->operation;
                     ...
                     int label_maxncpy)
{
  ...
  const int op = storage->operation;
  const char *name;
  bool enum_label = RNA_enum_name(rna_enum_node_math_items, op, &name);
  if (!enum_label) {
