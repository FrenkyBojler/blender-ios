...
struct SocketUsageInferencer {
  ...
  void do_inference()
  {
    ...
    while (!tasks_.is_empty()) {
      ...
      switch (task.type) {
        case TaskType::Value: {
          handle_value_task(task.socket, tasks_, scope_, all_socket_values_);
          break;
        }
        case TaskType::Usage: {
          handle_usage_task(task.socket, tasks_, scope_, all_socket_usages_, all_socket_values_);
          break;
        }
      }
  ...
  void handle_multi_function_node_output_value(
  ...
  void handle_foreach_element_output_node_input_usage(
      const SocketInContext &socket,
      Stack<Task> &tasks,
      Map<SocketInContext, bool> &all_socket_usages)
  {
