                                                 ...
                                                 const Span<bNodeSocket *> new_sockets)
{
  ...
  if (!moved_indices.is_empty()) {
    auto handle_rna_path = [&](const StringRef old_path) -> char * {
    ...
    animrig::foreach_fcurve_in_action_slot(action, slot.handle, [&](FCurve &fcurve) {
      if (char *new_path = handle_rna_path(fcurve.rna_path())) {
        fcurve.set_rna_path_move(new_path);
      }
    });
    for (FCurve &driver_fcurve : ntree.adt->drivers) {
      if (char *new_path = handle_rna_path(driver_fcurve.rna_path())) {
        driver_fcurve.set_rna_path_move(new_path);
      }
    }
  }
