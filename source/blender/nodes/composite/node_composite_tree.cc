                                       ...
                                       ID **r_from)
{
  ...
  *r_from = nullptr;
  *r_id = &scene->id;
  *r_ntree = scene->compositing_nodetree;
}
...
void ntreeCompositTagRender(Scene *scene)
{
  ...
  {
    if (sce_iter->compositing_nodetree) {
      for (bNode *node : sce_iter->compositing_nodetree->all_nodes()) {
        if (node->id == (ID *)scene || node->type_legacy == CMP_NODE_COMPOSITE) {
          BKE_ntree_update_tag_node_property(sce_iter->compositing_nodetree, node);
        }
