...
class BoxMaskOperation : public NodeOperation {
 public:
  ...
  void execute_cpu()
  {
    ...
    switch (this->get_operation()) {
      case CMP_NODE_MASKTYPE_ADD:
        box_mask<CMP_NODE_MASKTYPE_ADD>(
            base_mask, value_mask, output_mask, domain, location, size, cos_angle, sin_angle);
        return;
      case CMP_NODE_MASKTYPE_SUBTRACT:
        box_mask<CMP_NODE_MASKTYPE_SUBTRACT>(
            base_mask, value_mask, output_mask, domain, location, size, cos_angle, sin_angle);
        return;
      case CMP_NODE_MASKTYPE_MULTIPLY:
        box_mask<CMP_NODE_MASKTYPE_MULTIPLY>(
            base_mask, value_mask, output_mask, domain, location, size, cos_angle, sin_angle);
        return;
      case CMP_NODE_MASKTYPE_NOT:
        box_mask<CMP_NODE_MASKTYPE_NOT>(
            base_mask, value_mask, output_mask, domain, location, size, cos_angle, sin_angle);
        return;
    }
