...
static void node_declare(NodeDeclarationBuilder &b)
{
  ...
  b.add_input<decl::Menu>("RGB Limit Channel"_ustr)
      ...
      .make_available([](bNode &node) {
        bNodeSocket &limit_method_socket = *bke::node_find_socket(node, SOCK_IN, "Limit Method"_ustr);
        limit_method_socket.default_value_typed<bNodeSocketValueMenu>()->value =
            CMP_NODE_CHANNEL_MATTE_LIMIT_ALGORITHM_SINGLE;

        bNodeSocket &color_space_socket = *bke::node_find_socket(node, SOCK_IN, "Color Space"_ustr);
