...
struct FieldToGridItemsAccessor : public socket_items::SocketItemsAccessorDefaults {
  ...
  static socket_items::SocketItemsRef<NodeEnumItem> get_items_from_node(bNode &node)
  {
    ...
    return {&storage.enum_definition.items_array, &storage.enum_definition.items_num, &storage.enum_definition.active_index};
  }
  ...
  static std::string input_socket_identifier_for_item(const NodeEnumItem &item)
  {
    return "Field_" + std::to_string(item.identifier);
  }

  static std::string output_socket_identifier_for_item(const NodeEnumItem &item)
  {
