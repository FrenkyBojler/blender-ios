...
struct FieldToGridItemsAccessor : public socket_items::SocketItemsAccessorDefaults {
  ...
  static socket_items::SocketItemsRef<FieldToGridItem> get_items_from_node(bNode &node)
  {
    auto &storage = *static_cast<NodeFieldToGrid *>(node.storage);
    return {&storage.items, &storage.items_num, nullptr};
  }

  static void copy_item(const FieldToGridItem &src, FieldToGridItem &dst)
  {
    dst = src;
  }

  static void destruct_item(FieldToGridItem * /*item*/) {}

  static void blend_write_item(BlendWriter *writer, const ItemT &item);
  static void blend_read_data_item(BlendDataReader *reader, ItemT &item);

  static void init(bNode &node, FieldToGridItem &item)
  {
    auto &storage = *static_cast<NodeFieldToGrid *>(node.storage);
  ...
  static std::string socket_identifier_for_item(const FieldToGridItem &item)
  {
