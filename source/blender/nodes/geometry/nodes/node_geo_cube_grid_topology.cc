...
static void node_declare(NodeDeclarationBuilder &b)
{
  ...
  b.add_input<decl::Int>("Min X").default_value(0).min(0).description(
      "Minimum coordinate in X axis (grid index space)");
  b.add_input<decl::Int>("Min Y").default_value(0).min(0).description(
      "Minimum coordinate in Y axis (grid index space)");
  b.add_input<decl::Int>("Min Z").default_value(0).min(0).description(
...
static void node_geo_exec(GeoNodeExecParams params)
{
  ...
  if (grid_min.x < 0 || grid_min.y < 0 || grid_min.z < 0) {
    params.error_message_add(NodeWarningType::Error, TIP_("Grid minimum must be positive"));
    params.set_default_remaining_outputs();
    return;
  }
  ...
  const int3 grid_max = grid_min + resolution - 1;
  ...
  const double3 scale_fac = double3(bounds_max - bounds_min) / double3(resolution + grid_min - 1);
