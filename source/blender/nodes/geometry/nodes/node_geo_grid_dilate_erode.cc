...
static void node_declare(NodeDeclarationBuilder &b)
{
  ...
  static EnumPropertyItem connectivity_items[] = {
      {.value = int(Connectivity::Face),
       .identifier = "FACE",
       .icon = 0,
       .name = "Face",
       .description = "6-connectivity: affect voxels connected by faces only"},
      {.value = int(Connectivity::FaceEdge),
       .identifier = "FACE_EDGE",
       .icon = 0,
       .name = "Edge",
       .description = "18-connectivity: affect voxels connected by faces or edges only"},
      {.value = int(Connectivity::FaceEdgeVertex),
       .identifier = "FACE_EDGE_VERTEX",
       .icon = 0,
       .name = "Vertex",
       .description = "26-connectivity: affect voxels connected by faces, edges, or vertices"},
      {.value = 0, .identifier = nullptr, .icon = 0, .name = nullptr, .description = nullptr},
  };

  static EnumPropertyItem tile_policy_items[] = {
      {.value = int(TilePolicy::Ignore),
       .identifier = "IGNORE",
       .icon = 0,
       .name = "Ignore",
       .description =
           "Ignore active tiles; they are neither dilated/eroded nor contribute to the operation"},
      {.value = int(TilePolicy::Expand),
       .identifier = "EXPAND",
       .icon = 0,
       .name = "Expand",
       .description = "Voxelize active tiles, apply operation, and leave in voxelized state"},
      {.value = int(TilePolicy::Preserve),
       .identifier = "PRESERVE",
       .icon = 0,
       .name = "Preserve",
       .description = "Keep tiles unchanged when possible, only voxelizing if necessary. More "
                      "memory efficient"},
      {.value = 0, .identifier = nullptr, .icon = 0, .name = nullptr, .description = nullptr},
  };
