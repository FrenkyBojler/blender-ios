...
static void create_selection_output(CurveComponent &component, const UString name)
{
  SpanAttributeWriter<bool> selection =
      component.attributes_for_write()->lookup_or_add_for_write_only_span<bool>(name,
                                                                                AttrDomain::Point);
...
static void node_geo_exec(GeoNodeExecParams params)
{
  ...
  {
    create_selection_output(output.get_component_for_write<CurveComponent>(),
                            UString(*outer_points_id));
  }
