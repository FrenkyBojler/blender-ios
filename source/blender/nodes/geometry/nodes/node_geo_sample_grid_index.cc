...
static void node_geo_exec(GeoNodeExecParams params)
{
  ...
  auto fn_ptr = std::make_shared<SampleGridIndexFunction>(std::move(grid));
  const mf::MultiFunction &fn = *fn_ptr;

  bke::SocketValueVariant x = params.extract_input<bke::SocketValueVariant>("X");
  bke::SocketValueVariant y = params.extract_input<bke::SocketValueVariant>("Y");
  bke::SocketValueVariant z = params.extract_input<bke::SocketValueVariant>("Z");

  bke::SocketValueVariant output_value;
  std::string error_message;
  const bool success = execute_multi_function_on_value_variant(
      fn, std::move(fn_ptr), {&x, &y, &z}, {&output_value}, params.user_data(), error_message);
  if (!success) {
    params.set_default_remaining_outputs();
