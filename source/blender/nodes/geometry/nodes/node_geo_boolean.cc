...
static void node_rna(StructRNA *srna)
{
  static const EnumPropertyItem rna_node_geometry_boolean_method_items[] = {
      {.value = int(geometry::boolean::Operation::Intersect),
       .identifier = "INTERSECT",
       .icon = 0,
       .name = "Intersect",
       .description = "Keep the part of the mesh that is common between all operands"},
      {.value = int(geometry::boolean::Operation::Union),
       .identifier = "UNION",
       .icon = 0,
       .name = "Union",
       .description = "Combine meshes in an additive way"},
      {.value = int(geometry::boolean::Operation::Difference),
       .identifier = "DIFFERENCE",
       .icon = 0,
       .name = "Difference",
       .description = "Combine meshes in a subtractive way"},
  ...
  static const EnumPropertyItem rna_geometry_boolean_solver_items[] = {
      {.value = int(geometry::boolean::Solver::MeshArr),
       .identifier = "EXACT",
       .icon = 0,
       .name = "Exact",
       .description = "Slower solver with the best results for coplanar faces"},
      {.value = int(geometry::boolean::Solver::Float),
       .identifier = "FLOAT",
       .icon = 0,
       .name = "Float",
       .description =
           "Simple solver with good performance, without support for overlapping geometry"},
      {.value = int(geometry::boolean::Solver::Manifold),
       .identifier = "MANIFOLD",
       .icon = 0,
       .name = "Manifold",
       .description =
