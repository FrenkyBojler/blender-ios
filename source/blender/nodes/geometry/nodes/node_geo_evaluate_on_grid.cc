...
class GridFieldContext : public FieldContext {
 private:
  int3 resolution_;
  float3 bounds_min_;
  float3 bounds_max_;
 ...
 public:
  GridFieldContext(const int3 resolution, const float3 bounds_min, const float3 bounds_max)
  ...
  bke::attribute_math::convert_to_static_type(
      *bke::socket_type_to_geo_nodes_base_cpp_type(data_type), [&](auto type_tag) {
        ...
        if constexpr (!std::is_same_v<typename type_traits::BlenderType, void>) {
          ...
          GridFieldContext context(resolution, bounds_min, bounds_max);
