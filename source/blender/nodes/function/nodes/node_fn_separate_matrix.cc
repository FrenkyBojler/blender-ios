...
static void node_eval_elem(value_elem::ElemEvalParams &params)
{
  ...
  for (const int col : IndexRange(4)) {
    for (const int row : IndexRange(4)) {
      ...
      params.set_output_elem(UString(socket.identifier), output_elems[col][row]);
    }
  }
}
...
static void node_eval_inverse_elem(value_elem::InverseElemEvalParams &params)
{
  ...
  for (const int col : IndexRange(4)) {
    for (const int row : IndexRange(4)) {
      const bNodeSocket &socket = params.node.output_socket(col * 4 + row);
      output_elems[col][row] = params.get_output_elem<FloatElem>(UString(socket.identifier));
    }
  }
...
static void node_eval_inverse(inverse_eval::InverseEvalParams &params)
{
  ...
  for (const int col : IndexRange(4)) {
    for (const int row : IndexRange(4)) {
      const bNodeSocket &socket = params.node.output_socket(col * 4 + row);
      matrix[col][row] = params.get_output<float>(UString(socket.identifier));
    }
  }
