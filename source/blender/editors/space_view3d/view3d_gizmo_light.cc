...
/* -------------------------------------------------------------------- */
/** \name Light Normal Positioning Gizmo
 * \{ */

static bool WIDGETGROUP_light_normal_positioning_poll(const bContext *C, wmGizmoGroupType * /*gzgt*/)
{
  View3D *v3d = CTX_wm_view3d(C);
  if (v3d->gizmo_flag & (V3D_GIZMO_HIDE | V3D_GIZMO_HIDE_CONTEXT)) {
    return false;
  ...
  if (base && BASE_SELECTABLE(v3d, base)) {
    ...
    if (BKE_id_is_editable(CTX_data_main(C), &ob->id)) {
      if (ob->type == OB_LAMP) {
        /* Available for all light types that can be positioned */
        const Light *la = static_cast<Light *>(ob->data);
        if (ELEM(la->type, LA_SUN, LA_SPOT, LA_AREA, LA_LOCAL)) {
          return true;
        }
      }
    }
...
static void WIDGETGROUP_light_normal_positioning_setup(const bContext * /*C*/, wmGizmoGroup *gzgroup)
{
  wmGizmoWrapper *wwrapper = MEM_mallocN<wmGizmoWrapper>(__func__);
  wwrapper->gizmo = WM_gizmo_new("GIZMO_GT_move_3d", gzgroup, nullptr);
  wmGizmo *gz = wwrapper->gizmo;
  ...
  /* Use a distinct color for normal positioning gizmo */
  float color[3] = {0.2f, 0.5f, 0.8f}; /* Orange color */
  copy_v3_v3(gz->color, color);
  float color_hi[3] = {0.0f, 0.8f, 1.0f}; /* Bright orange highlight */
  copy_v3_v3(gz->color_hi, color_hi);

  gz->scale_basis = 0.08f; /* Slightly larger than target gizmo */

  wmOperatorType *ot = WM_operatortype_find("OBJECT_OT_light_normal_positioning", true);
...
static void WIDGETGROUP_light_normal_positioning_draw_prepare(const bContext *C, wmGizmoGroup *gzgroup)
{
  wmGizmoWrapper *wwrapper = static_cast<wmGizmoWrapper *>(gzgroup->customdata);
  const Scene *scene = CTX_data_scene(C);
  ViewLayer *view_layer = CTX_data_view_layer(C);
  ...
  if (ob->type == OB_LAMP) {
    ...
    /* Position the gizmo based on light type */
    if (la->type == LA_SPOT) {
      /* Position in front of spot light, along the direction */
      madd_v3_v3fl(gz->matrix_basis[3], gz->matrix_basis[2], -la->spotsize * 1.0f);
    }
    else if (la->type == LA_SUN) {
      /* Position slightly offset for sun light */
      madd_v3_v3fl(gz->matrix_basis[3], gz->matrix_basis[2], -1.0f);
    }
    else {
      /* For area and point lights, position slightly in front */
      madd_v3_v3fl(gz->matrix_basis[3], gz->matrix_basis[2], -0.5f);
    }
  }
  
  /* Offset the gizmo to avoid overlap with target gizmo */
  gz->matrix_offset[3][2] -= 20.0f; /* Slightly more offset than target gizmo */
  WM_gizmo_set_flag(gz, WM_GIZMO_DRAW_OFFSET_SCALE, true);
}

void VIEW3D_GGT_light_normal_positioning(wmGizmoGroupType *gzgt)
{
  gzgt->name = "Light Normal Positioning Widgets";
  gzgt->idname = "VIEW3D_GGT_light_normal_positioning";

  gzgt->flag |= (WM_GIZMOGROUPTYPE_PERSISTENT | WM_GIZMOGROUPTYPE_3D);

  gzgt->poll = WIDGETGROUP_light_normal_positioning_poll;
  gzgt->setup = WIDGETGROUP_light_normal_positioning_setup;
  gzgt->setup_keymap = WM_gizmogroup_setup_keymap_generic_maybe_drag;
  gzgt->draw_prepare = WIDGETGROUP_light_normal_positioning_draw_prepare;
}

/** \} */

/* -------------------------------------------------------------------- */
/** \name Light Reflection Positioning Gizmo
 * \{ */

static bool WIDGETGROUP_light_reflection_positioning_poll(const bContext *C, wmGizmoGroupType * /*gzgt*/)
{
  View3D *v3d = CTX_wm_view3d(C);
  if (v3d->gizmo_flag & (V3D_GIZMO_HIDE | V3D_GIZMO_HIDE_CONTEXT)) {
    return false;
...
static void WIDGETGROUP_light_reflection_positioning_setup(const bContext * /*C*/, wmGizmoGroup *gzgroup)
{
  wmGizmoWrapper *wwrapper = MEM_mallocN<wmGizmoWrapper>(__func__);
  wwrapper->gizmo = WM_gizmo_new("GIZMO_GT_move_3d", gzgroup, nullptr);
  wmGizmo *gz = wwrapper->gizmo;
  ...
  /* Use a distinct red color for reflection positioning gizmo */
  float color[3] = {0.8f, 0.2f, 0.2f}; /* Red color */
  copy_v3_v3(gz->color, color);
  float color_hi[3] = {1.0f, 0.4f, 0.4f}; /* Bright red highlight */
  copy_v3_v3(gz->color_hi, color_hi);

  gz->scale_basis = 0.1f; /* Same size as normal positioning gizmo */

  wmOperatorType *ot = WM_operatortype_find("OBJECT_OT_light_reflection_positioning", true);
...
static void WIDGETGROUP_light_reflection_positioning_draw_prepare(const bContext *C, wmGizmoGroup *gzgroup)
{
  wmGizmoWrapper *wwrapper = static_cast<wmGizmoWrapper *>(gzgroup->customdata);
  const Scene *scene = CTX_data_scene(C);
  ViewLayer *view_layer = CTX_data_view_layer(C);
  ...
  if (ob->type == OB_LAMP) {
    ...
    if (la->type == LA_SPOT) {
      /* Position in front of spot light, along the direction */
      madd_v3_v3fl(gz->matrix_basis[3], gz->matrix_basis[2], -la->spotsize * 1.f);
    }
    else if (la->type == LA_SUN) {
      /* Position slightly offset for sun light */
      madd_v3_v3fl(gz->matrix_basis[3], gz->matrix_basis[2], -1.f);
    }
    else {
      /* For area and point lights, position slightly in front */
      madd_v3_v3fl(gz->matrix_basis[3], gz->matrix_basis[2], -0.5f);
  ...
  /* Offset the gizmo to avoid overlap with other gizmos */
  gz->matrix_offset[3][2] -= 19.0f; /* Between normal and target gizmo offsets */
  WM_gizmo_set_flag(gz, WM_GIZMO_DRAW_OFFSET_SCALE, true);
}

void VIEW3D_GGT_light_reflection_positioning(wmGizmoGroupType *gzgt)
{
  gzgt->name = "Light Reflection Positioning Widgets";
  gzgt->idname = "VIEW3D_GGT_light_reflection_positioning";

  gzgt->flag |= (WM_GIZMOGROUPTYPE_PERSISTENT | WM_GIZMOGROUPTYPE_3D);

  gzgt->poll = WIDGETGROUP_light_reflection_positioning_poll;
  gzgt->setup = WIDGETGROUP_light_reflection_positioning_setup;
  gzgt->setup_keymap = WM_gizmogroup_setup_keymap_generic_maybe_drag;
  gzgt->draw_prepare = WIDGETGROUP_light_reflection_positioning_draw_prepare;
}
