...
static OperatorStatus gizmo_ruler_modal(bContext *C,
                                        wmGizmo *gz,
                                        const wmEvent *event,
                                        eWM_GizmoFlagTweak tweak_flag)
{
  bool do_draw = false;
  OperatorStatus exit_code = OPERATOR_RUNNING_MODAL;
  RulerInfo *ruler_info = static_cast<RulerInfo *>(gz->parent_gzgroup->customdata);
  RulerItem *ruler_item = (RulerItem *)gz;
...
static OperatorStatus gizmo_ruler_invoke(bContext *C, wmGizmo *gz, const wmEvent *event)
{
...
static OperatorStatus view3d_ruler_add_invoke(bContext *C, wmOperator *op, const wmEvent *event)
{
  ARegion *region = CTX_wm_region(C);
...
static OperatorStatus view3d_ruler_remove_invoke(bContext *C,
                                                 wmOperator *op,
                                                 const wmEvent * /*event*/)
{
  ARegion *region = CTX_wm_region(C);
