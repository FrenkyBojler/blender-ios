...
static OperatorStatus view3d_navigation_invoke_generic(bContext *C,
                                                       ViewOpsData *vod,
                                                       const wmEvent *event,
                                                       PointerRNA *ptr,
                                                       const ViewOpsType *nav_type,
                                           ...
                                           const ViewOpsType *nav_type)
{
  ...
  vod->init_context(C);
  OperatorStatus ret = view3d_navigation_invoke_generic(C, vod, event, op->ptr, nav_type, nullptr);
...
OperatorStatus view3d_navigate_modal_fn(bContext *C, wmOperator *op, const wmEvent *event)
{
  ...
  OperatorStatus ret = vod->nav_type->apply_fn(C, vod, event_code, event->xy);
