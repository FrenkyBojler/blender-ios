...
static OperatorStatus view3d_camera_to_view_exec(bContext *C, wmOperator * /*op*/)
{
...
static OperatorStatus view3d_camera_to_view_selected_exec(bContext *C, wmOperator *op)
{
...
static OperatorStatus view3d_setobjectascamera_exec(bContext *C, wmOperator *op)
{
  View3D *v3d;
  ARegion *region;
  RegionView3D *rv3d;
...
static OperatorStatus localview_exec(bContext *C, wmOperator *op)
{
  const Depsgraph *depsgraph = CTX_data_ensure_evaluated_depsgraph(C);
  const int smooth_viewtx = WM_operator_smooth_viewtx_get(op);
...
static OperatorStatus localview_remove_from_exec(bContext *C, wmOperator *op)
{
  View3D *v3d = CTX_wm_view3d(C);
  Main *bmain = CTX_data_main(C);
  Scene *scene = CTX_data_scene(C);
