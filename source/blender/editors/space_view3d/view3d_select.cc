...
static OperatorStatus view3d_lasso_select_exec(bContext *C, wmOperator *op)
{
...
static OperatorStatus object_select_menu_exec(bContext *C, wmOperator *op)
{
...
static OperatorStatus bone_select_menu_exec(bContext *C, wmOperator *op)
{
  const int name_index = RNA_enum_get(op->ptr, "name");
...
static OperatorStatus view3d_select_exec(bContext *C, wmOperator *op)
{
  Scene *scene = CTX_data_scene(C);
...
static OperatorStatus view3d_select_invoke(bContext *C, wmOperator *op, const wmEvent *event)
{
  RNA_int_set_array(op->ptr, "location", event->mval);

  const OperatorStatus retval = view3d_select_exec(C, op);
...
static OperatorStatus view3d_box_select_exec(bContext *C, wmOperator *op)
{
  using namespace blender;
  Depsgraph *depsgraph = CTX_data_ensure_evaluated_depsgraph(C);
  rcti rect;
...
static OperatorStatus view3d_circle_select_modal(bContext *C, wmOperator *op, const wmEvent *event)
{
  OperatorStatus result = WM_gesture_circle_modal(C, op, event);
...
static OperatorStatus view3d_circle_select_exec(bContext *C, wmOperator *op)
{
  Depsgraph *depsgraph = CTX_data_ensure_evaluated_depsgraph(C);
