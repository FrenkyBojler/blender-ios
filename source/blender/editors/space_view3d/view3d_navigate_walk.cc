...
static OperatorStatus walkEnd(bContext *C, WalkInfo *walk)
{
  wmWindow *win;
  RegionView3D *rv3d;
...
static OperatorStatus walk_invoke(bContext *C, wmOperator *op, const wmEvent *event)
{
...
static OperatorStatus walk_modal(bContext *C, wmOperator *op, const wmEvent *event)
{
  bool do_draw = false;
  WalkInfo *walk = static_cast<WalkInfo *>(op->customdata);
  View3D *v3d = walk->v3d;
  ...
  const OperatorStatus exit_code = walkEnd(C, walk);
