...
static OperatorStatus wm_usd_export_invoke(bContext *C, wmOperator *op, const wmEvent * /*event*/)
{
...
static OperatorStatus wm_usd_export_exec(bContext *C, wmOperator *op)
{
...
static OperatorStatus wm_usd_import_invoke(bContext *C, wmOperator *op, const wmEvent *event)
{
  USDOperatorOptions *options = MEM_callocN<USDOperatorOptions>("USDOperatorOptions");
  options->as_background_job = true;
  op->customdata = options;
...
static OperatorStatus wm_usd_import_exec(bContext *C, wmOperator *op)
{
  if (!RNA_struct_property_is_set_ex(op->ptr, "filepath", false)) {
    BKE_report(op->reports, RPT_ERROR, "No filepath given");
    return OPERATOR_CANCELLED;
