...
#ifdef WITH_AUDASPACE
static OperatorStatus sound_open_exec(bContext *C, wmOperator *op)
{
...
static OperatorStatus sound_open_invoke(bContext *C, wmOperator *op, const wmEvent *event)
{
...
static OperatorStatus sound_update_animation_flags_exec(bContext *C, wmOperator * /*op*/)
{
...
static OperatorStatus sound_bake_animation_exec(bContext *C, wmOperator * /*op*/)
{
  Scene *scene = CTX_data_scene(C);
...
static OperatorStatus sound_mixdown_exec(bContext *C, wmOperator *op)
{
#ifdef WITH_AUDASPACE
  char filepath[FILE_MAX];
...
static OperatorStatus sound_mixdown_invoke(bContext *C, wmOperator *op, const wmEvent *event)
{
  if (RNA_struct_property_is_set(op->ptr, "filepath")) {
...
static OperatorStatus sound_pack_exec(bContext *C, wmOperator *op)
{
  Main *bmain = CTX_data_main(C);
  Editing *ed = CTX_data_scene(C)->ed;
  bSound *sound;
...
static OperatorStatus sound_unpack_exec(bContext *C, wmOperator *op)
{
  Main *bmain = CTX_data_main(C);
...
static OperatorStatus sound_unpack_invoke(bContext *C, wmOperator *op, const wmEvent * /*event*/)
{
  Editing *ed = CTX_data_scene(C)->ed;
  bSound *sound;
