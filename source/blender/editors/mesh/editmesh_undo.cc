...
static void um_arraystore_cd_create(CustomData *cdata,
...
#  endif
                                       ...
                                       BArrayCustomData **r_bcd_first)
  {
    ...
    for (const CustomDataLayer &layer : blender::Span(cdata->layers, cdata->totlayer)) {
      ArraysWithType &arrays_for_type = arrays.lookup_or_add_cb(
  ...
  static void um_arraystore_cd_expand(
      const BArrayCustomData *bcd, CustomData *cdata, const size_t data_len)
  {
    using namespace blender;
...
#  endif
  ...
  static void um_arraystore_compact(UndoMesh * um, const UndoMesh *um_ref)
  {
    ...
    blender::threading::parallel_invoke(
        ...
        [&]() {
          um_arraystore_cd_create(&mesh->vert_data,
                                  mesh->verts_num,
        ...
        [&]() {
          um_arraystore_cd_create(&mesh->edge_data,
                                  mesh->edges_num,
                                  ARRAY_STORE_INDEX_EDGE,
        ...
        [&]() {
          um_arraystore_cd_create(&mesh->corner_data,
                                  mesh->corners_num,
                                  ARRAY_STORE_INDEX_LOOP,
        ...
        [&]() {
          um_arraystore_cd_create(&mesh->face_data,
                                  mesh->faces_num,
                                  ARRAY_STORE_INDEX_POLY,
...
#  endif /* USE_ARRAY_STORE_THREAD */
  ...
  static void um_arraystore_free(UndoMesh * um)
  {
    ...
    if (um->store.mselect) {
      const size_t stride = sizeof(*mesh->mselect);
      BArrayStore *bs = BLI_array_store_at_size_get(
          &um_arraystore.bs_stride[ARRAY_STORE_INDEX_MSEL], stride);
...
#  ifdef USE_ARRAY_STORE_THREAD
      ...
      BLI_task_pool_push(
          um_arraystore.task_pool, um_arraystore_compact_cb, um_data, true, nullptr);
#  else
...
#  endif
   ...
   * what the source of truth is for mesh data - which can make the logic difficult to reason
   * about.
   */
...
#  endif
  ...
  static bool mesh_undosys_step_encode(bContext * C, Main * bmain, UndoStep * us_p)
  {
    ...
    blender::Vector<Object *> objects = ED_undo_editmode_objects_from_view_layer(scene,
                                                                                 view_layer);
