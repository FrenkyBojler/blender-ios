...
UvVertMap *BM_uv_vert_map_create(BMesh *bm, const bool use_select, const bool respect_hide)
{
  ...
  UvVertMap *vmap = static_cast<UvVertMap *>(MEM_callocN(sizeof(*vmap), "UvVertMap"));
  if (!vmap) {
    return nullptr;
  }
                                       ...
                                       const bool do_islands)
{
  ...
  UvElementMap *element_map = static_cast<UvElementMap *>(
      MEM_callocN(sizeof(*element_map), "UvElementMap"));
  element_map->total_uvs = totuv;
  element_map->vertex = static_cast<UvElement **>(
      MEM_callocN(sizeof(*element_map->vertex) * bm->totvert, "UvElementVerts"));
  element_map->storage = static_cast<UvElement *>(
      MEM_callocN(sizeof(*element_map->storage) * totuv, "UvElement"));
