...
static OperatorStatus edbm_subdivide_exec(bContext *C, wmOperator *op)
{
...
static OperatorStatus edbm_subdivide_edge_ring_exec(bContext *C, wmOperator *op)
{
  const Scene *scene = CTX_data_scene(C);
  ViewLayer *view_layer = CTX_data_view_layer(C);
  const Vector<Object *> objects = BKE_view_layer_array_from_objects_in_edit_mode_unique_data(
...
static OperatorStatus edbm_unsubdivide_exec(bContext *C, wmOperator *op)
{
...
static OperatorStatus edbm_delete_exec(bContext *C, wmOperator *op)
{
  const Scene *scene = CTX_data_scene(C);
  ViewLayer *view_layer = CTX_data_view_layer(C);
...
static OperatorStatus edbm_delete_loose_exec(bContext *C, wmOperator *op)
{
  const Scene *scene = CTX_data_scene(C);
  ViewLayer *view_layer = CTX_data_view_layer(C);
...
static OperatorStatus edbm_collapse_edge_exec(bContext *C, wmOperator *op)
{
  const Scene *scene = CTX_data_scene(C);
  ViewLayer *view_layer = CTX_data_view_layer(C);
  const Vector<Object *> objects = BKE_view_layer_array_from_objects_in_edit_mode_unique_data(
...
static OperatorStatus edbm_add_edge_face_exec(bContext *C, wmOperator *op)
{
...
static OperatorStatus edbm_mark_seam_exec(bContext *C, wmOperator *op)
{
...
static OperatorStatus edbm_mark_sharp_exec(bContext *C, wmOperator *op)
{
  BMEdge *eed;
  BMIter iter;
  const bool clear = RNA_boolean_get(op->ptr, "clear");
...
static OperatorStatus edbm_vert_connect_exec(bContext *C, wmOperator *op)
{
  const Scene *scene = CTX_data_scene(C);
  ViewLayer *view_layer = CTX_data_view_layer(C);
...
static OperatorStatus edbm_vert_connect_path_exec(bContext *C, wmOperator *op)
{
  const Scene *scene = CTX_data_scene(C);
  ViewLayer *view_layer = CTX_data_view_layer(C);
...
static OperatorStatus edbm_vert_connect_concave_exec(bContext *C, wmOperator *op)
{
  const Scene *scene = CTX_data_scene(C);
  ViewLayer *view_layer = CTX_data_view_layer(C);
  const Vector<Object *> objects = BKE_view_layer_array_from_objects_in_edit_mode_unique_data(
...
static OperatorStatus edbm_vert_connect_nonplaner_exec(bContext *C, wmOperator *op)
{
  const Scene *scene = CTX_data_scene(C);
  ViewLayer *view_layer = CTX_data_view_layer(C);
...
static OperatorStatus edbm_face_make_planar_exec(bContext *C, wmOperator *op)
{
  const Scene *scene = CTX_data_scene(C);
  ViewLayer *view_layer = CTX_data_view_layer(C);
  const Vector<Object *> objects = BKE_view_layer_array_from_objects_in_edit_mode_unique_data(
...
static OperatorStatus edbm_edge_split_exec(bContext *C, wmOperator *op)
{
  const int type = RNA_enum_get(op->ptr, "type");
...
static OperatorStatus edbm_duplicate_exec(bContext *C, wmOperator *op)
{
  const Scene *scene = CTX_data_scene(C);
  ViewLayer *view_layer = CTX_data_view_layer(C);
  const Vector<Object *> objects = BKE_view_layer_array_from_objects_in_edit_mode_unique_data(
...
static OperatorStatus edbm_duplicate_invoke(bContext *C, wmOperator *op, const wmEvent * /*event*/)
{
  WM_cursor_wait(true);
  const OperatorStatus retval = edbm_duplicate_exec(C, op);
...
static OperatorStatus edbm_flip_quad_tessellation_exec(bContext *C, wmOperator *op)
{
  const Scene *scene = CTX_data_scene(C);
  ViewLayer *view_layer = CTX_data_view_layer(C);
  const Vector<Object *> objects = BKE_view_layer_array_from_objects_in_edit_mode_unique_data(
...
static OperatorStatus edbm_flip_normals_exec(bContext *C, wmOperator *op)
{
 ...
 */
static OperatorStatus edbm_edge_rotate_selected_exec(bContext *C, wmOperator *op)
{
  BMEdge *eed;
  BMIter iter;
...
static OperatorStatus edbm_hide_exec(bContext *C, wmOperator *op)
{
...
static OperatorStatus edbm_reveal_exec(bContext *C, wmOperator *op)
{
...
static OperatorStatus edbm_normals_make_consistent_exec(bContext *C, wmOperator *op)
{
  const Scene *scene = CTX_data_scene(C);
  ViewLayer *view_layer = CTX_data_view_layer(C);
...
static OperatorStatus edbm_do_smooth_vertex_exec(bContext *C, wmOperator *op)
{
  const float fac = RNA_float_get(op->ptr, "factor");
...
static OperatorStatus edbm_do_smooth_laplacian_vertex_exec(bContext *C, wmOperator *op)
{
  int tot_selected = 0, tot_locked = 0;
  const Scene *scene = CTX_data_scene(C);
  ViewLayer *view_layer = CTX_data_view_layer(C);
...
static OperatorStatus edbm_faces_shade_smooth_exec(bContext *C, wmOperator * /*op*/)
{
  const Scene *scene = CTX_data_scene(C);
  ViewLayer *view_layer = CTX_data_view_layer(C);
  const Vector<Object *> objects = BKE_view_layer_array_from_objects_in_edit_mode_unique_data(
...
static OperatorStatus edbm_faces_shade_flat_exec(bContext *C, wmOperator * /*op*/)
{
  const Scene *scene = CTX_data_scene(C);
  ViewLayer *view_layer = CTX_data_view_layer(C);
  const Vector<Object *> objects = BKE_view_layer_array_from_objects_in_edit_mode_unique_data(
...
static OperatorStatus edbm_rotate_uvs_exec(bContext *C, wmOperator *op)
{
...
static OperatorStatus edbm_reverse_uvs_exec(bContext *C, wmOperator *op)
{
  const Scene *scene = CTX_data_scene(C);
  ViewLayer *view_layer = CTX_data_view_layer(C);
  const Vector<Object *> objects = BKE_view_layer_array_from_objects_in_edit_mode_unique_data(
...
static OperatorStatus edbm_rotate_colors_exec(bContext *C, wmOperator *op)
{
  /* get the direction from RNA */
  const bool use_ccw = RNA_boolean_get(op->ptr, "use_ccw");
...
static OperatorStatus edbm_reverse_colors_exec(bContext *C, wmOperator *op)
{
  const Scene *scene = CTX_data_scene(C);
  ViewLayer *view_layer = CTX_data_view_layer(C);
  const Vector<Object *> objects = BKE_view_layer_array_from_objects_in_edit_mode_unique_data(
...
static OperatorStatus edbm_merge_exec(bContext *C, wmOperator *op)
{
  Scene *scene = CTX_data_scene(C);
  ViewLayer *view_layer = CTX_data_view_layer(C);
  const Vector<Object *> objects = BKE_view_layer_array_from_objects_in_edit_mode_unique_data(
...
static OperatorStatus edbm_remove_doubles_exec(bContext *C, wmOperator *op)
{
...
static OperatorStatus edbm_shape_propagate_to_all_exec(bContext *C, wmOperator *op)
{
  const Scene *scene = CTX_data_scene(C);
  ViewLayer *view_layer = CTX_data_view_layer(C);
  int tot_shapekeys = 0;
...
/* BMESH_TODO this should be properly encapsulated in a bmop.  but later. */
static OperatorStatus edbm_blend_from_shape_exec(bContext *C, wmOperator *op)
{
...
static OperatorStatus edbm_solidify_exec(bContext *C, wmOperator *op)
{
...
static OperatorStatus edbm_separate_exec(bContext *C, wmOperator *op)
{
...
static OperatorStatus edbm_fill_exec(bContext *C, wmOperator *op)
{
...
static OperatorStatus edbm_fill_grid_exec(bContext *C, wmOperator *op)
{
...
static OperatorStatus edbm_fill_holes_exec(bContext *C, wmOperator *op)
{
...
static OperatorStatus edbm_beautify_fill_exec(bContext *C, wmOperator *op)
{
  const Scene *scene = CTX_data_scene(C);
  ViewLayer *view_layer = CTX_data_view_layer(C);
  const Vector<Object *> objects = BKE_view_layer_array_from_objects_in_edit_mode_unique_data(
...
static OperatorStatus edbm_poke_face_exec(bContext *C, wmOperator *op)
{
...
static OperatorStatus edbm_quads_convert_to_tris_exec(bContext *C, wmOperator *op)
{
...
static OperatorStatus edbm_tris_convert_to_quads_exec(bContext *C, wmOperator *op)
{
  const Scene *scene = CTX_data_scene(C);
  ViewLayer *view_layer = CTX_data_view_layer(C);
...
static OperatorStatus edbm_decimate_exec(bContext *C, wmOperator *op)
{
...
static OperatorStatus edbm_dissolve_verts_exec(bContext *C, wmOperator *op)
{
...
static OperatorStatus edbm_dissolve_edges_exec(bContext *C, wmOperator *op)
{
  const bool use_verts = RNA_boolean_get(op->ptr, "use_verts");
  const bool use_face_split = RNA_boolean_get(op->ptr, "use_face_split");
...
static OperatorStatus edbm_dissolve_faces_exec(bContext *C, wmOperator *op)
{
  const bool use_verts = RNA_boolean_get(op->ptr, "use_verts");
  const Scene *scene = CTX_data_scene(C);
  ViewLayer *view_layer = CTX_data_view_layer(C);
...
static OperatorStatus edbm_dissolve_mode_exec(bContext *C, wmOperator *op)
{
  Object *obedit = CTX_data_edit_object(C);
  BMEditMesh *em = BKE_editmesh_from_object(obedit);
  PropertyRNA *prop;
...
static OperatorStatus edbm_dissolve_limited_exec(bContext *C, wmOperator *op)
{
  const float angle_limit = RNA_float_get(op->ptr, "angle_limit");
...
static OperatorStatus edbm_dissolve_degenerate_exec(bContext *C, wmOperator *op)
{
  const Scene *scene = CTX_data_scene(C);
  ViewLayer *view_layer = CTX_data_view_layer(C);
...
static OperatorStatus edbm_delete_edgeloop_exec(bContext *C, wmOperator *op)
{
  const bool use_face_split = RNA_boolean_get(op->ptr, "use_face_split");
  const Scene *scene = CTX_data_scene(C);
  ViewLayer *view_layer = CTX_data_view_layer(C);
...
static OperatorStatus edbm_split_exec(bContext *C, wmOperator *op)
{
  const Scene *scene = CTX_data_scene(C);
  ViewLayer *view_layer = CTX_data_view_layer(C);
  const Vector<Object *> objects = BKE_view_layer_array_from_objects_in_edit_mode_unique_data(
...
static OperatorStatus edbm_sort_elements_exec(bContext *C, wmOperator *op)
{
  Scene *scene = CTX_data_scene(C);
  ViewLayer *view_layer = CTX_data_view_layer(C);
...
static OperatorStatus edbm_bridge_edge_loops_exec(bContext *C, wmOperator *op)
{
  const int type = RNA_enum_get(op->ptr, "type");
...
static OperatorStatus edbm_wireframe_exec(bContext *C, wmOperator *op)
{
...
static OperatorStatus edbm_offset_edgeloop_exec(bContext *C, wmOperator *op)
{
...
#ifdef WITH_BULLET
static OperatorStatus edbm_convex_hull_exec(bContext *C, wmOperator *op)
{
...
static OperatorStatus mesh_symmetrize_exec(bContext *C, wmOperator *op)
{
  const float thresh = RNA_float_get(op->ptr, "threshold");
  const Scene *scene = CTX_data_scene(C);
  ViewLayer *view_layer = CTX_data_view_layer(C);
...
static OperatorStatus mesh_symmetry_snap_exec(bContext *C, wmOperator *op)
{
...
static OperatorStatus edbm_mark_freestyle_edge_exec(bContext *C, wmOperator *op)
{
  BMEdge *eed;
  BMIter iter;
  FreestyleEdge *fed;
...
static OperatorStatus edbm_mark_freestyle_face_exec(bContext *C, wmOperator *op)
{
  BMFace *efa;
  BMIter iter;
  FreestyleFace *ffa;
...
static OperatorStatus edbm_point_normals_modal(bContext *C, wmOperator *op, const wmEvent *event)
{
  ...
  OperatorStatus ret = OPERATOR_PASS_THROUGH;
...
static OperatorStatus edbm_point_normals_invoke(bContext *C,
                                                wmOperator *op,
                                                const wmEvent * /*event*/)
{
...
static OperatorStatus edbm_point_normals_exec(bContext *C, wmOperator *op)
{
  Object *obedit = CTX_data_edit_object(C);
...
static OperatorStatus normals_split_merge(bContext *C, const bool do_merge)
{
  const Scene *scene = CTX_data_scene(C);
  ViewLayer *view_layer = CTX_data_view_layer(C);
  const Vector<Object *> objects = BKE_view_layer_array_from_objects_in_edit_mode_unique_data(
...
static OperatorStatus edbm_merge_normals_exec(bContext *C, wmOperator * /*op*/)
{
...
static OperatorStatus edbm_split_normals_exec(bContext *C, wmOperator * /*op*/)
{
...
static OperatorStatus edbm_average_normals_exec(bContext *C, wmOperator *op)
{
  const Scene *scene = CTX_data_scene(C);
  ViewLayer *view_layer = CTX_data_view_layer(C);
  const Vector<Object *> objects = BKE_view_layer_array_from_objects_in_edit_mode_unique_data(
...
static OperatorStatus edbm_normals_tools_exec(bContext *C, wmOperator *op)
{
  Scene *scene = CTX_data_scene(C);
  ViewLayer *view_layer = CTX_data_view_layer(C);
  const Vector<Object *> objects = BKE_view_layer_array_from_objects_in_edit_mode_unique_data(
...
static OperatorStatus edbm_set_normals_from_faces_exec(bContext *C, wmOperator *op)
{
  const Scene *scene = CTX_data_scene(C);
  ViewLayer *view_layer = CTX_data_view_layer(C);
  const Vector<Object *> objects = BKE_view_layer_array_from_objects_in_edit_mode_unique_data(
...
static OperatorStatus edbm_smooth_normals_exec(bContext *C, wmOperator *op)
{
  const Scene *scene = CTX_data_scene(C);
  ViewLayer *view_layer = CTX_data_view_layer(C);
  const Vector<Object *> objects = BKE_view_layer_array_from_objects_in_edit_mode_unique_data(
...
static OperatorStatus edbm_mod_weighted_strength_exec(bContext *C, wmOperator *op)
{
  const Scene *scene = CTX_data_scene(C);
  ViewLayer *view_layer = CTX_data_view_layer(C);
  const Vector<Object *> objects = BKE_view_layer_array_from_objects_in_edit_mode_unique_data(
