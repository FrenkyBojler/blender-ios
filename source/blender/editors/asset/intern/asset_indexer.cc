 ...
 *     "license": "<license>",
 *     "tags": ["<tag>"],
 *     "default_import_method": eAssetImportMethod,
 *     "properties": [..]
 *   }]
 * }
 * \endcode
 ...
 * attributes. If default_import_method is set, the #ASSETDATA_USE_OWN_IMPORT_METHOD flag will be
...
constexpr StringRef ATTRIBUTE_ENTRIES_DEFAULT_IMPORT_METHOD("default_import_method");
                                               ...
                                               const FileIndexerEntry *indexer_entry)
{
  ...
  if (asset_data.flag & ASSETDATA_USE_OWN_IMPORT_METHOD) {
    ...
    RNA_enum_identifier(
        rna_enum_asset_import_method_items, asset_data.default_import_method, &identifier);
    if (identifier) {
      result.append_str(ATTRIBUTE_ENTRIES_DEFAULT_IMPORT_METHOD, identifier);
    }
  }
...
AssetMetaData *asset_metadata_from_dictionary(const DictionaryValue &entry)
{
  ...
  if (const std::optional<StringRefNull> import_method_identifier = entry.lookup_str(
          ATTRIBUTE_ENTRIES_DEFAULT_IMPORT_METHOD))
  {
    int default_import_method = 0;
    if (RNA_enum_value_from_identifier(rna_enum_asset_import_method_items,
                                       import_method_identifier->c_str(),
                                       &default_import_method))
    {
      asset_data->default_import_method = eAssetImportMethod(default_import_method);
