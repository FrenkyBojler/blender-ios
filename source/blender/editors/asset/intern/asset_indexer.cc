                                               ...
                                               const FileIndexerEntry *indexer_entry)
{
  ...
  if (const char *label = asset_data.label) {
    result.append_str(ATTRIBUTE_ENTRIES_LABEL, label);
  }
                                          ...
                                          const DictionaryValue &entry)
{
  ...
  if (const std::optional<StringRef> value = entry.lookup_str(ATTRIBUTE_ENTRIES_LABEL)) {
    asset_data->label = BLI_strdupn(value->data(), value->size());
  }
