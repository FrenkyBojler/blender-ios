                                                                ...
                                                                const char *library_name)
{
  ...
  BLI_listbase_clear(&settings->catalog_states);
                                                                 ...
                                                                 const char *catalog_path)
{
  ...
  return BKE_asset_catalog_path_is_collapsed(settings->catalog_states, catalog_path);
}
...
void BKE_preferences_asset_browser_settings_cleanup_all_old(UserDef *userdef)
{
  ...
  LISTBASE_FOREACH (bUserAssetBrowserSettings *, settings, &userdef->asset_browser_settings) {
    int removed_count = 0;
    LISTBASE_FOREACH_MUTABLE (AssetCatalogState *, collapsed_state, &settings->catalog_states) {
