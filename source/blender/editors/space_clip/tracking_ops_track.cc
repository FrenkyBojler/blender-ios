...
static OperatorStatus track_markers(bContext *C, wmOperator *op, bool use_job)
{
  TrackMarkersJob *tmj;
  SpaceClip *sc = CTX_wm_space_clip(C);
  MovieClip *clip = ED_space_clip_get_clip(sc);
...
static OperatorStatus track_markers_exec(bContext *C, wmOperator *op)
{
  return track_markers(C, op, false);
}

static OperatorStatus track_markers_invoke(bContext *C, wmOperator *op, const wmEvent * /*event*/)
{
  return track_markers(C, op, true);
}

static OperatorStatus track_markers_modal(bContext *C, wmOperator * /*op*/, const wmEvent *event)
{
...
static OperatorStatus refine_marker_exec(bContext *C, wmOperator *op)
{
  SpaceClip *sc = CTX_wm_space_clip(C);
  MovieClip *clip = ED_space_clip_get_clip(sc);
  const MovieTrackingObject *tracking_object = BKE_tracking_object_get_active(&clip->tracking);
