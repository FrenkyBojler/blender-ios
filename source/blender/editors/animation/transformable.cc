...
Transformable::Transformable(Object &object)
...
std::string Transformable::rna_path_to_rotation_mode(const eRotationModes rotation_mode) const
{
  StringRefNull property_name = get_rotation_mode_path(rotation_mode);
  if (rna_path_from_id_.empty()) {
    return std::string(property_name);
  }
  return fmt::format("{}.{}", rna_path_from_id_, property_name);
...
Rotation Transformable::get_rotation_for_mode(eRotationModes mode) const
{
  Rotation rotation;
