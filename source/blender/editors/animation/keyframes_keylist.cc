                                    ...
                                    blender::float2 range)
{
  ...
  if (!slot) {
     ...
     * in its summary line. That Action channel is there, even when there is no slot assigned. So
     * this function needs to be able to handle the "no slot" case as valid. It just doesn't
     * produce any keys.
     *
