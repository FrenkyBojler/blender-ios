...
static void acf_fcurve_name(bAnimListElem *ale, char *name)
{
      ...
      ale->slot_handle != Slot::unassigned)
  {
    ...
    if (slot->users(*ale->bmain).contains(ale->id)) {
      ...
      PointerRNA ptr;
      PropertyRNA *prop;
      if (!RNA_path_resolve_property(&id_ptr, fcurve->rna_path_ptr, &ptr, &prop)) {
        fcurve->flag |= FCURVE_DISABLED;
      }
    }
...
static void acf_nla_curve_name(bAnimListElem *ale, char *name)
{
  ...
  prop = RNA_struct_type_find_property(RNA_NlaStrip, fcu->rna_path_ptr);
  if (prop) {
    ...
    /* unknown property... */
    BLI_snprintf_utf8(name, ANIM_CHAN_NAME_SIZE, "%s[%d]", fcu->rna_path_ptr, fcu->array_index);
  }
}
        ...
        ale->type == ANIMTYPE_SHAPEKEY)
    {
      ...
      if (ale->owner) { /* Slider using custom RNA Access ---------- */
        if (ale->type == ANIMTYPE_NLACURVE) {
          ...
          prop = RNA_struct_find_property(&ptr, fcu->rna_path_ptr);
