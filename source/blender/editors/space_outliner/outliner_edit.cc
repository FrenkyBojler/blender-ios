...
void OUTLINER_OT_id_remap(wmOperatorType *ot)
{
  ...
  prop = RNA_def_int(ot->srna, "old_id", 0, 0, 0, "Old ID", "Old ID to replace", 0, 0);
  RNA_def_property_flag(prop, PROP_HIDDEN);

  ot->prop = RNA_def_int(
      ot->srna, "new_id", 0, 0, 0, "New ID", "New ID to remap all selected IDs' users to", 0, 0);
}
      ...
      if (TreeElementRNAProperty *tem_rna_prop = tree_element_cast<TreeElementRNAProperty>(tem)) {
        ...
        if (RNA_property_type(prop) == PROP_POINTER) {
          ...
          if (name) {
            ...
            for (TreeElement &temsub : tem->subtree) {
              if (&temsub == temnext) {
                break;
              }
              index++;
