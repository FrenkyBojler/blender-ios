                                                    ...
                                                    const wmEvent *event)
{
  ...
  if (te->ys < v2d->cur.ymin + UI_UNIT_Y) {
    /* Try to show one full row below. */
    const int delta_y = te->ys - (v2d->cur.ymin + UI_UNIT_Y);
    outliner_scroll_view(space_outliner, region, delta_y);
  }
  else if (te->ys > v2d->cur.ymax - (UI_UNIT_Y * 2.0f)) {
    /* Try to show one full row above. */
    const int delta_y = te->ys - (v2d->cur.ymax - (UI_UNIT_Y * 2.0f));
    outliner_scroll_view(space_outliner, region, delta_y);
  }
