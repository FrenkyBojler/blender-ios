  ...
  tree_iterator::all_open(*space_outliner, [&](const TreeElement *te) {
    ...
    if (is_object) {
      ...
      if (!real_users && !has_fake_user) {
        ...
        ui::button_icon_decoration_color_set(bt, overlay_color);
      }
      ui::button_icon_decoration_set(bt, overlay);
    }
  });
}
                             ...
                             const int num_elements)
{
  ...
  if (!is_clickable || x >= xmax || is_collection) {
    ...
    if (ui::icon_get_theme_color(data.icon, color)) {
      ...
      icon_draw_ex(x, y, data.icon, UI_INV_SCALE_FAC, alpha, 0.0f, nullptr, false, &decoration);
    }
  }
  else {
