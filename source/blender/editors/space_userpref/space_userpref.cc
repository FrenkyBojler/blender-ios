                                                 ...
                                                 bool is_project_defined)
{
  ...
  switch (library_type) {
    case bUserAssetLibraryAddType::Local: {
      ...
      if (!final_name[0]) {
        // TODO: This will not work that nicely on project assets as the names saved will be
        // localized on creation
    ...
    case bUserAssetLibraryAddType::Remote: {
      ...
      new_library = BKE_preferences_remote_asset_library_add(&U, final_name, dirpath);
      break;
    }
  }

  if (is_project_defined) {
    new_library->flag |= ASSET_LIBRARY_PROJECT_DEFINED;
  }
  else {
