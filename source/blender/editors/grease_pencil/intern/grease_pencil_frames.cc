...
static OperatorStatus insert_blank_frame_exec(bContext *C, wmOperator *op)
{
...
static OperatorStatus frame_clean_duplicate_exec(bContext *C, wmOperator *op)
{
  using namespace blender::bke::greasepencil;
  Object *object = CTX_data_active_object(C);
  GreasePencil &grease_pencil = *static_cast<GreasePencil *>(object->data);
...
static OperatorStatus grease_pencil_frame_duplicate_exec(bContext *C, wmOperator *op)
{
  using namespace blender::bke::greasepencil;
  Scene *scene = CTX_data_scene(C);
  Object *object = CTX_data_active_object(C);
...
static OperatorStatus grease_pencil_active_frame_delete_exec(bContext *C, wmOperator *op)
{
  using namespace blender::bke::greasepencil;
  Scene *scene = CTX_data_scene(C);
  Object *object = CTX_data_active_object(C);
...
static OperatorStatus grease_pencil_delete_breakdown_frames_exec(bContext *C, wmOperator * /*op*/)
{
  const Object &ob = *CTX_data_active_object(C);
  const Scene &scene = *CTX_data_scene(C);
