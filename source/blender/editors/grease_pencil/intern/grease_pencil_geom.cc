                            ...
                            MutableSpan<int> r_merge_indices)
{
  ...
  blender::KDTree_1d *tree = blender::kdtree_1d_new(selection.size());
  ...
  selection.foreach_index_optimized<int64_t>([&](const int64_t i, const int64_t pos) {
    blender::kdtree_1d_insert(tree, pos, &distances[i - points.first()]);
  });
  blender::kdtree_1d_balance(tree);

  Array<int> selection_merge_indices(selection.size(), -1);
  const int duplicate_count = blender::kdtree_1d_calc_duplicates_fast(
    ...
    const bke::AttributeFilter &attribute_filter)
{
  ...
  blender::KDTree_2d *tree = blender::kdtree_2d_new(2 * src_curves.curves_num());
  ...
  for (const int src_i : src_curves.curves_range()) {
    ...
    blender::kdtree_2d_insert(tree, src_i * 2, screen_start_points[src_i]);
    blender::kdtree_2d_insert(tree, src_i * 2 + 1, screen_end_points[src_i]);
  }
  ...
  selection.foreach_index(GrainSize(512), [&](const int src_i) {
    ...
    const bool is_end_ok = (blender::kdtree_find_nearest_cb_cpp<2>(
                                tree,
                                end_co,
                                &nearest_end,
                                [&](const int other, const float * /*co*/, const float dist_sq) {
