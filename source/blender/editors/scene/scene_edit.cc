...
static OperatorStatus scene_new_exec(bContext *C, wmOperator *op)
{
...
static OperatorStatus scene_new_sequencer_exec(bContext *C, wmOperator *op)
{
  Main *bmain = CTX_data_main(C);
  int type = RNA_enum_get(op->ptr, "type");
...
static OperatorStatus scene_delete_exec(bContext *C, wmOperator * /*op*/)
{
  Scene *scene = CTX_data_scene(C);
