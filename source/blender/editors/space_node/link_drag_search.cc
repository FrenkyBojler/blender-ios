...
static void add_group_input_node_fn(nodes::LinkSearchOpParams &params)
{
  ...
  bNodeSocket *socket = bke::node_find_socket(group_input, SOCK_OUT, UString(socket_iface->identifier));
  if (socket) {
                                        ...
                                        const bNodeTreeInterfaceSocket &interface_socket)
{
  ...
  bNodeSocket *socket = bke::node_find_socket(group_input, SOCK_OUT, UString(interface_socket.identifier));
  if (socket != nullptr) {
    socket->flag &= ~SOCK_HIDDEN;
    bke::node_add_link(params.node_tree, group_input, *socket, params.node, params.socket);
  }
