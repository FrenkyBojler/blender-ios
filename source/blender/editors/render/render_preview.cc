...
static void icon_preview_startjob_all_sizes(void *customdata, wmJobWorkerStatus *worker_status)
{
  ...
  LISTBASE_FOREACH (IconPreviewSize *, cur_size, &ip->sizes) {
     ...
     * they can skip this test. Same is true for Images, they can also skip this test since their
     * preview is just pulled from ImBuf which is not dependent on the render engine. */
    ...
    const bool use_solid_render_mode = (ip->id != nullptr) &&
                                       ELEM(GS(ip->id->name), ID_OB, ID_AC, ID_IM);
