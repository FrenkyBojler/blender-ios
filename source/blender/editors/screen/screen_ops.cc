...
static void focus_mode_menu(wmWindow *win, bScreen *screen, ScrArea *area, uiLayout *layout)
{
  ...
  layout->op("WM_OT_window_fullscreen_toggle",
             ...
             is_full ? ICON_FULLSCREEN_EXIT : ICON_FULLSCREEN_ENTER);
  layout->separator();

  ptr = layout->op("SCREEN_OT_screen_full_area", IFACE_("Exit Focus Mode"), ICON_SCREEN_BACK);
...
static wmOperatorStatus actionzone_invoke(bContext *C, wmOperator *op, const wmEvent *event)
{
  ...
  if (ELEM(sad->az->type, AZONE_REGION, AZONE_REGION_QUAD)) {
  ...
  if (sad->az->type == AZONE_FULLSCREEN) {
    ...
    focus_mode_menu(CTX_wm_window(C), screen, sad->sa1, layout);
