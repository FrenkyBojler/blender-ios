...
static wmOperatorStatus area_secondary_exec(bContext *C, wmOperator *op)
{
  ...
  for (ScrArea &ar : screen->areabase) {
    ...
    else if (ar.totrct.xmax > (win->sizex - 10)) {
      secondary_area = &ar;
    }
  }
  ...
  if ((primary_area && !secondary_area) || (primary_area == secondary_area)) {
    ...
    secondary_area = area_split(win, screen, primary_area, SCREEN_AXIS_V, 0.6f);
    WM_event_add_notifier(C, NC_SCREEN | NA_EDITED, nullptr);
  }
  ...
  *bigger = *smaller = 100000;
  ...
  for (ScrArea &area : screen->areabase) {
    if (dir_axis == SCREEN_AXIS_H) {
      ...
      const int x1 = area.winx - (U.tablet_mode ? 0 : (int(AREAMINX * UI_SCALE_FAC) - 1));
