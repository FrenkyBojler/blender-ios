...
static void area_split_move_apply(bContext *C,
                                  sAreaSplitData *sd,
                                  const eScreenAxis dir_axis,
                                  const bool snap)
{
  ...
  status.item(IFACE_("Confirm"), ICON_MOUSE_LMB);
  status.item(IFACE_("Cancel"), ICON_EVENT_ESC);
  status.item_bool(IFACE_("Snap"), snap, ICON_EVENT_CTRL);
  ...
  CLAMP(sd->delta, -sd->smaller, sd->bigger);

  if (snap) {
    final_loc = area_split_snap_calc_location(
        screen, sd->delta, sd->origval, dir_axis, sd->bigger, sd->smaller);
  }
  else {
    final_loc = sd->origval + sd->delta;
  }
