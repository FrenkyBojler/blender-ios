...
static OperatorStatus objects_add_active_exec(bContext *C, wmOperator *op)
{
...
static OperatorStatus objects_remove_active_exec(bContext *C, wmOperator *op)
{
  Main *bmain = CTX_data_main(C);
  Scene *scene = CTX_data_scene(C);
...
static OperatorStatus collection_objects_remove_all_exec(bContext *C, wmOperator * /*op*/)
{
  Main *bmain = CTX_data_main(C);
  Scene *scene = CTX_data_scene(C);
...
static OperatorStatus collection_objects_remove_exec(bContext *C, wmOperator *op)
{
  Object *ob = context_object(C);
  Main *bmain = CTX_data_main(C);
  Scene *scene = CTX_data_scene(C);
...
static OperatorStatus collection_create_exec(bContext *C, wmOperator *op)
{
  Main *bmain = CTX_data_main(C);
...
static OperatorStatus collection_exporter_add_exec(bContext *C, wmOperator *op)
{
...
static OperatorStatus collection_exporter_remove_exec(bContext *C, wmOperator *op)
{
  Collection *collection = CTX_data_collection(C);
  ListBase *exporters = &collection->exporters;
...
static OperatorStatus collection_exporter_remove_invoke(bContext *C,
                                                        wmOperator *op,
                                                        const wmEvent * /*event*/)
{
...
static OperatorStatus collection_exporter_export(bContext *C,
                                                 wmOperator *op,
                                                 ...
                                                 const bool report_success)
{
  ...
  OperatorStatus op_result = WM_operator_name_call_ptr(
...
static OperatorStatus collection_exporter_export_exec(bContext *C, wmOperator *op)
{
  Collection *collection = CTX_data_collection(C);
  ListBase *exporters = &collection->exporters;
...
static OperatorStatus collection_export(bContext *C,
                                        wmOperator *op,
                                        Collection *collection,
...
static OperatorStatus collection_io_export_all_exec(bContext *C, wmOperator *op)
{
  Collection *collection = CTX_data_collection(C);
  CollectionExportStats stats;
  OperatorStatus result = collection_export(C, op, collection, stats);
...
static OperatorStatus wm_collection_export_all_exec(bContext *C, wmOperator *op)
{
  ViewLayer *view_layer = CTX_data_view_layer(C);
...
static OperatorStatus collection_add_exec(bContext *C, wmOperator * /*op*/)
{
  Object *ob = context_object(C);
  Main *bmain = CTX_data_main(C);
...
static OperatorStatus collection_link_exec(bContext *C, wmOperator *op)
{
  Main *bmain = CTX_data_main(C);
  Object *ob = context_object(C);
...
static OperatorStatus collection_remove_exec(bContext *C, wmOperator *op)
{
  Main *bmain = CTX_data_main(C);
  Object *ob = context_object(C);
  Collection *collection = static_cast<Collection *>(
...
static OperatorStatus collection_unlink_exec(bContext *C, wmOperator *op)
{
  Main *bmain = CTX_data_main(C);
  Collection *collection = CTX_data_collection(C);
...
static OperatorStatus select_grouped_exec(bContext *C, wmOperator * /*op*/)
{
  Scene *scene = CTX_data_scene(C);
  Collection *collection = static_cast<Collection *>(
      CTX_data_pointer_get_type(C, "collection", &RNA_Collection).data);
