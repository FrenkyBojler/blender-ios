...
bool editmode_enter_ex(Main *bmain, Scene *scene, Object *ob, int flag)
{
  ...
  if (ELEM(nullptr, ob, ob->data) || !ID_IS_EDITABLE(ob) || ID_IS_OVERRIDE_LIBRARY(ob->data)) {
    return false;
  }
...
static bool editmode_toggle_poll(bContext *C)
{
  ...
  if (ELEM(nullptr, ob, ob->data) || !ID_IS_EDITABLE(ob->data) || ID_IS_OVERRIDE_LIBRARY(ob->data))
  {
    return false;
  }
