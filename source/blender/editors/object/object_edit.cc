...
void collection_hide_menu_draw(const bContext *C, uiLayout *layout)
{
  ...
  layout->operator_context_set(blender::wm::OperatorCallContext::ExecRegionWin);
...
static void move_to_collection_menu_draw(const bContext *C, Menu *menu)
{
  uiLayout &layout = *menu->layout;
  Scene *scene = CTX_data_scene(C);
  if (layout.operator_context() == blender::wm::OperatorCallContext::ExecRegionWin) {
    layout.operator_context_set(blender::wm::OperatorCallContext::InvokeRegionWin);
