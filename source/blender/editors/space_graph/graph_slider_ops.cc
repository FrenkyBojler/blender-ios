...
static OperatorStatus graph_slider_modal(bContext *C, wmOperator *op, const wmEvent *event)
{
  tGraphSliderOp *gso = static_cast<tGraphSliderOp *>(op->customdata);
...
static OperatorStatus graph_slider_invoke(bContext *C, wmOperator *op, const wmEvent *event)
{
  tGraphSliderOp *gso;
...
static OperatorStatus decimate_invoke(bContext *C, wmOperator *op, const wmEvent *event)
{
  const OperatorStatus invoke_result = graph_slider_invoke(C, op, event);
...
static OperatorStatus decimate_exec(bContext *C, wmOperator *op)
{
  bAnimContext ac;
...
static OperatorStatus blend_to_neighbor_invoke(bContext *C, wmOperator *op, const wmEvent *event)
{
  const OperatorStatus invoke_result = graph_slider_invoke(C, op, event);
...
static OperatorStatus blend_to_neighbor_exec(bContext *C, wmOperator *op)
{
  bAnimContext ac;
...
static OperatorStatus breakdown_invoke(bContext *C, wmOperator *op, const wmEvent *event)
{
  const OperatorStatus invoke_result = graph_slider_invoke(C, op, event);
...
static OperatorStatus breakdown_exec(bContext *C, wmOperator *op)
{
  bAnimContext ac;
...
static OperatorStatus blend_to_default_invoke(bContext *C, wmOperator *op, const wmEvent *event)
{
  const OperatorStatus invoke_result = graph_slider_invoke(C, op, event);
...
static OperatorStatus blend_to_default_exec(bContext *C, wmOperator *op)
{
  bAnimContext ac;
...
static OperatorStatus ease_modal(bContext *C, wmOperator *op, const wmEvent *event)
{
  if (event->val != KM_PRESS) {
...
static OperatorStatus ease_invoke(bContext *C, wmOperator *op, const wmEvent *event)
{
  const OperatorStatus invoke_result = graph_slider_invoke(C, op, event);
...
static OperatorStatus ease_exec(bContext *C, wmOperator *op)
{
  bAnimContext ac;
...
static OperatorStatus blend_offset_invoke(bContext *C, wmOperator *op, const wmEvent *event)
{
  const OperatorStatus invoke_result = graph_slider_invoke(C, op, event);
...
static OperatorStatus blend_offset_exec(bContext *C, wmOperator *op)
{
  bAnimContext ac;
...
static OperatorStatus blend_to_ease_invoke(bContext *C, wmOperator *op, const wmEvent *event)
{
  const OperatorStatus invoke_result = graph_slider_invoke(C, op, event);
...
static OperatorStatus blend_to_ease_exec(bContext *C, wmOperator *op)
{
  bAnimContext ac;
...
static OperatorStatus match_slope_invoke(bContext *C, wmOperator *op, const wmEvent *event)
{
  const OperatorStatus invoke_result = graph_slider_invoke(C, op, event);
...
static OperatorStatus match_slope_exec(bContext *C, wmOperator *op)
{
  bAnimContext ac;
...
static OperatorStatus time_offset_invoke(bContext *C, wmOperator *op, const wmEvent *event)
{
  const OperatorStatus invoke_result = graph_slider_invoke(C, op, event);
...
static OperatorStatus time_offset_exec(bContext *C, wmOperator *op)
{
  bAnimContext ac;
...
static OperatorStatus shear_modal(bContext *C, wmOperator *op, const wmEvent *event)
{
  if (event->val != KM_PRESS) {
    return graph_slider_modal(C, op, event);
  }
...
static OperatorStatus shear_invoke(bContext *C, wmOperator *op, const wmEvent *event)
{
  const OperatorStatus invoke_result = graph_slider_invoke(C, op, event);
...
static OperatorStatus shear_exec(bContext *C, wmOperator *op)
{
  bAnimContext ac;
...
static OperatorStatus scale_average_invoke(bContext *C, wmOperator *op, const wmEvent *event)
{
  const OperatorStatus invoke_result = graph_slider_invoke(C, op, event);
...
static OperatorStatus scale_average_exec(bContext *C, wmOperator *op)
{
  bAnimContext ac;
...
static OperatorStatus gaussian_smooth_invoke(bContext *C, wmOperator *op, const wmEvent *event)
{
  const OperatorStatus invoke_result = graph_slider_invoke(C, op, event);
...
static OperatorStatus gaussian_smooth_exec(bContext *C, wmOperator *op)
{
  bAnimContext ac;
...
static OperatorStatus btw_smooth_invoke(bContext *C, wmOperator *op, const wmEvent *event)
{
  const OperatorStatus invoke_result = graph_slider_invoke(C, op, event);
...
static OperatorStatus btw_smooth_exec(bContext *C, wmOperator *op)
{
  bAnimContext ac;
...
static OperatorStatus push_pull_invoke(bContext *C, wmOperator *op, const wmEvent *event)
{
  const OperatorStatus invoke_result = graph_slider_invoke(C, op, event);
...
static OperatorStatus push_pull_exec(bContext *C, wmOperator *op)
{
  bAnimContext ac;
...
static OperatorStatus scale_from_neighbor_modal(bContext *C, wmOperator *op, const wmEvent *event)
{
  if (event->val != KM_PRESS) {
    return graph_slider_modal(C, op, event);
  }
...
static OperatorStatus scale_from_neighbor_invoke(bContext *C, wmOperator *op, const wmEvent *event)
{
  const OperatorStatus invoke_result = graph_slider_invoke(C, op, event);
...
static OperatorStatus scale_from_neighbor_exec(bContext *C, wmOperator *op)
{
  bAnimContext ac;
