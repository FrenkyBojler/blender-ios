...
static OperatorStatus run_node_group_exec(bContext *C, wmOperator *op)
{
  Main *bmain = CTX_data_main(C);
  Scene *scene = CTX_data_scene(C);
  Object *active_object = CTX_data_active_object(C);
...
static OperatorStatus run_node_group_invoke(bContext *C, wmOperator *op, const wmEvent *event)
{
