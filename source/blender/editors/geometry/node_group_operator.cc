                                              ...
                                              const StringRefNull name)
{
          ...
          geometry.get_component_for_write<bke::GreasePencilComponent>().release())
  {
    ...
    BKE_id_material_from_eval_data(&new_grease_pencil->id, &src_grease_pencil->id);
    ...
    new_ids.append(&new_grease_pencil->id);
  }
  if (new_ids.is_empty()) {
                                                   ...
                                                   Set<ID *> &objects_to_delete)
{
  for (Object *object : selected_objects) {
    ...
    reference_to_updated_data.add_new(reference_index, object->data);
  }
...
static wmOperatorStatus run_node_group_exec(bContext *C, wmOperator *op)
{
  ...
  operator_eval_data.self_object_orig = nullptr;
  ...
  if (objects_as_instances) {
    ...
    bke::Instances *new_instances = result_geometry.get_instances_for_write();
    if (!new_instances) {
      return OPERATOR_CANCELLED;
    }
    ...
    for (Object *object : objects) {
      ...
      call_data.socket_log_contexts = object == active_object ? &socket_log_contexts : nullptr;
