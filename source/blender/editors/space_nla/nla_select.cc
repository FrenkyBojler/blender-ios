...
static OperatorStatus nlaedit_deselectall_exec(bContext *C, wmOperator *op)
{
  bAnimContext ac;
...
static OperatorStatus nlaedit_box_select_invoke(bContext *C, wmOperator *op, const wmEvent *event)
{
  bAnimContext ac;
  if (ANIM_animdata_get_context(C, &ac) == 0) {
    return OPERATOR_CANCELLED;
...
static OperatorStatus nlaedit_box_select_exec(bContext *C, wmOperator *op)
{
  bAnimContext ac;
  rcti rect;
  short mode = 0;
...
static OperatorStatus nlaedit_select_leftright_exec(bContext *C, wmOperator *op)
{
  bAnimContext ac;
...
static OperatorStatus nlaedit_select_leftright_invoke(bContext *C,
                                                      wmOperator *op,
                                                      const wmEvent *event)
{
  bAnimContext ac;
...
static OperatorStatus mouse_nla_strips(bContext *C,
                                       bAnimContext *ac,
                                       const int mval[2],
                                       short select_mode,
                                       ...
                                       bool wait_to_deselect_others)
{
  ...
  OperatorStatus ret_value = OPERATOR_FINISHED;
...
/* handle clicking */
static OperatorStatus nlaedit_clickselect_exec(bContext *C, wmOperator *op)
{
  bAnimContext ac;
  OperatorStatus ret_value;
