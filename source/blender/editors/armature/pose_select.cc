                                             ...
                                             const bool extend)
{
  blender::animrig::pose_bone_descendent_depth_iterator(
      *ob.pose, pose_bone, [extend](bPoseChannel &child) {
        ...
        if (extend) {
          blender::animrig::deselect_bone(&child);
        }
        else {
          blender::animrig::select_bone(&child);
        }
        return true;
      });
}
                                                     ...
                                                     const wmEvent *event)
{
  ...
  for (curBone = pchan; curBone; curBone = next) {
    ...
    if ((curBone->flag & BONE_UNSELECTABLE) == 0) {
      if (extend) {
        blender::animrig::deselect_bone(curBone);
      }
      else {
        blender::animrig::select_bone(curBone);
      }
...
static wmOperatorStatus pose_select_parent_exec(bContext *C, wmOperator * /*op*/)
{
  ...
  if (pchan) {
    ...
    {
      blender::animrig::select_bone(parent);
          ...
          LISTBASE_FOREACH (bConstraintTarget *, ct, &targets) {
                ...
                (ob->mode == OB_MODE_POSE))
            {
              ...
              if ((pchanc) && !(pchanc->bone->flag & BONE_UNSELECTABLE)) {
                blender::animrig::select_bone(pchanc);
                ED_pose_bone_select_tag_update(ob);
                found = true;
              }
            }
  ...
  if (direction == BONE_SELECT_PARENT) {
    if (pchan_act->parent) {
      ...
      if (blender::animrig::bone_is_selectable(arm, bone_parent)) {
        if (!add_to_sel) {
          blender::animrig::deselect_bone(pchan_act);
        }
        blender::animrig::select_bone(pchan_act->parent);
    ...
    if (bone_child) {
      ...
      blender::animrig::select_bone(bone_child);
