...
static OperatorStatus add_vertex_handle_cyclic_at_point(bContext *C,
                                                        Mask *mask,
                                                        MaskSpline *spline,
...
static OperatorStatus add_vertex_handle_cyclic(
...
static OperatorStatus add_vertex_exec(bContext *C, wmOperator *op)
{
      ...
      MASKPOINT_ISSEL_ANY(mask_layer->act_point))
  {
    ...
    const OperatorStatus cyclic_result = add_vertex_handle_cyclic(
...
static OperatorStatus add_vertex_invoke(bContext *C, wmOperator *op, const wmEvent *event)
{
  ScrArea *area = CTX_wm_area(C);
  ARegion *region = CTX_wm_region(C);
...
static OperatorStatus add_feather_vertex_exec(bContext *C, wmOperator *op)
{
  Mask *mask = CTX_data_edit_mask(C);
  MaskLayer *mask_layer;
  MaskSpline *spline;
...
static OperatorStatus add_feather_vertex_invoke(bContext *C, wmOperator *op, const wmEvent *event)
{
  ScrArea *area = CTX_wm_area(C);
  ARegion *region = CTX_wm_region(C);
...
static OperatorStatus primitive_add_invoke(bContext *C, wmOperator *op, const wmEvent * /*event*/)
{
  ScrArea *area = CTX_wm_area(C);
  float cursor[2];
  int width, height;
...
static OperatorStatus primitive_circle_add_exec(bContext *C, wmOperator *op)
{
...
static OperatorStatus primitive_square_add_exec(bContext *C, wmOperator *op)
{
