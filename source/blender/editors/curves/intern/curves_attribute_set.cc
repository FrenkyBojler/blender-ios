...
static OperatorStatus set_attribute_exec(bContext *C, wmOperator *op)
{
...
static OperatorStatus set_attribute_invoke(bContext *C, wmOperator *op, const wmEvent *event)
{
  Object *active_object = CTX_data_active_object(C);
  Curves &active_curves_id = *static_cast<Curves *>(active_object->data);
