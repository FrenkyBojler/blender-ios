...
static OperatorStatus curves_convert_to_particle_system_exec(bContext *C, wmOperator *op)
{
...
static OperatorStatus curves_convert_from_particle_system_exec(bContext *C, wmOperator * /*op*/)
{
  Main &bmain = *CTX_data_main(C);
  Scene &scene = *CTX_data_scene(C);
...
static OperatorStatus snap_curves_to_surface_exec(bContext *C, wmOperator *op)
{
...
static OperatorStatus curves_set_selection_domain_exec(bContext *C, wmOperator *op)
{
...
static OperatorStatus split_exec(bContext *C, wmOperator * /*op*/)
{
  VectorSet<Curves *> unique_curves = get_unique_editable_curves(*C);
  for (Curves *curves_id : unique_curves) {
    CurvesGeometry &curves = curves_id->geometry.wrap();
...
static OperatorStatus surface_set_exec(bContext *C, wmOperator *op)
{
  Main *bmain = CTX_data_main(C);
  Scene *scene = CTX_data_scene(C);
...
static OperatorStatus exec(bContext *C, wmOperator * /*op*/)
{
  for (Curves *curves_id : get_unique_editable_curves(*C)) {
    bke::CurvesGeometry &curves = curves_id->geometry.wrap();
    IndexMaskMemory memory;
