 ...
 */
static OperatorStatus ed_undo_step_direction(bContext *C,
...
static OperatorStatus ed_undo_exec(bContext *C, wmOperator *op)
{
  /* "last operator" should disappear, later we can tie this with undo stack nicer */
  WM_operator_stack_clear(CTX_wm_manager(C));
  OperatorStatus ret = ed_undo_step_direction(C, STEP_UNDO, op->reports);
...
static OperatorStatus ed_undo_push_exec(bContext *C, wmOperator *op)
{
  if (G.background) {
...
static OperatorStatus ed_redo_exec(bContext *C, wmOperator *op)
{
  OperatorStatus ret = ed_undo_step_direction(C, STEP_REDO, op->reports);
  if (ret & OPERATOR_FINISHED) {
    ed_undo_refresh_for_op(C);
  }
  return ret;
...
static OperatorStatus ed_undo_redo_exec(bContext *C, wmOperator * /*op*/)
{
  wmOperator *last_op = WM_operator_last_redo(C);
  OperatorStatus ret = ED_undo_operator_repeat(C, last_op) ? OPERATOR_FINISHED :
                                                             OPERATOR_CANCELLED;
  if (ret & OPERATOR_FINISHED) {
    /* Keep button under the cursor active. */
    WM_event_add_mousemove(CTX_wm_window(C));
...
static OperatorStatus undo_history_exec(bContext *C, wmOperator *op)
{
...
static OperatorStatus undo_history_invoke(bContext *C, wmOperator *op, const wmEvent * /*event*/)
{
  PropertyRNA *prop = RNA_struct_find_property(op->ptr, "item");
  if (RNA_property_is_set(op->ptr, prop)) {
