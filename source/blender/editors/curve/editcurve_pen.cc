...
static OperatorStatus curve_pen_modal(bContext *C, wmOperator *op, const wmEvent *event)
{
...
static OperatorStatus curve_pen_invoke(bContext *C, wmOperator *op, const wmEvent *event)
{
  Depsgraph *depsgraph = CTX_data_ensure_evaluated_depsgraph(C);
  ViewContext vc = ED_view3d_viewcontext_init(C, depsgraph);
  Curve *cu = static_cast<Curve *>(vc.obedit->data);
