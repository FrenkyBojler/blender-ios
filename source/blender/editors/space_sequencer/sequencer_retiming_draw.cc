                                            ...
                                            int &r_frame)
{
  ...
  const int left_frame = left_fake_key_frame_get(scene, strip);
  const float left_distance = fabs(ui::view2d_view_to_region_x(v2d, left_frame) - mval[0]);

  const int right_frame = right_fake_key_frame_get(scene, strip);
                           ...
                           const KeyframeShaderBindings &sh_bindings)
{
  ...
  const int left_key_frame = left_fake_key_frame_get(scene, strip);
  ...
  int right_key_frame = right_fake_key_frame_get(scene, strip);
