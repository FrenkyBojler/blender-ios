                           ...
                           bool skip_border)
{
  ...
  if (!skip_border) {
    /* Horizontal lines. */
    quads.add_line(area.xmin, area.ymin, area.xmax, area.ymin, col_grid);
    quads.add_line(area.xmin, area.ymax, area.xmax, area.ymax, col_grid);
  }
                                       ...
                                       const ocio::ScopeInfo &scope_info)
{
  ...
  for (int i = 0; i < 6; i++) {
    ...
    float2 pos = center + float2(rgb_to_cbcr(yuv_matrix, safe).x * (radius * 2),
                                 rgb_to_cbcr(yuv_matrix, safe).y * (radius * y_corr * 2));
    quads.add_wire_quad(
                                  ...
                                  bool premultiplied)
{
  ...
  for (int i = 0; i < scopes_count; i++) {
    ...
    switch (scp) {
      ...
      case SEQ_DRAW_IMG_RGBPARADE:
        use_blend = true;
        draw_waveform_graticule(&region,
                                quads,
                                area,
                                &scene->view_settings,
                                &scene->display_settings,
                                skip_individual_borders);
        break;
      ...
      case SEQ_DRAW_IMG_VECTORSCOPE: {
        ...
        const ocio::ScopeInfo scope_info = IMB_colormanagement_get_scope_info(
            &display_settings, view_settings.view_transform);
        draw_vectorscope_graticule(&region, quads, area, scope_info);
      } break;
