...
static void draw_strip_offsets(const TimelineDrawContext &ctx, const StripDrawContext &strip_ctx)
{
  ...
  if ((ctx.sseq->timeline_overlay.flag & SEQ_TIMELINE_SHOW_STRIP_OFFSETS) == 0 &&
      ...
      (strip_ctx.strip->runtime->flag & seq::STRIP_SHOW_OFFSETS) == 0)
  {
    return;
  }
                                       ...
                                       Vector<StripDrawContext> &r_top_layer)
{
  ...
  for (Strip *strip : strips) {
    ...
    if ((strip->runtime->flag & seq::STRIP_OVERLAP) == 0) {
                                          ...
                                          SeqStripDrawData &data)
{
  ...
  const bool overlaps = (strip.strip->runtime->flag & seq::STRIP_OVERLAP) && translating;

  const bool clamped_l = (strip.strip->runtime->flag & seq::STRIP_CLAMPED_LH);
  const bool clamped_r = (strip.strip->runtime->flag & seq::STRIP_CLAMPED_RH);
