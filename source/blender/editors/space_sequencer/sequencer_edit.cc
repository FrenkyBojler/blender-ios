...
static wmOperatorStatus sequencer_gap_remove_exec(bContext *C, wmOperator *op)
{
  ...
  seq::edit_remove_gaps(scene, ed->active_strips(), scene->r.cfra, do_all);
...
static wmOperatorStatus sequencer_gap_insert_exec(bContext *C, wmOperator *op)
{
  ...
  const Editing *ed = seq::editing_get(scene);
  seq::transform_offset_after_frame(scene, ed->active_strips(), frames, scene->r.cfra);
...
static wmOperatorStatus sequencer_snap_exec(bContext *C, wmOperator *op)
{
  ...
  LISTBASE_FOREACH (Strip *, strip, ed->active_strips()) {
    if (strip->flag & SELECT && !seq::transform_is_locked(channels, strip)) {
      ...
      if (seq::transform_test_overlap(scene, ed->active_strips(), strip)) {
...
static SlipData *slip_data_init(const Scene *scene)
{
  ...
  VectorSet<Strip *> strips = seq::query_selected_strips(ed->active_strips());
                                               ...
                                               bool ignore_active)
{
  ...
  Editing *ed = seq::editing_get(scene);
  blender::VectorSet<Strip *> selected_strips = seq::query_selected_strips(ed->active_strips());
...
static wmOperatorStatus sequencer_split_exec(bContext *C, wmOperator *op)
{
  ...
  LISTBASE_FOREACH_BACKWARD (Strip *, strip, ed->active_strips()) {
    ...
    if (ignore_selection || strip->flag & SELECT) {
      ...
      if (seq::edit_strip_split(
              bmain, scene, ed->active_strips(), strip, split_frame, method, &error_msg) !=
          nullptr)
      {
        changed = true;
      }
...
static wmOperatorStatus sequencer_add_duplicate_exec(bContext *C, wmOperator *op)
{
  ...
  seq::seqbase_duplicate_recursive(
      bmain, scene, scene, &duplicated_strips, ed->active_strips(), dupe_flag, 0);
  ...
  BLI_movelisttolist(ed->active_strips(), &duplicated_strips);
...
static wmOperatorStatus sequencer_offset_clear_exec(bContext *C, wmOperator * /*op*/)
{
  ...
  for (strip = static_cast<Strip *>(ed->active_strips()->first); strip;
  ...
  /* Update lengths, etc. */
  strip = static_cast<Strip *>(ed->active_strips()->first);
  while (strip) {
    seq::relations_invalidate_cache(scene, strip);
    strip = strip->next;
  }
...
static wmOperatorStatus sequencer_meta_separate_exec(bContext *C, wmOperator * /*op*/)
{
   ...
   * Strip is moved within the same edit, no need to re-generate the UID. */
  BLI_movelisttolist(ed->active_strips(), &active_strip->seqbase);
