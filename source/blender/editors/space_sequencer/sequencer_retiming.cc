                                           ...
                                           const int timeline_frame)
{
  ...
  const float frame_index = (BKE_scene_frame_get(scene) - strip->start_frame_get()) *
  ...
  const float end_frame = strip->start + strip->length_get(scene);
...
static void realize_fake_keys_in_rect(bContext *C, Strip *strip, const rctf &rectf)
{
  ...
  const int content_start = strip->start_frame_get();
  const int left_key_frame = max_ii(content_start, strip->left_handle_frame_get());
  const int content_end = strip->content_end_frame_get(scene);
  const int right_key_frame = min_ii(content_end, strip->right_handle_frame_get(scene));
...
wmOperatorStatus sequencer_retiming_box_select_exec(bContext *C, wmOperator *op)
{
  ...
  for (Strip *strip : sequencer_visible_strips_get(C)) {
    ...
    for (SeqRetimingKey &key : seq::retiming_keys_get(strip)) {
      ...
      const int strip_start = strip->left_handle_frame_get();
