...
static OperatorStatus strip_modifier_add_exec(bContext *C, wmOperator *op)
{
...
static OperatorStatus strip_modifier_remove_exec(bContext *C, wmOperator *op)
{
  Scene *scene = CTX_data_scene(C);
  Strip *strip = seq::select_active_get(scene);
  char name[MAX_NAME];
...
static OperatorStatus strip_modifier_move_exec(bContext *C, wmOperator *op)
{
  Scene *scene = CTX_data_scene(C);
  Strip *strip = seq::select_active_get(scene);
  char name[MAX_NAME];
...
static OperatorStatus strip_modifier_copy_exec(bContext *C, wmOperator *op)
{
  Scene *scene = CTX_data_scene(C);
  Editing *ed = scene->ed;
  Strip *strip = seq::select_active_get(scene);
...
static OperatorStatus strip_modifier_equalizer_redefine_exec(bContext *C, wmOperator *op)
{
  Scene *scene = CTX_data_scene(C);
  Strip *strip = seq::select_active_get(scene);
  SequenceModifierData *smd;
