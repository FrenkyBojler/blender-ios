...
/* Global mutex to the paint tile map. The individual #PaintTile have their own mutex. */
static blender::Mutex paint_tiles_mutex;
                                      ...
                                      bool **r_valid)
{
  ...
  bool created = false;
  {
    std::scoped_lock map_lock(paint_tiles_mutex);
...
{
  for (UndoImageHandle &uh : undo_handles->items_mutable()) {
                                                   ...
                                                   ImageUser *iuser)
{
  ...
  for (ImageTile &current_tile : image->tiles) {
