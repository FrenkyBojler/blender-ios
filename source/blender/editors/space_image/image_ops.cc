...
static wmOperatorStatus image_file_browse_invoke(bContext *C, wmOperator *op, const wmEvent *event)
{
   ...
   * #uiLayout::context_set(std::string,PointerRNA). The File Browser doesn't support restoring
   * this context when calling `exec()` though, so we have to pass it the image via custom data. */
  op->customdata = ima;
