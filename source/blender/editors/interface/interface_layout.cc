...
namespace internal {
void pie_menu_create_scroll_pages(Block *block, Layout *layout);
}
...
void LayoutRootPieMenu::resolve_impl()
{
  ...
  internal::pie_menu_create_scroll_pages(this->block(), this);
}
...
void pie_menu_workspace_status(bContext *C, const Block *block)
{
...
void pie_menu_apply_page_scroll(const Block *block)
{
  for (int i : block->pie_data.pages.index_range()) {
    for (Button *but : block->pie_data.pages[i]) {
...
static void pie_menu_add_buts_to_page(PieMenuPage &page, const Item *item)
{
  if (item->type() != ItemType::Button) {
    const Layout *litem = static_cast<const Layout *>(item);
    for (const Item *subitem : litem->items()) {
...
void pie_menu_create_scroll_pages(Block *block, Layout *layout)
{
  BLI_assert(layout->root()->type == LayoutType::PieMenu);
  const Item *const *pie_menu = std::find_if(
      layout->items().begin(), layout->items().end(), [](const Item *item) {
        return item->type() == ItemType::LayoutRadial;
      });
  ...
  for (const Item *subitem : static_cast<const Layout *>(*pie_menu)->items()) {
