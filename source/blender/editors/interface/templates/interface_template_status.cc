...
void uiTemplateStatusInfo(Layout *layout, bContext *C)
{
  ...
  if (U.statusbar_flag & STATUSBAR_SHOW_EXTENSIONS_UPDATES) {
    ...
    if (wm->extensions_blocked > 0) {
      ...
      ui::button_icon_decoration_number_set(but, wm->extensions_blocked);
      ui::button_icon_decoration_color_set(but, color);
             ...
             (wm->extensions_updates == WM_EXTENSIONS_UPDATE_CHECKING))
    {
      ...
      if (wm->extensions_updates > 0) {
        ui::button_icon_decoration_number_set(but, wm->extensions_updates);
        ui::button_icon_decoration_color_set(but, color);
      }
