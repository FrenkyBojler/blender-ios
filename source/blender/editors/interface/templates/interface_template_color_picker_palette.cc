...
void template_colorpicker_palette(Layout *layout, PointerRNA *ptr, const StringRefNull propname)
{
  ...
  template_id_simple(
      panel.body, C, ptr, propname, "PALETTE_OT_new", "PALETTE_OT_unlink", 0, std::nullopt);
  ...
  const float button_size = (cols_per_row > 1) ? (panel.body->width() / cols_per_row) :
                                                 base_button_size;
  ...
  if (color_count == 0) {
    ...
    Button *empty_label = uiItemL_ex(
        empty_layout, IFACE_("No colors in palette"), ICON_NONE, false, false);
    ...
    for (PaletteColor &color : palette->colors) {
      ...
      if (paint && paint->brush) {
        if (fabsf(color.color[0] - current_brush_color[0]) < 0.01f &&
            fabsf(color.color[1] - current_brush_color[1]) < 0.01f &&
            fabsf(color.color[2] - current_brush_color[2]) < 0.01f)
        {
...
static void ui_colorpicker_palette_add_cb(bContext *C, void *but1, void *arg)
{
  ...
  if (color_found) {
    for (PaletteColor &existing_color : palette->colors) {
      if (compare_v3v3(existing_color.color, color, 0.01f)) {
        printf("DEBUG: Palette Add: DUPLICATE FOUND (%.3f, %.3f, %.3f) vs (%.3f, %.3f, %.3f)\n",
               existing_color.color[0],
               existing_color.color[1],
               existing_color.color[2],
               color[0],
               color[1],
               color[2]);
        /* Show user feedback */
