...
{
  PointerRNA *rnapoin = &but.rnapoin;
  PropertyRNA *rnaprop = but.rnaprop;
  int rnaindex = but.rnaindex;

  /* Decorator button RNA data also needs to match the 'main' Button RNA data. */
  if (button_is_decorator(&but)) {
    ButtonDecorator &decorator_but = static_cast<ButtonDecorator &>(but);
    rnapoin = &decorator_but.decorated_rnapoin;
    rnaprop = decorator_but.decorated_rnaprop;
    rnaindex = decorator_but.rnaindex;
  }

  if (!rnapoin->data || !rnaprop) {
    return;
  }
  ...
  but.dynoverride_target.rnapoin = *rnapoin;
  but.dynoverride_target.rnaprop = rnaprop;
  but.dynoverride_target.rnaindex = rnaindex;

  if (button_is_decorator(&but)) {
    /* Decorator buttons keep their 'main' rnapoin/rnaprop unset. */
    ButtonDecorator &decorator_but = static_cast<ButtonDecorator &>(but);
    decorator_but.decorated_rnapoin = dynoverride_rule_data_ptr;
    decorator_but.decorated_rnaprop = dynoverride_prop;
    decorator_but.decorated_rnaindex = rnaindex;
  }
  else {
    but.rnapoin = dynoverride_rule_data_ptr;
    but.rnaprop = dynoverride_prop;
    but.rnaindex = rnaindex;
  }
                  ...
                  int r_xy[2])
{
  ...
  for (Button &but : block->buttons()) {
    ...
    if (button_is_decorator(&but)) {
      /* Decorator buttons keep their 'main' rnapoin/rnaprop unset. */
      ButtonDecorator &decorator_but = static_cast<ButtonDecorator &>(but);
      but.override_status = RNA_property_override_status(bmain,
                                                         scene,
                                                         &decorator_but.decorated_rnapoin,
                                                         decorator_but.decorated_rnaprop,
                                                         decorator_but.decorated_rnaindex);
    }
    else {
