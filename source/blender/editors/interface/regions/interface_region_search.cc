                     ...
                     const uint8_t name_prefix_offset)
{
  ...
  if (name_prefix_offset != 0) {
    ...
    if (items->name_prefix_offsets == nullptr) {
      items->name_prefix_offsets = static_cast<uint8_t *>(
          MEM_callocN(items->maxitem * sizeof(*items->name_prefix_offsets), __func__));
    }
                                            ...
                                            const bool use_shortcut_sep)
{
  ...
  data->items.totitem = 0;
  data->items.names = static_cast<char **>(
      MEM_callocN(data->items.maxitem * sizeof(void *), __func__));
  data->items.pointers = static_cast<void **>(
      MEM_callocN(data->items.maxitem * sizeof(void *), __func__));
  ...
  for (int i = 0; i < data->items.maxitem; i++) {
    data->items.names[i] = static_cast<char *>(MEM_callocN(data->items.maxstrlen + 1, __func__));
  }
...
void button_search_refresh(ButtonSearch *but)
{
  ...
  items->maxitem = 10;
  items->maxstrlen = 256;
  items->names = static_cast<char **>(MEM_callocN(items->maxitem * sizeof(void *), __func__));
  for (int i = 0; i < items->maxitem; i++) {
    items->names[i] = static_cast<char *>(MEM_callocN(but->hardmax + 1, __func__));
  }
