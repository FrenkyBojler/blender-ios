...
{
  BLI_assert(but->type == eButType::Menu);
...
static uiBut *ui_popup_menu_memory__internal(uiBlock *block, uiBut *but)
{
  ...
  for (const std::unique_ptr<uiBut> &but_iter : block->buttons) {
    ...
    if (ELEM(but_iter->type, eButType::Label, eButType::Sepr, eButType::SeprLine)) {
      continue;
    }
...
static void create_title_button(uiLayout *layout, const char *title, int icon)
{
  ...
  if (icon) {
    ...
    uiDefIconTextBut(
        block, eButType::Label, 0, icon, titlestr, 0, 0, 200, UI_UNIT_Y, nullptr, 0.0, 0.0, "");
  }
  else {
    uiBut *but = uiDefBut(
        block, eButType::Label, 0, title, 0, 0, 200, UI_UNIT_Y, nullptr, 0.0, 0.0, "");
