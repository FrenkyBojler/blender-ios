    ...
    bContext *C, uiBut *but, uiButExtraOpIcon *extra_icon, const bool is_quick_tip)
{
  ...
  if (!but_tip.empty()) {
    ...
    if ((but->type == eButType::Row) && rnaprop && RNA_property_flag(rnaprop) & PROP_ENUM_FLAG) {
      UI_tooltip_text_field_add(*data,
  ...
  if (ELEM(but->type, eButType::Text, eButType::SearchMenu)) {
  ...
  if (ELEM(but->type, eButType::Text) &&
    ...
    bContext *C, ARegion *butregion, uiBut *but, uiButExtraOpIcon *extra_icon, bool is_quick_tip)
{
  ...
  else if (but->type == eButType::Label && BLI_rctf_size_y(&but->rect) > UI_UNIT_Y) {
