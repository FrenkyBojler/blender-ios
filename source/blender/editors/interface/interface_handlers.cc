...
struct AfterFunc {
  ...
  UndoEncodeHints undo_hints = UndoEncodeHints(0);
};
...
static void apply_but_undo(Button *but, bool use_undo_grouped = false)
{
  ...
  std::optional<UndoEncodeHints> undo_hints_or_none = UndoEncodeHints(0);
  ...
  after->undo_hints = undo_hints_or_none.has_value() ? undo_hints_or_none.value() :
                                                       UndoEncodeHints(0);
}
