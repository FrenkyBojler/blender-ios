...
/* Codepoints from IBM Code Page 437 / DOS. */
static const wchar_t ibm437_chars[] = {
    ...
    0x2193, 0x2192, 0x2190, 0x221F, 0x2194, 0x25B2, 0x25BC, 0x0020, 0x0021, 0x0022, 0x0023, 0x0024,
    ...
    0x0079, 0x007A, 0x007B, 0x007C, 0x007D, 0x007E, 0x2302, 0x00C7, 0x00FC, 0x00E9, 0x00E2, 0x00E4,
    0x00E0, 0x00E5, 0x00E7, 0x00EA, 0x00EB, 0x00E8, 0x00EF, 0x00EE, 0x00EC, 0x00C4, 0x00C5, 0x00C9,
    0x00E6, 0x00C6, 0x00F4, 0x00F6, 0x00F2, 0x00FB, 0x00F9, 0x00FF, 0x00D6, 0x00DC, 0x00A2, 0x00A3,
    0x00A5, 0x20A7, 0x0192, 0x00E1, 0x00ED, 0x00F3, 0x00FA, 0x00F1, 0x00D1, 0x00AA, 0x00BA, 0x00BF,
    0x2310, 0x00AC, 0x00BD, 0x00BC, 0x00A1, 0x00AB, 0x00BB, 0x2591, 0x2592, 0x2593, 0x2502, 0x2524,
    ...
    0x2518, 0x250C, 0x2588, 0x2584, 0x258C, 0x2590, 0x2580, 0x03B1, 0x00DF, 0x0393, 0x03C0, 0x03A3,
    0x03C3, 0x00B5, 0x03A4, 0x03A6, 0x0398, 0x03A9, 0x03B4, 0x221E, 0x03C6, 0x03B5, 0x2229, 0x2261,
    0x00B1, 0x2265, 0x2264, 0x2320, 0x2321, 0x00F7, 0x2248, 0x00B0, 0x2219, 0x00B7, 0x221A, 0x207F,
                                   ...
                                   bool *changed)
{
  if (ELEM(event->type, EVT_LEFTALTKEY, EVT_RIGHTALTKEY)) {
    if (event->val == KM_PRESS) {
      ...
      if (len == 1 && unicode_input[0] >= '!' && unicode_input[0] <= '~') {
      ...
      else if (len == 2 && unicode_input[0] == '`') {
        // À È Ì Ò Ù à è ì ò ù
        const size_t pos = std::string("AEIOUaeiou").find(unicode_input[1]);
        if (pos != std::string::npos) {
          const wchar_t chars[] = {
              0x00C0, 0x00C8, 0x00CC, 0x00D2, 0x00D9, 0x00E0, 0x00E8, 0x00EC, 0x00F2, 0x00F9};
          val = chars[pos];
        }
      }
      else if (len == 2 && unicode_input[0] == 'e') {
        // Á É Í Ó Ú á é í ó ú
        const size_t pos = std::string("AEIOUaeiou").find(unicode_input[1]);
        if (pos != std::string::npos) {
          const wchar_t chars[] = {
              0x00C1, 0x00C9, 0x00CD, 0x00D3, 0x00DA, 0x00E1, 0x00E9, 0x00ED, 0x00F3, 0x00FA};
          val = chars[pos];
        }
      }
      else if (len == 2 && unicode_input[0] == 'i') {
        // Â Ê Î Ô Û â ê î ô û
        const size_t pos = std::string("AEIOUaeiou").find(unicode_input[1]);
        if (pos != std::string::npos) {
          const wchar_t chars[] = {
              0x00C2, 0x00CA, 0x00CE, 0x00D4, 0x00DC, 0x00E2, 0x00EA, 0x00EE, 0x00F4, 0x00FB};
          val = chars[pos];
        }
      }
      else if (len == 2 && unicode_input[0] == 'n') {
        // Ã Ñ Õ ã ñ õ
        const size_t pos = std::string("ANOano").find(unicode_input[1]);
        if (pos != std::string::npos) {
          const wchar_t chars[] = {0x00C3, 0x00D1, 0x00D5, 0x00E3, 0x00F1, 0x00F5};
          val = chars[pos];
        }
      }
      else if (len == 2 && unicode_input[0] == 'u') {
        // Ä Ë Ï Ö Ü Ÿ ä ë ï ö ü ÿ
        const size_t pos = std::string("AEIOUYaeiouy").find(unicode_input[1]);
        if (pos != std::string::npos) {
          const wchar_t chars[] = {0x00C4,
                                   0x00CB,
                                   0x00CF,
                                   0x00D6,
                                   0x00DC,
        ...
        if (val10 > 1 && val10 < 256) {
          val = ibm437_chars[val10 - 1];
        }
      }
      else {
        val = strtoul(unicode_input, NULL, 16);
      }
      ...
      if (val > 31 && val < 0x10FFFF) {
        ...
        *changed = ui_textedit_insert_buf(but, text_edit, utf8, utf8_buf_len);
    ...
    bContext *C, uiBlock *block, uiBut *but, uiHandleButtonData *data, const wmEvent *event)
{
  ...
  retval = ui_handle_unicode_input(but, text_edit, event, &changed);
