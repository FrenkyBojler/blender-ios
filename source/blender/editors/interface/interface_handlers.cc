...
static void textedit_begin(bContext *C, Button *but, HandleButtonData *data)
{
  ...
  if (is_textbox) {
    if (bool(U.flag & USER_TEXTBOX_SHIFT_SWAP)) {
      status.item(IFACE_("Confirm"), ICON_EVENT_SHIFT, ICON_EVENT_RETURN);
    ...
    bContext *C, Block *block, Button *but, HandleButtonData *data, const wmEvent *event)
{
  ...
  if (event->val == KM_PRESS && !is_ime_composing) {
    switch (event->type) {
      ...
      case EVT_RETKEY:
        if (but->type == ButtonType::TextBox &&
            (bool(USER_TEXTBOX_SHIFT_SWAP & U.flag) != bool(event->modifier & KM_SHIFT)))
        {
          char utf8_buf[2] = "\n";
