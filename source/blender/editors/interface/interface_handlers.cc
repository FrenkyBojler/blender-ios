...
{
  if ((block->flag & BLOCK_NUMSELECT) && event->val == KM_PRESS) {
    for (Button *but : block->pie_data.pages[block->pie_data.active_page]) {
...
static bool pie_menu_page_scroll_step_add(Block *block, int direction, bool cycle)
{
  ...
  if (cycle) {
    block->pie_data.active_page = math::mod_periodic(block->pie_data.active_page + direction,
                                                     pages);
    return true;
  }
...
static void pie_menu_page_scroll_step(
    bContext *C, Block *block, const PopupBlockHandle *menu, int direction, bool cycle = false)
{
  if (pie_menu_page_scroll_step_add(block, direction, cycle)) {
...
static int ui_pie_handler(bContext *C, const wmEvent *event, PopupBlockHandle *menu)
{
  ...
  auto test_pie_menu_page_scroll_handle = [&]() {
    ...
    if (event->type == LEFTMOUSE && event->val == KM_RELEASE) {
      pie_data.active_scroll_handle = active_handler;
      pie_menu_page_scroll_step(C, block, menu, direction, false);
    }
    else if (event->type == LEFTMOUSE && event->val == KM_PRESS) {
      ...
      switch (event->type) {
        ...
        case EVT_RIGHTARROWKEY: {
          if (event->val == KM_PRESS) {
            ...
            pie_menu_page_scroll_step(C, block, menu, scroll_dir);
          }
          break;
        }
        case EVT_TABKEY: {
          ...
          if (event->val == KM_PRESS) {
            ...
            pie_menu_page_scroll_step(C, block, menu, scroll_dir, true);
          }
          break;
        }
        case EVT_ESCKEY:
