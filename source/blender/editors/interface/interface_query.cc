...
{
  return ELEM(but->type, eButType::Text, eButType::Num, eButType::NumSlider, eButType::SearchMenu);
}
...
{
  /* NOTE: #eButType::Label is included for highlights, this allows drags. */
  if (ELEM(but->type, eButType::Label, eButType::PreviewTile)) {
    if (for_tooltip) {
  ...
  if (ELEM(but->type, eButType::Roundbox, eButType::Sepr, eButType::SeprLine, eButType::ListBox)) {
    return false;
  }
  ...
  if ((but->type == eButType::Text) &&
      ELEM(but->emboss, blender::ui::EmbossType::None, blender::ui::EmbossType::NoneOrStatus) &&
      !labeledit)
  {
    return false;
  }
  if ((but->type == eButType::ListRow) && labeledit) {
    return false;
  }
  if (but->type == eButType::ViewItem) {
...
{
  return (ELEM(but->type, eButType::But, eButType::Decorator) || ui_but_is_toggle(but));
}
#endif
...
uiBut *ui_list_find_mouse_over_ex(const ARegion *region, const int xy[2])
{
  ...
  LISTBASE_FOREACH (uiBlock *, block, &region->runtime->uiblocks) {
    ...
    for (int i = block->buttons.size() - 1; i >= 0; i--) {
      uiBut *but = block->buttons[i].get();
      if (but->type == eButType::ListBox && ui_but_contains_pt(but, mx, my)) {
        return but;
      }
    }
  }
...
{
  BLI_assert(listbox_but->type == eButType::ListBox);
  BLI_assert(listrow_but->type == eButType::ListRow);
...
{
  return but->type == eButType::ListRow;
}
...
{
  BLI_assert(listbox->type == eButType::ListBox);
...
{
  return but->type == eButType::ViewItem;
}
...
static const uiBut *ui_but_next_non_separator(const uiBut *but)
{
  ...
  for (int i = but->block->but_index(but); i < but->block->buttons.size(); i++) {
    but = but->block->buttons[i].get();
    if (!ELEM(but->type, eButType::Sepr, eButType::SeprLine)) {
      return but;
    }
  }
  return nullptr;
...
bool UI_block_is_empty_ex(const uiBlock *block, const bool skip_title)
{
  ...
  if (skip_title) {
    ...
    if (but && but->type == eButType::Label) {
...
bool UI_block_can_add_separator(const uiBlock *block)
{
  if (ui_block_is_menu(block) && !ui_block_is_pie_menu(block)) {
    ...
    return (but && !ELEM(but->type, eButType::SeprLine, eButType::Sepr));
  }
  return true;
}
