...
static wmOperatorStatus view2d_ndof_invoke(bContext *C, wmOperator *op, const wmEvent *event)
{
  ...
  const bool has_translate = !is_zero_v2(WM_event_ndof_translation_get(ndof)) && view_pan_poll(C);
  const bool has_zoom = (WM_event_ndof_translation_get(ndof)[2] != 0.0f) && view_zoom_poll(C);
