...
bool ui_popup_context_menu_for_button(bContext *C, uiBut *but, const wmEvent *event)
{
  ...
  if (ELEM(but->type, eButType::Label, eButType::Image)) {
    return false;
  }
...

  {
    ...
    uiButViewItem *view_item_but = (but->type == eButType::ViewItem) ?
                                       static_cast<uiButViewItem *>(but) :
                                       static_cast<uiButViewItem *>(
                                           ui_view_item_find_mouse_over(region, event->xy));
    if (view_item_but) {
      BLI_assert(view_item_but->type == eButType::ViewItem);
  ...
  if (but->rnapoin.data && but->rnaprop) {
    ...
    if (((prop_type == PROP_POINTER) ||
         (prop_type == PROP_STRING && but->type == eButType::SearchMenu &&
