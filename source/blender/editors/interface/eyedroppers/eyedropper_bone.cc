...
static OperatorStatus bonedropper_modal(bContext *C, wmOperator *op, const wmEvent *event)
{
  BoneDropper *bdr = (BoneDropper *)op->customdata;
  if (!bdr) {
...
static OperatorStatus bonedropper_invoke(bContext *C, wmOperator *op, const wmEvent * /*event*/)
{
...
static OperatorStatus bonedropper_exec(bContext *C, wmOperator *op)
{
  if (bonedropper_init(C, op)) {
    bonedropper_exit(C, op);
