                                               ...
                                               VectorSet<Strip *> &time_dependent_strips)
{
   ...
   * If all inputs of any effect even indirectly(through another effect) points to selected
   * strip, its position will change. */
   ...
   * If any 2-input effect changes position because handles were moved, animation should be
   * offset. With single input effect, it is less likely desirable to move animation. */
...
static void create_trans_seq_clamp_data(TransInfo *t, const Scene *scene)
{
  ...
  /* TODO(john): This ensures that y-axis movement is restricted only if all of the selected
   * items are handles, since currently it is possible to select whole strips and handles at the
   * same time. This should be removed for 5.0 when we make this behavior impossible. */
...
static void flushTransSeq(TransInfo *t)
{
   ...
   *
   * Better solution would be to store effect position and calculate real offset. However with
   * many
