                                   ...
                                   void (*type_fn)(wmGizmoGroupType *))
{
  ...
  PointerRNA scene_ptr = RNA_id_pointer_create(&scene->id);
  {
    const PropertyRNA *props[] = {
        RNA_struct_type_find_property(RNA_Scene, "transform_orientation_slots"),
    };
  ...
  {
    const PropertyRNA *props[] = {
        RNA_struct_type_find_property(RNA_TransformOrientationSlot, "type"),
        RNA_struct_type_find_property(RNA_TransformOrientationSlot, "use"),
    };
    for (int i = 0; i < ARRAY_SIZE(props); i++) {
      if (props[i]) {
  ...
  if (ELEM(type_fn, VIEW3D_GGT_xform_gizmo, VIEW3D_GGT_xform_shear)) {
    const PropertyRNA *props[] = {
        RNA_struct_type_find_property(RNA_ToolSettings, "transform_pivot_point"),
    };
    for (int i = 0; i < ARRAY_SIZE(props); i++) {
      WM_msg_subscribe_rna(
  ...
  if (type_fn == VIEW3D_GGT_xform_gizmo) {
    ...
    if (ggd->use_twtype_refresh) {
      const PropertyRNA *props[] = {
          RNA_struct_type_find_property(RNA_SpaceView3D, "show_gizmo_object_translate"),
          RNA_struct_type_find_property(RNA_SpaceView3D, "show_gizmo_object_rotate"),
          RNA_struct_type_find_property(RNA_SpaceView3D, "show_gizmo_object_scale"),
      };
      for (int i = 0; i < ARRAY_SIZE(props); i++) {
