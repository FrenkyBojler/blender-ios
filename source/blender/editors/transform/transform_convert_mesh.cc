      ...
      if (next.i != -1) {
        ...
        BM_ITER_ELEM (l, &iter, curr.e, BM_LOOPS_OF_EDGE) {
          ...
          if (isect_curr_dirs) {
             ...
             * </pre>
             *
             * NOTE(@ideasman42): the diagram above is a represents the line A-B intersection
             * which may be used to define the point sliding the central vertex (`curr_orig`)
             * moves towards.
             *
             * While this often makes sense (intuitively) there are cases where the two lines
             * are close to parallel, the intersection point may be a distant location,
             * since there is no guarantee the intersection point is anywhere between the lines.
             *
             ...
             * so it's not necessary to attempt to use the intersection when it approaches
             * extreme values. On the other hand it's undesirable to be overly sensitive in
             * considering a location "invalid" since that may be viewed as erratic behavior
             * from a user perspective (selecting different points at random).
             *
             * Excluding the Intersection
             * ==========================
             *
             * There isn't one "correct" solution here, I think it's reasonable to consider
             * the triangle define by (`dst1`, `dst2` & `curr_orig`) to form a cone where
             * the tip of the cone is `curr_orig` and (`dst1` & `dst2`) are the sides.
