...
/* This file is a modified copy of transform_mode_edge_slide.cc, there might be duplicate and unused code
 * from the mesh edge to armature bone rewrite edits. */
#include <algorithm>
...
/* -------------------------------------------------------------------- */
/** \name Transform (Bone Slide)
 * \{ */

struct BoneSlideData {
  Array<TransDataBoneSlideVert> sv;
 ...
 public:
  ...
  void project(const TransDataBoneSlideVert *svert, float2 &r_sco_a, float2 &r_sco_b) const
  {
    float3 iloc = svert->loc_orig();
...
struct BoneSlideParams {
  wmOperator *op;
  float perc;

  /** When un-clamped - use this index: #TransDataBoneSlideVert.dir_side. */
 ...
 */
static TransDataContainer *bone_slide_container_first_ok(TransInfo *t)
{
  ...
  BLI_assert_msg(0, "Should never happen, at least one BoneSlideData should be valid");
  return nullptr;
}

static BoneSlideData *boneSlideFirstGet(TransInfo *t)
{
  TransDataContainer *tc = bone_slide_container_first_ok(t);
  return static_cast<BoneSlideData *>(tc->custom.mode.data);
}

static void calcBoneSlideCustomPoints(TransInfo *t)
{
  BoneSlideData *sld = boneSlideFirstGet(t);
...
/* Interpolates along a line made up of 2 segments (used for bone slide). */
...
static void bone_slide_data_init_mval(MouseInput *mi, BoneSlideData *sld, float *mval_dir)
{
  ...
  /* how would this translate to bones, maybe not necesary? could use for forks or chains. */
 ...
 * Unlike edge slide, bones have no per-vertex occlusion test — visibility is
 * determined by the transform convert layer which already omits hidden bones.
 */
static void calcBoneSlide_mval_range(TransInfo *t,
                                     BoneSlideData *sld,
                                     const int loop_nr,
                                     const float2 &mval,
                                     const bool use_calc_direction)
{
  /* Find mouse vectors, the global one, and one per chain group. */
  ...
  for (int i : sld->sv.index_range()) {
    TransDataBoneSlideVert *sv = &sld->sv[i];

    /* Screen-space coords of the two slide directions. */
    float2 sco_a, sco_b;
  ...
  if (use_calc_direction) {
    for (TransDataBoneSlideVert &sv : sld->sv) {
  ...
  bone_slide_data_init_mval(&t->mouse, sld, mval_dir);
}

/**
 * Build slide data for edit-armature bones.
 *
 * Each TransData entry in the container represents a selected bone endpoint (head or tail).
 * For each endpoint we compute two directional vectors:
 *   dir_side[0]: towards the "parent" side of the chain.
 *   dir_side[1]: towards the "child" side of the chain.
 *
 * Bone endpoints that have no neighbour on a given side get a zero vector for that side.
 * \param r_group_len: set to the number of distinct chain groups (always 1 for now).
 */
static Array<TransDataBoneSlideVert> transform_armature_bone_slide_data_create(
    const TransDataContainer *tc, int *r_group_len)
{
  const bArmature *arm = id_cast<bArmature *>(tc->obedit->data);

  /* Count entries that have a valid location pointer (some TransData entries use val instead). */
  int count = 0;
  for (int i = 0; i < tc->data_len; i++) {
    if (tc->data[i].loc != nullptr) {
      count++;
    }
  }
  ...
  Array<TransDataBoneSlideVert> result(count);
  int idx = 0;
  ...
  for (int i = 0; i < tc->data_len; i++) {
    ...
    const EditBone *ebo = static_cast<const EditBone *>(td->extra);
    if (ebo == nullptr) {
      /* When both BONE_TIPSEL and BONE_ROOTSEL are set on a bone, the tail TransData entry
       * has td->extra = nullptr (the root entry owns the ebo pointer for roll correction).
       * Recover the bone by matching td->loc to the tail pointer of each bone. */
      for (EditBone &candidate : *arm->edbo) {
        if (td->loc == candidate.tail) {
          ebo = &candidate;
          break;
        }
      }
    ...
    TransDataBoneSlideVert &sv = result[idx++];
    sv.td = td;
    sv.loop_nr = 0; /* All bones belong to a single slide group for now. */
    zero_v3(sv.dir_side[0]);
    zero_v3(sv.dir_side[1]);

    /* Determine which endpoint this TransData represents by comparing the location
     * pointer to the bone's head and tail arrays. */
    const bool is_head = (td->loc == ebo->head);

    if (is_head) {
      /* dir_side[1]: from head towards own tail (forward/child direction). */
      sub_v3_v3v3(sv.dir_side[1], ebo->tail, ebo->head);
      sv.bone_len = len_v3(sv.dir_side[1]);

      /* dir_side[0]: from head towards parent's head (backward/parent direction). */
      if (ebo->parent && (ebo->flag & BONE_CONNECTED)) {
        sub_v3_v3v3(sv.dir_side[0], ebo->parent->head, ebo->head);
      }
    }
    else {
      /* Tail endpoint. dir_side[0]: from tail towards own head. */
      sub_v3_v3v3(sv.dir_side[0], ebo->head, ebo->tail);
      sv.bone_len = len_v3(sv.dir_side[0]);

      /* dir_side[1]: from tail towards the first connected child's tail. */
      for (EditBone &child : *arm->edbo) {
        if (child.parent == ebo && (child.flag & BONE_CONNECTED)) {
          sub_v3_v3v3(sv.dir_side[1], child.tail, ebo->tail);
          break;
        }
      }
    }
  ...
  /* Trim the array to the actual number of valid entries filled. */
  result.reinitialize(idx);

  *r_group_len = 1;
  return result;
}

static BoneSlideData *createBoneSlideVerts(TransInfo *t,
                                           TransDataContainer *tc,
                                           const bool use_double_side)
{
  int group_len;
  BoneSlideData *sld = MEM_new<BoneSlideData>("sld");

  sld->sv = transform_armature_bone_slide_data_create(tc, &group_len);
  ...
  if (!use_double_side) {
    ...
    for (TransDataBoneSlideVert &sv : sld->sv) {
      ...
      sv.bone_len = math::length(sv.dir_side[0]);
    }
  }
  ...
  calcBoneSlide_mval_range(t, sld, group_len, t->mval, use_double_side);

  return sld;
}

static void freeBoneSlideVerts(TransInfo * /*t*/,
                               TransDataContainer * /*tc*/,
                               TransCustomData *custom_data)
{
  BoneSlideData *sld = static_cast<BoneSlideData *>(custom_data->data);
...
static eRedrawFlag handleEventBoneSlide(TransInfo *t, const wmEvent *event)
{
  BoneSlideParams *slp = static_cast<BoneSlideParams *>(t->custom.mode.data);
...
static void drawBoneSlide(TransInfo *t)
{
  BoneSlideData *sld = boneSlideFirstGet(t);
  if (sld == nullptr) {
    return;
  ...
  const BoneSlideParams *slp = static_cast<const BoneSlideParams *>(t->custom.mode.data);
  ...
  {
    int num_path_verts = 0;
    for (const TransDataBoneSlideVert &sv : sld->sv) {
      if (!math::is_zero(sv.dir_side[0])) {
        num_path_verts += 2;
      }
      if (!math::is_zero(sv.dir_side[1])) {
        num_path_verts += 2;
      }
    }
    if (num_path_verts > 0) {
      GPU_line_width(line_size);
      immUniformThemeColorShadeAlpha(TH_EDGE_SELECT, 50, -128);
      immBegin(GPU_PRIM_LINES, num_path_verts);
      for (const TransDataBoneSlideVert &sv : sld->sv) {
        const float3 sv_co = float3(sv.loc_orig());
        if (!math::is_zero(sv.dir_side[0])) {
          float3 co_a = sv_co + sv.dir_side[0];
          immVertex3fv(pos, co_a);
          immVertex3fv(pos, sv_co);
        }
        if (!math::is_zero(sv.dir_side[1])) {
          float3 co_b = sv_co + sv.dir_side[1];
          immVertex3fv(pos, sv_co);
    ...
    immBindBuiltinProgram(GPU_SHADER_3D_POINT_UNIFORM_SIZE_UNIFORM_COLOR_AA);
      ...
      immUniformThemeColorShadeAlpha(TH_VERTEX_SELECT, 50, -128);
      immBegin(GPU_PRIM_POINTS, sld->sv.size());
      for (const TransDataBoneSlideVert &sv : sld->sv) {
        /* sv.loc_orig() directly holds the coordinate for the specific head or tail */
        immVertex3fv(pos, sv.loc_orig());
      }
  ...
  TransDataBoneSlideVert *curr_sv = &sld->sv[sld->curr_sv_index];
  const float3 curr_sv_co_orig = curr_sv->loc_orig();
  ...
  if (slp->use_even == true) {
    ...
    for (TransDataBoneSlideVert &sv : sld->sv) {
      ...
      const float3 sv_co_orig = sv.loc_orig();
    ...
    GPU_line_width(line_size);
    immUniformThemeColorShadeAlpha(TH_EDGE_SELECT, 80, alpha_shade);
    immBeginAtMost(GPU_PRIM_LINES, sld->sv.size() * 2);
    
    for (const TransDataBoneSlideVert &sv : sld->sv) {
      float co_dir[3];
      const float3 sv_co_orig = sv.loc_orig();
      
      add_v3_v3v3(co_dir, sv_co_orig, sv.dir_side[slp->curr_side_unclamp]);
      
      immVertex3fv(pos, sv_co_orig);
...
static void bone_slide_snap_apply(TransInfo *t, float *value)
{
  TransDataContainer *tc = bone_slide_container_first_ok(t);
  BoneSlideParams *slp = static_cast<BoneSlideParams *>(t->custom.mode.data);
  BoneSlideData *sld_active = static_cast<BoneSlideData *>(tc->custom.mode.data);
  TransDataBoneSlideVert *sv = &sld_active->sv[sld_active->curr_sv_index];
  ...
  if (slp->use_even == false) {
    ...
    if (is_clamp) {
      ...
      /* Use the side indicated in `BoneSlideParams::curr_side_unclamp` as long as that side is not
       * zero length. */
...
static void bone_slide_apply_elem(const TransDataBoneSlideVert &sv,
                                  const float fac,
                                  ...
                                  float r_co[3])
{
  ...
  if (use_even == false) {
     ...
     */
    if (sv.bone_len > FLT_EPSILON) {
      float co_a[3], co_b[3];
      const float fac_final = min_ff(sv.bone_len, curr_length_fac) / sv.bone_len;
...
static void doBoneSlide(TransInfo *t, float perc)
{
  BoneSlideParams *slp = static_cast<BoneSlideParams *>(t->custom.mode.data);
  BoneSlideData *sld_active = boneSlideFirstGet(t);
  ...
  if (use_even) {
    TransDataBoneSlideVert *sv_active = &sld_active->sv[sld_active->curr_sv_index];
    curr_length_fac = sv_active->bone_len * (((use_flip ? perc : -perc) + 1.0f) / 2.0f);
  }
  else if (use_clamp) {
  ...
  FOREACH_TRANS_DATA_CONTAINER (t, tc) {
    BoneSlideData *sld = static_cast<BoneSlideData *>(tc->custom.mode.data);
...
static void applyBoneSlide(TransInfo *t)
{
  ...
  /* Header string. */
  ofs += BLI_strncpy_utf8_rlen(str + ofs, RPT_("Bone Slide: "), sizeof(str) - ofs);
...
static void bone_slide_transform_matrix_fn(TransInfo *t, float mat_xform[4][4])
{
  ...
  copy_v3_v3(orig_co, sv_active.loc_orig());

  const float fac = t->values_final[0];
  float curr_length_fac = 0.0f;
  if (slp->use_even) {
    curr_length_fac = sv_active.bone_len * (((slp->flipped ? fac : -fac) + 1.0f) / 2.0f);
  }

  bone_slide_apply_elem(sv_active,
                        fac,
                        curr_length_fac,
...
static void initBoneSlide_ex(TransInfo *t,
                             wmOperator *op,
                             ...
                             bool use_clamp)
{
  ...
  t->mode = TFM_BONE_SLIDE;

  {
    BoneSlideParams *slp = MEM_new_zeroed<BoneSlideParams>(__func__);
    slp->op = op;
  ...
  FOREACH_TRANS_DATA_CONTAINER (t, tc) {
    sld = createBoneSlideVerts(t, tc, use_double_side);
    if (sld) {
      tc->custom.mode.data = sld;
      tc->custom.mode.free_cb = freeBoneSlideVerts;
      ok = true;
    }
  }
...
static void initBoneSlide(TransInfo *t, wmOperator *op)
{
  ...
  if (op) {
     ...
     * then "G" again to go back to bone slide. */
  ...
  initBoneSlide_ex(t, op, use_double_side, use_even, flipped, use_clamp);
}
...
TransModeInfo TransMode_boneslide = {
    /*flags*/ T_NO_CONSTRAINT,
    /*init_fn*/ initBoneSlide,
    /*transform_fn*/ applyBoneSlide,
    /*transform_matrix_fn*/ bone_slide_transform_matrix_fn,
    /*handle_event_fn*/ handleEventBoneSlide,
    /*snap_distance_fn*/ transform_snap_distance_len_squared_fn,
    /*snap_apply_fn*/ bone_slide_snap_apply,
    /*draw_fn*/ drawBoneSlide,
};
