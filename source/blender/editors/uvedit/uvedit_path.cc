...
static OperatorStatus uv_shortest_path_pick_exec(bContext *C, wmOperator *op);
...
static OperatorStatus uv_shortest_path_pick_invoke(bContext *C,
                                                   wmOperator *op,
                                                   const wmEvent *event)
{
...
static OperatorStatus uv_shortest_path_select_exec(bContext *C, wmOperator *op)
{
  Depsgraph *depsgraph = CTX_data_ensure_evaluated_depsgraph(C);
  Scene *scene = CTX_data_scene(C);
  const char uv_selectmode = ED_uvedit_select_mode_get(scene);
