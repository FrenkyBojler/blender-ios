...
static bool uv_mesh_hide_no_sync(const Scene *scene, Object *ob, BMEditMesh *em, bool swap)
{
   ...
   * ===============
   *
   * Multi-pass hiding solves the following problems, in vertex/edge mesh selection modes
   * too many UV's are hidden (hiding in this case de-selects).
   * It's not needed for face-only selection.
   *
   * The most common example of this is when the user hides a selected island,
   * it hides faces from the boundaries of islands are hidden. See #153464.
   *
   ...
   *
   * Consider:
   * - "A" is a select island to be hidden.
   ...
   * - First it hides vertices not connected to any selected faces
   *   (which are visible in the UV editor)
   * - The second pass uses the simple hide-all-connected-vertices.
   *
   * This works because the second pass only de-select vertices still connected to visible faces,
