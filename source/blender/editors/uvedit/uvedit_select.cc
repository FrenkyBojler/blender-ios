...
static OperatorStatus uv_select_more_less(bContext *C, const bool select)
{
...
static OperatorStatus uv_select_more_exec(bContext *C, wmOperator * /*op*/)
{
...
static OperatorStatus uv_select_less_exec(bContext *C, wmOperator * /*op*/)
{
...
static OperatorStatus uv_select_all_exec(bContext *C, wmOperator *op)
{
...
static OperatorStatus uv_select_exec(bContext *C, wmOperator *op)
{
  float co[2];
...
static OperatorStatus uv_select_invoke(bContext *C, wmOperator *op, const wmEvent *event)
{
  const ARegion *region = CTX_wm_region(C);
  float co[2];
  ...
  const OperatorStatus retval = uv_select_exec(C, op);
...
static OperatorStatus uv_mouse_select_loop_generic_multi(bContext *C,
                                                         const Span<Object *> objects,
                                                         const float co[2],
                                                         const bool extend,
...
static OperatorStatus uv_mouse_select_loop_generic(bContext *C,
                                                   const float co[2],
                                                   const bool extend,
                                                   enum eUVLoopGenericType loop_type)
{
  ...
  Vector<Object *> objects = BKE_view_layer_array_from_objects_in_edit_mode_unique_data_with_uvs(
      scene, view_layer, nullptr);
  OperatorStatus ret = uv_mouse_select_loop_generic_multi(C, objects, co, extend, loop_type);
  return ret;
}
...
static OperatorStatus uv_select_loop_exec(bContext *C, wmOperator *op)
{
  float co[2];
...
static OperatorStatus uv_select_loop_invoke(bContext *C, wmOperator *op, const wmEvent *event)
{
  const ARegion *region = CTX_wm_region(C);
  float co[2];
  ...
  const OperatorStatus retval = uv_select_loop_exec(C, op);
...
static OperatorStatus uv_select_edge_ring_exec(bContext *C, wmOperator *op)
{
  float co[2];
  RNA_float_get_array(op->ptr, "location", co);
  const bool extend = RNA_boolean_get(op->ptr, "extend");
...
static OperatorStatus uv_select_edge_ring_invoke(bContext *C, wmOperator *op, const wmEvent *event)
{
  const ARegion *region = CTX_wm_region(C);
  float co[2];
  ...
  const OperatorStatus retval = uv_select_edge_ring_exec(C, op);
...
static OperatorStatus uv_select_linked_internal(bContext *C,
                                                wmOperator *op,
                                                const wmEvent *event,
                                                bool pick)
{
...
static OperatorStatus uv_select_linked_exec(bContext *C, wmOperator *op)
{
...
static OperatorStatus uv_select_linked_pick_invoke(bContext *C,
                                                   wmOperator *op,
                                                   const wmEvent *event)
{
...
static OperatorStatus uv_select_linked_pick_exec(bContext *C, wmOperator *op)
{
 ...
 */
static OperatorStatus uv_select_split_exec(bContext *C, wmOperator *op)
{
  Depsgraph *depsgraph = CTX_data_ensure_evaluated_depsgraph(C);
  Scene *scene = CTX_data_scene(C);
  ViewLayer *view_layer = CTX_data_view_layer(C);
...
static OperatorStatus uv_box_select_exec(bContext *C, wmOperator *op)
{
  Depsgraph *depsgraph = CTX_data_ensure_evaluated_depsgraph(C);
  Scene *scene = CTX_data_scene(C);
  const ToolSettings *ts = scene->toolsettings;
...
static OperatorStatus uv_circle_select_exec(bContext *C, wmOperator *op)
{
  Depsgraph *depsgraph = CTX_data_ensure_evaluated_depsgraph(C);
...
static OperatorStatus uv_lasso_select_exec(bContext *C, wmOperator *op)
{
...
static OperatorStatus uv_select_pinned_exec(bContext *C, wmOperator *op)
{
  Scene *scene = CTX_data_scene(C);
  const ToolSettings *ts = scene->toolsettings;
  ...
  for (Object *obedit : objects) {
    ...
    const char *active_uv_name = CustomData_get_active_layer_name(&em->bm->ldata, CD_PROP_FLOAT2);
    if (!BM_uv_map_attr_pin_exists(em->bm, active_uv_name)) {
      continue;
    }
...
static OperatorStatus uv_select_overlap(bContext *C, const bool extend)
{
  Depsgraph *depsgraph = CTX_data_ensure_evaluated_depsgraph(C);
  Scene *scene = CTX_data_scene(C);
  ViewLayer *view_layer = CTX_data_view_layer(C);
  ...
  if (overlap != nullptr) {
    ...
    for (int i = 0; i < tree_overlap_len; i++) {
      ...
      if (scene->toolsettings->uv_flag & UV_SYNC_SELECTION) {
        ...
        BM_uv_map_attr_vert_select_ensure(em_a->bm, uv_a_name);
        BM_uv_map_attr_vert_select_ensure(em_b->bm, uv_b_name);
        BM_uv_map_attr_edge_select_ensure(em_a->bm, uv_a_name);
        BM_uv_map_attr_edge_select_ensure(em_b->bm, uv_b_name);
      }
      const BMUVOffsets offsets_a = BM_uv_map_offsets_get(em_a->bm);
      const BMUVOffsets offsets_b = BM_uv_map_offsets_get(em_b->bm);
...
static OperatorStatus uv_select_overlap_exec(bContext *C, wmOperator *op)
{
...
static OperatorStatus uv_select_similar_vert_exec(bContext *C, wmOperator *op)
{
  Scene *scene = CTX_data_scene(C);
  ViewLayer *view_layer = CTX_data_view_layer(C);
  Depsgraph *depsgraph = CTX_data_ensure_evaluated_depsgraph(C);
...
static OperatorStatus uv_select_similar_edge_exec(bContext *C, wmOperator *op)
{
  Scene *scene = CTX_data_scene(C);
  ViewLayer *view_layer = CTX_data_view_layer(C);
  Depsgraph *depsgraph = CTX_data_ensure_evaluated_depsgraph(C);
...
static OperatorStatus uv_select_similar_face_exec(bContext *C, wmOperator *op)
{
  Scene *scene = CTX_data_scene(C);
  ViewLayer *view_layer = CTX_data_view_layer(C);
  Depsgraph *depsgraph = CTX_data_ensure_evaluated_depsgraph(C);
...
static OperatorStatus uv_select_similar_island_exec(bContext *C, wmOperator *op)
{
  Scene *scene = CTX_data_scene(C);
  ViewLayer *view_layer = CTX_data_view_layer(C);
  Depsgraph *depsgraph = CTX_data_ensure_evaluated_depsgraph(C);
...
static OperatorStatus uv_select_similar_exec(bContext *C, wmOperator *op)
{
  ToolSettings *ts = CTX_data_tool_settings(C);
...
static OperatorStatus uv_select_mode_exec(bContext *C, wmOperator *op)
{
  Scene *scene = CTX_data_scene(C);
...
static OperatorStatus uv_select_mode_invoke(bContext *C, wmOperator *op, const wmEvent * /*event*/)
{
  const ToolSettings *ts = CTX_data_tool_settings(C);
