...
{
  BLI_assert(ts->uv_flag & UV_FLAG_SYNC_SELECT);
...
static wmOperatorStatus uv_select_more_less(bContext *C, const bool select)
{
  ...
  const bool is_uv_face_selectmode = (ts->uv_flag & UV_FLAG_SYNC_SELECT) ?
                                         (ts->selectmode == SCE_SELECT_FACE) :
                                         (ts->uv_selectmode == UV_SELECT_FACE);
...
static void uv_select_flush_from_tag_loop(const Scene *scene, Object *obedit, const bool select)
{
  ...
  if ((use_mesh_select == false) && ts->uv_sticky == SI_STICKY_VERTEX) {
  ...
  else if ((use_mesh_select == false) && (ts->uv_sticky == SI_STICKY_LOC)) {
    BM_ITER_MESH (efa, &iter, bm, BM_FACES_OF_MESH) {
      BM_ITER_ELEM (l, &liter, efa, BM_LOOPS_OF_FACE) {
        if (BM_elem_flag_test(l, BM_ELEM_TAG)) {
          uvedit_uv_select_flush_from_tag_sticky_loc_internal(scene, bm, l, select, offsets);
...
static wmOperatorStatus uv_box_select_exec(bContext *C, wmOperator *op)
{
  ...
  const bool use_select_linked = !(ts->uv_flag & UV_FLAG_SYNC_SELECT) &&
...
static wmOperatorStatus uv_select_similar_exec(bContext *C, wmOperator *op)
{
  ...
  int selectmode = (ts->uv_flag & UV_FLAG_SYNC_SELECT) ? ts->selectmode : ts->uv_selectmode;
...
{
  BLI_assert((scene->toolsettings->uv_flag & UV_FLAG_SYNC_SELECT) == 0);
  BMFace *efa;
  BMIter iter, liter;
