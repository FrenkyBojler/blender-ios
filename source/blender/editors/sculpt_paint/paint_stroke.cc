  ...
  *r_location_is_set = false;
  ...
  if (do_random) {
    if (brush.mtex_accessor.color.brush_angle_mode & MTEX_ANGLE_RANDOM) {
      paint_runtime.brush_rotation += -brush.mtex_accessor.color.random_angle / 2.0f +
                                      brush.mtex_accessor.color.random_angle *
                                          stroke->rng->get_float();
    }
  }

  if (do_random_mask) {
    if (brush.mtex_accessor.mask.brush_angle_mode & MTEX_ANGLE_RANDOM) {
      paint_runtime.brush_rotation_sec += -brush.mtex_accessor.mask.random_angle / 2.0f +
                                          brush.mtex_accessor.mask.random_angle *
                                              stroke->rng->get_float();
    }
  }
                              ...
                              const int event_type)
{
  ...
  if (br->mtex_accessor.color.tex && br->mtex_accessor.color.tex->type == TEX_IMAGE &&
      br->mtex_accessor.color.tex->ima)
  {
