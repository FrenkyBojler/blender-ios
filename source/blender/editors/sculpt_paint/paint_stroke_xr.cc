...
static bool paint_brush_update(bContext *C,
                               ...
                               PaintMode mode,
                               PaintStroke *stroke,
                               const float mouse_init[3], // TODO rename as controller_init ?
                               float mouse[3], // TODO rename as controller ? controller means any I/O device using up to three axes
                               float pressure,
                               float r_location[3],
                               bool *r_location_is_set) // is_xr
{
                              ...
                              int event_type)
{
  ...
  stroke->done = done;
  stroke->event_type = event_type; /* for modal, return event  We can know if we are in XR here EVT_XR_ACTION*/
  stroke->ups = ups;
