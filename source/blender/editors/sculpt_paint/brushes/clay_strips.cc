                          ...
                          const IndexMask &node_mask)
{
  ...
  const float plane_offset = brush_plane_offset_get(brush, ss);
  const float displace = radius * (0.18f + plane_offset);
  ...
  const float3 offset = area_normal * ss.cache->bstrength * ss.cache->radius;
