...
static wmOperatorStatus mask_from_cavity_exec(bContext *C, wmOperator *op)
{
  ...
  switch (src) {
    case MaskSettingsSource::Operator:
      if (RNA_boolean_get(op->ptr, "invert")) {
        scene_copy.automasking_settings.flags = BRUSH_AUTOMASKING_CAVITY_INVERTED;
      }
      else {
        scene_copy.automasking_settings.flags = BRUSH_AUTOMASKING_CAVITY_NORMAL;
      }

      if (RNA_boolean_get(op->ptr, "use_curve")) {
        scene_copy.automasking_settings.flags |= BRUSH_AUTOMASKING_CAVITY_USE_CURVE;
      }

      scene_copy.automasking_settings.cavity_blur_steps = RNA_int_get(op->ptr, "blur_steps");
      scene_copy.automasking_settings.cavity_factor = RNA_float_get(op->ptr, "factor");

      scene_copy.automasking_settings.cavity_curve = paint.automasking_settings.cavity_curve_op;
      break;
    case MaskSettingsSource::Brush:
      if (brush) {
        scene_copy.automasking_settings.flags = brush->automasking_flags;
        scene_copy.automasking_settings.cavity_factor = brush->automasking_cavity_factor;
        scene_copy.automasking_settings.cavity_curve = brush->automasking_cavity_curve;
        scene_copy.automasking_settings.cavity_blur_steps = brush->automasking_cavity_blur_steps;

        /* Ensure only cavity masking is enabled. */
        scene_copy.automasking_settings.flags &= BRUSH_AUTOMASKING_CAVITY_ALL |
                                                 BRUSH_AUTOMASKING_CAVITY_USE_CURVE;
      }
      else {
        scene_copy.automasking_settings.flags = 0;
  ...
  if (!(scene_copy.automasking_settings.flags & BRUSH_AUTOMASKING_CAVITY_ALL)) {
    scene_copy.automasking_settings.flags |= BRUSH_AUTOMASKING_CAVITY_NORMAL;
  }
  ...
  brush_copy.automasking_cavity_curve = scene_copy.automasking_settings.cavity_curve;
...
static wmOperatorStatus mask_from_boundary_exec(bContext *C, wmOperator *op)
{
  ...
  switch (src) {
    case MaskSettingsSource::Operator: {
      ...
      switch (boundary_mode) {
        case MaskBoundaryMode::Mesh:
          scene_copy.automasking_settings.flags = BRUSH_AUTOMASKING_BOUNDARY_EDGES;
          break;
        case MaskBoundaryMode::FaceSets:
          scene_copy.automasking_settings.flags = BRUSH_AUTOMASKING_BOUNDARY_FACE_SETS;
          break;
      }
      scene_copy.automasking_settings.boundary_edges_propagation_steps = RNA_int_get(
    ...
    case MaskSettingsSource::Brush:
      if (brush) {
        scene_copy.automasking_settings.flags = brush->automasking_flags;
        scene_copy.automasking_settings.boundary_edges_propagation_steps =
