...
{
  int automasking = paint.automasking_settings.flags;
...
{
  if (brush) {
    int flags = paint.automasking_settings.flags | brush->automasking_flags;
    ...
    else if (paint.automasking_settings.flags & BRUSH_AUTOMASKING_CAVITY_ALL) {
      flags &= ~(BRUSH_AUTOMASKING_CAVITY_ALL | BRUSH_AUTOMASKING_CAVITY_USE_CURVE |
                 BRUSH_AUTOMASKING_CAVITY_NORMAL);
      flags |= paint.automasking_settings.flags;
    }

    return flags;
  }
  return paint.automasking_settings.flags;
}
...
static int boundary_propagation_steps(const Paint &paint, const Brush *brush)
{
  return brush && brush->automasking_flags &
             ...
             paint.automasking_settings.boundary_edges_propagation_steps;
}
                                  ...
                                  const Brush *brush)
{
  ...
  if (brush && (brush->automasking_flags & BRUSH_AUTOMASKING_VIEW_NORMAL)) {
    ...
    automasking.settings.view_normal_limit = paint.automasking_settings.view_normal_limit;
    automasking.settings.view_normal_falloff = paint.automasking_settings.view_normal_falloff;
  }
  ...
  if (brush && (brush->automasking_flags & BRUSH_AUTOMASKING_BRUSH_NORMAL)) {
    ...
    automasking.settings.start_normal_limit = paint.automasking_settings.start_normal_limit;
    automasking.settings.start_normal_falloff = paint.automasking_settings.start_normal_falloff;
  }
  ...
  if (brush && (brush->automasking_flags & BRUSH_AUTOMASKING_CAVITY_ALL)) {
    ...
    automasking.settings.cavity_curve = paint.automasking_settings.cavity_curve;
    automasking.settings.cavity_factor = paint.automasking_settings.cavity_factor;
    automasking.settings.cavity_blur_steps = paint.automasking_settings.cavity_blur_steps;
  }
}
                                  ...
                                  Object &ob)
{
  ...
  if (mode & BRUSH_AUTOMASKING_CAVITY_ALL) {
    if (mode_enabled(paint, brush, BRUSH_AUTOMASKING_CAVITY_USE_CURVE)) {
      ...
      BKE_curvemapping_init(paint.automasking_settings.cavity_curve);
    }
