    ...
    BrushPainter *painter, ImagePaintTile *tile, const int size, float pressure, float distance)
{
  ...
  ImBuf *ibuf = IMB_allocImBuf(
      size, size, ImColorMode::RGBA, (is_float) ? IB_float_data : IB_byte_data);
                                               ...
                                               const int diameter)
{
  ...
  if (!cache->ibuf) {
    cache->ibuf = IMB_allocImBuf(diameter, diameter, ImColorMode::RGBA, imbflag);
  }
  ibuf = cache->ibuf;

  /* create new texture image buffer with coordinates relative to old */
  oldtexibuf = cache->texibuf;
  cache->texibuf = IMB_allocImBuf(diameter, diameter, ImColorMode::RGBA, imbflag);
...
static ImBuf *paint_2d_lift_clone(ImBuf *ibuf, ImBuf *ibufb, const int *pos)
{
  ...
  ImBuf *clonebuf = IMB_allocImBuf(w, h, ibufb->color_mode, ibufb->flags);
...
{
  ImBuf tmpbuf;
  IMB_initImBuf(&tmpbuf, ED_IMAGE_UNDO_TILE_SIZE, ED_IMAGE_UNDO_TILE_SIZE, ImColorMode::RGBA, 0);
