...
static void cloth_brush_collision_cb(void *userdata,
                                     ...
                                     BVHTreeRayHit *hit)
{
  ClothBrushCollision *col = static_cast<ClothBrushCollision *>(userdata);
      ...
      node_mask.foreach_index(
          [&](const int i) {
            ...
            auto_mask::calc_grids_factors(
                depsgraph, object, automasking, nodes[i], grids, factors);
            if (ss.cache) {
  ...
  switch (pbvh.type()) {
    ...
    case bke::pbvh::Type::Grids: {
      ...
      node_mask.foreach_index(
          [&](const int i) {
            FilterLocalData &tls = all_tls.local();
            apply_filter_forces_grids(*depsgraph,
                                      face_sets,
                                      filter_type,
                                      filter_strength,
                                      gravity,
