                            ...
                            const int img_y)
{
  ...
  const int tile_x0 = img_x0 >> ED_IMAGE_UNDO_TILE_BITS;
  const int tile_x1 = img_x1 >> ED_IMAGE_UNDO_TILE_BITS;
...
static void mark_seam_tiles_modified(MutableSpan<uint8_t> mask,
                                     ...
                                     const int py)
{
  const int tile_y = py >> SEAM_TILE_BITS;
  const int tile_x0 = px0 >> SEAM_TILE_BITS;
  const int tile_x1 = px1 >> SEAM_TILE_BITS;
  const int tile_y_min = (tile_y > 0) ? tile_y - 1 : 0;
  const int tile_y_max = (tile_y + 1 < tiles_y) ? tile_y + 1 : tiles_y - 1;
  const int tile_x_min = (tile_x0 > 0) ? tile_x0 - 1 : 0;
  const int tile_x_max = (tile_x1 + 1 < tiles_x) ? tile_x1 + 1 : tiles_x - 1;
  ...
  for (image::TileNumber tile_number : tile_numbers_to_fix) {
    ...
    bke::pbvh::pixels::copy_pixels(
        ...
        [&](const int x0, const int x1, const int y) {
          push_undo_tiles(image_data, tile_number, *image_buffer, undo_tile_pushed, x0, x1, y);
        });
  }
}
