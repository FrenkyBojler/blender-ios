...
{
  /* #ExecuteSerial because this is called from tasks with TLS that don't use isolation. */
  array_utils::gather(src, indices, dst, ExecuteSerial());
}
template<typename T>
...
{
  /* #ExecuteSerial because this is called from tasks with TLS that don't use isolation. */
  array_utils::scatter(src, indices, dst, ExecuteSerial());
}
template<typename T>
