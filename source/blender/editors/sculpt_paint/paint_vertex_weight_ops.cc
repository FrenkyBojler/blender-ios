...
static OperatorStatus weight_from_bones_exec(bContext *C, wmOperator *op)
{
 ...
 */
static OperatorStatus weight_sample_invoke(bContext *C, wmOperator *op, const wmEvent *event)
{
  Depsgraph *depsgraph = CTX_data_ensure_evaluated_depsgraph(C);
  Mesh *mesh;
  bool changed = false;
...
static OperatorStatus weight_sample_group_invoke(bContext *C, wmOperator *op, const wmEvent *event)
{
...
static OperatorStatus weight_paint_set_exec(bContext *C, wmOperator *op)
{
  Scene *scene = CTX_data_scene(C);
...
static OperatorStatus paint_weight_gradient_modal(bContext *C,
                                                  wmOperator *op,
                                                  const wmEvent *event)
{
...
static OperatorStatus paint_weight_gradient_exec(bContext *C, wmOperator *op)
{
  using namespace blender;
  wmGesture *gesture = static_cast<wmGesture *>(op->customdata);
  WPGradient_vertStoreBase *vert_cache;
...
static OperatorStatus paint_weight_gradient_invoke(bContext *C,
                                                   wmOperator *op,
                                                   const wmEvent *event)
{
  OperatorStatus ret;
