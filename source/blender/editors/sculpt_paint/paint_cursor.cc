...
static int load_tex(Brush *br, ViewContext *vc, float zoom, bool col, bool primary)
{
  ...
  if (refresh) {
    ...
    const float radius = BKE_brush_size_get(vc->scene, br, paint) * zoom;

    make_tex_snap(target, vc, zoom);

    if (mtex->brush_map_mode == MTEX_MAP_MODE_VIEW) {
      int s = BKE_brush_size_get(vc->scene, br, paint);
      int r = 1;
...
static int load_tex_cursor(Brush *br, ViewContext *vc, float zoom)
{
  ...
  if (refresh) {
    ...
    s = BKE_brush_size_get(vc->scene, br, paint);
    r = 1;
  ...
  if (load_tex(brush, vc, zoom, col, primary)) {
    ...
    if (mtex->brush_map_mode == MTEX_MAP_MODE_VIEW) {
      ...
      if (ups->draw_anchored) {
        ...
        const int radius = BKE_brush_size_get(vc->scene, brush, &paint) * zoom;
                                                   ...
                                                   const float location[3])
{
  ...
  if (!BKE_brush_use_locked_size(vc.scene, &brush, &paint)) {
    ...
    if (ups.draw_anchored) {
      ...
      if (brush.flag & BRUSH_ANCHORED) {
        ...
        projected_radius = BKE_brush_size_get(vc.scene, &brush, &paint);
      }
    }
    ...
    BKE_brush_unprojected_radius_set(vc.scene, &brush, unprojected_radius, &paint);
  }
}
                                      ...
                                      PaintCursorContext &pcontext)
{
  ...
  pcontext.final_radius = (BKE_brush_size_get(pcontext.scene, pcontext.brush, pcontext.paint) * zoomx);
...
static void paint_cursor_update_pixel_radius(PaintCursorContext &pcontext)
{
  if (pcontext.is_cursor_over_mesh) {
    ...
    pcontext.pixel_radius = project_brush_radius(
        &pcontext.vc, BKE_brush_unprojected_radius_get(pcontext.scene, brush, pcontext.paint), pcontext.location);

    if (pcontext.pixel_radius == 0) {
      pcontext.pixel_radius = BKE_brush_size_get(pcontext.scene, brush, pcontext.paint);
    }
...
static void paint_cursor_sculpt_session_update_and_init(PaintCursorContext &pcontext)
{
  ...
  if (BKE_brush_use_locked_size(&scene, &brush, pcontext.paint)) {
    BKE_brush_size_set(&scene, &brush, pcontext.pixel_radius, pcontext.paint);
  }
...
static void grease_pencil_brush_cursor_draw(PaintCursorContext &pcontext)
{
  ...
  if (pcontext.mode == PaintMode::GPencil) {
    ...
    pcontext.pixel_radius = BKE_brush_size_get(pcontext.vc.scene, brush, pcontext.paint);
...
static void paint_draw_3D_view_inactive_brush_cursor(PaintCursorContext &pcontext)
{
  ...
  imm_draw_circle_wire_3d(
      ...
      pcontext.final_radius *
          clamp_f(BKE_brush_alpha_get(pcontext.scene, pcontext.brush, pcontext.paint), 0.0f, 1.0f),
      80);
}
...
static void paint_cursor_draw_main_inactive_cursor(PaintCursorContext &pcontext)
{
  ...
  imm_draw_circle_wire_3d(
      ...
      0,
      0,
      pcontext.radius * clamp_f(BKE_brush_alpha_get(pcontext.scene, pcontext.brush, pcontext.paint), 0.0f, 1.0f),
      80);
}
