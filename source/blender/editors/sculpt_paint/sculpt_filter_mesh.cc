...
static OperatorStatus sculpt_mesh_filter_confirm(SculptSession &ss,
                                                 wmOperator *op,
...
static OperatorStatus sculpt_mesh_filter_modal(bContext *C, wmOperator *op, const wmEvent *event)
{
  Object &ob = *CTX_data_active_object(C);
  ...
  if (event->type == EVT_MODAL_MAP) {
    OperatorStatus ret = OPERATOR_FINISHED;
    switch (event->val) {
...
static OperatorStatus sculpt_mesh_filter_start(bContext *C, wmOperator *op)
{
...
static OperatorStatus sculpt_mesh_filter_invoke(bContext *C, wmOperator *op, const wmEvent *event)
{
  RNA_int_set_array(op->ptr, "start_mouse", event->mval);
  OperatorStatus ret = sculpt_mesh_filter_start(C, op);
...
static OperatorStatus sculpt_mesh_filter_exec(bContext *C, wmOperator *op)
{
  OperatorStatus ret = sculpt_mesh_filter_start(C, op);
