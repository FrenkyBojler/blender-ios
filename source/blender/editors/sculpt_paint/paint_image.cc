...
static OperatorStatus grab_clone_exec(bContext *C, wmOperator *op)
{
...
static OperatorStatus grab_clone_invoke(bContext *C, wmOperator *op, const wmEvent *event)
{
  const Scene *scene = CTX_data_scene(C);
  const ToolSettings *settings = scene->toolsettings;
  const ImagePaintSettings &image_paint_settings = settings->imapaint;
...
static OperatorStatus grab_clone_modal(bContext *C, wmOperator *op, const wmEvent *event)
{
  const Scene *scene = CTX_data_scene(C);
  ToolSettings *settings = scene->toolsettings;
  ImagePaintSettings &image_paint_settings = settings->imapaint;
...
static OperatorStatus sample_color_exec(bContext *C, wmOperator *op)
{
...
static OperatorStatus sample_color_modal(bContext *C, wmOperator *op, const wmEvent *event)
{
  Scene *scene = CTX_data_scene(C);
...
static OperatorStatus texture_paint_toggle_exec(bContext *C, wmOperator *op)
{
...
static OperatorStatus brush_colors_flip_exec(bContext *C, wmOperator * /*op*/)
{
  Scene &scene = *CTX_data_scene(C);
