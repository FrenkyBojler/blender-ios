...
static void stencil_set_target(StencilControlData *scd)
{
  ...
  if (scd->mask) {
    ...
    scd->init_rot = br->mtex_accessor.mask.rot;

    scd->dim_target = br->mask_stencil_dimension;
    scd->rot_target = &br->mtex_accessor.mask.rot;
    ...
    scd->init_rot = br->mtex_accessor.color.rot;

    scd->dim_target = br->stencil_dimension;
    scd->rot_target = &br->mtex_accessor.color.rot;
...
static wmOperatorStatus stencil_control_invoke(bContext *C, wmOperator *op, const wmEvent *event)
{
  ...
  if (mask) {
    if (br->mtex_accessor.mask.brush_map_mode != MTEX_MAP_MODE_STENCIL) {
      return OPERATOR_CANCELLED;
    }
  }
  else {
    if (br->mtex_accessor.color.brush_map_mode != MTEX_MAP_MODE_STENCIL) {
      return OPERATOR_CANCELLED;
    }
  }
...
static bool stencil_control_poll(bContext *C)
{
  ...
  return (br && (br->mtex_accessor.color.brush_map_mode == MTEX_MAP_MODE_STENCIL ||
                 br->mtex_accessor.mask.brush_map_mode == MTEX_MAP_MODE_STENCIL));
}
...
static wmOperatorStatus stencil_fit_image_aspect_exec(bContext *C, wmOperator *op)
{
  ...
  MTex *mtex = nullptr;
  if (br) {
    mtex = do_mask ? &br->mtex_accessor.mask : &br->mtex_accessor.color;
    tex = mtex->tex;
  }
