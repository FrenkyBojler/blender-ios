...
static OperatorStatus grease_pencil_brush_stroke_invoke(bContext *C,
                                                        wmOperator *op,
                                                        const wmEvent *event)
{
  ...
  OperatorStatus return_value = ed::greasepencil::grease_pencil_draw_operator_invoke(
...
static OperatorStatus grease_pencil_brush_stroke_modal(bContext *C,
                                                       wmOperator *op,
                                                       const wmEvent *event)
{
...
static OperatorStatus grease_pencil_sculpt_paint_invoke(bContext *C,
                                                        wmOperator *op,
                                                        const wmEvent *event)
{
...
static OperatorStatus grease_pencil_sculpt_paint_modal(bContext *C,
                                                       wmOperator *op,
                                                       const wmEvent *event)
{
  return paint_stroke_modal(C, op, event, reinterpret_cast<PaintStroke **>(&op->customdata));
...
static OperatorStatus grease_pencil_weight_brush_stroke_invoke(bContext *C,
                                                               wmOperator *op,
                                                               const wmEvent *event)
{
  const Scene *scene = CTX_data_scene(C);
...
static OperatorStatus grease_pencil_weight_brush_stroke_modal(bContext *C,
                                                              wmOperator *op,
                                                              const wmEvent *event)
{
  return paint_stroke_modal(C, op, event, reinterpret_cast<PaintStroke **>(&op->customdata));
...
static OperatorStatus grease_pencil_vertex_brush_stroke_invoke(bContext *C,
                                                               wmOperator *op,
                                                               const wmEvent *event)
{
  const Scene *scene = CTX_data_scene(C);
...
static OperatorStatus grease_pencil_vertex_brush_stroke_modal(bContext *C,
                                                              wmOperator *op,
                                                              const wmEvent *event)
{
  return paint_stroke_modal(C, op, event, reinterpret_cast<PaintStroke **>(&op->customdata));
...
static OperatorStatus grease_pencil_fill_invoke(bContext *C,
                                                wmOperator *op,
                                                const wmEvent * /*event*/)
{
  const ARegion &region = *CTX_wm_region(C);
...
static OperatorStatus grease_pencil_fill_event_modal_map(bContext *C,
                                                         wmOperator *op,
                                                         const wmEvent *event)
{
  auto &op_data = *static_cast<GreasePencilFillOpData *>(op->customdata);
...
static OperatorStatus grease_pencil_fill_modal(bContext *C, wmOperator *op, const wmEvent *event)
{
  const RegionView3D &rv3d = *CTX_wm_region_view3d(C);

  auto &op_data = *static_cast<GreasePencilFillOpData *>(op->customdata);

  OperatorStatus estate = OPERATOR_CANCELLED;
  if (!op_data.show_extension) {
...
static OperatorStatus grease_pencil_erase_lasso_exec(bContext *C, wmOperator *op)
{
  using namespace bke::greasepencil;
  using namespace ed::greasepencil;
  const Scene *scene = CTX_data_scene(C);
...
static OperatorStatus grease_pencil_erase_box_exec(bContext *C, wmOperator *op)
{
  using namespace bke::greasepencil;
  using namespace ed::greasepencil;
  const Scene *scene = CTX_data_scene(C);
