                                                           ...
                                                           const wmEvent *event)
{
  ...
  switch (event->val) {
    ...
    case int(FillToolModalKey::ExtensionLengthen):
      op_data.extension_length = std::min(op_data.extension_length + extension_delta, FLT_MAX);
      grease_pencil_update_extend(*C, op_data);
      break;
  ...
  if (!op_data.show_extension) {
    ...
    switch (event->type) {
      ...
      case MOUSEMOVE: {
        ...
        op_data.extension_length = std::clamp(op_data.extension_length + delta, 0.0f, FLT_MAX);
