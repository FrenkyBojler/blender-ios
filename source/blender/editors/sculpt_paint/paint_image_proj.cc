...
static void proj_paint_layer_clone_init(ProjPaintState *ps, ProjPaintLayerClone *layer_clone)
{
  ...
  Mesh &mesh_orig = *static_cast<Mesh *>(ps->ob->data);
  ImagePaintSettings &imapaint = ps->scene->toolsettings->imapaint;
  ...
  if (ps->do_layer_clone) {
    ...
    uv_map_clone_base = static_cast<const float (*)[2]>(
        CustomData_get_layer_named(&mesh_orig.corner_data, CD_PROP_FLOAT2, imapaint.clone_uv_map));
                                ...
                                const char symmetry_flag)
{
  ...
  if (ps->do_layer_stencil || ps->do_stencil_brush) {
    ...
    ps->uv_map_stencil_eval = static_cast<const float (*)[2]>(CustomData_get_layer_named(
        &mesh_orig.corner_data, CD_PROP_FLOAT2, imapaint.stencil_uv_map));
