...
static wmOperatorStatus sample_color_exec(bContext *C, wmOperator *op)
{
  ...
  paint_sample_color(C, region, location, use_merged_texture, use_palette);
...
static wmOperatorStatus sample_color_invoke(bContext *C, wmOperator *op, const wmEvent *event)
{
  ...
  paint_sample_color(C, region, int2(event->mval[0], event->mval[1]), use_merged_texture, false);
...
static wmOperatorStatus sample_color_modal(bContext *C, wmOperator *op, const wmEvent *event)
{
  ...
  switch (event->type) {
    case MOUSEMOVE: {
      ...
      RNA_int_set_array(op->ptr, "location", event->mval);
      paint_sample_color(
          C, region, int2(event->mval[0], event->mval[1]), use_merged_texture, false);
      WM_event_add_notifier(C, NC_BRUSH | NA_EDITED, brush);
      break;
    }
    ...
    case LEFTMOUSE:
      if (event->val == KM_PRESS) {
        ...
        RNA_int_set_array(op->ptr, "location", event->mval);
        paint_sample_color(
            C, region, int2(event->mval[0], event->mval[1]), use_merged_texture, true);
        if (!data->sample_palette) {
