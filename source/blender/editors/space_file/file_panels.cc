...
static void file_panel_execution_buttons_draw(const bContext *C, Panel *panel)
{
  ...
  if (params->flag & FILE_CHECK_EXISTING) {
    but_extra_rna_ptr = UI_but_extra_operator_icon_add(
        but, "FILE_OT_filenum", blender::wm::OperatorCallContext::ExecRegionWin, ICON_REMOVE);
    RNA_int_set(but_extra_rna_ptr, "increment", -1);
    but_extra_rna_ptr = UI_but_extra_operator_icon_add(
        but, "FILE_OT_filenum", blender::wm::OperatorCallContext::ExecRegionWin, ICON_ADD);
    RNA_int_set(but_extra_rna_ptr, "increment", 1);
  }
...

  {
    ...
    sub->operator_context_set(blender::wm::OperatorCallContext::ExecRegionWin);
