...
void folderlist_popdir(ListBaseT<FolderList> *folderlist, char *dir, char *dir_template)
{
  const char *prev_dir;
  FolderList *folder;
  ...
  if (folder) {
    ...
    MEM_delete(folder->foldertemplate);
...
static ListBaseT<FolderList> folderlist_duplicate(ListBaseT<FolderList> *folderlist)
{
  ...
  for (FolderList &folder : folderlistn) {
    ...
    folder.foldertemplate = MEM_dupalloc(folder.foldertemplate);
  }
  return folderlistn;
}
