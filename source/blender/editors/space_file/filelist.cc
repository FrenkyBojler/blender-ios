...
void filelist_setdir(FileList *filelist, char dirpath[FILE_MAX_LIBEXTRA])
{
  ...
  BLI_path_apply_variables(
      dirpath, BKE_build_path_variables(BKE_main_blendfile_path_from_global(), nullptr));
                                                     ...
                                                     float *progress)
{
  ...
  BLI_path_apply_variables(dir, BKE_build_path_variables(job_params->main_filepath, nullptr));
  ...
  while (!BLI_stack_is_empty(todo_dirs) && !(*stop)) {
    ...
    BLI_path_apply_variables(rel_subdir, BKE_build_path_variables(root, nullptr));
    ...
    LISTBASE_FOREACH (FileListInternEntry *, entry, &entries) {
              ...
              max_recursion, is_lib, recursion_level, entry))
      {
        ...
        BLI_path_apply_variables(dir,
                                 BKE_build_path_variables(job_params->main_filepath, nullptr));
        BLI_path_abs(dir, job_params->main_filepath);
        BLI_path_normalize_dir(dir, sizeof(dir));
        td_dir = static_cast<TodoDir *>(BLI_stack_push_r(todo_dirs));
