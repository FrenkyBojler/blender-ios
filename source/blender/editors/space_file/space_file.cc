...
static void file_free(SpaceLink *sl)
{
  ...
  if (sfile->asset_params) {
    BKE_asset_catalog_path_list_free(sfile->asset_params->catalog_states);
  }
...
static SpaceLink *file_duplicate(SpaceLink *sl)
{
  ...
  if (sfileo->asset_params) {
    ...
    BLI_listbase_clear(&sfilen->asset_params->catalog_states);
    LISTBASE_FOREACH (const AssetCatalogState *, state, &sfileo->asset_params->catalog_states) {
      ...
      BLI_addtail(&sfilen->asset_params->catalog_states, new_state);
    }
  }
...
static void file_space_blend_read_data(BlendDataReader *reader, SpaceLink *sl)
{
  ...
  if (sfile->asset_params) {
    ...
    BLO_read_struct_list(reader, AssetCatalogState, &sfile->asset_params->catalog_states);
    /* Read path strings for each catalog collapsed state */
    LISTBASE_FOREACH (AssetCatalogState *, catalog_path, &sfile->asset_params->catalog_states) {
...
static void file_space_blend_write(BlendWriter *writer, SpaceLink *sl)
{
  ...
  if (sfile->asset_params) {
    ...
    LISTBASE_FOREACH (
        const AssetCatalogState *, catalog_path, &sfile->asset_params->catalog_states)
    {
