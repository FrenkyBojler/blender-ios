...
static PointerRNA rna_BoidState_active_boid_rule_get(PointerRNA *ptr)
{
  ...
  for (; rule; rule = rule->next) {
    if (rule->flag & BOIDRULE_CURRENT) {
      return RNA_pointer_create_with_ancestors(*ptr, &RNA_BoidRule, rule);
    }
  }
  return RNA_pointer_create_with_ancestors(*ptr, &RNA_BoidRule, nullptr);
}
...
static PointerRNA rna_BoidSettings_active_boid_state_get(PointerRNA *ptr)
{
  ...
  for (; state; state = state->next) {
    if (state->flag & BOIDSTATE_CURRENT) {
      return RNA_pointer_create_with_ancestors(*ptr, &RNA_BoidState, state);
    }
  }
  return RNA_pointer_create_with_ancestors(*ptr, &RNA_BoidState, nullptr);
}
