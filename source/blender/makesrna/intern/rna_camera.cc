...
{
  const Camera *cam = blender::id_cast<const Camera *>(ptr->owner_id);
...
{
  Camera *cam = blender::id_cast<Camera *>(ptr->owner_id);
  float sensor = BKE_camera_sensor_size(cam->sensor_fit, cam->sensor_x, cam->sensor_y);
...
{
  Camera *camera = blender::id_cast<Camera *>(ptr->owner_id);
...
static std::optional<std::string> rna_Camera_background_image_path(const PointerRNA *ptr)
{
  ...
  const Camera *camera = blender::id_cast<const Camera *>(ptr->owner_id);
    ...
    Main *bmain, RNAPropertyOverrideApplyContext &rnaapply_ctx)
{
  ...
  Camera *cam_dst = blender::id_cast<Camera *>(ptr_dst->owner_id);
  const Camera *cam_src = blender::id_cast<const Camera *>(ptr_src->owner_id);
