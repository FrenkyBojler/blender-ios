...
static void rna_def_userdef_theme_space_node(BlenderRNA *brna)
{
  ...
  prop = RNA_def_property(srna, "foreach_geometry_zone", PROP_FLOAT, PROP_COLOR_GAMMA);
  RNA_def_property_float_sdna(prop, nullptr, "node_zone_foreach_geometry");
  RNA_def_property_array(prop, 4);
  RNA_def_property_ui_text(prop, "For Each Geometry", "");
  RNA_def_property_update(prop, 0, "rna_userdef_theme_update");
}
