...
static void rna_def_curveprofilepoint(BlenderRNA *brna)
{
  ...
  RNA_def_property_enum_funcs(prop,
                              "rna_CurveProfilePoint_handle_types_get",
                              "rna_CurveProfilePoint_handle_types_set",
                              nullptr);
  RNA_def_property_ui_text(
      prop, "Handle Types", "Type of both handles if they are the same, None otherwise.");

  prop = RNA_def_property(srna, "handle_type_1", PROP_ENUM, PROP_NONE);
  RNA_def_property_enum_items(prop, prop_handle_type_items);
  RNA_def_property_enum_funcs(prop,
                              "rna_CurveProfilePoint_handle_type_1_get",
                              "rna_CurveProfilePoint_handle_type_1_set",
                              nullptr);
