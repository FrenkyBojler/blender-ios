...
#  define DEF_SHOW_REGION_PROPERTY(identifier, label, description, icon) \
    { \
      ...
      RNA_def_property_ui_icon(prop, icon, 1); \
  ...
  if (region_type_mask & (1 << RGN_TYPE_TOOL_HEADER)) {
    ...
    DEF_SHOW_REGION_PROPERTY(show_region_tool_header, "Tool Settings", "", ICON_HEADER_OFF);
  }
  if (region_type_mask & (1 << RGN_TYPE_HEADER)) {
    region_type_mask &= ~(1 << RGN_TYPE_HEADER);
    DEF_SHOW_REGION_PROPERTY(show_region_header, "Header", "", ICON_HEADER_OFF);
  }
  if (region_type_mask & (1 << RGN_TYPE_FOOTER)) {
    region_type_mask &= ~(1 << RGN_TYPE_FOOTER);
    DEF_SHOW_REGION_PROPERTY(show_region_footer, "Footer", "", ICON_FOOTER_OFF);
  }
  if (region_type_mask & (1 << RGN_TYPE_TOOLS)) {
    region_type_mask &= ~(1 << RGN_TYPE_TOOLS);
    DEF_SHOW_REGION_PROPERTY(show_region_toolbar, "Toolbar", "", ICON_LEFT_OFF);
  }
  if (region_type_mask & (1 << RGN_TYPE_TOOL_PROPS)) {
    region_type_mask &= ~(1 << RGN_TYPE_TOOL_PROPS);
    DEF_SHOW_REGION_PROPERTY(show_region_tool_props, "Toolbar", "", ICON_HUD_OFF);
  }
  if (region_type_mask & (1 << RGN_TYPE_CHANNELS)) {
    region_type_mask &= ~(1 << RGN_TYPE_CHANNELS);
    DEF_SHOW_REGION_PROPERTY(show_region_channels, "Channels", "", ICON_LEFT_OFF);
  }
  if (region_type_mask & (1 << RGN_TYPE_UI)) {
    region_type_mask &= ~(1 << RGN_TYPE_UI);
    DEF_SHOW_REGION_PROPERTY(show_region_ui, "Sidebar", "", ICON_RIGHT_OFF);
  }
  if (region_type_mask & (1 << RGN_TYPE_HUD)) {
    region_type_mask &= ~(1 << RGN_TYPE_HUD);
    DEF_SHOW_REGION_PROPERTY(show_region_hud, "Adjust Last Operation", "", ICON_HUD_OFF);
  }
