...
{
  ColorManagedDisplaySettings *display_settings = static_cast<ColorManagedDisplaySettings *>(
      ptr->data);
  ID *id = ptr->owner_id;
  ...
  switch (GS(id->name)) {
    case ID_SCE: {
      ...
      if (&scene->r.im_format.display_settings == display_settings) {
        im_format = &scene->r.im_format;
        found = true;
      }
      else if (&scene->r.bake.im_format.display_settings == display_settings) {
        im_format = &scene->r.bake.im_format;
        found = true;
      }
      break;
    }
    case ID_NT: {
      ...
      for (node = static_cast<bNode *>(ntree->nodes.first); node && !found; node = node->next) {
        if (node->type_legacy != CMP_NODE_OUTPUT_FILE) {
          continue;
        }

        NodeImageMultiFile *storage = static_cast<NodeImageMultiFile *>(node->storage);
        if (&storage->format.display_settings == display_settings) {
          im_format = &storage->format;
          found = true;
          break;
        }
        else {
          ...
          for (sock = static_cast<bNodeSocket *>(node->inputs.first); sock; sock = sock->next) {
            NodeImageMultiFileSocket *sockdata = static_cast<NodeImageMultiFileSocket *>(
                sock->storage);
            if (&sockdata->format.display_settings == display_settings) {
              im_format = &sockdata->format;
              found = true;
              break;
            }
          }
  ...
  if (found) {
    PointerRNA imf_ptr = RNA_pointer_create_discrete(id, &RNA_ImageFormatSettings, im_format);
    std::string imf_path = RNA_path_from_ID_to_struct(&imf_ptr).value_or("");
    return fmt::format("{}.display_settings", imf_path);
  }
...
static std::optional<std::string> rna_ColorManagedViewSettings_path(const PointerRNA *ptr)
{
  ...
  switch (GS(id->name)) {
    case ID_SCE: {
      ...
      if (&scene->r.im_format.view_settings == view_settings) {
        im_format = &scene->r.im_format;
        found = true;
      }
      else if (&scene->r.bake.im_format.view_settings == view_settings) {
        im_format = &scene->r.bake.im_format;
        found = true;
      }
      break;
    ...
    case ID_NT: {
      ...
      for (node = static_cast<bNode *>(ntree->nodes.first); node && !found; node = node->next) {
        ...
        NodeImageMultiFile *storage = static_cast<NodeImageMultiFile *>(node->storage);
        if (&storage->format.view_settings == view_settings) {
          im_format = &storage->format;
          found = true;
          break;
        }
          ...
          for (sock = static_cast<bNodeSocket *>(node->inputs.first); sock; sock = sock->next) {
            NodeImageMultiFileSocket *sockdata = static_cast<NodeImageMultiFileSocket *>(
                sock->storage);
            if (&sockdata->format.view_settings == view_settings) {
              im_format = &sockdata->format;
              found = true;
              break;
            }
  ...
  if (found) {
    PointerRNA imf_ptr = RNA_pointer_create_discrete(id, &RNA_ImageFormatSettings, im_format);
    std::string imf_path = RNA_path_from_ID_to_struct(&imf_ptr).value_or("");
    return fmt::format("{}.view_settings", imf_path);
  }
    ...
    const PointerRNA *ptr)
{
  ...
  switch (GS(id->name)) {
    case ID_SCE: {
      ...
      if (&scene->r.im_format.linear_colorspace_settings == colorspace) {
        im_format = &scene->r.im_format;
        found = true;
      }
      else if (&scene->r.bake.im_format.linear_colorspace_settings == colorspace) {
        im_format = &scene->r.bake.im_format;
        found = true;
      }
      break;
    ...
    case ID_NT: {
      ...
      for (node = static_cast<bNode *>(ntree->nodes.first); node && !found; node = node->next) {
        ...
        NodeImageMultiFile *storage = static_cast<NodeImageMultiFile *>(node->storage);
        if (&storage->format.linear_colorspace_settings == colorspace) {
          im_format = &storage->format;
          found = true;
          break;
        }
          ...
          for (sock = static_cast<bNodeSocket *>(node->inputs.first); sock; sock = sock->next) {
            NodeImageMultiFileSocket *sockdata = static_cast<NodeImageMultiFileSocket *>(
                sock->storage);
            if (&sockdata->format.linear_colorspace_settings == colorspace) {
              im_format = &sockdata->format;
              found = true;
              break;
            }
  ...
  if (found) {
    PointerRNA imf_ptr = RNA_pointer_create_discrete(id, &RNA_ImageFormatSettings, im_format);
    std::string imf_path = RNA_path_from_ID_to_struct(&imf_ptr).value_or("");
    return fmt::format("{}.linear_colorspace_settings", imf_path);
  }
