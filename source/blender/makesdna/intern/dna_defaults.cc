 ...
 * ============
 *
 * This API provides direct access to DNA default structs to avoid duplicating
 * values for initialization, versioning and RNA.
 *
 * Default values are retrieved from the C++11 member initialization in the
 * structs.
 *
 * Defining the defaults is optional since it doesn't make sense for some structs to
 * have defaults, and some structs don't need it. However to auto initialize defaults
 * in RNA the struct must be defined here.
 *
 * Adding Defaults
 * ---------------
 *
 * Defaults must be registered by adding the #SDNA_DEFAULT_DECL_STRUCT and
...
#define SDNA_DEFAULT_DECL_STRUCT(struct_name) static const struct_name DNA_DEFAULT_##struct_name
...
static bool init_dna_default_table()
{
/**
...
#define SDNA_DEFAULT_DECL(struct_name) \
  DNA_default_table[SDNA_TYPE_FROM_STRUCT(struct_name)] = SDNA_TYPE_CHECKED( \
      DNA_DEFAULT_##struct_name, struct_name)

#define SDNA_DEFAULT_DECL_EX(struct_name, struct_path) \
  DNA_default_table[SDNA_TYPE_FROM_STRUCT(struct_name)] = SDNA_TYPE_CHECKED( \
      DNA_DEFAULT_##struct_path, struct_name)
...
static bool initialized_dna_default_table = init_dna_default_table();

uint8_t *_DNA_struct_default_alloc_impl(const uint8_t *data_src,
                                        size_t size,
                                        const char *alloc_str)
{
  uint8_t *data_dst = static_cast<uint8_t *>(MEM_mallocN(size, alloc_str));
